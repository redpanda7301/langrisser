var data = [
{
  "Skins": [
    {
      "ImageUrl": "라나/1.webp",
      "Desc": "기본"
    },
    {
      "ImageUrl": "라나/2.webp",
      "Desc": "스킨상점"
    },
    {
      "ImageUrl": "라나/3.webp",
      "Desc": "빛의 메아리"
    },
    {
      "ImageUrl": "라나/4.webp",
      "Desc": "한정판매"
    },
    {
      "ImageUrl": "라나/5.webp",
      "Desc": "한정판매"
    },
    {
      "ImageUrl": "라나/6.webp",
      "Desc": "한정판매"
    },
    {
      "ImageUrl": "라나/7.webp",
      "Desc": "빛의 메아리"
    },
    {
      "ImageUrl": "라나/sp.webp",
      "Desc": "SP 각성"
    },
    {
      "ImageUrl": "라나/8.webp",
      "Desc": "한정판매"
    }
  ],
  "Name": "라나",
  "ChName": "拉娜",
  "EngName": "Lana",
  "JapName": "ラーナ",
  "CV": "코시미즈 아미",
  "Rank": [
    0
  ],
  "SP": true,
  "Groups": [
    5,
    4
  ],
  "HP": 10,
  "SPHP": 20,
  "Attk": 40,
  "SPAttk": 40,
  "Def": 10,
  "SPDef": 15,
  "MagicDef": 40,
  "SPMagicDef": 25,
  "HowToGet": "영웅 소환",
  "Fetters": [
    {
      "Condition": "라나를 출전시켜 아레나에서 5회 승리"
    },
    {
      "Condition": "라나를 출전시켜 시공의 균열 3-1 [정예] 클리어"
    },
    {
      "Condition": "레온의 도움 하에 '먼 구름 속의 청룡' 클리어"
    },
    {
      "Condition": "엘윈의 도움 하에 '봉인의 무녀' 클리어"
    }
  ],
  "HeartFetter": "생명+400, 방어+30, 마방+60",
  "Talent": [
    {
      "Name": "마력 강화",
      "Icon": "45.webp",
      "Abilities": [
        {
          "Desc": "1성"
        },
        {
          "Desc": "2성"
        },
        {
          "Desc": "전투 진입 시 마법 피해 <font color=\"red\">+10%</font> 스킬 사거리 <font color=\"red\">+1</font>."
        },
        {
          "Desc": "전투 진입 시 마법 피해 <font color=\"red\">+15%</font> 스킬 사거리 <font color=\"red\">+1</font>."
        },
        {
          "Desc": "전투 진입 시 마법 피해 <font color=\"red\">+20%</font> 스킬 사거리 <font color=\"red\">+1</font>."
        },
        {
          "Desc": "전투 진입 시 마법 피해 <font color=\"red\">+30%</font> 스킬 사거리 <font color=\"red\">+1</font>."
        }
      ]
    }
  ],
  "SPTalent": [
    {
      "Name": "암광의 성재",
      "Icon": "SP_Gift_Lana.webp",
      "Abilities": [
        {
          "Desc": "1성"
        },
        {
          "Desc": "2성"
        },
        {
		      "Desc": "자신이 보유한 <font color=\"blue\">버프</font> <font color=\"red\">1</font>개당 피해 <font color=\"red\">+4</font>%, 최대 <font color=\"red\">+20</font>%<br/><font color=#ffa700>근접 병사</font> 보유 시 <font color=\"blue\">광역 스킬</font> 범위 <font color=\"red\">+1</font>, 사거리 <font color=\"red\">+1</font><br/><font color=#e25822>원거리 병사</font> 보유 시 <b>전투 진입하는 <font color=\"blue\">원거리 스킬</font>을 사용할 경우 부대 사거리</b> <font color=\"red\">+1</font>"
        },
        {
		      "Desc": "자신이 보유한 <font color=\"blue\">버프</font> <font color=\"red\">1</font>개당 피해 <font color=\"red\">+5</font>%, 최대 <font color=\"red\">+25</font>%<br/><font color=#ffa700>근접 병사</font> 보유 시 <font color=\"blue\">광역 스킬</font> 범위 <font color=\"red\">+1</font>, 사거리 <font color=\"red\">+1</font><br/><font color=#e25822>원거리 병사</font> 보유 시 <b>전투 진입하는 <font color=\"blue\">원거리 스킬</font>을 사용할 경우 부대 사거리</b> <font color=\"red\">+1</font>"
        },
        {
		      "Desc": "자신이 보유한 <font color=\"blue\">버프</font> <font color=\"red\">1</font>개당 피해 <font color=\"red\">+6</font>%, 최대 <font color=\"red\">+30</font>%<br/><font color=#ffa700>근접 병사</font> 보유 시 <font color=\"blue\">광역 스킬</font> 범위 <font color=\"red\">+1</font>, 사거리 <font color=\"red\">+1</font><br/><font color=#e25822>원거리 병사</font> 보유 시 <b>전투 진입하는 <font color=\"blue\">원거리 스킬</font>을 사용할 경우 부대 사거리</b> <font color=\"red\">+1</font>"
        },
        {
		      "Desc": "자신이 보유한 <font color=\"blue\">버프</font> <font color=\"red\">1</font>개당 피해 <font color=\"red\">+7</font>%, 최대 <font color=\"red\">+35</font>%<br/><font color=#ffa700>근접 병사</font> 보유 시 <font color=\"blue\">광역 스킬</font> 범위 <font color=\"red\">+1</font>, 사거리 <font color=\"red\">+1</font><br/><font color=#e25822>원거리 병사</font> 보유 시 <b>전투 진입하는 <font color=\"blue\">원거리 스킬</font>을 사용할 경우 부대 사거리</b> <font color=\"red\">+1</font>"
        }
      ]
    }
  ],
  "Equip": {
    "Name": "검정 드레스(갑옷)",
    "Stats": "생명 583, 방어 54",
    "Icon": "라나.webp",
    "Ability": "생명 +5%<br/>전투진입전, 적군에게 랜덤으로 1개의 강력한 디버프 부여"
  },
  "Soldiers": [
    0,
    13,
    22,
    26,
    43,
    44,
    62,
    70,
    67,
    76,
    85,
    87
  ],
  "Jobs": [
    {
      "Name": "다크 프린세스",
      "Icon": "Icon_Occupation_Magician.webp",
      "SD": [
        {
          "ImageUrl": "라나/1/1.webp"
        },
        {
          "ImageUrl": "라나/2/1.webp"
        },
        {
          "ImageUrl": "라나/3/1.webp"
        },
        {
          "ImageUrl": "라나/4/1.webp"
        },
        {
          "ImageUrl": "라나/5/1.webp"
        },
        {
          "ImageUrl": "라나/6/1.webp"
        },
        {
          "ImageUrl": "라나/7/1.webp"
        },
        {
          "ImageUrl": "라나/sp/1.webp"
        },
        {
          "ImageUrl": "라나/8/1.webp"
        }
      ],
      "AttkRange": 2,
      "Move": 3,
      "MoveType": "Walk",
      "HP": 3340,
      "HPRate": 56,
      "Attk": 262,
      "AttkRate": 40,
      "Int": 554,
      "IntRate": 92,
      "Def": 245,
      "DefRate": 54,
      "MagicDef": 401,
      "MagicDefRate": 89,
      "Skill": 96,
      "SkillRate": 27,
      "HeartFetterBouns1": "'<font color =\"blue\">약화효과</font>'가 있는 대상에게 공격받아 전투 진입시 <br/>받는 피해량 <font color=\"red\">-10</font>%",
      "HeartFetterBouns2": "<b>물리공격을 받아 전투 진입 시</b> <br/>주는 피해량<font color=\"red\">+10</font>%"
    },
    {
      "Name": "단죄의 성자",
      "Icon": "Icon_Occupation_MonkSP.webp",
      "SD": [
        {
          "ImageUrl": "라나/sp/1.webp"
        },
        {
          "ImageUrl": "라나/2/1.webp"
        },
        {
          "ImageUrl": "라나/3/1.webp"
        },
        {
          "ImageUrl": "라나/4/1.webp"
        },
        {
          "ImageUrl": "라나/5/1.webp"
        },
        {
          "ImageUrl": "라나/6/1.webp"
        },
        {
          "ImageUrl": "라나/7/1.webp"
        },
        {
          "ImageUrl": "라나/sp/1.webp"
        },
        {
          "ImageUrl": "라나/8/1.webp"
        }        
      ],
      "AttkRange": 2,
      "Move": 3,
      "MoveType": "Walk",
      "HP": 3925,
      "HPRate": 72,
      "Attk": 290,
      "AttkRate": 43,
      "Int": 590,
      "IntRate": 96,
      "Def": 265,
      "DefRate": 57,
      "MagicDef": 362,
      "MagicDefRate": 83,
      "Skill": 167,
      "SkillRate": 57,
      "HeartFetterBouns1": "부대 생명이 <font color=\"red\">70</font>% 이하시, <br/><b>전투중 받는 피해량</b> <font color=\"red\">-10</font>%",
      "HeartFetterBouns2": "<b><font color =\"blue\">약화효과</font>를 보유한 부대와 전투 시</b><br/> 주는 피해량 <font color=\"red\">+10</font>%"
    },
    {
      "Name": "세인트",
      "Icon": "Icon_Occupation_Monk.webp",
      "SD": [
        {
          "ImageUrl": "라나/1/2.webp"
        },
        {
          "ImageUrl": "라나/2/1.webp"
        },
        {
          "ImageUrl": "라나/3/1.webp"
        },
        {
          "ImageUrl": "라나/4/1.webp"
        },
        {
          "ImageUrl": "라나/5/1.webp"
        },
        {
          "ImageUrl": "라나/6/1.webp"
        },
        {
          "ImageUrl": "라나/7/1.webp"
        },
        {
          "ImageUrl": "라나/sp/1.webp"
        },
        {
          "ImageUrl": "라나/8/1.webp"
        }
      ],
      "AttkRange": 2,
      "Move": 3,
      "MoveType": "Walk",
      "HP": 3503,
      "HPRate": 58,
      "Attk": 316,
      "AttkRate": 49,
      "Int": 523,
      "IntRate": 87,
      "Def": 265,
      "DefRate": 59,
      "MagicDef": 401,
      "MagicDefRate": 89,
      "Skill": 96,
      "SkillRate": 27,
      "HeartFetterBouns1": "생명 <font color=\"red\">100</font>%시 <br/>받는 피해량 <font color=\"red\">-10</font>%",
      "HeartFetterBouns2": "<b>마법공격을 받아 전투 진입 시</b> <br/>주는 피해량 <font color=\"red\">+10</font>%"
    }
  ],
  "Materials": {
    "HeartFetter": [
      "h3",
      "h4",
      "h2",
      "h1",
      "h3-1",
      "h4-1",
      "h2-1",
      "h1-1",
      "x"
    ],
    "Fetter": [
      "4",
      "4-1",
      "4-2",
      "4-3",
      "4-4"
    ],
    "Awaken": [
      "ranka4",
      "ranka5",
      "ranka4-1",
      "ranka5-1"
    ]
  },
  "ClassUp": [
    {
      "ClassTitle": "메이지 : 마방+4, 지력+9",
      "Tiers": [
        {
          "Name": "T1",
          "Materials": [
            "rank1-5"
          ]
        },
        {
          "Name": "T2",
          "Materials": [
            "rank1-5"
          ]
        }
      ]
    },
    {
      "ClassTitle": "서모너 : 생명+80, 방어+5",
      "Tiers": [
        {
          "Name": "T1",
          "Materials": [
            "rank2-4",
            "rank1-5",
            "rank1-4"
          ]
        },
        {
          "Name": "T2",
          "Materials": [
            "rank3-5",
            "rank3-6",
            "rank2-4",
            "rank2-5"
          ]
        },
        {
          "Name": "T3",
          "Materials": [
            "rank4-4",
            "rank4-5",
            "rank3-5",
            "rank3-6"
          ]
        }
      ]
    },
    {
      "ClassTitle": "아크메이지 : 생명+48, 지력+12",
      "Tiers": [
        {
          "Name": "T1",
          "Materials": [
            "rank2-4",
            "rank1-5",
            "rank1-4"
          ]
        },
        {
          "Name": "T2",
          "Materials": [
            "rank3-5",
            "rank3-4",
            "rank2-4",
            "rank2-5"
          ]
        },
        {
          "Name": "T3",
          "Materials": [
            "rank4-4",
            "rank4-5",
            "rank3-5",
            "rank3-4"
          ]
        }
      ]
    },
    {
      "ClassTitle": "다크 프린세스 : 생명+90, 마방+10, 지력+25",
      "Tiers": [
        {
          "Name": "T1",
          "Materials": [
            "rank5-5",
            "rank5-4",
            "rank4-4",
            "rank4-5"
          ]
        },
        {
          "Name": "T2",
          "Materials": [
            "rank6-7",
            "rank6-4",
            "rank5-5",
            "rank5-4"
          ]
        },
        {
          "Name": "T3",
          "Materials": [
            "rank7-6",
            "rank7-5",
            "rank6-7",
            "rank6-4"
          ]
        },
        {
          "Name": "T4",
          "Materials": [
            "rank8-4",
            "rank8-8",
            "rank7-6",
            "rank7-4"
          ]
        }
      ]
    },
    {
      "ClassTitle": "마터 : 마방+8, 지력+7",
      "Tiers": [
        {
          "Name": "T1",
          "Materials": [
            "rank2-5",
            "rank1-5",
            "rank1-4"
          ]
        },
        {
          "Name": "T2",
          "Materials": [
            "rank3-4",
            "rank3-5",
            "rank2-5",
            "rank2-4"
          ]
        },
        {
          "Name": "T3",
          "Materials": [
            "rank4-5",
            "rank4-4",
            "rank3-4",
            "rank3-5"
          ]
        }
      ]
    },
    {
      "ClassTitle": "세인트 : 지력+25, 마방+10, 생명+90",
      "Tiers": [
        {
          "Name": "T1",
          "Materials": [
            "rank5-4",
            "rank5-5",
            "rank4-5",
            "rank4-4"
          ]
        },
        {
          "Name": "T2",
          "Materials": [
            "rank6-8",
            "rank6-7",
            "rank5-4",
            "rank5-5"
          ]
        },
        {
          "Name": "T3",
          "Materials": [
            "rank7-5",
            "rank7-6",
            "rank6-8",
            "rank6-7"
          ]
        },
        {
          "Name": "T4",
          "Materials": [
            "rank8-6",
            "rank8-4",
            "rank7-5",
            "rank7-6"
          ]
        }
      ]
    }
  ],
  "SPOpenConditions": [
    {
      "Name": "개방",
      "Materials": [
        {
          "MaterialImgUrl": "../img/item/AlterEgo_Lana.webp",
          "Amount": 1
        }
      ],
      "Desc": ""
    },
    {
      "Name": "1단계 재료",
      "Materials": [
        {
          "MaterialImgUrl": "../img/item/오락기.webp",
          "Amount": 40
        },
        {
          "MaterialImgUrl": "../img/classup/rank8-4.webp",
          "Amount": 30
        },
        {
          "MaterialImgUrl": "../img/classup/rank7-6.webp",
          "Amount": 40
        },
        {
          "MaterialImgUrl": "../img/classup/rank6-7.webp",
          "Amount": 50
        }
      ],
      "Desc": ""
    },
    {
      "Name": "1단계 미션",
      "Materials": [],
      "Desc": "라나로 파티를 짜서 형귀 헬스장 바란(창병) 60클리어"
    },
    {
      "Name": "2단계 재료",
      "Materials": [
        {
          "MaterialImgUrl": "../img/item/오락기.webp",
          "Amount": 40
        },
        {
          "MaterialImgUrl": "../img/item/쓰알열쇠.webp",
          "Amount": 30
        },
        {
          "MaterialImgUrl": "../img/item/스알열쇠.webp",
          "Amount": 40
        },
        {
          "MaterialImgUrl": "../img/item/알열쇠.webp",
          "Amount": 50
        }
      ],
      "Desc": ""
    },
    {
      "Name": "2단계 미션",
      "Materials": [],
      "Desc": "검정 드레스(갑옷) 레벨 50 달성"
    },
    {
      "Name": "3단계 재료",
      "Materials": [
        {
          "MaterialImgUrl": "../img/item/오락기.webp",
          "Amount": 40
        },
        {
          "MaterialImgUrl": "../img/item/룬스톤.webp",
          "Amount": 2
        }
      ],
      "Desc": ""
    },
    {
      "Name": "3단계 미션",
      "Materials": [],
      "Desc": "라나로 파티를 짜서 여신의 시련 - 빙룡 아산테 60레벨 클리어"
    },
    {
      "Name": "",
      "Materials": [],
      "Desc": "단죄의 성자 전직 가능, 암광의 성재(SP 고유기술)스킬 획득, SP 일러스트 획득"
    },
    {
      "Name": "4단계 재료",
      "Materials": [
        {
          "MaterialImgUrl": "../img/item/오락기.webp",
          "Amount": 40
        },
        {
          "MaterialImgUrl": "../img/heart/h3.webp",
          "Amount": 60
        },
        {
          "MaterialImgUrl": "../img/heart/h3-1.webp",
          "Amount": 30
        },
        {
          "MaterialImgUrl": "../img/heart/x.webp",
          "Amount": 12
        }
      ],
      "Desc": ""
    },
    {
      "Name": "4단계 미션",
      "Materials": [],
      "Desc": "라나로 파티를 짜서 형귀 헬스장 바란(창병) 65클리어"
    },
    {
      "Name": "5단계 재료",
      "Materials": [
        {
          "MaterialImgUrl": "../img/item/오락기.webp",
          "Amount": 40
        },
        {
          "MaterialImgUrl": "../img/item/Awaken_MoonBrillance.webp",
          "Amount": 5
        },
        {
          "MaterialImgUrl": "../img/item/Awaken_Stardust.webp",
          "Amount": 5
        }
      ],
      "Desc": ""
    },
    {
      "Name": "5단계 미션",
      "Materials": [],
      "Desc": "라나로 파티를 짜서 - 빙룡 아산테 65레벨 클리어"
    },
    {
      "Name": "6단계 재료",
      "Materials": [
        {
          "MaterialImgUrl": "../img/item/Item_MemoryEssence.webp",
          "Amount": 600
        },
        {
          "MaterialImgUrl": "../img/stone/StoneA_Holy3.webp",
          "Amount": 5
        },
        {
          "MaterialImgUrl": "../img/stone/StoneA_Holy5.webp",
          "Amount": 5
        },
        {
          "MaterialImgUrl": "../img/stone/StoneA_Holy6.webp",
          "Amount": 5
        }
      ],
      "Desc": ""
    },
    {
      "Name": "6단계 미션",
      "Materials": [],
      "Desc": "라나로 파티를 짜서 영원의 신전 - 리바이엘 65레벨 클리어"
    },
    {
      "Name": "",
      "Materials": [],
      "Desc": "SP 스탯, SP 스킬 2개 활성화, 속죄자 고용가능"
    }
  ],
  
  "Skills": [
    {
      "Name": "어둠의 낫",
      "Cost": "COST 2",
      "Entries": [
        {
          "Label": "유형",
          "Value": "마법 피해"
        },
        {
          "Label": "쿨타임",
          "Value": "2턴"
        },
        {
          "Label": "사거리",
          "Value": "2칸"
        },
        {
          "Label": "범위",
          "Value": "단일"
        }
      ],
      "IconImageUrl": "../img/skills/Skill_DarkCrash.webp",
      "Desc": "[마법 피해]단일 적 부대를 공격해 <font color =\"red\">1.3</font>배의 피해를 준다.<br/>적의 마방을 <font color =\"red\">30</font>% 무시. <br/>전투 후 준 피해량의 <font color =\"red\">50</font>%만큼 회복한다.",
      "SubSkills": null,
      "Animation": null
    },
    {
      "Name": "빙결",
      "Cost": "COST 1",
      "Entries": [
        {
          "Label": "유형",
          "Value": "마법 피해"
        },
        {
          "Label": "쿨타임",
          "Value": "1턴"
        },
        {
          "Label": "사거리",
          "Value": "2칸"
        },
        {
          "Label": "범위",
          "Value": "단일"
        }
      ],
      "IconImageUrl": "../img/skills/Skill_IceBolt.webp",
      "Desc": "단일 개체에게 <font color =\"red\">1.5</font>배의 피해를 준다.<br/>'창병'에게 더 큰 피해를 준다.",
      "SubSkills": null,
      "Animation": null
    },
    {
      "Name": "아이스",
      "Cost": "COST 2",
      "Entries": [
        {
          "Label": "유형",
          "Value": "마법 피해"
        },
        {
          "Label": "쿨타임",
          "Value": "3턴"
        },
        {
          "Label": "사거리",
          "Value": "3칸"
        },
        {
          "Label": "범위",
          "Value": "3칸"
        }
      ],
      "IconImageUrl": "../img/skills/Skill_IceCrash.webp",
      "Desc": "[마법 피해]범위 내 다수의 적군을 공격하여 <font color =\"red\">0.3</font>배의 범위 피해를 입힌다.<br/>'창병'에게 더 큰 피해를 준다. <br/>적군 이동력 <font color =\"red\">-1</font>칸 및 <font color=\"blue\">호위 불가</font>를 부여한다, <br/><font color =\"red\">2</font>턴 지속.",
      "SubSkills": null,
      "Animation": null
    },
    {
      "Name": "마방 위협",
      "Cost": "COST 2",
      "Entries": [
        {
          "Label": "유형",
          "Value": "패시브"
        }
      ],
      "IconImageUrl": "../img/skills/Passive_BreakMDef.webp",
      "Desc": "[패시브]행동 종료 시 주위 <font color =\"red\">3</font>칸 내 모든 적 부대 마방 <font color =\"red\">-20</font>% <br/><font color =\"red\">1</font>턴 지속.",
      "SubSkills": null,
      "Animation": null
    },
    {
      "Name": "블랙홀",
      "Cost": "COST 2",
      "Entries": [
        {
          "Label": "유형",
          "Value": "마법 피해"
        },
        {
          "Label": "쿨타임",
          "Value": "5턴"
        },
        {
          "Label": "사거리",
          "Value": "3칸"
        },
        {
          "Label": "범위",
          "Value": "4칸"
        }
      ],
      "IconImageUrl": "../img/skills/Skill_DarkHole.webp",
      "Desc": "[마법 피해]범위 내 적들을 공격해 <font color =\"red\">0.36</font>배의 광역 피해를 준다.<br/>각 부대에게 <b><font color=\"blue\">무작위 디버프</font></b>를 <font color =\"red\">2</font>개씩 부여한다.",
      "SubSkills": null,
      "Animation": null
    },
    {
      "Name": "썬더",
      "Cost": "COST 1",
      "Entries": [
        {
          "Label": "유형",
          "Value": "마법 피해"
        },
        {
          "Label": "쿨타임",
          "Value": "1턴"
        },
        {
          "Label": "사거리",
          "Value": "2칸"
        },
        {
          "Label": "범위",
          "Value": "단일"
        }
      ],
      "IconImageUrl": "../img/skills/Skill_ThunderBolt.webp",
      "Desc": "[마법 피해]단일 적 부대를 공격해 <font color =\"red\">1.5</font>배의 피해를 준다.<br/>'기병'에게 더큰 피해를 준다.",
      "SubSkills": null,
      "Animation": null
    },
    {
      "Name": "턴언데드",
      "Cost": "COST 2",
      "Entries": [
        {
          "Label": "유형",
          "Value": "마법 피해"
        },
        {
          "Label": "쿨타임",
          "Value": "2턴"
        },
        {
          "Label": "사거리",
          "Value": "2칸"
        },
        {
          "Label": "범위",
          "Value": "단일"
        }
      ],
      "IconImageUrl": "../img/skills/Skill_ValkCrash2.webp",
      "Desc": "[마법 피해]단일 적 부대를 공격해 <font color =\"red\">1.5</font>배의 피해를 준다.<br/>'마물'에게 더큰 피해를 준다. <br/><b>전투 후</b> 대상의 <font color =\"blue\">버프</font>를 <font color =\"red\">2</font>개 제거한다.",
      "SubSkills": null,
      "Animation": null
    },
    {
      "Name": "천벌",
      "Cost": "COST 2",
      "Entries": [
        {
          "Label": "유형",
          "Value": "마법 피해"
        },
        {
          "Label": "쿨타임",
          "Value": "5턴"
        },
        {
          "Label": "사거리",
          "Value": "자신"
        },
        {
          "Label": "범위",
          "Value": "5칸"
        }
      ],
      "IconImageUrl": "../img/skills/Skill_HolySuper.webp",
      "Desc": "[마법 피해]치명타 확률 <font color =\"red\">+50</font>% <br/>주위 <font color =\"red\">5</font>칸 내 모든 적군에게 <font color =\"red\">0.35</font>배 범위 피해를 입힌다.<br/>'마물'과 전투 시 더 큰 피해를 준다.",
      "SubSkills": null,
      "Animation": null
    },
    {
      "Name": "망령 소환",
      "Cost": "COST 2",
      "Entries": [
        {
          "Label": "유형",
          "Value": "지원"
        },
        {
          "Label": "쿨타임",
          "Value": "4턴"
        },
        {
          "Label": "사거리",
          "Value": "3칸"
        },
        {
          "Label": "범위",
          "Value": "단일"
        }
      ],
      "IconImageUrl": "../img/skills/Skill_Skeleton04.webp",
      "Desc": "망령술사 부대를 <font color =\"red\">1</font>개 소환한다.",
      "SubSkills": null,
      "Animation": null
    },
    {
      "Name": "원소의 울림",
      "Cost": "COST 1",
      "Entries": [
        {
          "Label": "유형",
          "Value": "패시브"
        }
      ],
      "IconImageUrl": "../img/skills/sp/라나/1.webp",
      "Desc": "<b>선공으로 <font color=\"blue\">스킬</font>을 사용해 피해를 입힌 후<br/>해당 스킬 쿨타임 무작위로</b> <font color =\"red\">1</font> ~ <font color =\"red\">5</font>턴",
      "SubSkills": null,
      "Animation": null
    },
    {
      "Name": "위상이동",
      "Cost": "COST 2",
      "Entries": [
        {
          "Label": "유형",
          "Value": "지원"
        },
        {
          "Label": "쿨타임",
          "Value": "4턴"
        },
        {
          "Label": "사거리",
          "Value": "자신"
        },
        {
          "Label": "범위",
          "Value": "단일"
        }
      ],
      "IconImageUrl": "../img/skills/sp/라나/2.webp",
      "Desc": "<b>현재 위치를 <font color=\"blue\">마킹</font> 후</b>, 자신을 일정 범위내 장소로 <font color=\"blue\">텔레포트</font> 및 '<font color=\"blue\">위상</font>' 효과 획득<br/><font color=\"blue\">위상</font> : 영웅이 받는 피해 <font color=\"red\">-75</font>%, <b>선공으로 전투 진입 전 자신 부대</b> 생명 <font color=\"red\">100</font>% 회복<br/><b>행동 종료 시 자신 부대를 <font color=\"blue\">마킹</font>한 위치로 <font color=\"blue\">텔레포트</font></b><br/>지속 <font color=\"red\">1</font>행동<br/><b>해제 불가</b><br/><b>(텔레포트 범위는 대상을 기준으로 <font color=\"red\">7</font>칸)</b>",
      "SubSkills": null,
      "Animation": null
    },
    {
      "Name": "원소 파괴",
      "Cost": "COST 3",
      "Entries": [
        {
          "Label": "유형",
          "Value": "마법피해"
        },
        {
          "Label": "쿨타임",
          "Value": "3턴"
        },
        {
          "Label": "사거리",
          "Value": "2칸"
        },
        {
          "Label": "범위",
          "Value": "단일"
        }
      ],
      "IconImageUrl": "../img/Super/라나.webp",
      "Desc": "[마법 피해]단일 적 부대를 공격해 <font color =\"red\">1.5</font>배의 피해를 준다.<br/><b>전투 진입 전 병사의 사거리</b> <font color =\"red\">+1</font>, <br/>전투 후 준 피해량의 <font color =\"red\">30</font>%만큼 회복한다.<br/>'보병','기병','창병','비병'에게 더 큰 피해를 준다.",
      "SubSkills": null,
      "Animation": "../img/Super/라나_Ani.webp"
    }
  ],
  "Propose": true,
  "Bio": [
    {
      "Entry": "영웅소개",
      "Desc": "◆키/몸무게：167cm / 45kg<br/>◆신체 사이즈：83/52/84<br/>◆출연：랑그릿사II<br/>◆소속：어둠의 왕국 벨제리아"
    },
    {
      "Entry": "영웅담1",
      "Desc": "성검을 봉인하고 그 봉인을 풀 수 있는 두 명의 무녀 중 한 사람이자 리아나의 언니.어린 시절 어둠의 왕자 보젤에게 납치되고 조종당했다. 조종에서 풀려난 그녀는 자신이 저지른 죄를 씻기 위해 평생 최선을 다했다.<br/><font color=\"#DC143C\"><size=33><i>「지금이라도 뉘우친다면 아직 늦지 않았을 겁니다... 분명...」</i>  </size=33></font>"
    },
    {
      "Entry": "영웅담2",
      "Desc": "엘윈 일행이 성에 당도하자 라나는 다크 프린세스로 나타나 빛의 군단을 가로막는 강력한 적이 되었다. 이후 어둠의 왕자 보젤이 베른하르트 황제에게 죽자 라나 역시 제정신을 찾을 수 있었다."
    },
    {
      "Entry": "영웅담3",
      "Desc": "속죄하기 위해 빛의 군단에 합류했다. 순진하고 선량하며 굳은 의지의 그 모습은 마치 리아나와 비슷했지만 레온에 대한 강렬한 마음만은 언제나 그대로였다.하지만 그 마음은 영원히 보답받지 못하였다."
    },
    {
      "Entry": "영웅담4",
      "Desc": "전쟁 후 라나는 빛의 대신전으로 가서 빛의 여신을 모시는 사제가 되었다. 이후 온 대륙에 전염병이 만연할 때, 라나가 만든 약으로 병세를 잠재워 수많은 생명을 구할 수 있었다.열성적이고 선량한 성품을 지닌 그녀는 이후 새로운 대사제가 되어 세상을 행복하게 만들기 위해 평생을 노력하였다."
    },
    {
      "Entry": "영웅담5",
      "Desc": "성검의 기억 속 라나에게는 여전히 어둠의 흔적이 많이 남아 있었다. 하지만 동료들은 그녀가 그들의 곁을 지켜줄 것이라 굳게 믿고 있다.<br/><font color=\"#DC143C\"><size=33><i>「어둠의 무녀와 빛의 무녀, 이렇게 대조를 이루는 것도 괜찮지 않나요?」</i>  </size=33></font>"
    }
  ],
  "Performance": [
    {
      "Entry": "「소환 - 등장대사」",
      "Desc": "이게 소환이라는 거네요. 일단 의식을 끝내주세요. 아, 안지는 마세요!"
    },
    {
      "Entry": "「전투 - 전직1」",
      "Desc": "그리 뻔히 쳐다보지 마세요. 단세포 생물이 쳐다보는건 익숙치 않으니까요."
    },
    {
      "Entry": "「전투 - 전직2」",
      "Desc": "딱히 당신을 위해 필사적으로 힘을 기른건 아니야!"
    },
    {
      "Entry": "「전투 - 스킬시전1」",
      "Desc": "승부를 가릴 때 입니다!"
    },
    {
      "Entry": "「전투 - 스킬시전2」",
      "Desc": "지금입니다!"
    },
    {
      "Entry": "「전투 - 선택1」",
      "Desc": "응?"
    },
    {
      "Entry": "「전투 - 선택2」",
      "Desc": "타겟은?"
    },
    {
      "Entry": "「전투 - 전투불능」",
      "Desc": "방심했습니다... 미안해요."
    },
    {
      "Entry": "「대화 - 친밀도1」",
      "Desc": "미안합니다. 단세포 생물의 심리상태에는 흥미가 없습니다."
    },
    {
      "Entry": "「대화 - 친밀도2」",
      "Desc": "옷을 갈아 입는 도중에는 들어오지 마세요. 죽을 거예요."
    },
    {
      "Entry": "「대화 - 친밀도3」",
      "Desc": "건드리기 싫은 타인의 비밀을 무신경하게 듣다니 섬세함이 부족한 사람은 정말 곤란하네요."
    },
    {
      "Entry": "「대화 - 친밀도4」",
      "Desc": "여동생 말인가요? 단순하고 순수한 아이죠...그런데 쌍둥이 언니에게 여동생의 일을 묻는 것은 실례가 아닐까요?"
    },
    {
      "Entry": "「대화 - 친밀도5」",
      "Desc": "제 몸만 쳐다보면서 망상을 하는 건 아니겠지요? 만약 저에게 손가락 하나라도 댄다면 혀를 깨물겠어요!.....당신의 혀를 말이예요!"
    },
    {
      "Entry": "「대화 - 일상1」",
      "Desc": "가슴을 펴세요! 자, 빛과 어둠이 싸우는 이 시대에 종지부를 찍는 겁니다."
    },
    {
      "Entry": "「대화 - 기쁨」",
      "Desc": "동료를 고르라고...제가요? 잘생기고 신사적인 레온이 동행한다면 좋겠는데... 리아나? 음... 리아나는 괜찮습니다."
    },
    {
      "Entry": "「대화 - 분노」",
      "Desc": "위에서 쳐다보는 듯한 태도는 그만두세요! 당신은 이 공주의 하인에 불과합니다. 하인이면 하인다운 자각을 하세요!"
    },
    {
      "Entry": "「대화 - 슬픔」",
      "Desc": "저에겐 성검의 봉인을 푸는 힘이 있습니다. 어둠에서 눈을 떠 빛 속에서 각성합니다."
    },
    {
      "Entry": "「대화 - 수줍음」",
      "Desc": "후후... 저와 함께 있으면 아무것도 무서워할 필요가 없습니다."
    },
    {
      "Entry": "「상호작용 - 대사1」",
      "Desc": "앗, 빈틈을 찔렸습니다..."
    },
    {
      "Entry": "「상호작용 - 대사2」",
      "Desc": "안돼..."
    },
    {
      "Entry": "「상호작용 - 대사3」",
      "Desc": "난처해졌는 걸. 빠... 빨리... 그만둬..."
    },
    {
      "Entry": "「상호작용 - 대사4」",
      "Desc": "아, 싫어...."
    },
    {
      "Entry": "「상호작용 - 대사5」",
      "Desc": "우우...그...그만, 괴로워, 하아아...."
    },
    {
      "Entry": "「상호작용 - 호감1」",
      "Desc": "저, 정말 싫습니다. 하지만 당신에겐 많은 빚이 있어요...이건, 제가 원하는 것."
    },
    {
      "Entry": "「상호작용 - 호감2」",
      "Desc": "자...제 몸 속에 잠들어 있는 빛의 봉인을 풀어주세요...."
    },
    {
      "Entry": "「상호작용 - 반감1」",
      "Desc": "저에게 그런 말을 하다니 나중에 후회하지 마세요."
    },
    {
      "Entry": "「상호작용 - 반감2」",
      "Desc": "무슨!"
    },
    {
      "Entry": "「상호작용 - 반감3」",
      "Desc": "이...이번에는 용서해 주겠어!"
    },
    {
      "Entry": "「상호작용 - 특수1」",
      "Desc": "당신에 대한 평가를 조금 올려야겠네요!"
    },
    {
      "Entry": "「상호작용 - 특수2」",
      "Desc": "그럼 실례하겠습니다."
    }
  ],
  "Performance2": null,  
  "Misc": ""
}];