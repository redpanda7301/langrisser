var data = [
{
  "Skins": [
    {
      "ImageUrl": "레온하르트/1.webp",
      "Desc": "기본"
    },
    {
      "ImageUrl": "레온하르트/2.webp",
      "Desc": "빛의 메아리"
    }
  ],
  "Name": "레온하르트",
  "ChName": "莱恩哈特",
  "EngName": "Leonhardt",
  "JapName": "レオンハルト",
  "CV": "미도리카와 히카루",
  "Rank": [
    0
  ],
  "SP": false,
  "Groups": [
    2,
    3,
    9
  ],
  "HP": 20,
  "SPHP": 0,
  "Attk": 20,
  "SPAttk": 0,
  "Def": 25,
  "SPDef": 0,
  "MagicDef": 35,
  "SPMagicDef": 0,
  "HowToGet": "한정 콜라보 소환",
  "Fetters": [
    {
      "Condition": "레온하르트를 사용하여 시공의 균열3-6 클리어"
    },
    {
      "Condition": "레온하르트를 사용하여 시공의 균열[정예]3-1 클리어"
    },
    {
      "Condition": "레온하르트가 검제로 전직"
    },
    {
      "Condition": "레온하르트가 수라로 전직"
    }
  ],
  "HeartFetter": "생명+750, 방어+50, 마방+50",
  "Talent": [
    {
      "Name": "검의 수라",
      "Icon": "62.webp",
      "Abilities": [
        {
          "Desc": "1성"
        },
        {
          "Desc": "2성"
        },
        {
          "Desc": "주위 <font color=\"red\">3</font>칸 내 아군이 없을 시 생명 외 모든 속성 <font color=\"red\">+10%</font>.<br/> 1회 공격으로 <font color=\"red\">2</font>명 이상 적군에게 피해를 입힐 시<br/>생명 <font color=\"red\">20%</font>회복 <font color=\"red\">1</font>회 재행동 가능.<br/> (재행동 효과는 <font color=\"red\">4</font>턴 후 재발동 가능.)"
        },
        {
          "Desc": "주위 <font color=\"red\">2</font>칸 내 아군이 없을 시 생명 외 모든 속성 <font color=\"red\">+13%</font>.<br/> 1회 공격으로 <font color=\"red\">2</font>명 이상 적군에게 피해를 입힐 시<br/>생명 <font color=\"red\">30%</font>회복 <font color=\"red\">1</font>회 재행동 가능.<br/> (재행동 효과는 <font color=\"red\">3</font>턴 후 재발동 가능.)"
        },
        {
          "Desc": "주위 <font color=\"red\">2</font>칸 내 아군이 없을 시 생명 외 모든 속성 <font color=\"red\">+16%</font>.<br/> 1회 공격으로 <font color=\"red\">2</font>명 이상 적군에게 피해를 입힐 시<br/>생명 <font color=\"red\">40%</font>회복 <font color=\"red\">1</font>회 재행동 가능.<br/> (재행동 효과는 <font color=\"red\">3</font>턴 후 재발동 가능.)"
        },
        {
          "Desc": "주위 <font color=\"red\">1</font>칸 내 아군이 없을 시 생명 외 모든 속성 <font color=\"red\">+20%</font>.<br/> 1회 공격으로 <font color=\"red\">2</font>명 이상 적군에게 피해를 입힐 시<br/>생명 <font color=\"red\">50%</font>회복 <font color=\"red\">1</font>회 재행동 가능.<br/> (재행동 효과는 <font color=\"red\">2</font>턴 후 재발동 가능.)"
        }
      ]
    }
  ],
  "SPTalent": null,
  "Second_Awaken": null,
  "Equip": null,
  "Soldiers": [
    20,
    27,
    29,
    33,
    35,
    52,
    61
  ],
  "Jobs": [
    {
      "Name": "검제",
      "Icon": "Icon_Occupation_Infantryman.webp",
      "SD": [
        {
          "ImageUrl": "레온하르트/1/1.webp"
        },
        {
          "ImageUrl": "레온하르트/2/1.webp"
        }
      ],
      "AttkRange": 1,
      "Move": 3,
      "MoveType": "Walk",
      "HP": 4754,
      "HPRate": 79,
      "Attk": 549,
      "AttkRate": 84,
      "Int": 294,
      "IntRate": 49,
      "Def": 353,
      "DefRate": 78,
      "MagicDef": 301,
      "MagicDefRate": 67,
      "Skill": 116,
      "SkillRate": 33,
      "HeartFetterBouns1": "생명비율이 낮은상대와 전투중 받는피해 -10%",
      "HeartFetterBouns2": "생명비율이 낮은상대와 전투중 입히는피해 +10%"
    },
    {
      "Name": "수라",
      "Icon": "Icon_Occupation_Infantryman.webp",
      "SD": [
        {
          "ImageUrl": "레온하르트/1/2.webp"
        },
        {
          "ImageUrl": "레온하르트/2/1.webp"
        }
      ],
      "AttkRange": 1,
      "Move": 3,
      "MoveType": "Walk",
      "HP": 4448,
      "HPRate": 74,
      "Attk": 579,
      "AttkRate": 89,
      "Int": 294,
      "IntRate": 49,
      "Def": 327,
      "DefRate": 73,
      "MagicDef": 280,
      "MagicDefRate": 62,
      "Skill": 130,
      "SkillRate": 37,
      "HeartFetterBouns1": "생명이 80%이상일때 치명타를 받을확률 -10%",
      "HeartFetterBouns2": "생명이 80%이상일때 치명타확률 +10%"
    }
  ],
  "Materials": {
    "HeartFetter": [
      "h3",
      "h4",
      "h2",
      "h1",
      "h3-1",
      "h4-1",
      "h2-1",
      "h1-1",
      "x"
    ],
    "Fetter": [
      "2",
      "2-1",
      "2-2",
      "2-3",
      "2-4"
    ],
    "Awaken": [
      "ranka2",
      "ranka6",
      "ranka2-1",
      "ranka6-1"
    ]
  },
  "ClassUp": [
    {
      "ClassTitle": "파이터 : 공격+9, 생명+36",
      "Tiers": [
        {
          "Name": "T1",
          "Materials": [
            "rank1-1"
          ]
        },
        {
          "Name": "T2",
          "Materials": [
            "rank1-1"
          ]
        }
      ]
    },
    {
      "ClassTitle": "소드맨 : 생명+80, 기술+5",
      "Tiers": [
        {
          "Name": "T1",
          "Materials": [
            "rank2-1",
            "rank1-1",
            "rank1-2"
          ]
        },
        {
          "Name": "T2",
          "Materials": [
            "rank3-1",
            "rank3-6",
            "rank2-1",
            "rank2-2"
          ]
        },
        {
          "Name": "T3",
          "Materials": [
            "rank4-1",
            "rank4-2",
            "rank3-1",
            "rank3-6"
          ]
        }
      ]
    },
    {
      "ClassTitle": "검제 : 공격+25, 방어+10, 마방+10",
      "Tiers": [
        {
          "Name": "T1",
          "Materials": [
            "rank5-1",
            "rank5-6",
            "rank4-1",
            "rank4-2"
          ]
        },
        {
          "Name": "T2",
          "Materials": [
            "rank6-1",
            "rank6-2",
            "rank5-1",
            "rank5-6"
          ]
        },
        {
          "Name": "T3",
          "Materials": [
            "rank7-2",
            "rank7-1",
            "rank6-1",
            "rank6-2"
          ]
        },
        {
          "Name": "T4",
          "Materials": [
            "rank8-2",
            "rank8-1",
            "rank7-1",
            "rank7-7"
          ]
        }
      ]
    },
    {
      "ClassTitle": "소위 : 생명+80, 방어+5",
      "Tiers": [
        {
          "Name": "T1",
          "Materials": [
            "rank2-1",
            "rank1-1",
            "rank1-2"
          ]
        },
        {
          "Name": "T2",
          "Materials": [
            "rank3-1",
            "rank3-6",
            "rank2-1",
            "rank2-2"
          ]
        },
        {
          "Name": "T3",
          "Materials": [
            "rank4-1",
            "rank4-2",
            "rank3-1",
            "rank3-6"
          ]
        }
      ]
    },
    {
      "ClassTitle": "수라 : 생명+90, 공격+25, 방어+10",
      "Tiers": [
        {
          "Name": "T1",
          "Materials": [
            "rank5-1",
            "rank5-2",
            "rank4-1",
            "rank4-3"
          ]
        },
        {
          "Name": "T2",
          "Materials": [
            "rank6-1",
            "rank6-3",
            "rank5-1",
            "rank5-2"
          ]
        },
        {
          "Name": "T3",
          "Materials": [
            "rank7-2",
            "rank7-4",
            "rank6-1",
            "rank6-3"
          ]
        },
        {
          "Name": "T4",
          "Materials": [
            "rank8-1",
            "rank8-5",
            "rank7-2",
            "rank7-4"
          ]
        }
      ]
    }
  ],
  "SPOpenConditions": null,
  "Skills": [
    {
      "Name": "제로 스톰",
      "Cost": "COST 1",
      "Entries": [
        {
          "Label": "유형",
          "Value": "물리 피해"
        },
        {
          "Label": "쿨타임",
          "Value": "3턴"
        },
        {
          "Label": "사거리",
          "Value": "자신"
        },
        {
          "Label": "범위",
          "Value": "주위 1칸"
        }
      ],
      "IconImageUrl": "../img/skills/Skill_FireOnikiri.webp",
      "Desc": "[물리 피해]자신 주위 1칸 내 모든 적군에게 0.25배 범위 피해를 입힌다.<br/>모든 스킬 쿨타임 -1턴.",
      "SubSkills": null,
      "Animation": null
    },
    {
      "Name": "바람 칼날",
      "Cost": "COST 2",
      "Entries": [
        {
          "Label": "유형",
          "Value": "물리 피해"
        },
        {
          "Label": "쿨타임",
          "Value": "2턴"
        },
        {
          "Label": "사거리",
          "Value": "2칸"
        },
        {
          "Label": "범위",
          "Value": "단일"
        }
      ],
      "IconImageUrl": "../img/skills/0.webp",
      "Desc": "[물리 피해]단일 적 부대를 공격해 1.5배의 피해를 준다.<br/>근접전 병사를 이끌고 있다면 병사도 함께 공격한다.",
      "SubSkills": null,
      "Animation": null
    },
    {
      "Name": "간파",
      "Cost": "COST 1",
      "Entries": [
        {
          "Label": "유형",
          "Value": "패시브"
        }
      ],
      "IconImageUrl": "../img/skills/Passive_Cancel.webp",
      "Desc": "[패시브]치명타율 +10%. 피해를 줄 때 50%의 확률로 적 부대의 패시브 스킬을 무효화한다.<br/>2턴 지속, 치명타 발생 시 반드시 발동.",
      "SubSkills": null,
      "Animation": null
    },
    {
      "Name": "순간이동",
      "Cost": "COST 1",
      "Entries": [
        {
          "Label": "유형",
          "Value": "지원"
        },
        {
          "Label": "쿨타임",
          "Value": "3턴"
        },
        {
          "Label": "사거리",
          "Value": "자신"
        },
        {
          "Label": "범위",
          "Value": "단일"
        }
      ],
      "IconImageUrl": "../img/skills/Skill_Teleporting.webp",
      "Desc": "[지원]액티브 스킬, 자신을 순간이동하여 임의의 위치로 이동할 시<br/>이동력 +1, 자신 부대가 받는 피해 -30%,[검기]버프 시전:<br/>공격하여 전투 시 주위 1칸 내 모든 적군에게 0.1배 범위 피해를 입힌다. 1턴 지속.",
      "SubSkills": null,
      "Animation": null
    },
    {
      "Name": "귀염참",
      "Cost": "COST 2",
      "Entries": [
        {
          "Label": "유형",
          "Value": "물리 피해"
        },
        {
          "Label": "쿨타임",
          "Value": "3턴"
        },
        {
          "Label": "사거리",
          "Value": "5칸"
        },
        {
          "Label": "범위",
          "Value": "직선"
        }
      ],
      "IconImageUrl": "../img/skills/Skill_ZeroStorm.webp",
      "Desc": "[물리 피해]라인에 있는 모든 적군을 관통하며 0.25배 범위 피해를 입힌다.<br/>25%확률로 적에게 기절 시전, 1턴 지속.",
      "SubSkills": null,
      "Animation": null
    },
    {
      "Name": "태양 베기",
      "Cost": "COST 2",
      "Entries": [
        {
          "Label": "유형",
          "Value": "물리 피해"
        },
        {
          "Label": "쿨타임",
          "Value": "2턴"
        },
        {
          "Label": "사거리",
          "Value": "1칸"
        },
        {
          "Label": "범위",
          "Value": "단일"
        }
      ],
      "IconImageUrl": "../img/skills/1.webp",
      "Desc": "[물리 피해]단일 적 부대를 공격해 1.5배의 피해를 준다.<br/>전투 후 가한 피해량 30%만큼 생명을 회복한다.",
      "SubSkills": null,
      "Animation": null
    },
    {
      "Name": "사투",
      "Cost": "COST 1",
      "Entries": [
        {
          "Label": "유형",
          "Value": "패시브"
        }
      ],
      "IconImageUrl": "../img/skills/Passive_AtkBuf6.webp",
      "Desc": "전투 진입 시 부대 공격, 기술 <font color=\"red\">+12%</font>, 방어, 마방 <font color=\"red\">-7%</font>.",
      "SubSkills": null,
      "Animation": null
    },
    {
      "Name": "절기 명황검",
      "Cost": "COST 2",
      "Entries": [
        {
          "Label": "유형",
          "Value": "물리 피해"
        },
        {
          "Label": "쿨타임",
          "Value": "6턴"
        },
        {
          "Label": "사거리",
          "Value": "자신"
        },
        {
          "Label": "범위",
          "Value": "2바퀴"
        }
      ],
      "IconImageUrl": "../img/skills/Skill_ShadowSword.webp",
      "Desc": "[물리 피해]자신 주위 <font color=\"red\">2</font>칸 내 모든 적군에게 <font color=\"red\">0.3</font>배 범위 피해를 입힌다.<br/>적의 <font color=\"red\">2</font>개 버프를 제거, 적이 <font color=\"blue\">재행동 불가</font>, <br/><font color=\"red\">3</font>턴 지속.(제거 불가).",
      "SubSkills": null,
      "Animation": null
    },
    {
      "Name": "수라 분신",
      "Cost": "COST 3",
      "Entries": [
        {
          "Label": "유형",
          "Value": "물리피해"
        },
        {
          "Label": "쿨타임",
          "Value": "4턴"
        },
        {
          "Label": "사거리",
          "Value": "자신"
        },
        {
          "Label": "범위",
          "Value": "2바퀴"
        }
      ],
      "IconImageUrl": "../img/Super/레온하르트.webp",
      "Desc": "[물리피해]주위 <font color =\"red\">2</font>바퀴 내 모든 적에게 <font color =\"red\">0.2</font>배 피해를 준다.<br/><b><font color =\"red\">50</font>% 확률로 패시브 봉인</b> 지속 <font color =\"red\">2</font>행동<br/><b>공격 후 곁에 자신의 현재 스탯과 동일한 <font color =\"dark purple\">분신</font>소환</b><br/>분신은 '고유 스킬 : 주는 피해량<font color =\"red\">-30</font>%, 받는피해량 <font color =\"red\">+50</font>%',  '<font color =\"blue\">제로 스톰</font>' '<font color =\"blue\">귀염참</font>' '<font color =\"blue\">대역</font>' 스킬 보유<br/>'<font color =\"blue\">대역</font>' : <b>주변 <font color =\"red\">2</font>칸이내 소환자의 물리피해를 대신</b> 하여 공격받는다.<br/><b>분신이 사망후</b> 레온하르트 본인의 생명 <font color =\"red\">30</font>%회복 및 모든 스킬 쿨타임 <font color =\"red\">-1</font><br/><b>해당 분신 최대 <font color =\"red\">1</font>기 까지 보유가능</b>",
      "SubSkills": null,
      "Animation": "../img/Super/레온하르트_Ani.webp"
    }
  ],
  "Propose": false,
  "Bio": [
    {
      "Entry": "영웅소개",
      "Desc": "◆키/체중: 184cm / 75kg<br/>◆출연: 영웅전설·하늘의 궤적 시리즈<br/>◆소속: 우로보로스"
    },
    {
      "Entry": "영웅담1",
      "Desc": "애칭은 레베, '우로보로스' 결사의 집행자 No.II로, 코드 네임은 '검제'이다. 애쉬블론드의 청년으로 과묵한 성격을 지녔고, '외부의 이치'로 만든 장검 '케른바이터'를 사용한다. 압도적인 검기와 전투력으로 '검제'라는 칭호를 받았다.<br/><font color=\"#DC143C\"><size=33><i>「굳은 각오를 했다한들, 그에 걸맞는 실력이 없으면 아무 의미가 없어... 알겠나?」</i> </size=33></font>"
    },
    {
      "Entry": "영웅담2",
      "Desc": "하멜 마을 출신의 청년으로 '하멜의 비극'을 직접 경험했다. 당시 일생의 연인 카린을 잃고 절망한 그는 카린의 동생 요슈아와 함께 세상을 떠돌았다. 왕국 정부가 사건의 진실을 은폐했다는 사실에 분노한 레온하르트는 수라의 길을 걷기 시작하고 '우로보로스'의 집행자가 되었다. 제 7사도 '강철의 성녀'의 추천으로, 맹주에게 '케른바이터'를 선물받고 '검제'라는 코드 네임을 받았다."
    },
    {
      "Entry": "영웅담3",
      "Desc": "에스텔 일행의 모험에서 레온하르트는 '로렌스 소위'로 위장해 몰래 활동하며 리샤드 대령의 쿠데타를 감시 및 통제하고, 에스텔 일행과 여러 차례 맞붙었다. 결국 왕성에서 정체를 밝히고 압도적인 실력으로 에스텔 일행을 꺾었다. 집행자의 신분으로 다시 등장한 레온하르트는 '복음 계획'의 진행을 돕고, 붉은 방주에서 에스텔에게 하멜의 비극에 대한 진실을 알려주었다. 그가 결사에 들어간 목적은 세상에 이 사건을 알리고 같은 일이 벌어지는 것을 막기 위해서였다. <br/><font color=\"#DC143C\"><size=33><i>「내가 지켜볼 것이다... 희망의 날개가 꺾였을 때, 너희들은 어떤 절망을 느끼게 될까.」</i> </size=33></font>"
    },
    {
      "Entry": "영웅담4",
      "Desc": "레온하르트는 중추탑 꼭대기에서 요슈아의 도전을 받게 되고, 결국 모두의 합동 공격에 패한다. 그리고 상상에서나마 갈구하던 답을 찾게 된다. 대결 후 요슈아와 화해한 레베는 결사에서 나오려 했지만, 와이스맨의 기습을 받고 쓰러졌다. 이후 빛의 반지와 합체된 와이스맨에게 붙잡힌 요슈아를 구하기 위해, 레온하르트는 모든 힘을 쏟아부어 절대 장벽을 파괴했지만, 빛의 반지의 반격을 받고 사망한다..."
    },
    {
      "Entry": "영웅담5",
      "Desc": "압도적인 실력과 냉정한 성격 그대로 소환된 레온하르트는 엘사리아 대륙에서 답을 찾기 위한 여행을 시작한다. 그는 이곳에서... 답을 찾을 수 있을까?<br/><font color=\"#DC143C\"><size=33><i>「수라의 화신이 되려면, 모든 것을 버릴 각오가 필요하지. 이 세상에서 살아남으려면... 모든 분노와 슬픔을 잊어야 한다.」</i> </size=33></font>"
    }
  ],
  "Performance": [
    {
      "Entry": "「소환 - 등장대사」",
      "Desc": "집행자NO•II, 레온하르트다. 나를 검제라고 부르는 자도 있다."
    },
    {
      "Entry": "「전투 - 전직1」",
      "Desc": "넘은 건가―― 수라의 경계를."
    },
    {
      "Entry": "「전투 - 전직2」",
      "Desc": "인간에겐... 아직 가능성이 남아 있는 것인가?"
    },
    {
      "Entry": "「전투 - 스킬시전1」",
      "Desc": "날 방해하지 마라."
    },
    {
      "Entry": "「전투 - 스킬시전2」",
      "Desc": "연습은 여기까지만 해도 되겠지."
    },
    {
      "Entry": "「전투 - 선택1」",
      "Desc": "흥, 귀찮은 일은 맡겨도 좋다."
    },
    {
      "Entry": "「전투 - 선택2」",
      "Desc": "여신의 가호든, 뭐든 좋다."
    },
    {
      "Entry": "「전투 - 전투불능」",
      "Desc": "검이... 떨어진 건가..."
    },
    {
      "Entry": "「대화 - 친밀도1」",
      "Desc": "레베? 그 이름은 본의가 아니다.  주변인이 붙여준 애칭일 뿐이다."
    },
    {
      "Entry": "「대화 - 친밀도2」",
      "Desc": "검을 휘두르는 이유? 나는 그저 인간으로서의 모든 것을 버리고 그 수라에 몸을 던진 것뿐이다..."
    },
    {
      "Entry": "「대화 - 친밀도3」",
      "Desc": "수라와 하나가 되려면, 모든 것을 버릴 각오가 필요하다. 이 세상에서 살려면... 모든 분노와 슬픔을 잊어서는 안 된다."
    },
    {
      "Entry": "「대화 - 친밀도4」",
      "Desc": "확고한 각오가 있다 해도, 그에 걸맞는 실력이 없으면 아무 의미가 없다. 너라면... 알겠는가?"
    },
    {
      "Entry": "「대화 - 친밀도5」",
      "Desc": "미천한 인간을 구한다? 그런 결의를 가지고 있다면 너의 그 숭고한 의지에 경의를 표해야 할지도 모르겠군."
    },
    {
      "Entry": "「대화 - 일상1」",
      "Desc": "정면 대결이 있다면, 누구도 나를 능가하지 못할 것이다."
    },
    {
      "Entry": "「대화 - 기쁨」",
      "Desc": "하하... 이 마음은 이미 얼음보다 차가워지고 있는걸..."
    },
    {
      "Entry": "「대화 - 분노」",
      "Desc": "보고 싶구나, 사람은 과연 자신의 나약함과 기만의 속죄를 할지 안 할지!"
    },
    {
      "Entry": "「대화 - 슬픔」",
      "Desc": "이 몸이 연옥에 떨어진다 해도, 후회는 하지 않는다."
    },
    {
      "Entry": "「상호작용 - 대사1」",
      "Desc": "보여 주겠는가... 희망의 날개가 꺾였을 때, 너희들이 어떤 절망을 느끼게 될지."
    },
    {
      "Entry": "「상호작용 - 대사2」",
      "Desc": "진실은 사람에 따라 쉽게 변할 수 있다. 마치 달의 이지러짐이 사람마다 다르게 해석하듯 말이다."
    },
    {
      "Entry": "「상호작용 - 대사3」",
      "Desc": "물론 저 녀석들과 동료가 될 생각도 했었지. 하지만 나 같은 녀석에게는 그건 정말 아이러니한 일이지..."
    },
    {
      "Entry": "「상호작용 - 호감1」",
      "Desc": "훗, 말이 필요 없구나. 네 녀석의 힘을 인정한다."
    },
    {
      "Entry": "「상호작용 - 반감1」",
      "Desc": "인간은 이 세계에 하나의 작은 돌에 불과하지, 파도에 뒤척여지는 거와 같다."
    },
    {
      "Entry": "「상호작용 - 특수1」",
      "Desc": "자, 그럼."
    },
    {
      "Entry": "「상호작용 - 특수2」",
      "Desc": "그럼 다음에"
    }
  ],
  "Performance2": null,  
  "Misc": ""
}];