var data = [
    {        
        "Skins": [
            {
                "ImageUrl": "엘윈/1.png",
                "Desc": "기본"
            },
            {
                "ImageUrl": "엘윈/2.png",
                "Desc": "스킨상점"
            },
            {
                "ImageUrl": "엘윈/3.png",
                "Desc": "빛의 메아리"
            },
            {
                "ImageUrl": "엘윈/4.png",
                "Desc": "비경상점"
            },
            {
                "ImageUrl": "엘윈/5.png",
                "Desc": "빛의 메아리"
            },
            {
                "ImageUrl": "엘윈/sp.png",                
                "Desc": "SP 각성"
            },
            {
                "ImageUrl": "엘윈/6.png",
                "Desc": "한정판매"
            }
        ],       
        "Name": "엘윈",
        "CV": "호시 소이치로",
        "Rank": [ // 0 - SSR, 1 - SR, 2 - R, 3 - N
            0
        ], 
        "SP": true,
        "Groups": [
            0, // 주인공
            1, // 빛
            2  // 제국
        ],
        "HP": 15,
        "Attk": 35,
        "Def": 35,
        "MagicDef": 15,
        "HowToGet": "영웅 소환",
        "Fetters": [ // 유대 순서를 반드시 지킬 것
            {
                "Condition": "엘윈을 출전시켜 시공의 균열 3-6 클리어"
            },
            {
                "Condition": "엘윈을 출전시켜 시공의 균열 3-1[정예] 클리어"
            },
            {
                "Condition": "헤인의 도움 하에 '어둠의 길' 클리어"
            },
            {
                "Condition": "리아나의 도움 하에 '고독의 왕' 클리어"
            }
        ],
        "HeartFetter": "생명+900, 방어+60, 마방+40",
        "Talent": [
            {
                "Name": "용사의 의지",
                "Icon": "46.png",
                "Abilities": [ // 성급 순서를 반드시 지킬 것
                    {
                        "Desc": "1성"
                    },
                    {
                        "Desc": "2성"
                    },
                    {
                        "Desc": "공격하여 전투 진입 시 피해 <font color=\"red\">+10%</font>.<br> 전투 후 <font color=\"red\">50%</font>확률로 생명 회복,<br> 회복량은 영웅이 입힌 피해의 <font color=\"red\">30%</font>."
                    },
                    {
                        "Desc": "공격하여 전투 진입 시 피해 <font color=\"red\">+15%</font>.<br> 전투 후 <font color=\"red\">60%</font>확률로 생명 회복,<br> 회복량은 영웅이 입힌 피해의 <font color=\"red\">30%</font>."
                    },
                    {
                        "Desc": "공격하여 전투 진입 시 피해 <font color=\"red\">+20%</font>.<br> 전투 후 <font color=\"red\">80%</font>확률로 생명 회복,<br> 회복량은 영웅이 입힌 피해의 <font color=\"red\">30%</font>."
                    },
                    {
                        "Desc": "공격하여 전투 진입 시 피해 <font color=\"red\">+30%</font>.<br> 전투 후 <font color=\"red\">100%</font>확률로 생명 회복,<br> 회복량은 영웅이 입힌 피해의 <font color=\"red\">30%</font>."
                    }
                ]
            }
        ],
        "SPTalent": [
            {
                "Name": "광룡의 의지",
                "Icon": "SP_Gift_Elwin.png",
                "Abilities": [ // 성급 순서를 반드시 지킬 것
                    {
                        "Desc": "1성"
                    },
                    {
                        "Desc": "2성"
                    },
                    {
                        "Desc": "선공 시 피해량 및 방어 <font color=\"red\">+10%</font>,<br> 자신 주위 <font color=\"red\">2</font>칸 범위 내에 아군이 없을 경우 사망 방지 (최대 <font color=\"red\">1</font>회 발동)<br> 전투 진입 전 이동한 거리 <font color=\"red\">1</font>칸 당 <font color=\"red\">10%</font> 회복 <br> <font color=\"red\">4</font>칸 이상일 경우 선제공격."
                    },
                    {
                        "Desc": "선공 시 피해량 및 방어 <font color=\"red\">+15%</font>,<br> 자신 주위 <font color=\"red\">2</font>칸 범위 내에 아군이 없을 경우 사망 방지 (최대 <font color=\"red\">1</font>회 발동)<br> 전투 진입 전 이동한 거리 <font color=\"red\">1</font>칸 당 <font color=\"red\">13%</font> 회복 <br> <font color=\"red\">4</font>칸 이상일 경우 선제공격."
                    },
                    {
                        "Desc": "선공 시 피해량 및 방어 <font color=\"red\">+20%</font>,<br> 자신 주위 <font color=\"red\">2</font>칸 범위 내에 아군이 없을 경우 사망 방지 (최대 <font color=\"red\">1</font>회 발동)<br> 전투 진입 전 이동한 거리 <font color=\"red\">1</font>칸 당 <font color=\"red\">16%</font> 회복 <br> <font color=\"red\">4</font>칸 이상일 경우 선제공격."
                    },
                    {
                        "Desc": "선공 시 피해량 및 방어 <font color=\"red\">+25%</font>,<br> 자신 주위 <font color=\"red\">2</font>칸 범위 내에 아군이 없을 경우 사망 방지 (최대 <font color=\"red\">1</font>회 발동)<br> 전투 진입 전 이동한 거리 <font color=\"red\">1</font>칸 당 <font color=\"red\">20%</font> 회복 <br> <font color=\"red\">4</font>칸 이상일 경우 선제공격."
                    }
                ]
            }
        ],
        "Equip": {
            "Name": "방랑의 기사(무기)",
            "Stats": "생명 437,공격 107",
            "Icon": "엘윈.png",
            "Ability": "공격 +<font color=\"red\">10%</font></br>기술을 사용하여 피해를 입힐시, <font color=\"red\">30%</font> 확률로 해당 기술의 쿨타임 -5"
        },
        "Soldiers": [
            20,
            25,
            27,
            31,
            34,
            35,
            49,
            57,
            54
        ],
        "Jobs": [
            {
                "Name": "히어로",
                "Icon": "Icon_Occupation_Infantryman.png",
                "SD": [
                    {
                        "ImageUrl": "엘윈/1/1.png"
                    },
                    {
                        "ImageUrl": "엘윈/2/1.png"
                    },
                    {
                        "ImageUrl": "엘윈/3/1.png"
                    },
                    {
                        "ImageUrl": "엘윈/4/1.png"
                    },
                    {
                        "ImageUrl": "엘윈/5/1.png"
                    },
                    {
                        "ImageUrl": "엘윈/sp/1.png"
                    },
                    {
                        "ImageUrl": "엘윈/6/1.png"
                    }
                ],
                "AttkRange": 1,
                "Move": 3,
                "MoveType": "Walk", // "Walk, Ride, Fly, Water, FieldArmy"
                "HP": 4822,
                "HPRate": 80,
                "Attk": 573,
                "AttkRate": 88,
                "Int": 258,
                "IntRate": 43,
                "Def": 356,
                "DefRate": 79,
                "MagicDef": 270,
                "MagicDefRate": 60,
                "Skill": 154,
                "SkillRate": 44,
                "HeartFetterBouns1": "생명 80%이상일때<br>전투중 받는피해 -10%",
                "HeartFetterBouns2": "생명 80%이상일때<br>전투중 입히는피해 +10%"
            },
            {
                "Name": "광룡 기사단장",
                "Icon": "Icon_Occupation_CavalrySP.png",
                "SD": [
                    {
                        "ImageUrl": "엘윈/1/2.png"
                    },
                    {
                        "ImageUrl": "엘윈/2/2.png"
                    },
                    {
                        "ImageUrl": "엘윈/3/2.png"
                    },
                    {
                        "ImageUrl": "엘윈/4/2.png"
                    },
                    {
                        "ImageUrl": "엘윈/5/2.png"
                    },
                    {
                        "ImageUrl": "엘윈/sp/2.png"
                    },
                    {
                        "ImageUrl": "엘윈/6/2.png"
                    }
                ],
                "AttkRange": 1,
                "Move": 5,
                "MoveType": "Ride", // "Walk, Ride, Fly, Water, FieldArmy"
                "HP": 5066,
                "HPRate": 81,
                "Attk": 578,
                "AttkRate": 88,
                "Int": 235,
                "IntRate": 41,
                "Def": 340,
                "DefRate": 76,
                "MagicDef": 270,
                "MagicDefRate": 60,
                "Skill": 130,
                "SkillRate": 37,
                "HeartFetterBouns1": "선공으로 전투 진입 시<br>부대가 받는 피해 -10%",
                "HeartFetterBouns2": "부대 생명 70% 미만 시<br>피해 +10%"
            },
            {
                "Name": "마샬",
                "Icon": "Icon_Occupation_Marines.png",
                "SD": [
                    {
                        "ImageUrl": "엘윈/1/3.png"
                    },
                    {
                        "ImageUrl": "엘윈/2/3.png"
                    },
                    {
                        "ImageUrl": "엘윈/3/3.png"
                    },
                    {
                        "ImageUrl": "엘윈/4/3.png"
                    },
                    {
                        "ImageUrl": "엘윈/5/3.png"
                    },
                    {
                        "ImageUrl": "엘윈/sp/3.png"
                    },
                    {
                        "ImageUrl": "엘윈/6/3.png"
                    }
                ],
                "AttkRange": 1,
                "Move": 3,
                "MoveType": "Walk", // "Walk, Ride, Fly, Water, FieldArmy"
                "HP": 5129,
                "HPRate": 85,
                "Attk": 544,
                "AttkRate": 84,
                "Int": 235,
                "IntRate": 39,
                "Def": 378,
                "DefRate": 84,
                "MagicDef": 217,
                "MagicDefRate": 48,
                "Skill": 116,
                "SkillRate": 33,
                "HeartFetterBouns1": "생명비율이 낮은상대와<br>전투시 받는피해 -10%",
                "HeartFetterBouns2": "생명비율이 낮은상대와<br>전투시 입히는피해 +10%"
            }
        ],
        "Materials": {
            "HeartFetter": [
                "h3",
                "h4",
                "h2",
                "h1",
                "h3-1",
                "h4-1",
                "h2-1",
                "h1-1",
                "x",
            ],
            "Fetter": [
                "5",
                "5-1",
                "5-2",
                "5-3",
                "5-4"
            ],
            "Awaken": [
                "ranka2",
                "ranka5",
                "ranka2-1",
                "ranka5-1"
            ]
        },
        "ClassUp": [ // 2개씩 한묶음으로 표시됨.
            {
                "ClassTitle": "파이터 : 공격+9, 생명+36",
                "Tiers": [
                    {
                        "Name": "T1",
                        "Materials": [
                            "rank1-1"
                        ]
                    },
                    {
                        "Name": "T2",
                        "Materials": [
                            "rank1-1"
                        ]
                    }
                ]
            },
            {
                "ClassTitle": "하이랜더 : 생명+48, 공격+12",
                "Tiers": [
                    {
                        "Name": "T1",
                        "Materials": [
                            "rank2-3",
                            "rank1-3",
                            "rank1-1"
                        ]
                    },
                    {
                        "Name": "T2",
                        "Materials": [
                            "rank3-3",
                            "rank3-2",
                            "rank2-3",
                            "rank2-1"
                        ]
                    },
                    {
                        "Name": "T3",
                        "Materials": [
                            "rank4-6",
                            "rank4-1",
                            "rank3-3",
                            "rank3-2"
                        ]
                    }
                ]
            },
            {
                "ClassTitle": "소드맨 : 생명+80, 기술+5",
                "Tiers": [
                    {
                        "Name": "T1",
                        "Materials": [
                            "rank2-1",
                            "rank1-1",
                            "rank1-2"
                        ]
                    },
                    {
                        "Name": "T2",
                        "Materials": [
                            "rank3-1",
                            "rank3-6",
                            "rank2-1",
                            "rank2-2"
                        ]
                    },
                    {
                        "Name": "T3",
                        "Materials": [
                            "rank4-1",
                            "rank4-2",
                            "rank3-1",
                            "rank3-6"
                        ]
                    }
                ]
            },
            {
                "ClassTitle": "히어로 : 공격+25, 생명+90, 방어+10",
                "Tiers": [
                    {
                        "Name": "T1",
                        "Materials": [
                            "rank5-1",
                            "rank5-6",
                            "rank4-1",
                            "rank4-2"
                        ]
                    },
                    {
                        "Name": "T2",
                        "Materials": [
                            "rank6-1",
                            "rank6-2",
                            "rank5-1",
                            "rank5-6"
                        ]
                    },
                    {
                        "Name": "T3",
                        "Materials": [
                            "rank7-2",
                            "rank7-1",
                            "rank6-1",
                            "rank6-2"
                        ]
                    },
                    {
                        "Name": "T4",
                        "Materials": [
                            "rank8-1",
                            "rank8-5",
                            "rank7-2",
                            "rank7-4"
                        ]
                    }
                ]
            },
            {
                "ClassTitle": "제네럴 : 생명+80, 공격+7",
                "Tiers": [
                    {
                        "Name": "T1",
                        "Materials": [
                            "rank2-2",
                            "rank1-2",
                            "rank1-1"
                        ]
                    },
                    {
                        "Name": "T2",
                        "Materials": [
                            "rank3-6",
                            "rank3-1",
                            "rank2-2",
                            "rank2-1"
                        ]
                    },
                    {
                        "Name": "T3",
                        "Materials": [
                            "rank4-2",
                            "rank4-6",
                            "rank3-6",
                            "rank3-1"
                        ]
                    }
                ]
            },
            {
                "ClassTitle": "마샬 : 생명+150, 공격+15, 방어+10",
                "Tiers": [
                    {
                        "Name": "T1",
                        "Materials": [
                            "rank5-6",
                            "rank5-3",
                            "rank4-2",
                            "rank4-6"
                        ]
                    },
                    {
                        "Name": "T2",
                        "Materials": [
                            "rank6-2",
                            "rank6-1",
                            "rank5-6",
                            "rank5-3"
                        ]
                    },
                    {
                        "Name": "T3",
                        "Materials": [
                            "rank7-1",
                            "rank7-7",
                            "rank6-2",
                            "rank6-1"
                        ]
                    },
                    {
                        "Name": "T4",
                        "Materials": [
                            "rank8-2",
                            "rank8-1",
                            "rank7-1",
                            "rank7-7"
                        ]
                    }
                ]
            }
        ],
        "SPOpenConditions": [
            {
                "Name": "개방",
                "Materials": [
                    {
                        "MaterialImgUrl": "/item/AlterEgo_Elwin.png",
                        "Amount": 1
                    }
                ],
                "Desc": ""
            },
            {
                "Name": "1단계 재료",
                "Materials": [
                    {
                        "MaterialImgUrl": "/item/오락기.png",
                        "Amount": 40
                    },
                    {
                        "MaterialImgUrl": "/classup/rank8-1.png",
                        "Amount": 30
                    },
                    {
                        "MaterialImgUrl": "/classup/rank7-1.png",
                        "Amount": 40
                    },
                    {
                        "MaterialImgUrl": "/classup/rank6-1.png",
                        "Amount": 50
                    }
                ],
                "Desc": ""
            },
            {
                "Name": "1단계 미션",
                "Materials": [],
                "Desc": "엘윈으로 IF 스테이지 대륙 최강의 기사 클리어"
            },
            {
                "Name": "2단계 재료",
                "Materials": [
                    {
                        "MaterialImgUrl": "/item/오락기.png",
                        "Amount": 40
                    },
                    {
                        "MaterialImgUrl": "/item/쓰알열쇠.png",
                        "Amount": 30
                    },
                    {
                        "MaterialImgUrl": "/item/스알열쇠.png",
                        "Amount": 40
                    },
                    {
                        "MaterialImgUrl": "/item/알열쇠.png",
                        "Amount": 50
                    }
                ],
                "Desc": ""
            },
            {
                "Name": "2단계 미션",
                "Materials": [],
                "Desc": "방랑의 기사(무기) 레벨 50 달성"
            },
            {
                "Name": "3단계 재료",
                "Materials": [
                    {
                        "MaterialImgUrl": "/item/오락기.png",
                        "Amount": 40
                    },
                    {
                        "MaterialImgUrl": "/item/룬스톤.png",
                        "Amount": 2
                    }
                ],
                "Desc": ""
            },
            {
                "Name": "3단계 미션",
                "Materials": [],
                "Desc": "엘윈, 헤인으로 파티를 짜서 여신의 시련 - 화룡 파프니르 60레벨 클리어"
            },
            {
                "Name": "",
                "Materials": [],
                "Desc": "광룡 기사단장 전직 가능, 광룡의 의지(SP 고유기술)스킬 획득, SP 일러스트 획득"
            },
            {
                "Name": "4단계 재료",
                "Materials": [
                    {
                        "MaterialImgUrl": "/item/오락기.png",
                        "Amount": 40
                    },
                    {
                        "MaterialImgUrl": "/heart/h1.png",
                        "Amount": 60
                    },
                    {
                        "MaterialImgUrl": "/heart/h1-1.png",
                        "Amount": 30
                    },
                    {
                        "MaterialImgUrl": "/heart/x.png",
                        "Amount": 12
                    }
                ],
                "Desc": ""
            },
            {
                "Name": "4단계 미션",
                "Materials": [],
                "Desc": "엘윈, 헤인으로 파티를 짜서 여신의 시련 - 뇌룡 카르코사 65레벨 클리어"
            },
            {
                "Name": "5단계 재료",
                "Materials": [
                    {
                        "MaterialImgUrl": "/item/오락기.png",
                        "Amount": 40
                    },
                    {
                        "MaterialImgUrl": "/item/Awaken_MoonBrillance.png",
                        "Amount": 5
                    },
                    {
                        "MaterialImgUrl": "/item/Awaken_Stardust.png",
                        "Amount": 5
                    }
                ],
                "Desc": ""
            },
            {
                "Name": "5단계 미션",
                "Materials": [],
                "Desc": "엘윈 혼자서 형귀 헬스장 샘슨(기병) 70레벨 클리어"
            },
            {
                "Name": "6단계 재료",
                "Materials": [
                    {
                        "MaterialImgUrl": "/item/Item_MemoryEssence.png",
                        "Amount": 600
                    },
                    {
                        "MaterialImgUrl": "/stone/StoneA_Rider1.png",
                        "Amount": 5
                    },
                    {
                        "MaterialImgUrl": "/stone/StoneA_Rider2.png",
                        "Amount": 5
                    },
                    {
                        "MaterialImgUrl": "/stone/StoneA_Rider4.png",
                        "Amount": 5
                    }
                ],
                "Desc": ""
            },
            {
                "Name": "6단계 미션",
                "Materials": [],
                "Desc": "엘윈, 헤인으로 파티를 짜서 영원의 신전 - 스퀼라 70레벨 클리어<br />(쉐리 SP 업데이트 후 65레벨 발키리 스퀼라로 하향조정, 업데이트 당일 클라이언트 데이터 재확인 필요)"
            },
            {
                "Name": "",
                "Materials": [],
                "Desc": "SP 스탯, SP 스킬 2개 활성화, 유니콘 고용가능"
            }            
        ],
        "Skills": [
            {
                "Name": "태양 베기",
                "IsPassive": false,
                "Cost": 2,
                "Type": 1, // 0 - 해당사항 없음, 1 - 물리피해, 2 - 마법피해, 3 - 초절강화, 4 - 지원, 5 - 치료
                "CoolTime": 2, // 0 - 해당사항 없음
                "Distance": 1, // 0 - 자신, 1 ~ - 사거리
                "Range": 1, // 1 - 단일, 2~98 - 범위, 99 - 맵전체
                "IconImageUrl": "/img/skills/1.png",
                "Desc": "[물리 피해]단일 적 부대를 공격해 1.5배의 피해를 준다.</br>전투 후 가한 피해량 30%만큼 생명을 회복한다.",
                "Animation": null
            },
            {
                "Name": "간파",
                "IsPassive": true,
                "Cost": 1,
                "Type": 0, // 0 - 해당사항 없음, 1 - 물리피해, 2 - 마법피해, 3 - 초절강화, 4 - 지원, 5 - 치료
                "CoolTime": 0, // 0 - 해당사항 없음
                "Distance": 0, // 0 - 자신 / 해당사항 없음, 1 ~ - 사거리
                "Range": 0, // 0 - 해당사항 없음, 1 - 단일, 2~97 - 범위, 98 - 직선, 99 - 맵전체
                "IconImageUrl": "/img/skills/Passive_Cancel.png",
                "Desc": "[패시브]치명타율 +10%. 피해를 줄 때 50%의 확률로 적 부대의 패시브 스킬을 무효화한다.</br>2턴 지속, 치명타 발생 시 반드시 발동.",
                "Animation": null
            },
            {
                "Name": "힘의 외침",
                "IsPassive": false,
                "Cost": 2,
                "Type": 1, // 0 - 해당사항 없음, 1 - 물리피해, 2 - 마법피해, 3 - 초절강화, 4 - 지원, 5 - 치료
                "CoolTime": 2, // 0 - 해당사항 없음
                "Distance": 1, // 0 - 자신 / 해당사항 없음, 1 ~ - 사거리
                "Range": 1, // 0 - 해당사항 없음, 1 - 단일, 2~97 - 범위, 98 - 직선, 99 - 맵전체
                "IconImageUrl": "/img/skills/36.png",
                "Desc": "[물리 피해] 단일 적군을 공격하여 1.3배 피해를 준다.</br>전투 전 상대 버프 2개 제거.동시에 상대의 공격 및 지력 -20%, 1턴 지속.",
                "Animation": null
            },
            {
                "Name": "불멸의 휘광",
                "IsPassive": false,
                "Cost": 2,
                "Type": 3, // 0 - 해당사항 없음, 1 - 물리피해, 2 - 마법피해, 3 - 초절강화, 4 - 지원, 5 - 치료
                "CoolTime": 3, // 0 - 해당사항 없음
                "Distance": 0, // 0 - 자신 / 해당사항 없음, 1 ~ - 사거리
                "Range": 99, // 0 - 해당사항 없음, 1 - 단일, 2~97 - 범위, 98 - 직선, 99 - 맵전체
                "IconImageUrl": "/img/skills/SuperBuff_Light2.png",
                "Desc": "[초절강화] 액티브 스킬, 범위 내 모든 '빛의 군단' 아군의</br>공격, 방어 능력치가 크게 상승하고</br>생명이 80% 이상일 경우 전투 중 주는 피해량 +15%, 4턴 지속.</br>(이효과는 다른 초절강화 스킬과 중첩불가)",
                "Animation": null
            },
            {
                "Name": "검혼",
                "IsPassive": false,
                "Cost": 2,
                "Type": 1, // 0 - 해당사항 없음, 1 - 물리피해, 2 - 마법피해, 3 - 초절강화, 4 - 지원, 5 - 치료
                "CoolTime": 5, // 0 - 해당사항 없음
                "Distance": 1, // 0 - 자신 / 해당사항 없음, 1 ~ - 사거리
                "Range": 1, // 0 - 해당사항 없음, 1 - 단일, 2~97 - 범위, 98 - 직선, 99 - 맵전체
                "IconImageUrl": "/img/skills/Skill_SwordSoul.png",
                "Desc": "[물리 피해] 단일 적군을 공격하여 1.8배 피해를 준다.</br>전투 전 적 부대의 버프 5개 제거 및 적 부대에게 '치료 방해' 부여, 2턴 지속</br>이 효과는 제거되지 않는다.",
                "Animation": null
            },
            {
                "Name": "장갑 파괴",
                "IsPassive": true,
                "Cost": 1,
                "Type": 0, // 0 - 해당사항 없음, 1 - 물리피해, 2 - 마법피해, 3 - 초절강화, 4 - 지원, 5 - 치료
                "CoolTime": 0, // 0 - 해당사항 없음
                "Distance": 0, // 0 - 자신 / 해당사항 없음, 1 ~ - 사거리
                "Range": 0, // 0 - 해당사항 없음, 1 - 단일, 2~97 - 범위, 98 - 직선, 99 - 맵전체
                "IconImageUrl": "/img/skills/Passive_BreakDef.png",
                "Desc": "[패시브]전투 진입전 50% 확률로 적의 방어 -20%, 1턴 지속.",
                "Animation": null
            },
            {
                "Name": "미늘",
                "IsPassive": true,
                "Cost": 1,
                "Type": 0, // 0 - 해당사항 없음, 1 - 물리피해, 2 - 마법피해, 3 - 초절강화, 4 - 지원, 5 - 치료
                "CoolTime": 0, // 0 - 해당사항 없음
                "Distance": 0, // 0 - 자신 / 해당사항 없음, 1 ~ - 사거리
                "Range": 0, // 0 - 해당사항 없음, 1 - 단일, 2~97 - 범위, 98 - 직선, 99 - 맵전체
                "IconImageUrl": "/img/skills/Passive_Barb.png",
                "Desc": "[패시브]근접 공격을 받아 전투 진입시 전투 후 부대 생명이 50%이상이라면 [미늘]을 발동한다:</br>전투 종료 후 적 부대에게 영웅 방어 2.5배만큼의 [고정 피해]를 준다.",
                "Animation": null
            },
            {
                "Name": "정면일격",
                "IsPassive": false,
                "Cost": 2,
                "Type": 1, // 0 - 해당사항 없음, 1 - 물리피해, 2 - 마법피해, 3 - 초절강화, 4 - 지원, 5 - 치료
                "CoolTime": 5, // 0 - 해당사항 없음
                "Distance": 1, // 0 - 자신 / 해당사항 없음, 1 ~ - 사거리
                "Range": 1, // 0 - 해당사항 없음, 1 - 단일, 2~97 - 범위, 98 - 직선, 99 - 맵전체
                "IconImageUrl": "/img/skills/Skill_Support.png",
                "Desc": "[물리 피해] 단일 적군을 공격하여 1.7배 피해를 준다.</br>전투 전 공격의 20%가 방어에 추가되며, 적 부대에게 '모든 패시브 스킬을 무효화한다'부여,2턴 지속,</br>이 효과는 제거되지 않는다.",
                "Animation": null
            },
            {
                "Name": "제압",
                "IsPassive": true,
                "Cost": 1,
                "Type": 0, // 0 - 해당사항 없음, 1 - 물리피해, 2 - 마법피해, 3 - 초절강화, 4 - 지원, 5 - 치료
                "CoolTime": 0, // 0 - 해당사항 없음
                "Distance": 0, // 0 - 자신 / 해당사항 없음, 1 ~ - 사거리
                "Range": 0, // 0 - 해당사항 없음, 1 - 단일, 2~97 - 범위, 98 - 직선, 99 - 맵전체
                "IconImageUrl": "/img/skills/Passive_AtkBuf1.png",
                "Desc": "[패시브]부대의 남은 생명 비율이 상대보다 높을 경우 전투 중 공격 +12%.",
                "Animation": null
            },
            {
                "Name": "SP : 빛의 급습",
                "IsPassive": false,
                "Cost": 2,
                "Type": 4, // 0 - 해당사항 없음, 1 - 물리피해, 2 - 마법피해, 3 - 초절강화, 4 - 지원, 5 - 치료
                "CoolTime": 4, // 0 - 해당사항 없음
                "Distance": 0, // 0 - 자신 / 해당사항 없음, 1 ~ - 사거리
                "Range": 1, // 0 - 해당사항 없음, 1 - 단일, 2~97 - 범위, 98 - 직선, 99 - 맵전체
                "IconImageUrl": "/img/skills/sp/엘윈/1.png",
                "Desc": "[액티브] 자신에게 '전설의 빛' 효과 부여 :</br>엘윈 전용 장비 효과 발동 확률을 <font color=\"red\">100%</font>로 조절, 지속 1행동 및 '면역' 효과, 지속 2행동,</br>사용 후 재행동 <font color=\"red\">3</font>칸 가능,부대가 혼합 병종이 아닐 경우 <font color=\"red\">4</font>칸 재행동</br>(해당 스킬은 버프 지속 시간을 감소시키지 않음)",
                "Animation": null
            },
            {
                "Name": "SP : 고독한 강자",
                "IsPassive": true,
                "Cost": 1,
                "Type": 0, // 0 - 해당사항 없음, 1 - 물리피해, 2 - 마법피해, 3 - 초절강화, 4 - 지원, 5 - 치료
                "CoolTime": 0, // 0 - 해당사항 없음
                "Distance": 0, // 0 - 자신 / 해당사항 없음, 1 ~ - 사거리
                "Range": 0, // 0 - 해당사항 없음, 1 - 단일, 2~97 - 범위, 98 - 직선, 99 - 맵전체
                "IconImageUrl": "/img/skills/sp/엘윈/2.png",
                "Desc": "[패시브]아군 퇴각 시 부대 생명 <font color=\"red\">20%</font> 회복 및 고독한 강자 효과 획득.<br>피격으로 전투 진입 전 생명 회복 <font color=\"red\">30%</font>,<br>전투 후 피해량 <font color=\"red\">30%</font> 수치만큼 생명 회복, 지속 <font color=\"red\">1</font>행동",
                "Animation": null
            }
        ],
        "Bio": [
            {
                "Entry": "영웅소개",
                "Desc": "◆키/몸무게：176cm / 70kg<br>◆출연：랑그릿사II<br>◆소속：빛의 군단·빛의 후예"
            },
            {
                "Entry": "영웅담1",
                "Desc": "대륙에서 가장 유명한 전설 속 영웅 중 한 명이자 몰락한 빛의 후예.어릴 때 부모를 잃고 홀로 모험을 떠난다. 그러다 우연한 기회에 견습 마법사 헤인을 만나 빛의 무녀 리아나를 구출하는 것으로 그의 영웅 서사시가 시작되었다.<br><font color=#DC143C><size =33><i>「누구든 어떠한 좌절을 겪어도 다시 일어날 수 있어. 나 역시 그래.」</i> </size> </font>"
            },
            {
                "Entry": "영웅담2",
                "Desc": "정직하고 명예를 중시하나 때때로 충동적이다. 거창한 업적 같은 건 생각해 본 적도 없다. 벨제리아에서 양부 드렌의 원수인 파이어스를 죽인 후 동료들은 그의 혈통이 아닌 자신을 믿기에 같이 싸워왔다는 것을 깨닫는다."
            },
            {
                "Entry": "영웅담3",
                "Desc": "빛의 군단을 모아 어둠을 물리친 엘윈의 전설은 수백 년 동안 음유시인들에 의해 전해져 왔다. 하지만 다소 이상한 점이 있는데 바로 이야기의 끝이 전하는 사람마다 다르다는 것이다. 누군가는 그가 어둠의 세력을 쓰러뜨렸다 하고 누군가는 그가 제국의 지도자가 되었다고 말했으며 다른 누군가는 그가 마지막에 모든 적과 친구마저 쓰러뜨리고 고독한 왕이 되었다고 말한다."
            },
            {
                "Entry": "영웅담4",
                "Desc": "제시카의 기록을 보면 엘윈에 대해 이렇게 적어 놓았다.<br><font color=#DC143C><size =33><i>「빛과 어둠의 장렬한 싸움이 끝나고 엘윈과 리아나는 보다 평화로운 세상을 만들기 위해 곳곳을 돌아다녔다.」</i> </size> </font>"
            },
            {
                "Entry": "영웅담5",
                "Desc": "엘윈은 성검의 영령이 되었으며 성검의 힘으로 자신이 품은 여러 가지 가능성을 살펴볼 수 있었다.<br><font color=#DC143C><size =33><i>「어쩌면 이 세계에 수많은 내가 있었을지도 모르지. 신기하지 않아?」</i> </size> </font>"
            }
        ],
        "Performance": [
            {
                "Entry": "「소환 - 등장대사」",
                "Desc": "내 이름은 엘윈, 검사 엘윈이라고 해. 이 세계를 반드시 구해내겠어...."
            },
            {
                "Entry": "「전투 - 전직1」",
                "Desc": "좋아, 흥미가 생겼어, 어디까지 갈 수 있는지 보자고!"
            },
            {
                "Entry": "「전투 - 전직2」",
                "Desc": "후회하지 않을 선택을 하는거야. 나는 언제나 모두의 행복을 지키고 싶어."
            },
            {
                "Entry": "「전투 - 스킬시전1」",
                "Desc": "전부 나에게 맡겨!"
            },
            {
                "Entry": "「전투 - 스킬시전2」",
                "Desc": "느려!"
            },
            {
                "Entry": "「전투 - 선택1」",
                "Desc": "전력으로 간다!"
            },
            {
                "Entry": "「전투 - 선택2」",
                "Desc": "예!"
            },
            {
                "Entry": "「전투 - 전투불능」",
                "Desc": "지키지... 못하는 건가...."
            },
            {
                "Entry": "「대화 - 친밀도1」",
                "Desc": "너에 대한 믿음이 잘못된 게 아니길 바래."
            },
            {
                "Entry": "「대화 - 친밀도2」",
                "Desc": "왜 정면으로 적과 맞서지 않는거야? 설마 자신 없는 거야?"
            },
            {
                "Entry": "「대화 - 친밀도3」",
                "Desc": "또 다른 가능성? 그래... 패왕으로 사는 것도 나쁘지 않다 생각한 적은 있었지만... 그래도..."
            },
            {
                "Entry": "「대화 - 친밀도4」",
                "Desc": "역시 현재가 즐거워. 나는 모두의 행복을 지킬 수 있다면 그걸로 충분해."
            },
            {
                "Entry": "「대화 - 친밀도5」",
                "Desc": "당신의 지시가 제 여행의 목적지입니다. 제 검은 언제나 당신과 함께할 겁니다. 이건 당신과 저 사이의 약속입니다."
            },
            {
                "Entry": "「대화 - 일상1」",
                "Desc": "성검의 힘만이 아니야. 악을 쓰러뜨린 것은, 모두의 힘을 합쳤기 때문이야!"
            },
            {
                "Entry": "「대화 - 일상1」",
                "Desc": "성검의 힘만이 아니야. 악을 쓰러뜨린 것은, 모두의 힘을 합쳤기 때문이야!"
            },
            {
                "Entry": "「대화 - 기쁨」",
                "Desc": "그 말은... 수백년 동안 사람들이 행복하고 풍요로운 삶을 살 수 있었다는 거지? 내게 있어 최고의 보상이군!"
            },
            {
                "Entry": "「대화 - 분노」",
                "Desc": "목숨 마저도 버릴 전투... 그것이 마검의 힘에 대항 할 때다!"
            },
            {
                "Entry": "「대화 - 슬픔」",
                "Desc": "분노와 증오만으로 싸우는 건 소용없어. 아무것도 바뀌지 않아. 냉정함을 잃는다면 이길 수 있는 싸움도 이길 수 없어."
            },
            {
                "Entry": "「상호작용 - 대사1」",
                "Desc": "아아! 계속하자고!"
            },
            {
                "Entry": "「상호작용 - 대사2」",
                "Desc": "응?"
            },
            {
                "Entry": "「상호작용 - 대사3」",
                "Desc": "상쾌했어!"
            },
            {
                "Entry": "「상호작용 - 대사4」",
                "Desc": "하핫!"
            },
            {
                "Entry": "「상호작용 - 대사5」",
                "Desc": "효과적인 치료 방법이네! 고마워!"
            },
            {
                "Entry": "「상호작용 - 반감1」",
                "Desc": "우리 사이에 불쾌한 감정따윈 없었으면 좋겠어."
            },
            {
                "Entry": "「상호작용 - 반감2」",
                "Desc": "핫!"
            },
            {
                "Entry": "「상호작용 - 반감3」",
                "Desc": "그만둬! 실례잖아!"
            },
            {
                "Entry": "「상호작용 - 특수1」",
                "Desc": "우리들의 우정이 더욱 깊어진 것 같은 기분이 드는걸!"
            },
            {
                "Entry": "「상호작용 - 특수2」",
                "Desc": "앞으로도 잘 부탁해!"
            }
        ]
    }
];