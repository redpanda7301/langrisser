var data = [
{
  "Skins": [
    {
      "ImageUrl": "유스케/1.webp",
      "Desc": "기본"
    },
    {
      "ImageUrl": "유스케/2.webp",
      "Desc": "빛의 메아리"
    }
  ],
  "Name": "우라메시 유스케",
  "CV": "사사키 노조무",
  "Rank": [
    0
  ],
  "SP": false,
  "Groups": [
    0,
    10,
    9
  ],
  "HP": 15,
  "SPHP": 0,
  "Attk": 35,
  "SPAttk": 0,
  "Def": 35,
  "SPDef": 0,
  "MagicDef": 15,
  "SPMagicDef": 0,
  "HowToGet": "한정 콜라보 소환",
  "Fetters": [
    {
      "Condition": "우라메시 유스케가 레벨30에 도달"
    },
    {
      "Condition": "우라메시 유스케를 출전시켜 시공의 균열 3-1[정예] 클리어"
    },
    {
      "Condition": "우라메시 유스케를 영계탐정으로 전직"
    },
    {
      "Condition": "우라메시 유스케를 마계의 왕으로 전직"
    }
  ],
  "HeartFetter": "생명+700, 방어+60, 마방+60",
  "Talent": [
    {
      "Name": "동료의 힘",
      "Icon": "76.webp",
      "Abilities": [
        {
          "Desc": "1성"
        },
        {
          "Desc": "2성"
        },
        {
          "Desc": "주위 <font color=\"red\">2</font>칸 내 아군이 있을 경우 공격 방어 <font color=\"red\">+5%</font>.<br/> 아군 사망 시 생명 <font color=\"red\">30%</font>회복 동시에 [분노] 발동 <br/>(분노: 생명 외 모든 속성<font color=\"red\">+10%</font> 이동력 <font color=\"red\">+1</font> 공격 사거리 <font color=\"red\">+1</font>) (<font color=\"red\">1</font>턴 지속) (제거 불가)"
        },
        {
          "Desc": "주위 <font color=\"red\">2</font>칸 내 아군이 있을 경우 공격 방어 <font color=\"red\">+8%</font>.<br/> 아군 사망 시 생명 <font color=\"red\">30%</font>회복 동시에 [분노] 발동 <br/>(분노: 생명 외 모든 속성<font color=\"red\">+10%</font> 이동력 <font color=\"red\">+1</font> 공격 사거리 <font color=\"red\">+1</font>) (<font color=\"red\">2</font>턴 지속) (제거 불가)"
        },
        {
          "Desc": "주위 <font color=\"red\">2</font>칸 내 아군이 있을 경우 공격 방어 <font color=\"red\">+11%</font>.<br/> 아군 사망 시 생명 <font color=\"red\">30%</font>회복 동시에 [분노] 발동 <br/>(분노: 생명 외 모든 속성<font color=\"red\">+10%</font> 이동력 <font color=\"red\">+1</font> 공격 사거리 <font color=\"red\">+1</font>) (<font color=\"red\">2</font>턴 지속) (제거 불가)"
        },
        {
          "Desc": "주위 <font color=\"red\">2</font>칸 내 아군이 있을 경우 공격 방어 <font color=\"red\">+15%</font>.<br/> 아군 사망 시 생명 <font color=\"red\">30%</font>회복 동시에 [분노] 발동 <br/>(분노: 생명 외 모든 속성<font color=\"red\">+10%</font> 이동력 <font color=\"red\">+1</font> 공격 사거리 <font color=\"red\">+1</font>) (<font color=\"red\">3</font>턴 지속) (제거 불가)"
        }
      ]
    }
  ],
  "SPTalent": null,
  "Equip": null,
  "Soldiers": [
    3,
    4,
    31,
    32,
    36,
    63,
    64
  ],
  "Jobs": [
    {
      "Name": "영계탐정",
      "Icon": "Icon_Occupation_Monk.webp",
      "SD": [
        {
          "ImageUrl": "유스케/1/1.webp"
        },
        {
          "ImageUrl": "유스케/2/1.webp"
        }
      ],
      "AttkRange": 1,
      "Move": 3,
      "MoveType": "Walk",
      "HP": 3788,
      "HPRate": 63,
      "Attk": 557,
      "AttkRate": 86,
      "Int": 258,
      "IntRate": 43,
      "Def": 319,
      "DefRate": 71,
      "MagicDef": 351,
      "MagicDefRate": 78,
      "Skill": 165,
      "SkillRate": 47,
      "HeartFetterBouns1": "생명 비율이 높은 상대에게 공격받아 전투진입시 받는피해 -10%",
      "HeartFetterBouns2": "생명 비율이 높은 상대에게 공격하여 전투진입시 입히는피해 +10%"
    },
    {
      "Name": "마계의 왕",
      "Icon": "Icon_Occupation_Monster.webp",
      "SD": [
        {
          "ImageUrl": "유스케/1/2.webp"
        },
        {
          "ImageUrl": "유스케/2/1.webp"
        }
      ],
      "AttkRange": 1,
      "Move": 3,
      "MoveType": "Walk",
      "HP": 4177,
      "HPRate": 70,
      "Attk": 587,
      "AttkRate": 90,
      "Int": 258,
      "IntRate": 43,
      "Def": 319,
      "DefRate": 71,
      "MagicDef": 327,
      "MagicDefRate": 73,
      "Skill": 165,
      "SkillRate": 47,
      "HeartFetterBouns1": "공격을 받아 전투진입시 받는마법피해 -10%",
      "HeartFetterBouns2": "생명이 70%이하일떄 전투중 입히는피해 +10%"
    }
  ],
  "Materials": {
    "HeartFetter": [
      "h4",
      "h1",
      "h3",
      "h2",
      "h4-1",
      "h1-1",
      "h3-1",
      "h2-1",
      "x"
    ],
    "Fetter": [
      "4",
      "4-1",
      "4-2",
      "4-3",
      "4-4"
    ],
    "Awaken": [
      "ranka2",
      "ranka5",
      "ranka2-1",
      "ranka5-1"
    ]
  },
  "ClassUp": [
    {
      "ClassTitle": "불량소년 : 공격+9, 생명+36",
      "Tiers": [
        {
          "Name": "T1",
          "Materials": [
            "rank1-1"
          ]
        },
        {
          "Name": "T2",
          "Materials": [
            "rank1-1"
          ]
        }
      ]
    },
    {
      "ClassTitle": "영능력 소년 : 생명+80, 기술+5",
      "Tiers": [
        {
          "Name": "T1",
          "Materials": [
            "rank2-6",
            "rank1-1",
            "rank1-4"
          ]
        },
        {
          "Name": "T2",
          "Materials": [
            "rank3-3",
            "rank3-6",
            "rank2-3",
            "rank2-5"
          ]
        },
        {
          "Name": "T3",
          "Materials": [
            "rank4-5",
            "rank4-6",
            "rank3-3",
            "rank3-6"
          ]
        }
      ]
    },
    {
      "ClassTitle": "영계탐정 : 공격+25, 마방+10, 방어+10",
      "Tiers": [
        {
          "Name": "T1",
          "Materials": [
            "rank5-3",
            "rank5-4",
            "rank4-5",
            "rank4-2"
          ]
        },
        {
          "Name": "T2",
          "Materials": [
            "rank6-8",
            "rank6-7",
            "rank5-3",
            "rank5-4"
          ]
        },
        {
          "Name": "T3",
          "Materials": [
            "rank7-5",
            "rank7-7",
            "rank6-8",
            "rank6-7"
          ]
        },
        {
          "Name": "T4",
          "Materials": [
            "rank8-6",
            "rank8-7",
            "rank7-5",
            "rank7-4"
          ]
        }
      ]
    },
    {
      "ClassTitle": "마족의 후예 : 생명+80, 기술+5",
      "Tiers": [
        {
          "Name": "T1",
          "Materials": [
            "rank2-1",
            "rank1-1",
            "rank1-2"
          ]
        },
        {
          "Name": "T2",
          "Materials": [
            "rank3-1",
            "rank3-6",
            "rank2-1",
            "rank2-2"
          ]
        },
        {
          "Name": "T3",
          "Materials": [
            "rank4-1",
            "rank4-2",
            "rank3-1",
            "rank3-6"
          ]
        }
      ]
    },
    {
      "ClassTitle": "마계의 왕 : 공격+25, 마방+10, 방어+10",
      "Tiers": [
        {
          "Name": "T1",
          "Materials": [
            "rank5-1",
            "rank5-2",
            "rank4-1",
            "rank4-3"
          ]
        },
        {
          "Name": "T2",
          "Materials": [
            "rank6-1",
            "rank6-3",
            "rank5-1",
            "rank5-2"
          ]
        },
        {
          "Name": "T3",
          "Materials": [
            "rank7-2",
            "rank7-4",
            "rank6-1",
            "rank6-3"
          ]
        },
        {
          "Name": "T4",
          "Materials": [
            "rank8-1",
            "rank8-5",
            "rank7-2",
            "rank7-4"
          ]
        }
      ]
    }
  ],
  "SPOpenConditions": null,
  "Skills": [
    {
      "Name": "영력 폭발",
      "Cost": "COST 1",
      "Entries": [
        {
          "Label": "유형",
          "Value": "변신"
        },
        {
          "Label": "쿨타임",
          "Value": "5턴"
        },
        {
          "Label": "사거리",
          "Value": "자신"
        },
        {
          "Label": "범위",
          "Value": "3칸"
        }
      ],
      "IconImageUrl": "https://storage.googleapis.com/langmosheet-data/img/skills/Skill_Yuusuke1.webp",
      "Desc": "[패시브] 피해를 입을 경우, [폭발] 발동 <br/>(폭발: 이동력 <font color=\"red\">+1</font>, 공격하여 전투 진입 시, 주는 피해량 <font color=\"red\">+30%</font>) (1턴 지속)<br/>[액티브] 생명 <font color=\"red\">30%</font> 회복. [마인] 발동 (마인: [폭발] 발동) <br/>(폭발: 전투 후 생명을 입힌 피해량의 <font color=\"red\">30%</font>만큼 회복. )(<font color=\"red\">3</font>턴)<br/>효과 종료 시, 주는 피해량 <font color=\"red\">-30%</font>, 이동력 <font color=\"red\">-1</font> (<font color=\"red\">2</font>턴 지속) (제거 불가, 면역 불가)",
      "SubSkills": null,
      "Animation": null
    },
    {
      "Name": "영환",
      "Cost": "COST 2",
      "Entries": [
        {
          "Label": "유형",
          "Value": "물리 피해"
        },
        {
          "Label": "쿨타임",
          "Value": "3턴"
        },
        {
          "Label": "사거리",
          "Value": "2칸"
        },
        {
          "Label": "범위",
          "Value": "단일"
        }
      ],
      "IconImageUrl": "https://storage.googleapis.com/langmosheet-data/img/skills/Skill_Yuusuke2.webp",
      "Desc": "단일 적 부대를 공격해 <font color=\"red\">1.3</font>배의 피해를 입힌다.<br/>근접전 병사를 이끌고 있다면 병사도 함께 공격한다.<br/>[마인] 발동 시, 스킬 사거리<font color=\"red\">+1</font>, 주는 피해량 <font color=\"red\">+30%</font>,<br/>적을 쓰러트릴 경우 해당 스킬 쿨타임<font color=\"red\">-2</font>.",
      "SubSkills": null,
      "Animation": null
    },
    {
      "Name": "모두의 의지",
      "Cost": "COST 1",
      "Entries": [
        {
          "Label": "유형",
          "Value": "패시브"
        }
      ],
      "IconImageUrl": "https://storage.googleapis.com/langmosheet-data/img/skills/Skill_Solidarity.webp",
      "Desc": "[패시브] 주위 <font color=\"red\">2</font>칸 내 아군이 <font color=\"red\">1</font>명씩 있을 경우, 방어, 마방 <font color=\"red\">+5%</font>, 최대 <font color=\"red\">+15%</font>.",
      "SubSkills": null,
      "Animation": null
    },
    {
      "Name": "심판",
      "Cost": "COST 2",
      "Entries": [
        {
          "Label": "유형",
          "Value": "물리 피해"
        },
        {
          "Label": "쿨타임",
          "Value": "2턴"
        },
        {
          "Label": "사거리",
          "Value": "1칸"
        },
        {
          "Label": "범위",
          "Value": "단일"
        }
      ],
      "IconImageUrl": "https://storage.googleapis.com/langmosheet-data/img/skills/Skill_Trial.webp",
      "Desc": "[물리 피해] 단일 적 부대를 공격해 <font color=\"red\">1.2배</font>의 피해를 준다.<br/>전투 진입 전 자신 부대의 방어, 마방 <font color=\"red\">+30%</font>, <font color=\"red\">2턴</font> 지속.",
      "SubSkills": null,
      "Animation": null
    },
    {
      "Name": "혈전",
      "Cost": "COST 1",
      "Entries": [
        {
          "Label": "유형",
          "Value": "패시브"
        }
      ],
      "IconImageUrl": "https://storage.googleapis.com/langmosheet-data/img/skills/Passive_AtkBuf3.webp",
      "Desc": "[패시브]부대의 남은 생명 비율이 상대보다 낮을 경우 전투 진입 시 공격, 방어 <font color=\"red\">+10%</font>.",
      "SubSkills": null,
      "Animation": null
    },
    {
      "Name": "백열권",
      "Cost": "COST 2",
      "Entries": [
        {
          "Label": "유형",
          "Value": "물리 피해"
        },
        {
          "Label": "쿨타임",
          "Value": "3턴"
        },
        {
          "Label": "사거리",
          "Value": "1칸"
        },
        {
          "Label": "범위",
          "Value": "단일"
        }
      ],
      "IconImageUrl": "https://storage.googleapis.com/langmosheet-data/img/skills/Skill_Yuusuke3.webp",
      "Desc": "[물리 피해] 단일 적 부대를 공격해 <font color=\"red\">1.3</font>배의 피해를 입힌다.<br/>전투 전, 받는 피해량 <font color=\"red\">-20%</font> (<font color=\"red\">1</font>턴 지속)<br/>[마인] 발동 시, 주는 스킬 피해량 <font color=\"red\">+30%</font>, 받는 피해량 <font color=\"red\">-20%</font>,<br/>동시에, 적을 쓰러트렸을 경우 해당 스킬의 쿨타임 <font color=\"red\">-2</font>.",
      "SubSkills": null,
      "Animation": null
    },
    {
      "Name": "마강통일",
      "Cost": "COST 2",
      "Entries": [
        {
          "Label": "유형",
          "Value": "초절강화"
        },
        {
          "Label": "쿨타임",
          "Value": "3턴"
        },
        {
          "Label": "사거리",
          "Value": "자신"
        },
        {
          "Label": "범위",
          "Value": "맵전체"
        }
      ],
      "IconImageUrl": "https://storage.googleapis.com/langmosheet-data/img/skills/Skill_Yuusuke4.webp",
      "Desc": "[초절강화] 액티브 스킬. 범위 내의 [시공의 중심] 을 지닌 <br/>모든 아군의 공격, 방어 능력이 대폭 상승.<br/>동시에, 단일 대상 스킬을 사용할 경우 주는 피해량 <font color=\"red\">+15%</font><br/>(이 효과는 다른 초절강화 스킬과 중첩불가)",
      "SubSkills": null,
      "Animation": null
    },
    {
      "Name": "방어 지휘",
      "Cost": "COST 2",
      "Entries": [
        {
          "Label": "유형",
          "Value": "지휘,패시브"
        }
      ],
      "IconImageUrl": "https://storage.googleapis.com/langmosheet-data/img/skills/Passive_AuraDefUp.webp",
      "Desc": "[지휘] 주위 <font color=\"red\">2</font>칸 내 모든 아군 방어 <font color=\"red\">+10%</font>.",
      "SubSkills": null,
      "Animation": null
    },
    {
      "Name": "흑영환",
      "Cost": "COST 3",
      "Entries": [
        {
          "Label": "유형",
          "Value": "물리 피해"
        },
        {
          "Label": "쿨타임",
          "Value": "4턴"
        },
        {
          "Label": "사거리",
          "Value": "2칸"
        },
        {
          "Label": "범위",
          "Value": "단일"
        }
      ],
      "IconImageUrl": "../img/Super/유스케.webp",
      "Desc": "[물리 피해]단일 적군을 공격해 1.4배의 피해를 입힌다.<br/>근접병사를 휴대할 경우 병사도 같이 공격한다.<br/>[마인상태]일 경우 스킬 사거리 +1, 피해상승 +30%, 적군을 격파할 경우 쿨타임 -3<br/>적군을 격파하지 못할 경우 '치료 불가', 2턴 지속(해당 효과는 제거불가)",
      "SubSkills": null,
      "Animation": "../img/Super/유스케_Ani.gif"
    }
  ],
  "Bio": [
    {
      "Entry": "영웅소개",
      "Desc": "◆출연：유유백서<br/>◆소속：우라메시 팀/마계 라이젠 세력"
    },
    {
      "Entry": "영웅담1",
      "Desc": "사라야시키 중학교 2학년. 다혈질에 싸움만 하는 불량 소년이지만, 본성은 동료를 아끼는 상냥한 성격이다. 어머니 아츠코와 2인 가족. 차에 치일 뻔한 아이를 구하려다 죽고 말았지만, 그것이 예정에 없던 죽음이었기에 염라대왕의 아들 염라대왕 Jr.가 다시 살아나기 위한 시련을 내린다. 영계 안내인 보탄, 동급생 쿠와바라 카즈마, 그리고 소꿉친구인 유키무라 케이코의 도움을 받아 결국 무사히 부활한다. 부활한 뒤에는 염라대왕 Jr.의 명에 따라 영계탐정이 되었다."
    },
    {
      "Entry": "영웅담2",
      "Desc": "영계탐정으로서 여러 사건을 조사하던 도중, 유스케는 고명한 영능력자인 겐카이와 만난다. 그녀의 제자가 된 유스케는, 정식 계승자로서 영광파동권을 수련한다."
    },
    {
      "Entry": "영웅담3",
      "Desc": "암흑무술대회 준결승 도중, 도구로와 싸우던 겐카이는 그의 힘 앞에 처참히 목숨을 잃고 만다. 겐카이의 죽음에 전의를 상실한 유스케였지만, 염라대왕Jr.가 전한 그녀의 '이겨라'라는 말을 듣고, 도구로를 타도하기로 맹세한다. 결승전에서, 유스케는 주령정을 벗고 본인도 최대한의 힘을 해방하여 파워 100%가 된 도구로와 대결한다. 결사의 사투 끝에 드디어 도구로와의 인연에 종지부를 찍게 된다."
    },
    {
      "Entry": "영웅담4",
      "Desc": "마계와 인간계를 잇는 계경 터널 확장을 계획하는 센스이. 그의 야망을 저지하기 위해 유스케 일행은 다시 한번 싸움의 소용돌이로 뛰어든다. 유스케 일행은 센스이의 아지트에 성공적으로 잠입하였으나, 센스이와 대치한 유스케는 압도적인 힘 앞에 살해당하고 만다. 하지만 그때, 그의 몸속에 잠들어 있던 마족의 피가 각성하여 부활하게 된다. 마계까지 센스이를 추격하여 호각으로 싸우지만, 누군가가 힘을 제대로 내지 못하는 유스케를 보다 못해, 그 몸을 조종하여 센스이를 쓰러트린다. 그것은 바로, 마족인 유스케의 아버지 라이젠이었다."
    },
    {
      "Entry": "영웅담5",
      "Desc": "자신의 몸에 흐르는 피의 진실을 찾기 위해 유스케는 마계로 향하고, 그곳에서 자신의 아버지인 라이젠을 만난다. 마계의 3대 요괴중 하나인 라이젠은 식인귀면서도 사람을 잡아먹는 것을 그만둔 탓에, 앞으로 살날이 얼마 남지 않았다고 말한다. 라이젠은 자신의 후계자로서 유스케를 불러들여 남은 시간 동안 그를 단련시키고, 마계의 평화와 균형을 유지해줄 것을 유스케에게 당부하고 사망한다. 라이젠의 의지를 이어받은 유스케는 요미와 무쿠로에게 마계 통일 토너먼트를 제안한다. 격전 끝에 유스케는 요미에게 패배하지만, 최종적으로는 라이젠의 옛 친구인 엔키가 우승하여 뜻밖에도 라이젠의 의지를 이어받게 된다."
    }
  ],
  "Performance": [
    {
      "Entry": "「소환 - 등장대사」",
      "Desc": "영계탐정, 우라메시 유스케님이시다!"
    },
    {
      "Entry": "「전투 - 전직1」",
      "Desc": "말할 필요도 없어! 이런 시련이라면 대환영이라고!"
    },
    {
      "Entry": "「전투 - 전직2」",
      "Desc": "좋아! 컨디션 최고다!"
    },
    {
      "Entry": "「전투 - 스킬시전1」",
      "Desc": "으랴으랴으랴으랴으랴~!"
    },
    {
      "Entry": "「전투 - 스킬시전2」",
      "Desc": "받아라!"
    },
    {
      "Entry": "「전투 - 선택1」",
      "Desc": "좋아, 간다!"
    },
    {
      "Entry": "「전투 - 선택2」",
      "Desc": "내가 상대다!"
    },
    {
      "Entry": "「전투 - 전투불능」",
      "Desc": "제기라아아알―!"
    },
    {
      "Entry": "「대화 - 친밀도1」",
      "Desc": "흐~응... 싸움의 미학이라... 나는 전혀 모르겠는데..."
    },
    {
      "Entry": "「대화 - 친밀도2」",
      "Desc": "하지만 아직도 믿지 못믿겠어... 거짓말 같다니까... 어쩌면 별안간 여기 나타나는 게 아닐까 하고, 지금도 그런 생각이 든다고."
    },
    {
      "Entry": "「대화 - 친밀도3」",
      "Desc": "내 선조가 마족인지 귀족인지는 모르겠지만 말야. 센스이를 때려눕힌다는 내 목표는 바뀌지 않는다고!"
    },
    {
      "Entry": "「대화 - 친밀도4」",
      "Desc": "귀찮은 건 생각하지 않는 주의라 말야. 맞짱 뜨는 게 가장 빠른 방법이었을 뿐이야."
    },
    {
      "Entry": "「대화 - 친밀도5」",
      "Desc": "수도 없이 싸워오면서, 그 중에는 뒷맛이 나쁜 적도 있었지만 말야... 저 바보처럼 싸움으로 밖에 서로를 이해할 수 없는 녀석도 있어."
    },
    {
      "Entry": "「대화 - 일상1」",
      "Desc": "자자. 그리 열 내지 말라고. 승부는 그 때의 운! 맘 편하게 하자고!"
    },
    {
      "Entry": "「대화 - 기쁨」",
      "Desc": "식은 죽 먹기지!"
    },
    {
      "Entry": "「대화 - 분노」",
      "Desc": "내 모든 힘을 이 한 방에 담겠어. 네가 혼을 버린 대신 얻은 힘 전부... 전부 다 모아서 덤벼봐! 네 모든 것을 쳐부수고, 내가 이길 거야"
    },
    {
      "Entry": "「대화 - 슬픔」",
      "Desc": "참 한심하지...? 친구 하나 구하지 못하다니 말야... 용서가 안 돼... 누구보다도, 나 자신을 용서 못하겠어!"
    },
    {
      "Entry": "「상호작용 - 대사2」",
      "Desc": "너무 심하게 다치면 통증도 안 느껴진다고 하던데... 공포도 그런 거였나."
    },
    {
      "Entry": "「상호작용 - 대사3」",
      "Desc": "오랜만이야... 이 긴장감!"
    },
    {
      "Entry": "「상호작용 - 반감1」",
      "Desc": "무슨 말이야?!"
    },
    {
      "Entry": "「상호작용 - 특수1」",
      "Desc": "하하하..."
    },
    {
      "Entry": "「상호작용 - 특수2」",
      "Desc": "그럼 안녕."
    }
  ]
}];