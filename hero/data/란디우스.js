var data = [
  {
    "Skins": [
      {
        "ImageUrl": "란디우스/1.webp",
        "Desc": "기본"
      },
      {
        "ImageUrl": "란디우스/2.webp",
        "Desc": "빛의 메아리"
      },
      {
        "ImageUrl": "란디우스/3.webp",
        "Desc": "형귀 스킨"
      },
      {
        "ImageUrl": "란디우스/4.webp",
        "Desc": "한정 판매"
      },
      {
        "ImageUrl": "란디우스/5.webp",
        "Desc": "서밋아레나 S12 패자스킨"
      },
      {
        "ImageUrl": "란디우스/6.webp",
        "Desc": "한정판매"
      },
      {
        "ImageUrl": "란디우스/sp.webp",
        "Desc": "SP 각성"
      }
    ],
    "Name": "란디우스",
    "CV": "오키아유 료타로",
    "ChName": "兰迪乌斯",
    "EngName": "Landius",
    "JapName": "ランディウス",
    "Rank": [
      0
    ],
    "SP": false,
    "Groups": [
      "주인공",
      "전설"
    ],
    "HP": 20,
    "SPHP": 20,
    "Attk": 35,
    "SPAttk": 35,
    "Def": 35,
    "SPDef": 35,
    "MagicDef": 10,
    "SPMagicDef": 10,
    "HowToGet": "영웅 소환",
    "Fetters": [
      {
        "Condition": "란디우스 30레벨 도달"
      },
      {
        "Condition": "란디우스를 로열 나이트전직"
      },
      {
        "Condition": "란디우스를 사용하여 아레나 5회 승리"
      },
      {
        "Condition": "레이첼을 도와 레이첼의 운명의 문4 클리어"
      }
    ],
    "HeartFetter": "생명+1000, 방어+40, 마방+30",
    "Talent": [
      {
        "TalentName": "단결의 의지",
        "Icon": "64.webp",
        "Abilities": [
          {
            "Desc": "1성"
          },
          {
            "Desc": "2성"
          },
          {
            "Desc": "행동 종료 시 자신의 <font color=\"red\">1</font>칸 내 아군이 있을 경우 공격<font color=\"red\"> +10%</font><br/> 공격의 <font color=\"red\">5%</font>를 방어와 마방에 추가 <font color=\"red\">1</font>행동 지속.<br/>원거리 공격을 받을 시 받는 피해 <font color=\"red\">-10%</font><br/> <font color=\"red\">2</font>칸 내 원거리 공격에 대해 반격을 할 수 있다."
          },
          {
            "Desc": "행동 종료 시 자신의 <font color=\"red\">2</font>칸 내 아군이 있을 경우 공격<font color=\"red\"> +13%</font><br/> 공격의 <font color=\"red\">8%</font>를 방어와 마방에 추가 <font color=\"red\">1</font>행동 지속.<br/>원거리 공격을 받을 시 받는 피해 <font color=\"red\">-13%</font><br/> <font color=\"red\">2</font>칸 내 원거리 공격에 대해 반격을 할 수 있다."
          },
          {
            "Desc": "행동 종료 시 자신의 <font color=\"red\">2</font>칸 내 아군이 있을 경우 공격<font color=\"red\"> +16%</font><br/> 공격의 <font color=\"red\">11%</font>를 방어와 마방에 추가 <font color=\"red\">1</font>행동 지속.<br/>원거리 공격을 받을 시 받는 피해 <font color=\"red\">-16%</font><br/> <font color=\"red\">2</font>칸 내 원거리 공격에 대해 반격을 할 수 있다."
          },
          {
            "Desc": "행동 종료 시 자신의 <font color=\"red\">3</font>칸 내 아군이 있을 경우 공격<font color=\"red\"> +20%</font><br/> 공격의 <font color=\"red\">15%</font>를 방어와 마방에 추가 <font color=\"red\">1</font>행동 지속.<br/>원거리 공격을 받을 시 받는 피해 <font color=\"red\">-20%</font><br/> <font color=\"red\">2</font>칸 내 원거리 공격에 대해 반격을 할 수 있다."
          }
        ]
      }
    ],
    "SPTalent": [
      {
        "Name": "마도성벽",
        "Icon": "SP_Gift_Landius.webp",
        "Abilities": [
          {
            "Desc": "1성"
          },
          {
            "Desc": "2성"
          },
          {
            "Desc": "<b>스킬 코스트 제한 해제</b><br/>공격 <font color=\"red\">+10%</font> 및 공격의 <font color=\"red\">5%</font>를 방어와 마방에 추가<br/>사망 시 체력 <font color=\"red\">20%</font> 수치로 부활. 최대 <font color=\"red\">1</font>회 발동<br/>행동 종료 시 자신의 <font color=\"red\">3</font>칸 내 아군이 있을 경우 자신은 <font color=\"blue\">영광스러운 공명</font> <font color=\"red\">1</font>스택 획득<br/><font color=\"blue\">영광스러운 공명</font>: 받는 원거리 피해 <font color=\"red\">-10%</font><br/>피격으로 전투 진입 시 피격 피해 <font color=\"red\">10%</font> 수치 반사. 최대 <font color=\"red\">3</font>중첩. 지속 <font color=\"red\">2</font>턴"
          },
          {
            "Desc": "<b>스킬 코스트 제한 해제</b><br/>공격 <font color=\"red\">+13%</font> 및 공격의 <font color=\"red\">8%</font>를 방어와 마방에 추가<br/>사망 시 체력 <font color=\"red\">30%</font> 수치로 부활. 최대 <font color=\"red\">1</font>회 발동<br/>행동 종료 시 자신의 <font color=\"red\">3</font>칸 내 아군이 있을 경우 자신은 <font color=\"blue\">영광스러운 공명</font> <font color=\"red\">1</font>스택 획득<br/><font color=\"blue\">영광스러운 공명</font>: 받는 원거리 피해 <font color=\"red\">-10%</font><br/>피격으로 전투 진입 시 피격 피해 <font color=\"red\">10%</font> 수치 반사. 최대 <font color=\"red\">3</font>중첩. 지속 <font color=\"red\">2</font>턴"
          },
          {
            "Desc": "<b>스킬 코스트 제한 해제</b><br/>공격 <font color=\"red\">+16%</font> 및 공격의 <font color=\"red\">11%</font>를 방어와 마방에 추가<br/>사망 시 체력 <font color=\"red\">40%</font> 수치로 부활. 최대 <font color=\"red\">1</font>회 발동<br/>행동 종료 시 자신의 <font color=\"red\">3</font>칸 내 아군이 있을 경우 자신은 <font color=\"blue\">영광스러운 공명</font> <font color=\"red\">1</font>스택 획득<br/><font color=\"blue\">영광스러운 공명</font>: 받는 원거리 피해 <font color=\"red\">-10%</font><br/>피격으로 전투 진입 시 피격 피해 <font color=\"red\">10%</font> 수치 반사. 최대 <font color=\"red\">3</font>중첩. 지속 <font color=\"red\">2</font>턴"
          },
          {
            "Desc": "<b>스킬 코스트 제한 해제</b><br/>공격 <font color=\"red\">+20%</font> 및 공격의 <font color=\"red\">15%</font>를 방어와 마방에 추가<br/>사망 시 체력 <font color=\"red\">50%</font> 수치로 부활. 최대 <font color=\"red\">1</font>회 발동<br/>행동 종료 시 자신의 <font color=\"red\">3</font>칸 내 아군이 있을 경우 자신은 <font color=\"blue\">영광스러운 공명</font> <font color=\"red\">1</font>스택 획득<br/><font color=\"blue\">영광스러운 공명</font>: 받는 원거리 피해 <font color=\"red\">-10%</font><br/>피격으로 전투 진입 시 피격 피해 <font color=\"red\">10%</font> 수치 반사. 최대 <font color=\"red\">3</font>중첩. 지속 <font color=\"red\">2</font>턴"
          }
        ]
      }
    ],
    "Second_Awaken": {
      "Name": "단결의 의지",
      "Stats": "<font color=\"blue\">패시브 봉인 면역</font><br/>주변 <font color=\"red\">3</font>칸 이내 아군 <font color=\"red\">1</font>명당 방어 <font color=\"red\">3%</font> 증가(최대 <font color=\"red\">12</font>)%"
    },
    "Equip": {
      "EqpName": "수호자의 추억 (무기)",
      "Stats": "생명 583,공격 85",
      "Icon": "란디우스.webp",
      "Ability": "공격 <font color=\"red\">+10%</font>   <br/><b>주위 <font color=\"red\">2</font>칸 범위 내에 아군이 있을 경우</b><br/><b>전투 진입 시 적 과의 거리 <font color=\"red\">1</font>칸당 ,</b><br/><b>부대가 받는 피해 <font color=\"red\">-5%</font> ( 최대 <font color=\"red\">-15%</font> )</b>"
    },
    "Soldiers": [
      "엔트 가디언",
      "근위 창병",
      "할버드",
      "암흑 근위대",
      "그레나디어",
      "근위기병",
      "템플러",
      "황가 기병",
      "황금기사",
      "엘프 투창병",
      "성스러운 방패 친위대",
      "파도 소환사",
      "송곳니 근위대"
    ],
    "Jobs": [
      {
        "Name": "마샬",
        "Icon": "창병",
        "SD": [
          {
            "ImageUrl": "란디우스/1/1.webp"
          },
          {
            "ImageUrl": "란디우스/2/1.webp"
          },
          {
            "ImageUrl": "란디우스/3/1.webp"
          },
          {
            "ImageUrl": "란디우스/4/1.webp"
          },
          {
            "ImageUrl": "란디우스/5/1.webp"
          },
          {
            "ImageUrl": "란디우스/6/1.webp"
          },
          {
            "ImageUrl": "란디우스/sp/1.webp"
          }
        ],
        "AttkRange": 1,
        "Move": 3,
        "MoveType": "보행",
        "HP": 5344,
        "HPRate": 89,
        "Attk": 534,
        "AttkRate": 82,
        "Int": 247,
        "IntRate": 41,
        "Def": 341,
        "DefRate": 76,
        "MagicDef": 211,
        "MagicDefRate": 47,
        "Skill": 118,
        "SkillRate": 34,
        "HeartFetterBouns1": "생명이 70%이하일때 전투중 받는피해 -10%",
        "HeartFetterBouns2": "생명이 70%이하일때 전투중 입히는피해 +10%"
      },
      {
        "Name": "마도 성기사",
        "Icon": "기병SP",
        "SD": [
          {
            "ImageUrl": "란디우스/1/2.webp"
          },
          {
            "ImageUrl": "란디우스/2/2.webp"
          },
          {
            "ImageUrl": "란디우스/3/2.webp"
          },
          {
            "ImageUrl": "란디우스/4/2.webp"
          },
          {
            "ImageUrl": "란디우스/5/2.webp"
          },
          {
            "ImageUrl": "란디우스/6/2.webp"
          },
          {
            "ImageUrl": "란디우스/sp/2.webp"
          }
        ],
        "AttkRange": 1,
        "Move": 5,
        "MoveType": "기마",
        "HP": 5494,
        "HPRate": 84,
        "Attk": 581,
        "AttkRate": 89,
        "Int": 247,
        "IntRate": 38,
        "Def": 330,
        "DefRate": 73,
        "MagicDef": 267,
        "MagicDefRate": 59,
        "Skill": 135,
        "SkillRate": 38,
        "HeartFetterBouns1": "부대 생명 <font color=\"red\">100%</font>일 경우<br/>받는 피해 <font color=\"red\">-10%</font>",
        "HeartFetterBouns2": "버프 <font color=\"red\">5</font>개 이상 보유 시<br/>전투 중 입히는 피해 <font color=\"red\">+10%</font>"
      },
      {
        "Name": "로열 나이트",
        "Icon": "기병",
        "SD": [
          {
            "ImageUrl": "란디우스/1/2.webp"
          },
          {
            "ImageUrl": "란디우스/2/2.webp"
          },
          {
            "ImageUrl": "란디우스/3/2.webp"
          },
          {
            "ImageUrl": "란디우스/4/2.webp"
          },
          {
            "ImageUrl": "란디우스/5/2.webp"
          },
          {
            "ImageUrl": "란디우스/6/2.webp"
          },
          {
            "ImageUrl": "란디우스/sp/3.webp"
          }
        ],
        "AttkRange": 1,
        "Move": 5,
        "MoveType": "기마",
        "HP": 5032,
        "HPRate": 84,
        "Attk": 563,
        "AttkRate": 87,
        "Int": 247,
        "IntRate": 41,
        "Def": 295,
        "DefRate": 66,
        "MagicDef": 265,
        "MagicDefRate": 59,
        "Skill": 132,
        "SkillRate": 38,
        "HeartFetterBouns1": "공격을 받아 전투진입시 받는물리피해 -10%",
        "HeartFetterBouns2": "공격하여 전투진입시 입히는피해 +10%"
      }
    ],
    "Materials": {
      "HeartFetter": [
        "h2",
        "h3",
        "h1",
        "h4",
        "h2-1",
        "h3-1",
        "h1-1",
        "h4-1",
        "x"
      ],
      "Fetter": [
        "6",
        "6-1",
        "6-2",
        "6-3",
        "6-4"
      ],
      "Awaken": [
        "ranka3",
        "ranka6",
        "ranka3-1",
        "ranka6-1"
      ]
    },
    "ClassUp": [
      {
        "ClassTitle": "나이트 : 생명+36, 공격+9",
        "Tiers": [
          {
            "Name": "T1",
            "Materials": [
              "rank1-3"
            ]
          },
          {
            "Name": "T2",
            "Materials": [
              "rank1-3"
            ]
          }
        ]
      },
      {
        "ClassTitle": "소드맨 : 생명+80, 기술+5",
        "Tiers": [
          {
            "Name": "T1",
            "Materials": [
              "rank2-1",
              "rank1-1",
              "rank1-2"
            ]
          },
          {
            "Name": "T2",
            "Materials": [
              "rank3-1",
              "rank3-6",
              "rank2-1",
              "rank2-2"
            ]
          },
          {
            "Name": "T3",
            "Materials": [
              "rank4-1",
              "rank4-2",
              "rank3-1",
              "rank3-6"
            ]
          }
        ]
      },
      {
        "ClassTitle": "제네럴 : 생명+80, 공격+7",
        "Tiers": [
          {
            "Name": "T1",
            "Materials": [
              "rank2-2",
              "rank1-2",
              "rank1-1"
            ]
          },
          {
            "Name": "T2",
            "Materials": [
              "rank3-6",
              "rank3-1",
              "rank2-2",
              "rank2-1"
            ]
          },
          {
            "Name": "T3",
            "Materials": [
              "rank4-2",
              "rank4-6",
              "rank3-6",
              "rank3-1"
            ]
          }
        ]
      },
      {
        "ClassTitle": "마샬 : 생명+150, 공격+15, 방어+10",
        "Tiers": [
          {
            "Name": "T1",
            "Materials": [
              "rank5-6",
              "rank5-3",
              "rank4-2",
              "rank4-6"
            ]
          },
          {
            "Name": "T2",
            "Materials": [
              "rank6-2",
              "rank6-1",
              "rank5-6",
              "rank5-3"
            ]
          },
          {
            "Name": "T3",
            "Materials": [
              "rank7-1",
              "rank7-7",
              "rank6-2",
              "rank6-1"
            ]
          },
          {
            "Name": "T4",
            "Materials": [
              "rank8-2",
              "rank8-1",
              "rank7-1",
              "rank7-7"
            ]
          }
        ]
      },
      {
        "ClassTitle": "하이랜더 : 생명+48, 공격+12",
        "Tiers": [
          {
            "Name": "T1",
            "Materials": [
              "rank2-3",
              "rank1-3",
              "rank1-1"
            ]
          },
          {
            "Name": "T2",
            "Materials": [
              "rank3-3",
              "rank3-2",
              "rank2-3",
              "rank2-1"
            ]
          },
          {
            "Name": "T3",
            "Materials": [
              "rank4-6",
              "rank4-1",
              "rank3-3",
              "rank3-2"
            ]
          }
        ]
      },
      {
        "ClassTitle": "로열나이트 : 생명+150, 공격+15, 방어+10",
        "Tiers": [
          {
            "Name": "T1",
            "Materials": [
              "rank5-3",
              "rank5-1",
              "rank4-6",
              "rank4-1"
            ]
          },
          {
            "Name": "T2",
            "Materials": [
              "rank6-6",
              "rank6-2",
              "rank5-3",
              "rank5-1"
            ]
          },
          {
            "Name": "T3",
            "Materials": [
              "rank7-7",
              "rank7-1",
              "rank6-6",
              "rank6-2"
            ]
          },
          {
            "Name": "T4",
            "Materials": [
              "rank8-7",
              "rank8-3",
              "rank7-7",
              "rank7-1"
            ]
          }
        ]
      }
    ],
    "SPOpenConditions": [
      {
        "Name": "개방",
        "Materials": [
          {
            "MaterialImgUrl": "../img/item/AlterEgo_Randiovs.webp",
            "Amount": 1
          }
        ],
        "Desc": ""
      },
      {
        "Name": "1단계 재료",
        "Materials": [
          {
            "MaterialImgUrl": "../img/item/오락기.webp",
            "Amount": 40
          },
          {
            "MaterialImgUrl": "../img/classup/rank8-7.webp",
            "Amount": 30
          },
          {
            "MaterialImgUrl": "../img/classup/rank7-7.webp",
            "Amount": 40
          },
          {
            "MaterialImgUrl": "../img/classup/rank6-6.webp",
            "Amount": 50
          }
        ],
        "Desc": ""
      },
      {
        "Name": "1단계 미션",
        "Materials": [],
        "Desc": "란디우스를 포함한 파티를 짜서 형귀 헬스장 바란(창병) 60레벨 이상 스테이지 클리어<br/>(화/금/일)"
      },
      {
        "Name": "2단계 재료",
        "Materials": [
          {
            "MaterialImgUrl": "../img/item/오락기.webp",
            "Amount": 40
          },
          {
            "MaterialImgUrl": "../img/item/쓰알열쇠.webp",
            "Amount": 30
          },
          {
            "MaterialImgUrl": "../img/item/스알열쇠.webp",
            "Amount": 40
          },
          {
            "MaterialImgUrl": "../img/item/알열쇠.webp",
            "Amount": 50
          }
        ],
        "Desc": ""
      },
      {
        "Name": "2단계 미션",
        "Materials": [],
        "Desc": "수호자의 추억(무기) 레벨 50 달성"
      },
      {
        "Name": "3단계 재료",
        "Materials": [
          {
            "MaterialImgUrl": "../img/item/오락기.webp",
            "Amount": 40
          },
          {
            "MaterialImgUrl": "../img/item/룬스톤.webp",
            "Amount": 2
          }
        ],
        "Desc": ""
      },
      {
        "Name": "3단계 미션",
        "Materials": [],
        "Desc": "란디우스를 포함한 파티를 짜서 여신의 시련 - 화룡 파프니르 60레벨 이상 스테이지 클리어<br/>(월/목)"
      },
      {
        "Name": "",
        "Materials": [],
        "Desc": "마도 성기사 전직 가능, 마도성벽(SP 고유기술)스킬 획득, SP 일러스트 획득"
      },
      {
        "Name": "4단계 재료",
        "Materials": [
          {
            "MaterialImgUrl": "../img/item/오락기.webp",
            "Amount": 40
          },
          {
            "MaterialImgUrl": "../img/heart/h2.webp",
            "Amount": 60
          },
          {
            "MaterialImgUrl": "../img/heart/h2-1.webp",
            "Amount": 30
          },
          {
            "MaterialImgUrl": "../img/heart/x.webp",
            "Amount": 12
          }
        ],
        "Desc": ""
      },
      {
        "Name": "4단계 미션",
        "Materials": [],
        "Desc": "란디우스를 포함한 파티를 짜서 형귀 헬스장 샘슨(기병) 65레벨 이상 스테이지 클리어<br/>(수/토/일)"
      },
      {
        "Name": "5단계 재료",
        "Materials": [
          {
            "MaterialImgUrl": "../img/item/오락기.webp",
            "Amount": 40
          },
          {
            "MaterialImgUrl": "../img/item/Awaken_MoonBrillance.webp",
            "Amount": 5
          },
          {
            "MaterialImgUrl": "../img/item/Awaken_Stardust.webp",
            "Amount": 5
          }
        ],
        "Desc": ""
      },
      {
        "Name": "5단계 미션",
        "Materials": [],
        "Desc": "란디우스를 포함한 파티를 짜서 여신의 시련 - 암룡 티아메트 65레벨 이상 스테이지 클리어<br/>(일)"
      },
      {
        "Name": "6단계 재료",
        "Materials": [
          {
            "MaterialImgUrl": "../img/item/Item_MemoryEssence.webp",
            "Amount": 600
          },
          {
            "MaterialImgUrl": "../img/stone/StoneA_Rider1.webp",
            "Amount": 5
          },
          {
            "MaterialImgUrl": "../img/stone/StoneA_Rider2.webp",
            "Amount": 5
          },
          {
            "MaterialImgUrl": "../img/stone/StoneA_Rider4.webp",
            "Amount": 5
          }
        ],
        "Desc": ""
      },
      {
        "Name": "6단계 미션",
        "Materials": [],
        "Desc": "란디우스를 포함한 파티를 짜서 영원의 신전 - 리바이엘 65레벨 이상 스테이지 클리어<br/>(화/금)"
      },
      {
        "Name": "",
        "Materials": [],
        "Desc": "SP 스탯, SP 스킬 2개 활성화, 송곳니 근위대 고용가능"
      }
    ],
    "Skills": [
      {
        "Name": "진형파괴",
        "Cost": "COST 2",
        "Entries": [
          {
            "Label": "유형",
            "Value": "물리피해"
          },
          {
            "Label": "쿨타임",
            "Value": "3턴"
          },
          {
            "Label": "사거리",
            "Value": "1칸"
          },
          {
            "Label": "범위",
            "Value": "단일"
          }
        ],
        "IconImageUrl": "../img/skills/Skill_KnightCrash3.webp",
        "Desc": "[물리 피해]단일 적 부대를 공격해 1배의 공격 피해를 준다.<br/>대상을 2칸 밀쳐내며 다음 턴 행동 불가 부여.",
        "SubSkills": null,
        "Animation": null
      },
      {
        "Name": "재이동",
        "Cost": "COST 1",
        "Entries": [
          {
            "Label": "유형",
            "Value": "패시브"
          }
        ],
        "IconImageUrl": "../img/skills/13.webp",
        "Desc": "[패시브]공격하여 전투 진입 시 주는 피해량 +10%.<br/>남은 이동력이 있을 경우 공격 후 재이동 가능.",
        "SubSkills": null,
        "Animation": null
      },
      {
        "Name": "창진",
        "Cost": "COST 2",
        "Entries": [
          {
            "Label": "유형",
            "Value": "액티브"
          },
          {
            "Label": "쿨타임",
            "Value": "2턴"
          },
          {
            "Label": "사거리",
            "Value": "자신"
          },
          {
            "Label": "범위",
            "Value": "단일"
          }
        ],
        "IconImageUrl": "../img/skills/Skill_BitterlyHit.webp",
        "Desc": "[패시브]근접한 아군 부대를 대신해 물리 공격을 받는다.<br/>[액티브]호위 범위가 <font color=\"#DC143C\">2칸</font>으로 늘어나며 방어 <font color=\"#DC143C\">+20%</font>.<br/>근접 전투 종료 후 [미늘]의 고정 피해가 반드시 발동한다. <font color=\"#DC143C\">2턴</font> 지속.",
        "SubSkills": null,
        "Animation": null
      },
      {
        "Name": "강철 투구",
        "Cost": "COST 2",
        "Entries": [
          {
            "Label": "유형",
            "Value": "패시브"
          }
        ],
        "IconImageUrl": "../img/skills/Skill_Helmet.webp",
        "Desc": "[패시브] 원거리 공격을 받을 시 <font color=\"#DC143C\">25%</font>확률로 부대가 받는 모든 피해 <font color=\"#DC143C\">-50%</font>.",
        "SubSkills": null,
        "Animation": null
      },
      {
        "Name": "불굴",
        "Cost": "COST 1",
        "Entries": [
          {
            "Label": "유형",
            "Value": "패시브"
          }
        ],
        "IconImageUrl": "../img/skills/Skill_NerverDie.webp",
        "Desc": "[패시브]사망 시 부활 및 생명 20%회복. 최대 1회 가능.",
        "SubSkills": null,
        "Animation": null
      },
      {
        "Name": "반격",
        "Cost": "COST 1",
        "Entries": [
          {
            "Label": "유형",
            "Value": "패시브"
          }
        ],
        "IconImageUrl": "../img/skills/Skill_CounterAttack.webp",
        "Desc": "[패시브]주위 아군을 대신해 물리 공격을 받는다.<br/>공격을 받아 전투 진입 시 공격 +12%.",
        "SubSkills": null,
        "Animation": null
      },
      {
        "Name": "찬란한 전설",
        "Cost": "COST 2",
        "Entries": [
          {
            "Label": "유형",
            "Value": "초절강화"
          },
          {
            "Label": "쿨타임",
            "Value": "3턴"
          },
          {
            "Label": "사거리",
            "Value": "자신"
          },
          {
            "Label": "범위",
            "Value": "맵전체"
          }
        ],
        "IconImageUrl": "../img/skills/Skill_Crystal.webp",
        "Desc": "[초절강화] 액티브 스킬, 범위 내 모든 '전설의 저편' 아군의 <br/>공격, 방어 능력치가 크게 상승하고<br/>상성관계가 없는 부대와 교전 시 추가 피해 +15%,<br/>4턴 지속.(이효과는 다른 초절강화 스킬과 중첩불가)",
        "SubSkills": null,
        "Animation": null
      },
      {
        "Name": "고요",
        "Cost": "COST 2",
        "Entries": [
          {
            "Label": "유형",
            "Value": "액티브"
          },
          {
            "Label": "쿨타임",
            "Value": "2턴"
          },
          {
            "Label": "사거리",
            "Value": "자신"
          },
          {
            "Label": "범위",
            "Value": "단일"
          }
        ],
        "IconImageUrl": "../img/skills/Skill_Static.webp",
        "Desc": "[패시브]근접한 아군을 대신해 모든 공격을 받는다.<br/>[지휘]주위 2칸 내 모든 아군이 받는 치명타 확률 -30%, 받는 원거리 피해 -15%.<br/>[액티브]호위 범위 2칸으로 상승, 지휘 범위 3칸으로 상승. 2턴 지속.",
        "SubSkills": null,
        "Animation": null
      },
      {
        "Name": "태양 베기",
        "Cost": "COST 2",
        "Entries": [
          {
            "Label": "유형",
            "Value": "물리피해"
          },
          {
            "Label": "쿨타임",
            "Value": "2턴"
          },
          {
            "Label": "사거리",
            "Value": "1칸"
          },
          {
            "Label": "범위",
            "Value": "단일"
          }
        ],
        "IconImageUrl": "../img/skills/1.webp",
        "Desc": "[물리 피해]단일 적 부대를 공격해 1.5배의 피해를 준다.<br/>전투 후 가한 피해량 30%만큼 생명을 회복한다.",
        "SubSkills": null,
        "Animation": null
      },
      {
        "Name": "SP : 영광스러운 호위",
        "Cost": "COST 2",
        "Entries": [
          {
            "Label": "유형",
            "Value": "액티브"
          },
          {
            "Label": "쿨타임",
            "Value": "1턴"
          },
          {
            "Label": "사거리",
            "Value": "3칸"
          },
          {
            "Label": "범위",
            "Value": "단일"
          }
        ],
        "IconImageUrl": "../img/skills/sp/란디우스/1.webp",
        "Desc": "[패시브]<font color=\"blue\">지수</font> 혹은 <font color=\"blue\">명경지수</font> 사용 후 행동 종료 시 전장의 모든 '전설의 저편' 아군의 공격, 방어 능력치가 크게 상승하고, '주는 피해량 <font color=\"red\">+8%</font>, 상성이 아닌 부대와 교전 시 추가 주는 피해량 <font color=\"red\">+8%</font> (지속 <font color=\"red\">4</font>턴) 효과 부여. (이효과는 다른 초절강화 스킬과 중첩불가)<br/>[액티브] 목표 아군에게 <font color=\"blue\">영광스러운 공명</font> <font color=\"red\">1</font>스택 부여.<br/><font color=\"blue\">영광스러운 공명</font>: 받는 원거리 피해 <font color=\"red\">-10%</font><br/>피격으로 전투 진입 시 피격 피해 <font color=\"red\">10%</font> 수치 반사. 최대 <font color=\"red\">3</font>중첩. 지속 <font color=\"red\">2</font>턴",
        "SubSkills": null,
        "Animation": null
      },
      {
        "Name": "SP : 기사단의 분노",
        "Cost": "COST 2",
        "Entries": [
          {
            "Label": "유형",
            "Value": "액티브"
          },
          {
            "Label": "쿨타임",
            "Value": "2턴"
          },
          {
            "Label": "사거리",
            "Value": "자신"
          },
          {
            "Label": "범위",
            "Value": "단일"
          }
        ],
        "IconImageUrl": "../img/skills/sp/란디우스/2.webp",
        "Desc": "[패시브]근접 피격으로 전투 진입 시 <font color=\"blue\">받는 피해량</font> <font color=\"red\">-10%</font>, 보유한 <font color=\"blue\">영광스러운 공명</font> <font color=\"red\">1</font>스택 당 추가로 <font color=\"red\">-5%</font>.<br/>[액티브] 주위 <font color=\"red\">3</font>칸 범위 안에 있는 부대 <font color=\"red\">1</font>개당 <font color=\"blue\">기사단의 분노</font> <font color=\"red\">1</font>스택 획득.<br/><font color=\"blue\">기사단의 분노</font>: 피격으로 전투 진입 시 사거리 <font color=\"red\">+1</font>.<br/>버프를 제거당할 경우 <font color=\"blue\">기사단의 분노</font>를 우선 제거. 최대 <font color=\"red\">5</font>중첩. 지속 <font color=\"red\">2</font>턴",
        "SubSkills": null,
        "Animation": null
      },
      {
        "Name": "명경지수",
        "Cost": "COST 3",
        "Entries": [
          {
            "Label": "유형",
            "Value": "액티브"
          },
          {
            "Label": "쿨타임",
            "Value": "3턴"
          },
          {
            "Label": "사거리",
            "Value": "자신"
          },
          {
            "Label": "범위",
            "Value": "단일"
          }
        ],
        "IconImageUrl": "../img/Super/란디우스.webp",
        "Desc": "[패시브]근접한 아군을 대신해 모든 공격을 받는다. 위치 이동 면역.<br/>[지휘]주위 2칸 내 모든 아군이 받는 치명타 확률 -30%, 받는 원거리 피해 -15%, 받는 치료 효과 +15%.<br/>[액티브]호위 범위 2칸으로 상승, 지휘 범위 3칸으로 상승,<br/>주위 2칸 범위내 모든 적군의 '재이동 불가', 3턴 지속.",
        "SubSkills": null,
        "Animation": "../img/Super/란디우스_Ani.webp"
      }
    ],
    "Propose": false,
    "Bio": [
      {
        "Entry": "영웅소개",
        "Desc": "◆키/체중: 185cm / 74kg<br/>◆출연: 랑그릿사IV<br/>◆소속: 정의로운 동료"
      },
      {
        "Entry": "영웅담1",
        "Desc": "과묵하고 선량한 용사이자, 자신의 뛰어난 무예와 지휘 능력으로 사람들을 이끄는 걸출한 리더이다. 예레스 대륙을 휩쓴 전쟁에서 성검 랑그릿사로 카오스를 부활시키려는 보젤의 음모와 기자로프의 세계 제패 야욕을 무너뜨렸다.<br/><font color=\"#DC143C\"><size=33><i>「이건 모두가 내게 준 힘이야!」</i> </size=33></font>"
      },
      {
        "Entry": "영웅담2",
        "Desc": "홍수로 양친을 잃고 레이첼과 리키의 아버지에게 길러졌다. 이후 기자로프가 마을을 파괴하고 레이첼을 납치하자 저항의 길로 들어선다. 란디우스는 도망치던 중 신비로운 검사 마크렌, 카콘시스의 두 공주와 인연을 맺고 카콘시스 왕국 편에 서서 연방의 침략을 막아낸다."
      },
      {
        "Entry": "영웅담3",
        "Desc": "모험 중 '페어라트'라는 가명을 사용한 어둠의 왕자 보젤에게 속기도 한다. 하지만 자신의 신념을 지켜낸 란디우스는 빛의 후예만이 사용할 수 있는 성검 '랑그릿사'를 손에 쥐고 자신의 정의를 실현했다. <br/><font color=\"#DC143C\"><size=33><i>「많은 것을 잃어봤기 때문에... 나는 목숨보다 소중한 것들을 지키려고 하는 거야.」</i> </size=33></font>"
      },
      {
        "Entry": "영웅담4",
        "Desc": "전쟁이 끝난 후, 그는 연방과 왕국의 수호용사로 대륙에 이름을 알렸다. 이후 레인폴스와의 전투에서 갇혀있던 란포드 장군을 구출하고 함께 마족을 막아냈다. 이 전쟁으로 그가 달의 백성 '크림조랜더'의 후손이라는 게 밝혀진다."
      },
      {
        "Entry": "영웅담5",
        "Desc": "소환된 란디우스는 놀라운 전투력을 과시하며, 사람들과 힘을 합쳐 전선에서 적과 맞서 싸웠다. 그는 성검 군단의 매튜와 찰떡같은... 호흡을 보인다.<font color=\"#DC143C\"><size=33><i>「랑그릿사, 알하자드... 두 검의 힘은 너무나 강해.」</i> </size=33></font>"
      }
    ],
    "Performance": [
      {
        "Entry": "「소환 - 등장대사」",
        "Desc": "내 이름은 란디우스야. 나는... 영웅이 아니야."
      },
      {
        "Entry": "「전투 - 전직1」",
        "Desc": "아니, 이건 모두가 내게 준 힘이야!"
      },
      {
        "Entry": "「전투 - 전직2」",
        "Desc": "힘이 강해진 만큼, 책임도 무거워졌어."
      },
      {
        "Entry": "「전투 - 스킬시전1」",
        "Desc": "말이 많으면 도움이 안돼, 막아 봐!"
      },
      {
        "Entry": "「전투 - 스킬시전2」",
        "Desc": "내게 힘을 빌려줘!"
      },
      {
        "Entry": "「전투 - 선택1」",
        "Desc": "전투가 아직 끝나지 않았으니 방심하지 마!"
      },
      {
        "Entry": "「전투 - 선택2」",
        "Desc": "모두, 조금만 더 힘내자!"
      },
      {
        "Entry": "「전투 - 전투불능」",
        "Desc": "아니, 난 여기서 죽을 수 없어..."
      },
      {
        "Entry": "「대화 - 친밀도1」",
        "Desc": "친부모...? 아니, 나는 고아야. 양친 모두 홍수로 돌아가셨어..."
      },
      {
        "Entry": "「대화 - 친밀도2」",
        "Desc": "생각해보면, 누나가 있었던 것 같은데... 잘, 기억이 안나."
      },
      {
        "Entry": "「대화 - 친밀도3」",
        "Desc": "하하하, 네가 말한 전투들은 사실 윌러 제독의 공이었어... 그 녀석의 지휘가 조금 그립기도 해."
      },
      {
        "Entry": "「대화 - 친밀도4」",
        "Desc": "많은 것을 잃어봤기 때문에... 나는 목숨보다 소중한 것들을 지키려고 하는 거야."
      },
      {
        "Entry": "「대화 - 친밀도5」",
        "Desc": "하, 내 혈통을 안다고? 정말 귀찮군... 외계인이라는 사실은 나 자신도 받아들이기 힘들니까..."
      },
      {
        "Entry": "「대화 - 일상1」",
        "Desc": "여동생을 구하려다가 전쟁에 휘말렸을 뿐인데, 후세에 영웅으로 불린다니... 정말 부끄럽네."
      },
      {
        "Entry": "「대화 - 기쁨」",
        "Desc": "성검의 기억에 의해 소환될 줄은 꿈에도 몰랐어. 너희들과 만난 건 정말 행운이야."
      },
      {
        "Entry": "「대화 - 분노」",
        "Desc": "기자로프... 이 이름을 조심해. 그는 어둠의 왕자 보젤보다 훨씬 위험한 괴물이니까."
      },
      {
        "Entry": "「대화 - 슬픔」",
        "Desc": "랑그릿사, 알하자드... 두 검의 힘은 너무나 강해."
      },
      {
        "Entry": "「상호작용 - 대사1」",
        "Desc": "이 세상의 종족이 모두 다르듯, 가치관도 다를 수 있어. 그 모든 건 각각 존재의 의미가 있을 거야. 적어도 나는 그렇게 생각해."
      },
      {
        "Entry": "「상호작용 - 대사2」",
        "Desc": "맞아, 나도 저 멀리  대륙에서 왔어... 고향은 어떤 모습으로 변했을까?"
      },
      {
        "Entry": "「상호작용 - 대사3」",
        "Desc": "난 이 세계로 소환해줘서 고마워... 별다른 뜻은 없어, 그냥 이 말이 하고 싶었을 뿐이야."
      },
      {
        "Entry": "「상호작용 - 호감1」",
        "Desc": "하, 크림조랜더의 피가 흐르기 때문에 내 수명은 보통 사람보다 훨씬 길겠지..."
      },
      {
        "Entry": "「상호작용 - 반감1」",
        "Desc": "그만해! 멈춰, 무의미한 싸움은 하지마!"
      },
      {
        "Entry": "「상호작용 - 특수1」",
        "Desc": "몸조심해!"
      },
      {
        "Entry": "「상호작용 - 특수2」",
        "Desc": "저는 이만."
      }
    ],
    "Performance2": null,
    "Misc": ""
  }
]