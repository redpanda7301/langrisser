var data = [
{
  "Skins": [
    {
      "ImageUrl": "팟시르/1.webp",
      "Desc": "기본"
    },
    {
      "ImageUrl": "팟시르/2.webp",
      "Desc": "빛의 메아리"
    }
  ],
  "Name": "팟시르",
  "CV": "타카하시 치아키",
  "Rank": [
    0
  ],
  "SP": false,
  "Groups": [
    4,
    11
  ],
  "HP": 30,
  "SPHP": 0,
  "Attk": 35,
  "SPAttk": 0,
  "Def": 15,
  "SPDef": 0,
  "MagicDef": 20,
  "SPMagicDef": 0,
  "HowToGet": "영웅 소환",
  "Fetters": [
    {
      "Condition": "팟시르를 출전시켜 아레나 5승 달성"
    },
    {
      "Condition": "핏빛 꿈의 주인 전직"
    },
    {
      "Condition": "보젤의 도움받아 유대의 문4 클리어"
    },
    {
      "Condition": "리코리스의 도움받아 유대의 문5 클리어"
    }
  ],
  "HeartFetter": "생명+600, 방어+30, 마방+40",
  "Talent": [
    {
      "Name": "그림자 매혹",
      "Icon": "Gift_Patsyr.webp",
      "Abilities": [
        {
          "Desc": "1성"
        },
        {
          "Desc": "2성"
        },
        {
          "Desc": "<b><font color=\"#DB00DB\">여성</font>이 아닌 영웅</b>과 전투 시 지력 <font color=\"red\">+15</font>% 받는 피해 <font color=\"red\">-15</font>%<br/>선공으로 피해를 입힐 경우 목표에게 <b><font color =\"purple\">피로</font></b>효과 부여<br/><b><font color =\"purple\">피로</font></b> : 피격 치명타 확률 <font color=\"red\">+20</font>% 지속 <font color=\"red\">3</font>행동 중첩 가능<br/><font color=\"red\">2</font>중첩 된 상태로 행동 종료 시 해당 효과는 <font color=\"purple\">최면</font>으로 대체<br/><font color=\"purple\">최면</font> : 기절 피격 치명타 확률 <font color=\"red\">+50</font>% 지속 <font color=\"red\">1</font>행동<br/>(최면 효과는 1번 피격시 사라짐 쿨 <font color=\"red\">3</font>행동)"
        },
        {
          "Desc": "<b><font color=\"#DB00DB\">여성</font>이 아닌 영웅</b>과 전투 시 지력 <font color=\"red\">+20</font>% 받는 피해 <font color=\"red\">-20</font>%<br/>선공으로 피해를 입힐 경우 목표에게 <b><font color =\"purple\">피로</font></b>효과 부여<br/><b><font color =\"purple\">피로</font></b> : 피격 치명타 확률 <font color=\"red\">+20</font>% 지속 <font color=\"red\">3</font>행동 중첩 가능<br/><font color=\"red\">2</font>중첩 된 상태로 행동 종료 시 해당 효과는 <font color=\"purple\">최면</font>으로 대체<br/><font color=\"purple\">최면</font> : 기절 피격 치명타 확률 <font color=\"red\">+50</font>% 지속 <font color=\"red\">1</font>행동<br/>(최면 효과는 1번 피격시 사라짐 쿨 <font color=\"red\">3</font>행동)"
        },
        {
          "Desc": "<b><font color=\"#DB00DB\">여성</font>이 아닌 영웅</b>과 전투 시 지력 <font color=\"red\">+25</font>% 받는 피해 <font color=\"red\">-25</font>%<br/>선공으로 피해를 입힐 경우 목표에게 <b><font color =\"purple\">피로</font></b>효과 부여<br/><b><font color =\"purple\">피로</font></b> : 피격 치명타 확률 <font color=\"red\">+20</font>% 지속 <font color=\"red\">3</font>행동 중첩 가능<br/><font color=\"red\">2</font>중첩 된 상태로 행동 종료 시 해당 효과는 <font color=\"purple\">최면</font>으로 대체<br/><font color=\"purple\">최면</font> : 기절 피격 치명타 확률 <font color=\"red\">+50</font>% 지속 <font color=\"red\">1</font>행동<br/>(최면 효과는 1번 피격시 사라짐 쿨 <font color=\"red\">3</font>행동)"
        },
        {
          "Desc": "<b><font color=\"#DB00DB\">여성</font>이 아닌 영웅</b>과 전투 시 지력 <font color=\"red\">+30</font>% 받는 피해 <font color=\"red\">-30</font>%<br/>선공으로 피해를 입힐 경우 목표에게 <b><font color =\"purple\">피로</font></b>효과 부여<br/><b><font color =\"purple\">피로</font></b> : 피격 치명타 확률 <font color=\"red\">+20</font>% 지속 <font color=\"red\">3</font>행동 중첩 가능<br/><font color=\"red\">2</font>중첩 된 상태로 행동 종료 시 해당 효과는 <font color=\"purple\">최면</font>으로 대체<br/><font color=\"purple\">최면</font> : 기절 피격 치명타 확률 <font color=\"red\">+50</font>% 지속 <font color=\"red\">1</font>행동<br/>(최면 효과는 1번 피격시 사라짐 쿨 <font color=\"red\">3</font>행동)"
        }
      ]
    }
  ],
  "SPTalent": null,
  "Equip": null,
  "Soldiers": [
    43,
    22,
    13,
    7,
    26,
    15,
    45,
    62
  ],
  "Jobs": [
    {
      "Name": "핏빛 꿈의 주인",
      "Icon": "Icon_Occupation_Assassin.webp",
      "SD": [
        {
          "ImageUrl": "팟시르/1/1.webp"
        },
        {
          "ImageUrl": "팟시르/2/1.webp"
        }
      ],
      "AttkRange": 2,
      "Move": 3,
      "MoveType": "Fly",
      "HP": 3731,
      "HPRate": 60,
      "Attk": 290,
      "AttkRate": 33,
      "Int": 550,
      "IntRate": 74,
      "Def": 273,
      "DefRate": 50,
      "MagicDef": 331,
      "MagicDefRate": 88,
      "Skill": 111,
      "SkillRate": 33,
      "HeartFetterBouns1": "<b>받는 범위피해량</b><font color=\"red\">-10</font>%",
      "HeartFetterBouns2": "<b>공격하여 전투 진입시</b><br/>피해 증가량 <font color=\"red\">+10</font>%"
    },
    {
      "Name": "꿈을 쫓는 자",
      "Icon": "Icon_Occupation_Monster.webp",
      "SD": [
        {
          "ImageUrl": "팟시르/1/2.webp"
        },
        {
          "ImageUrl": "팟시르/2/1.webp"
        }
      ],
	  "AttkRange": 2,
      "Move": 3,
      "MoveType": "Fly",
      "HP": 4034,
      "HPRate": 65,
      "Attk": 262,
      "AttkRate": 33,
      "Int": 579,
      "IntRate": 84,
      "Def": 293,
      "DefRate": 55,
      "MagicDef": 354,
      "MagicDefRate": 93,
      "Skill": 125,
      "SkillRate": 40,
	    "HeartFetterBouns1": "<b>공격받아 전투 진입시</b><br/> 받는 물리량<font color=\"red\">-10</font>%",
      "HeartFetterBouns2": "<b>부대 생명력이 <font color=\"red\">80</font>% 이상일시</b><br/><b>전투 진입후</b> 주는 피해량 <font color=\"red\">+10</font>%"
    }
  ],
  "Materials": {
    "HeartFetter": [
      "h4",
      "h1",
      "h3",
      "h2",
      "h4-1",
      "h1-1",
      "h3-1",
      "h2-1",
      "x"
    ],
    "Fetter": [
      "5",
      "5-1",
      "5-2",
      "5-3",
      "5-4"
    ],
    "Awaken": [
      "ranka2",
      "ranka5",
      "ranka2-1",
      "ranka5-1"
    ]
  },
  "ClassUp": [
    {
      "ClassTitle": "그림자 요녀 : 방어+6, 지력+5",
      "Tiers": [
        {
          "Name": "T1",
          "Materials": [
            "rank1-4"
          ]
        },
        {
          "Name": "T2",
          "Materials": [
            "rank1-4"
          ]
        }
      ]
    },
    {
      "ClassTitle": "주술사 : 생명+48, 지력+12",
      "Tiers": [
        {
          "Name": "T1",
          "Materials": [
            "rank2-6",
            "rank1-3",
            "rank1-4"
          ]
        },
        {
          "Name": "T2",
          "Materials": [
            "rank3-5",
            "rank3-2",
            "rank2-3",
            "rank2-2"
          ]
        },
        {
          "Name": "T3",
          "Materials": [
            "rank4-6",
            "rank4-2",
            "rank3-5",
            "rank3-2"
          ]
        }
      ]
    },
    {
      "ClassTitle": "달의 그림자 요녀 : 마방+5, 지력+12",
      "Tiers": [
        {
          "Name": "T1",
          "Materials": [
            "rank2-5",
            "rank1-4",
            "rank1-5"
          ]
        },
        {
          "Name": "T2",
          "Materials": [
            "rank3-4",
            "rank3-5",
            "rank2-5",
            "rank2-4"
          ]
        },
        {
          "Name": "T3",
          "Materials": [
            "rank4-5",
            "rank4-4",
            "rank3-4",
            "rank3-5"
          ]
        }
      ]
    },
    {
      "ClassTitle": "꿈을 쫒는 자 : 생명+90 지력+25, 방어+10",
      "Tiers": [
        {
          "Name": "T1",
          "Materials": [
            "rank5-1",
            "rank5-4",
            "rank4-5",
            "rank4-1"
          ]
        },
        {
          "Name": "T2",
          "Materials": [
            "rank6-5",
            "rank6-7",
            "rank5-1",
            "rank5-4"
          ]
        },
        {
          "Name": "T3",
          "Materials": [
            "rank7-5",
            "rank7-1",
            "rank6-5",
            "rank6-7"
          ]
        },
        {
          "Name": "T4",
          "Materials": [
            "rank8-6",
            "rank8-1",
            "rank7-5",
            "rank7-6"
          ]
        }
      ]
    },
    {
      "ClassTitle": "그림자 마녀 : 생명+48, 지력 +12",
      "Tiers": [
        {
          "Name": "T1",
          "Materials": [
            "rank2-6",
            "rank1-2",
            "rank1-1"
          ]
        },
        {
          "Name": "T2",
          "Materials": [
            "rank3-2",
            "rank3-1",
            "rank2-3",
            "rank2-6"
          ]
        },
        {
          "Name": "T3",
          "Materials": [
            "rank4-3",
            "rank4-1",
            "rank3-2",
            "rank3-1"
          ]
        }
      ]
    },
    {
      "ClassTitle": "핏빛 꿈의 주인 : 생명+150, 지력+15, 방어+10",
      "Tiers": [
        {
          "Name": "T1",
          "Materials": [
            "rank5-2",
            "rank5-1",
            "rank4-3",
            "rank4-1"
          ]
        },
        {
          "Name": "T2",
          "Materials": [
            "rank6-4",
            "rank6-3",
            "rank5-2",
            "rank5-1"
          ]
        },
        {
          "Name": "T3",
          "Materials": [
            "rank7-3",
            "rank7-6",
            "rank6-4",
            "rank6-3"
          ]
        },
        {
          "Name": "T4",
          "Materials": [
            "rank8-8",
            "rank8-5",
            "rank7-3",
            "rank7-6"
          ]
        }
      ]
    }
  ],
  "SPOpenConditions": null,
  "Skills": [
    {
      "Name": "파이어볼",
      "Cost": "COST 1",
      "Entries": [
        {
          "Label": "유형",
          "Value": "마법 피해"
        },
        {
          "Label": "쿨타임",
          "Value": "1턴"
        },
        {
          "Label": "사거리",
          "Value": "2칸"
        },
        {
          "Label": "범위",
          "Value": "단일"
        }
      ],
      "IconImageUrl": "https://storage.googleapis.com/langmosheet-data/img/skills/Skill_Fireball.webp",
      "Desc": "[마법 피해]단일 적 부대를 공격해 <font color=\"red\">1.5</font>배의 피해를 준다.<br/><b>'보병'에게 더 큰 피해를 준다.</b>",
      "SubSkills": null,
      "Animation": null
    },
    {
      "Name": "썬더",
      "Cost": "COST 1",
      "Entries": [
        {
          "Label": "유형",
          "Value": "마법 피해"
        },
        {
          "Label": "쿨타임",
          "Value": "1턴"
        },
        {
          "Label": "사거리",
          "Value": "2칸"
        },
        {
          "Label": "범위",
          "Value": "단일"
        }
      ],
      "IconImageUrl": "https://storage.googleapis.com/langmosheet-data/img/skills/Skill_ThunderBolt.webp",
      "Desc": "[마법 피해]단일 적 부대를 공격해 <font color=\"red\">1.5</font>배의 피해를 준다.<br/><b>'기병'에게 더큰 피해를 준다.</b>",
      "SubSkills": null,
      "Animation": null
    },
    {
      "Name": "망각",
      "Cost": "COST 2",
      "Entries": [
        {
          "Label": "유형",
          "Value": "마법 피해"
        },
        {
          "Label": "쿨타임",
          "Value": "3턴"
        },
        {
          "Label": "사거리",
          "Value": "3칸"
        },
        {
          "Label": "범위",
          "Value": "3칸"
        }
      ],
      "IconImageUrl": "https://storage.googleapis.com/langmosheet-data/img/skills/Skill_JamMove.webp",
      "Desc": "[마법 피해]범위 내 적들을 공격해 <font color=\"red\">0.3</font>배의 광역 피해를 주며<br/>적에게 '<font color=\"blue\">모든 패시브 스킬 무효</font>'부여<br/><font color=\"red\">2</font>턴 지속.",
      "SubSkills": null,
      "Animation": null
    },
    {
      "Name": "마방 위협",
      "Cost": "COST 2",
      "Entries": [
        {
          "Label": "유형",
          "Value": "패시브"
        }
      ],
      "IconImageUrl": "https://storage.googleapis.com/langmosheet-data/img/skills/Passive_BreakMDef.webp",
      "Desc": "[패시브]행동 종료 시 주위 <font color=\"red\">3</font>칸 내 모든 적 부대 마방 <font color=\"red\">-20</font>%, <font color=\"red\">1</font>턴 지속.",
      "SubSkills": null,
      "Animation": null
    },
    {
      "Name": "어둠의 낫",
      "Cost": "COST 2",
      "Entries": [
        {
          "Label": "유형",
          "Value": "마법 피해"
        },
        {
          "Label": "쿨타임",
          "Value": "2턴"
        },
        {
          "Label": "사거리",
          "Value": "2칸"
        },
        {
          "Label": "범위",
          "Value": "단일"
        }
      ],
      "IconImageUrl": "https://storage.googleapis.com/langmosheet-data/img/skills/Skill_DarkCrash.webp",
      "Desc": "[마법 피해]단일 적 부대를 공격해 <font color=\"red\">1.3</font>배의 피해를 준다.<br/>적의 마방을 <font color=\"red\">30</font>% 무시.<b>전투 후 준 피해량</b>f의 <font color=\"red\">50</font>%만큼 회복한다.",
      "SubSkills": null,
      "Animation": null
    },
    {
      "Name": "매혹",
      "Cost": "COST 2",
      "Entries": [
        {
          "Label": "유형",
          "Value": "마법 피해"
        },
        {
          "Label": "쿨타임",
          "Value": "3턴"
        },
        {
          "Label": "사거리",
          "Value": "3칸"
        },
        {
          "Label": "범위",
          "Value": "1칸"
        }
      ],
      "IconImageUrl": "https://storage.googleapis.com/langmosheet-data/img/skills/Skill_Patsyr1.webp",
      "Desc": "[마법 피해]전장에 있는 단일 개체 <font color=\"red\">0.1</font>배, 목표가 '<b><font color=\"#DB00DB\">여성</font>이 아닌 영웅</b>'일 경우 다음 턴 <font color=\"blue\">기절</font>,<br/><b>목표 행동 종료 시 주위 <font color=\"red\">1</font>바퀴 범위 다른 아군에게</b> <font color=\"blue\">고정피해</font><br/>(<b>고정피해량은 대상의 남은 생명력 <font color=\"red\">25</font>% 수치, 해당 효과는 해제 불가</b>)",
      "SubSkills": null,
      "Animation": null
    },
    {
      "Name": "꿈 탐식",
      "Cost": "COST 2",
      "Entries": [
        {
          "Label": "유형",
          "Value": "마법 피해"
        },
        {
          "Label": "쿨타임",
          "Value": "2턴"
        },
        {
          "Label": "사거리",
          "Value": "2칸"
        },
        {
          "Label": "범위",
          "Value": "단일"
        }
      ],
      "IconImageUrl": "https://storage.googleapis.com/langmosheet-data/img/skills/Skill_Patsyr2.webp",
      "Desc": "[마법 피해]단일 적 부대를 공격해 <font color=\"red\">1.5</font>배의 피해를 준다. <br/>목표가 '<b><font color=\"#DB00DB\">여성</font>이 아닌 영웅</b>'일 경우 피해 <font color=\"red\">+20</font>% 및 <font color=\"blue\">호위 무시</font>",
      "SubSkills": null,
      "Animation": null
    },
    {
      "Name": "몽마의 입맞춤",
      "Cost": "COST 1",
      "Entries": [
        {
          "Label": "유형",
          "Value": "패시브"
        }
      ],
      "IconImageUrl": "https://storage.googleapis.com/langmosheet-data/img/skills/Skill_Patsyr3.webp",
      "Desc": "[패시브]'<b><font color=\"#DB00DB\">여성</font>이 아닌 영웅과 교전하여 전투에 진입시</b><br/><b>전투 후 해당 전투에서 입힌 피해량</b> <font color=\"red\">30</font>% 수치로 생명 회복",
      "SubSkills": null,
      "Animation": null
    },
    {
      "Name": "파멸의 그림자 저주",
      "Cost": "COST 3",
      "Entries": [
        {
          "Label": "유형",
          "Value": "마법피해"
        },
        {
          "Label": "쿨타임",
          "Value": "4턴"
        },
        {
          "Label": "사거리",
          "Value": "6칸"
        },
        {
          "Label": "범위",
          "Value": "직선"
        }
      ],
      "IconImageUrl": "../img/Super/팟시르.webp",
      "Desc": "[패시브]<b>팟시르 사망 시, 전장에 있는 적군 중 모든 '<font color=\"#DB00DB\">여성</font>이 아닌 영웅</b>'에게 <br/>'<font color=\"blue\">패시브 봉인</font>' 효과 부여 (해제 불가)<br/>[마법피해]<font color=\"red\">3</font>줄 직선 상 적에게 <font color=\"red\">0.1</font>배<br/><b>대상이 방어 지형이 아닌 지형에 위치해 있을 경우</b><br/>추가 <font color=\"blue\">고정 피해</font> (지력 <font color=\"red\">2</font>배 수치, <b>면역 불가</b>)<br/>'<font color=\"purple\">그림자 저주</font>' 효과 부여 :<br/><b>'방어 지형이 아닌 지형'에서 행동 종료 시</b> <font color=\"blue\">고정 피해</font> (지력 <font color=\"red\">1.5</font>배 수치), <br/><b>면역 불가</b>, 지속 <font color=\"red\">2</font>행동 (<font color=\"purple\">그림자 저주</font> 효과는 <b>해제 불가</b>)",
      "SubSkills": null,
      "Animation": "../img/Super/팟시르_Ani.webp"
    }
  ],
  "Propose": false,
  "Bio": [
    {
      "Entry": "영웅소개",
      "Desc": "◆키/몸무게：166cm / 56kg <br/>◆출연：랑그릿사 리인카네이션 <br/>◆신체사이즈 : 91/60/89<br/>◆소속：어둠의 왕국 벨제리아"
    },
    {
      "Entry": "영웅담1",
      "Desc": "제멋대로에 변덕이 심한 마족 여성. 꿈의 경계를 마음대로 넘나들고 그림자를 조종할 수 있는 능력을 갖고 있는, <br/>리코리스의 가장 충성스러운 부하이자 동료이기도하다. <br/>매우 아름다운 모습과 마족다운 영혼의 소유자이지만 인간에 대해 호기심이 많으며 내기에 열중하는 모습을 보이기도 한다."
    },
    {
      "Entry": "영웅담2",
      "Desc": "쾌활하고 제멋대로인 성격에, 어떤 일에든 거리낌이 없는 그녀지만 <br/>마음 속 깊은 곳에 말로 설명하기 힘든 과거와 비밀을 간직하고 있다. <br/>독특하고 망설임 없는 그녀만의 스타일 때문에 동족인 마족들은 그녀를 조소하고 배척하며 별종 취급하였다. <br/>동족의 배척을 받은 그녀는 자신이 태어나고 머물던 곳을 떠나 <br/>흥미로운 일과 동료를 찾아 가기로 결심하게 된다."
    },
    {
      "Entry": "영웅담3",
      "Desc": "오랫동안 머물던 곳을 떠난 후, 마녀는 우연히 어디론가 급히 달려가는 병사를 보게 된다. <br/>줄곧 인간에게 관심을 갖고 있던 마녀는 나무 그림자 속에서 몰래 그 병사를 관찰했다. <br/>그리고 병사로 하여금 자신과 내기를 하도록 만들었다. <br/>하지만 마녀의 예상과는 달리 내기가 끝나자 그 병사는 그녀와 동행하기를 거부, <br/>자신을 압박하는 그녀에게 망설임 없는 자살로 답해주었다. <br/>충격을 받은 마녀는 내기를 깬 병사에게 복수하기 위해 그의 이름인 '팟시르'를 빼앗아 자신의 이름으로 삼았다.<br/><font color=\"#DC143C\"><size=33><i>「예전에 인간과 내기를 했단다. 결국 그 인간은 죽었지만 나는 그의 '팟시르'라는 이름을 받아 냈단다. <br/>후후, 그때의 내기가 정말 최고의 내기였다니까.」 </i>  </size=33></font>"
    },
    {
      "Entry": "영웅담4",
      "Desc": "변함없이 혼자인 팟시르는 자신의 '동류'를 찾기 위해 어둠의 군단에 합류했으며 벨제리아에서 리코리스를 만나게 된다. <br/>리코리스의 선량함과 순수함은 오랫동안 팟시르의 마음 속에 자리 잡은 안개를 거둬주었고, <br/>리코리스의 친절함 덕분에 팟시르는 점차 오랫동안 엉켜있던 마음의 매듭을 풀고 보다 밝게 변하였다.<br/><font color=\"#DC143C\"><size=33><i>「리코리스와 만났을 때, 나는 그녀의 눈동자에서 에서 따스함함을 느꼈어. <br/>그녀와 함께 있다보면, 내가 마족이였던 것을 잊게된단다.」 </i>  </size=33></font>"
    },
    {
      "Entry": "영웅담5",
      "Desc": "새로운 대륙에 오게 된 팟시르는 그 호기심과 충동으로 인해 또다른 곤경에 처하게 된다. <br/>과거의 어두운 안개가 다시 그녀의 꿈을 뒤덮을 때, 그녀는 순조롭게 위기를 극복할 수 있을 것인가? <br/>그리고 이 모든 것이 이쪽 세계의 영웅들과 부딪힐 때 어떤 흥미로운 불꽃을 일으키게 될 것인가?<br/><font color=\"#DC143C\"><size=33><i>「달빛 아래에 숨어 꿈 속에서 마음을 빼앗고... <br/>그리고 인간의 추악함을 폭로하는건 정말 즐겁다니까!」 </i>  </size=33></font>"
    }
  ],
  "Performance": [
    {
      "Entry": "「소환 - 등장대사」",
      "Desc": "달빛 안에 숨어 있는 건 우아한 존재 일까, 아니면 무서운 맹독과 같은 존재일까나? <br/>아아~ 이 길고 어두운 여정속에서 당신에게 더욱 많은 쾌락을 안겨 줄게~"
    },
    {
      "Entry": "「전투 - 전직1」",
      "Desc": "아하하, 이 새로운 변한 모습은 <br/>당신이 푹 빠져버릴꺼 같구나"
    },
    {
      "Entry": "「전투 - 전직2」",
      "Desc": "꿈을 깨뜨리는 힘이 강해진것 같사와요...넘쳐날거 같아!"
    },
    {
      "Entry": "「전투 - 스킬시전1」",
      "Desc": "어둠에 안겨 침묵해라!"
    },
    {
      "Entry": "「전투 - 스킬시전2」",
      "Desc": "꺠진 꿈의 경계속에서 해메거라!"
    },
    {
      "Entry": "「전투 - 선택1」",
      "Desc": "즐길 시간이구나"
    },
    {
      "Entry": "「전투 - 선택2」",
      "Desc": "더는 참을 수 없다고"
    },
    {
      "Entry": "「전투 - 전투불능」",
      "Desc": "으윽... 이것이 심연에 빠진다는 느낌인가..."
    },
    {
      "Entry": "「대화 - 친밀도1」",
      "Desc": "나는 인간이 무력한 모습을 좋아해. <br/>..... 특히 울상짓는 모습을 말이지. <br/>자연스럽게 귀엽다라는 표현이 떠오른다니까"
    },
    {
      "Entry": "「대화 - 친밀도2」",
      "Desc": "어둠의 군단에 합류한 이유~? <br/>물론, 어리석은 인간들에게 복수하기 위해서지~<br/>어머~? 무서운 얼굴을 짓지 마렴, 농담인게 당연하잖아~"
    },
    {
      "Entry": "「대화 - 친밀도3」",
      "Desc": "리코리스와 만났을 때, 나는 그녀의 눈동자에서 에서 따스함함을 느꼈어. <br/>그녀와 함께 있다보면, 내가 마족 이였던 것을 잊게된단다..."
    },
    {
      "Entry": "「대화 - 친밀도4」",
      "Desc": "아레스가 지금까지 만난 존재 중 내가 가장 쾌활한 마녀라고 했지만<br/>하지만 쾌활이라는 단어는 나에겐 어울리지 않단다.<br/>왜냐면, 나는 인간의 마음이 없는 마족인걸."
    },
    {
      "Entry": "「대화 - 친밀도5」",
      "Desc": "한명의 화가를 알고있단다. 나는 인간이 싫지만 <br/>그 인간의 조소와 광기에 공감을 해버린단다. 우리 마족의 본성을 보는 느낌인걸~"
    },
    {
      "Entry": "「대화 - 일상1」",
      "Desc": "달빛 아래에 숨어 꿈 속에서 마음을 빼앗고... <br/>그리고 인간의 추악함을 폭로하는건 정말 즐겁다니까!"
    },
    {
      "Entry": "「대화 - 기쁨」",
      "Desc": "예전에 인간과 내기를 했단다. 결국 그 인간은 죽었지만 나는 그의 '팟시르'라는 이름을 받아 냈단다. <br/>후후, 그때의 내기가 정말 최고의 내기였다니까."
    },
    {
      "Entry": "「대화 - 분노」",
      "Desc": "시시한 인간들의 잔꾀만큼 재미 없는건 없단다!"
    },
    {
      "Entry": "「대화 - 슬픔」",
      "Desc": "인간은 이렇게나 친밀한 존재구나... 인간의 마음이 없는 마족도 외롭다고 느껴지는구나.<br/>어느 한쪽만 일방적이여도 아무것도 해결할 수 없구나."
    },
    {
      "Entry": "「대화 - 수줍음」",
      "Desc": "흐응~  내 몸을 뚫어지게 쳐다보는것을 좋아하구나?<br/>당신이 궁금해하는 '마영'은 내가 조종하고 있는거란다.<br/>당신에게 마영을 조종하는 방법을 가르쳐줘도 좋다고 생각한단다~"
    },
    {
      "Entry": "「상호작용 - 대사1」",
      "Desc": "내 무기가 뭐냐고? 흥미가 생겼다면 특별히 보여줄까나~<br/>물론 당신의 피로 말이지~"
    },
    {
      "Entry": "「상호작용 - 대사2」",
      "Desc": "방금전에 당신이 타고 다녔던 말? 잡아 먹었버렸단다~<br/>무서워 하지마렴~ 장난도 유혹의 수단이라고 생각하렴?"
    },
    {
      "Entry": "「상호작용 - 대사3」",
      "Desc": "방금전에 했던 내기, 내가 그대로 이행할 거라고 생각하지 마렴<br/>왜냐하면~ ......당신에겐 내가 좋아하는 냄새가 나는걸~"
    },
    {
      "Entry": "「상호작용 - 호감」",
      "Desc": "좀더 가까이 오렴~ <br/>나에게도 인간들이 말하는 감미로운 기쁨이란것을 가르쳐주렴~"
    },
    {
      "Entry": "「상호작용 - 반감1」",
      "Desc": "저리가렴, 말 안들으면......당신의 귀를 잘라버릴거니까."
    },
    {
      "Entry": "「상호작용 - 특수1」",
      "Desc": "다음 만월때 함께 춤을 추자꾸나."
    }
  ]
}];