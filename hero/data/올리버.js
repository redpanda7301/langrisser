var data = [
{
  "Skins": [
    {
      "ImageUrl": "올리버/1.png",
      "Desc": "기본"
    }
  ],
  "Name": "올리버",
  "CV": "오카모토 노부히코",
  "Rank": [
    1
  ],
  "SP": false,
  "Groups": [
    8,
    7,
    1
  ],
  "HP": 20,
  "SPHP": 0,
  "Attk": 30,
  "SPAttk": 0,
  "Def": 25,
  "SPDef": 0,
  "MagicDef": 20,
  "SPMagicDef": 0,
  "HowToGet": "메인 시나리오, 시공의균열 보상",
  "Fetters": [
    {
      "Condition": "아레나 5승"
    },
    {
      "Condition": "클래스 쉐도우 나이트 마스터 전직"
    },
    {
      "Condition": "미지의기사를 도와 미지의기사 운명의문 4 클리어"
    },
    {
      "Condition": "셀파닐을 도와 셀파닐 운명의문 4 클리어"
    }
  ],
  "HeartFetter": "생명+600, 방어+40, 마방+40",
  "Talent": [
    {
      "Name": "독행 용사",
      "Icon": "40.png",
      "Abilities": [
        {
          "Desc": "1성"
        },
        {
          "Desc": "모든능력치 <font color=\"red\">+5%</font>. <br/>자신 주변 <font color=\"red\">2</font>칸 내에 아군이 있을경우</br>아군 <font color=\"red\">1</font>명 마다 모든능력치 <font color=\"red\">-5%</font> (최대 <font color=\"red\">-5%</font>)</br>전투 진입전 자신의 기술이 적군보다 높으면 적군의 강화버프 <font color=\"red\">1</font>개 강탈."
        },
        {
          "Desc": "모든능력치 <font color=\"red\">+10%</font>. <br/>자신 주변 <font color=\"red\">2</font>칸 내에 아군이 있을경우</br>아군 <font color=\"red\">1</font>명 마다 모든능력치 <font color=\"red\">-5%</font> (최대 <font color=\"red\">-10%</font>)</br>전투 진입전 자신의 기술이 적군보다 높으면 적군의 강화버프 <font color=\"red\">1</font>개 강탈."
        },
        {
          "Desc": "모든능력치 <font color=\"red\">+15%</font>. <br/>자신 주변 <font color=\"red\">2</font>칸 내에 아군이 있을경우</br>아군 <font color=\"red\">1</font>명 마다 모든능력치 <font color=\"red\">-5%</font> (최대 <font color=\"red\">-15%</font>)</br>전투 진입전 자신의 기술이 적군보다 높으면 적군의 강화버프 <font color=\"red\">1</font>개 강탈."
        },
        {
          "Desc": "모든능력치 <font color=\"red\">+20%</font>. <br/>자신 주변 <font color=\"red\">2</font>칸 내에 아군이 있을경우</br>아군 <font color=\"red\">1</font>명 마다 모든능력치 <font color=\"red\">-5%</font> (최대 <font color=\"red\">-20%</font>)</br>전투 진입전 자신의 기술이 적군보다 높으면 적군의 강화버프 <font color=\"red\">1</font>개 강탈."
        },
        {
          "Desc": "모든능력치 <font color=\"red\">+25%</font>. <br/>자신 주변 <font color=\"red\">2</font>칸 내에 아군이 있을경우</br>아군 <font color=\"red\">1</font>명 마다 모든능력치 <font color=\"red\">-5%</font> (최대 <font color=\"red\">-25%</font>)</br>전투 진입전 자신의 기술이 적군보다 높으면 적군의 강화버프 <font color=\"red\">1</font>개 강탈."
        }
      ]
    }
  ],
  "SPTalent": null,
  "Equip": {
    "Name": "위대한 왕의 증표(투구)",
    "Stats": "생명 437,마방 59",
    "Icon": "올리버.png",
    "Ability": "생명, 마방 +5%<br/>부대가 치명타공격을 입히면 1개의 강력한 디버프를 부여하고 버프를 1개 강탈."
  },
  "Soldiers": [
    8,
    10,
    17,
    18,
    40,
    57,
    59
  ],
  "Jobs": [
    {
      "Name": "하이 마스터",
      "Icon": "Icon_Occupation_Assassin.png",
      "SD": [
        {
          "ImageUrl": "올리버/1/1.png"
        }
      ],
      "AttkRange": 2,
      "Move": 3,
      "MoveType": "Walk",
      "HP": 3537,
      "HPRate": 58,
      "Attk": 503,
      "AttkRate": 77,
      "Int": 233,
      "IntRate": 38,
      "Def": 262,
      "DefRate": 58,
      "MagicDef": 250,
      "MagicDefRate": 55,
      "Skill": 317,
      "SkillRate": 90,
      "HeartFetterBouns1": "받는 범위피해 -10%",
      "HeartFetterBouns2": "생명이 100%일때 치명타확률 +10%"
    },
    {
      "Name": "쉐도우 나이트 마스터",
      "Icon": "Icon_Occupation_Assassin.png",
      "SD": [
        {
          "ImageUrl": "올리버/1/2.png"
        }
      ],
      "AttkRange": 2,
      "Move": 5,
      "MoveType": "Ride",
      "HP": 3273,
      "HPRate": 54,
      "Attk": 503,
      "AttkRate": 77,
      "Int": 233,
      "IntRate": 38,
      "Def": 262,
      "DefRate": 58,
      "MagicDef": 281,
      "MagicDefRate": 62,
      "Skill": 274,
      "SkillRate": 78,
      "HeartFetterBouns1": "생명이 100%일때 치명타를 받을확률 -10%",
      "HeartFetterBouns2": "강화버프를 5개이상 보유시 전투중 입히는피해 +10%"
    }
  ],
  "Materials": {
    "HeartFetter": [
      "h2",
      "h3",
      "h1",
      "h4",
      "h2-1",
      "h3-1",
      "h1-1",
      "h4-1",
      "x",
      "5",
      "5-1",
      "5-2",
      "5-3",
      "5-4"
    ],
    "Fetter": [
      "6",
      "6-1",
      "6-2",
      "6-3",
      "6-4"
    ],
    "Awaken": [
      "ranka3",
      "ranka6",
      "ranka3-1",
      "ranka6-1"
    ]
  },
  "ClassUp": [
    {
      "ClassTitle": "밴디트 : 공격+5, 기술+6",
      "Tiers": [
        {
          "Name": "T1",
          "Materials": [
            "rank1-2"
          ]
        },
        {
          "Name": "T2",
          "Materials": [
            "rank1-2"
          ]
        }
      ]
    },
    {
      "ClassTitle": "라이더 : 공격+12, 기술+5",
      "Tiers": [
        {
          "Name": "T1",
          "Materials": [
            "rank2-6",
            "rank1-2",
            "rank1-1"
          ]
        },
        {
          "Name": "T2",
          "Materials": [
            "rank3-2",
            "rank3-1",
            "rank2-3",
            "rank2-6"
          ]
        },
        {
          "Name": "T3",
          "Materials": [
            "rank4-3",
            "rank4-1",
            "rank3-2",
            "rank3-1"
          ]
        }
      ]
    },
    {
      "ClassTitle": "하이마스터 : 공격+25, 방어+10, 기술+10",
      "Tiers": [
        {
          "Name": "T1",
          "Materials": [
            "rank5-2",
            "rank5-1",
            "rank4-3",
            "rank4-1"
          ]
        },
        {
          "Name": "T2",
          "Materials": [
            "rank6-4",
            "rank6-3",
            "rank5-2",
            "rank5-1"
          ]
        },
        {
          "Name": "T3",
          "Materials": [
            "rank7-3",
            "rank7-6",
            "rank6-4",
            "rank6-3"
          ]
        },
        {
          "Name": "T4",
          "Materials": [
            "rank8-5",
            "rank8-9",
            "rank7-3",
            "rank7-6"
          ]
        }
      ]
    },
    {
      "ClassTitle": "쉐도우 나이트 : 생명+80, 기술+5",
      "Tiers": [
        {
          "Name": "T1",
          "Materials": [
            "rank2-3",
            "rank1-3",
            "rank1-1"
          ]
        },
        {
          "Name": "T2",
          "Materials": [
            "rank3-3",
            "rank3-2",
            "rank2-3",
            "rank2-1"
          ]
        },
        {
          "Name": "T3",
          "Materials": [
            "rank4-6",
            "rank4-1",
            "rank3-3",
            "rank3-2"
          ]
        }
      ]
    },
    {
      "ClassTitle": "쉐도우 나이트 마스터 : 생명+150, 공격+15, 기술+10",
      "Tiers": [
        {
          "Name": "T1",
          "Materials": [
            "rank5-3",
            "rank5-1",
            "rank4-6",
            "rank4-1"
          ]
        },
        {
          "Name": "T2",
          "Materials": [
            "rank6-6",
            "rank6-1",
            "rank5-3",
            "rank5-1"
          ]
        },
        {
          "Name": "T3",
          "Materials": [
            "rank7-7",
            "rank7-1",
            "rank6-6",
            "rank6-1"
          ]
        },
        {
          "Name": "T4",
          "Materials": [
            "rank8-7",
            "rank8-5",
            "rank7-7",
            "rank7-1"
          ]
        }
      ]
    }
  ],
  "SPOpenConditions": null,
  "Skills": [
    {
      "Name": "약탈",
      "Cost": "COST 2",
      "Entries": [
        {
          "Label": "유형",
          "Value": "물리 피해"
        },
        {
          "Label": "쿨타임",
          "Value": "2턴"
        },
        {
          "Label": "사거리",
          "Value": "2칸"
        },
        {
          "Label": "범위",
          "Value": "단일"
        }
      ],
      "IconImageUrl": "../img/skills/Skill_BladeCrash3.png",
      "Desc": "[물리 피해]호위를 무시하고 단일 적 부대를 공격해 1.1배의 피해를 준다.<br/>전투 후 상대가 가진 버프를 최대 3개 뺏어온다.",
      "SubSkills": null,
      "Animation": null
    },
    {
      "Name": "은폐",
      "Cost": "COST 1",
      "Entries": [
        {
          "Label": "유형",
          "Value": "패시브"
        }
      ],
      "IconImageUrl": "../img/skills/Passive_Hide.png",
      "Desc": "[패시브]공격하여 전투 진입 시 치명타율 +10%, 받는 피해량 -20%.",
      "SubSkills": null,
      "Animation": null
    },
    {
      "Name": "야전",
      "Cost": "COST 1",
      "Entries": [
        {
          "Label": "유형",
          "Value": "패시브"
        }
      ],
      "IconImageUrl": "../img/skills/10.png",
      "Desc": "[패시브]이동 시 통과할 수 있는 모든 지형을 '평지'로 취급한다.<br/>위치한 지형에 방어 상승 효과가 있을 경우 공격도 똑같이 상승한다.",
      "SubSkills": null,
      "Animation": null
    },
    {
      "Name": "구급",
      "Cost": "COST 1",
      "Entries": [
        {
          "Label": "유형",
          "Value": "치료"
        },
        {
          "Label": "쿨타임",
          "Value": "3턴"
        },
        {
          "Label": "사거리",
          "Value": "자신"
        },
        {
          "Label": "범위",
          "Value": "단일"
        }
      ],
      "IconImageUrl": "../img/skills/Skill_PHeal2.png",
      "Desc": "[치료]액티브 스킬, 자신 부대 생명을 100% 회복한다.",
      "SubSkills": null,
      "Animation": null
    },
    {
      "Name": "암살",
      "Cost": "COST 2",
      "Entries": [
        {
          "Label": "유형",
          "Value": "물리 피해"
        },
        {
          "Label": "쿨타임",
          "Value": "2턴"
        },
        {
          "Label": "사거리",
          "Value": "2칸"
        },
        {
          "Label": "범위",
          "Value": "단일"
        }
      ],
      "IconImageUrl": "../img/skills/Skill_BladeCrash2.png",
      "Desc": "[물리 피해]호위를 무시하고 단일 적 부대를 공격해 1.2배의 피해를 주고<br/>치명타율 +20%.",
      "SubSkills": null,
      "Animation": null
    },
    {
      "Name": "재이동",
      "Cost": "COST 1",
      "Entries": [
        {
          "Label": "유형",
          "Value": "패시브"
        }
      ],
      "IconImageUrl": "../img/skills/13.png",
      "Desc": "[패시브]공격하여 전투 진입 시 주는 피해량 +10%.<br/>남은 이동력이 있을 경우 공격 후 재이동 가능.",
      "SubSkills": null,
      "Animation": null
    },
    {
      "Name": "기술 위협",
      "Cost": "COST 2",
      "Entries": [
        {
          "Label": "유형",
          "Value": "패시브"
        }
      ],
      "IconImageUrl": "../img/skills/Passive_AuraDexDown.png",
      "Desc": "[패시브]행동 종료 시 주위 3칸 내 모든 적 부대 기술 -20%, 1턴 지속.",
      "SubSkills": null,
      "Animation": null
    },
    {
      "Name": "회심의 일격",
      "Cost": "COST 2",
      "Entries": [
        {
          "Label": "유형",
          "Value": "물리 피해"
        },
        {
          "Label": "쿨타임",
          "Value": "3턴"
        },
        {
          "Label": "사거리",
          "Value": "2칸"
        },
        {
          "Label": "범위",
          "Value": "단일"
        }
      ],
      "IconImageUrl": "../img/skills/Skill_BladeCrash5.png",
      "Desc": "단일 적 부대에 <font color=\"red\">1.3</font>배의 피해를 준다.<br/>전투 진입 전, <font color=\"red\">1</font>칸 이동할 때마다 기술 <font color=\"red\">+10%</font>. (최대 <font color=\"red\">30%</font> 중첩 가능)",
      "SubSkills": null,
      "Animation": null
    }
  ],
  "Bio": [
    {
      "Entry": "영웅소개",
      "Desc": "◆키/몸무게: 150cm / 50kg<br/>◆진영: 카콘시스 왕국"
    },
    {
      "Entry": "영웅담1",
      "Desc": "카콘시스의 왕위계승권을 지닌 쌍둥이 중 한 명이자, 전설의 혈통을 이어받은 열혈 왕자. 용사가 되는 것이 꿈인 겁 없는 소년. 예레스 전 국토를 덮친 전쟁 속에서 성검 군단 일행을 만나고, 예상조차 못 했던 모험의 여정을 떠나게 된다. <font color=\"#a52a2aff\"><size=33><i>'여어! 미래의 카콘시스 왕, 성검의 영웅 올리버님의 등장이시다!'</i></size=33></font>"
    },
    {
      "Entry": "영웅담2",
      "Desc": "세상 물정을 잘 모르고 쾌활하며, 용감하고 사소한 일은 신경 쓰지 않는 성격. 조용하고 어른스러운 여동생 멜파닐과는 정반대로, 어린애 같고 엉뚱한 면도 있다. 카콘시스 성이 벽록의 날개 병단에게 포위당했을 때, 단독으로 포위망에서 탈출하여 원군을 요청했다. 그때 성검 군단과 만나 일행과 함께 행동하기로 한다."
    },
    {
      "Entry": "영웅담3",
      "Desc": "올리버의 양친은 10년 전 제국의 기습에 목숨을 잃었다. 그 뒤 쌍둥이 전하의 뒷바라지나 교육은 올리버에게 '할머니'라고 불리는 대신 일루시아와, '누님'이라 불리는 미지의 기사가 맡았다.올리버는 미지의 기사의 정체를 알고 있다. 아직 어린 그에게 '누님'이 곁에 있어 주는 건 당연한 일이었다. 하지만, 언젠가 '누님'이 해준 모든 것에 보답을 하고 싶다는 마음을 가지고 있었다......"
    },
    {
      "Entry": "영웅담4",
      "Desc": "카콘시스 성 공방전의 와중, 벽록의 날개 병단의 단장 빈센트에게 소중한 여동생 멜파닐을 납치당하고 만다.동생을 구하기 위해, 올리버는 미지의 기사와 성검 군단 일행과 함께 수정 황야로 빈센트를 추격하는 여행을 떠난다. 올리버로서는 태어나서 처음으로 카콘시스의 바깥 세계에 발을 디딘 것이었다."
    },
    {
      "Entry": "영웅담5",
      "Desc": "여동생을 구하고 마물을 쓰러트린다. 이는 동화처럼 간단한 일  같았지만, 지금 그는 용사가 되기 위한 혹독한 시련을 서서히 실감하고 있다.올리버의 여행은 이제 막 시작했을 뿐...... 그에게 주어진 운명의 만남과 사건이 그를 기다리고 있다......<font color=\"#a52a2aff\"><size=33><i>'그러니까... 성검, 수정, 세계수... 으윽, 뭔지 하나도 모르겠어. 흑막이 누군지만 빨리 알려주지 않을래? '</i></size=33></font>"
    }
  ],
  "Performance": [
    {
      "Entry": "「소환 - 등장대사」",
      "Desc": "여어 ! 미래의 카콘시스 왕, 성검의 영웅 올리버님의 등장이다!"
    },
    {
      "Entry": "「전투 - 전직1」",
      "Desc": "좋았어~! 또 한 뼘 자란 기분이야!"
    },
    {
      "Entry": "「전투 - 전직2」",
      "Desc": "이걸로 누님과도 겨룰 수 있게 됐으려나?"
    },
    {
      "Entry": "「전투 - 스킬시전1」",
      "Desc": "돌격!"
    },
    {
      "Entry": "「전투 - 스킬시전2」",
      "Desc": "카콘시스를 위하여!"
    },
    {
      "Entry": "「전투 - 선택1」",
      "Desc": "다음 목표는 뭔데?"
    },
    {
      "Entry": "「전투 - 선택2」",
      "Desc": "준비는 끝났다!"
    },
    {
      "Entry": "「전투 - 전투불능」",
      "Desc": "미안... 멜파닐..."
    },
    {
      "Entry": "「대화 - 친밀도1」",
      "Desc": "일루시아 할머니는 못하는 게 없어. 그래서 할머니가 듬직하게 받쳐준다면, 난 여동생을 지키며 앞으로 나아갈 수 있어!"
    },
    {
      "Entry": "「대화 - 친밀도2」",
      "Desc": "누구든 멜파닐을 슬프게 하는 녀석은 오빠로서 용서하지 않아! 싸우는 이유는 그걸로 충분해!"
    },
    {
      "Entry": "「대화 - 친밀도3」",
      "Desc": "제국 녀석들... 그 녀석들도 사정이야 있겠지만, 카콘시스에 쳐들어온 죄만은 절대 용서치 못해!"
    },
    {
      "Entry": "「대화 - 친밀도4」",
      "Desc": "그러니까... 성검, 수정, 세계수... 으윽, 뭔가 하나도 모르겠어. 흑막이 누군지만 빨리 알려주지 않을래?"
    },
    {
      "Entry": "「대화 - 친밀도5」",
      "Desc": "나는 반드시 제국의 손에서, 동생과 일루시아 할머니와 이 카콘시스를 지켜낼 거야!"
    },
    {
      "Entry": "「대화 - 일상1」",
      "Desc": "하앗! 겨우 왕궁을 빠져나왔군! 자아, 드디어 내 영웅전설의 시작이다! 얏호!"
    },
    {
      "Entry": "「대화 - 기쁨」",
      "Desc": "후훗, 나 완전 멋있지? 그야말로 성검의 영웅! 이란 느낌 아냐?"
    },
    {
      "Entry": "「대화 - 분노」",
      "Desc": "내가 있는 한, 멜파닐한테는 손가락 하나 대지 못해!"
    },
    {
      "Entry": "「대화 - 슬픔」",
      "Desc": "카콘시스 왕국... 내 고향은 언제쯤 옛날의 영광과 평화를 되찾을 수 있는 걸까..."
    },
    {
      "Entry": "「상호작용 - 대사1」",
      "Desc": "저기, 혹시 너도 내 나이 때는, 빨리 어른이 되고 싶다거나, 강해지고 싶다고 생각했어?"
    },
    {
      "Entry": "「상호작용 - 대사2」",
      "Desc": "누님 얼굴은 언제 봐도 무섭네... 있지, 몰래 가서 누님 안대 뺏어볼까?"
    },
    {
      "Entry": "「상호작용 - 대사3」",
      "Desc": "어디서 왔는지는 모르지만, 저 수정병은 온 대륙에 퍼져서 사람들을 괴롭히고 있어. 나는 반드시 수정병으로부터 대륙을 지킬 거야. 미래의 카콘시스 왕의 명예에 걸고 맹세해!"
    },
    {
      "Entry": "「상호작용 - 호감1」",
      "Desc": "후훗, 내가 왕위에 오르면, 반드시 널 내 직속 기사로 임명할게!"
    },
    {
      "Entry": "「상호작용 - 반감1」",
      "Desc": "할머니이~! 이 녀석이 나 괴롭혀~!"
    },
    {
      "Entry": "「상호작용 - 특수1」",
      "Desc": "저기, 다음에는 언제 와줄 거야?"
    },
    {
      "Entry": "「상호작용 - 특수2」",
      "Desc": "바이바이~! 다음에 또 함께 모험을 떠나자고!"
    }
  ]
}];