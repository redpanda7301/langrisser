var data = [
{
  "Skins": [
    {
      "ImageUrl": "토와/1.webp",
      "Desc": "기본"
    },
    {
      "ImageUrl": "토와/2.webp",
      "Desc": "빛의 메아리"
    },
    {
      "ImageUrl": "토와/3.webp",
      "Desc": "형귀 뽑기"
    }
  ],
  "Name": "토와",
  "CV": "사와시로 미유키(토와)<br/>츠루오카 사토시(시타)",
  "Rank": [
    0
  ],
  "SP": false,
  "Groups": [
    11,
    3,
    6
  ],
  "HP": 30,
  "SPHP": 0,
  "Attk": 30,
  "SPAttk": 0,
  "Def": 40,
  "SPDef": 0,
  "MagicDef": 15,
  "SPMagicDef": 0,
  "HowToGet": "영웅 소환",
  "Fetters": [
    {
      "Condition": "토와를 출전시켜 시공의 균열 3-6 클리어"
    },
    {
      "Condition": "페가수스 별의 기사 전직"
    },
    {
      "Condition": "마이야의 도움받아 유대의 문3 클리어"
    },
    {
      "Condition": "아레스의 도움받아 유대의 문4 클리어"
    }
  ],
  "HeartFetter": "생명+900, 방어+60, 마방+30",
  "Talent": [
    {
      "Name": "방랑 군사",
      "Icon": "Gift_Towa.webp",
      "Abilities": [
        {
          "Desc": "1성"
        },
        {
          "Desc": "2성"
        },
        {
          "Desc": "<b><font color=\"blue\">아군에게 받는 버프</font></b> 지속 시간 <font color=\"red\">+1</font><br/><b>자신에게 걸린 버프 <font color=\"red\">1</font>개 마다 공격 방어</b><font color=\"red\">+1%</font><<br/>최대 <font color=\"red\">+5%</font>"
        },
        {
          "Desc": "<b><font color=\"blue\">아군에게 받는 버프</font></b> 지속 시간 <font color=\"red\">+1</font><br/><b>자신에게 걸린 버프 <font color=\"red\">1</font>개 마다 공격 방어</b><font color=\"red\">+2%</font><<br/>최대 <font color=\"red\">+10%</font>"
        },
        {
          "Desc": "<b><font color=\"blue\">아군에게 받는 버프</font></b> 지속 시간 <font color=\"red\">+1</font><br/><b>자신에게 걸린 버프 <font color=\"red\">1</font>개 마다 공격 방어</b><font color=\"red\">+3%</font><<br/>최대 <font color=\"red\">+15%</font>"
        },
        {
          "Desc": "<b><font color=\"blue\">아군에게 받는 버프</font></b> 지속 시간 <font color=\"red\">+1</font><br/><b>자신에게 걸린 버프 <font color=\"red\">1</font>개 마다 공격 방어</b><font color=\"red\">+4%</font><<br/>최대 <font color=\"red\">+20%</font>"
        }
      ]
    }
  ],
  "SPTalent": null,
  "Equip": null,
  "Soldiers": [
    48,
    52,
    47,
    74,
    57,
    68,
    35,
    24
  ],
  "Jobs": [
    {
      "Name": "방랑 검객",
      "Icon": "Icon_Occupation_Infantryman.webp",
      "SD": [
        {
          "ImageUrl": "토와/1/1.webp"
        },
        {
          "ImageUrl": "토와/2/1.webp"
        },
        {
          "ImageUrl": "토와/3/1.webp"
        }
      ],
      "AttkRange": 1,
      "Move": 3,
      "MoveType": "Walk",
      "HP": 4696,
      "HPRate": 72,
      "Attk": 560,
      "AttkRate": 93,
      "Int": 283,
      "IntRate": 33,
      "Def": 346,
      "DefRate": 82,
      "MagicDef": 283,
      "MagicDefRate": 50,
      "Skill": 159,
      "SkillRate": 50,
      "HeartFetterBouns1": "<b>부대 생명력이 <font color =\"red\">70</font>% 미만일시</b><br/>전투진입시 받는 피해량<font color =\"red\">-10</font>%",
      "HeartFetterBouns2": "<b>부대 생명력이 <font color =\"red\">100</font>% 일시</b><br/>주는 피해량 <font color =\"red\">+10</font>%"
    },
    {
      "Name": "페가수스 별의 기사",
      "Icon": "Icon_Occupation_Fly.webp",
      "SD": [
        {
          "ImageUrl": "토와/1/2.webp"
        },
        {
          "ImageUrl": "토와/2/2.webp"
        },
        {
          "ImageUrl": "토와/3/2.webp"
        }
      ],
      "AttkRange": 1,
      "Move": 5,
      "MoveType": "Fly",
      "HP": 4375,
      "HPRate": 75,
      "Attk": 560,
      "AttkRate": 80,
      "Int": 257,
      "IntRate": 41,
      "Def": 322,
      "DefRate": 67,
      "MagicDef": 319,
      "MagicDefRate": 70,
      "Skill": 176,
      "SkillRate": 70,
      "HeartFetterBouns1": "<b>공격하여 전투 진입시</b><br/><b>부대가</b> 받는 피해량<font color =\"red\">-10</font>%",
      "HeartFetterBouns2": "<b>공격받아 전투 진입시</b><br/>주는 피해량 <font color =\"red\">+10</font>%"
    }
  ],
  "Materials": {
    "HeartFetter": [
      "h3",
      "h4",
      "h2",
      "h1",
      "h3-1",
      "h4-1",
      "h2-1",
      "h1-1",
      "x"
    ],
    "Fetter": [
      "1",
      "1-1",
      "1-2",
      "1-3",
      "1-4"
    ],
    "Awaken": [
      "ranka1",
      "ranka4",
      "ranka1-1",
      "ranka4-1"
    ]
  },
  "ClassUp": [
    {
      "ClassTitle": "페가수스 나이트 : 공격+9, 기술+4",
      "Tiers": [
        {
          "Name": "T1",
          "Materials": [
            "rank1-3"
          ]
        },
        {
          "Name": "T2",
          "Materials": [
            "rank1-3"
          ]
        }
      ]
    },
    {
      "ClassTitle": "드래곤나이트 : 기술+8, 생명+48",
      "Tiers": [
        {
          "Name": "T1",
          "Materials": [
            "rank2-3",
            "rank1-3",
            "rank1-1"
          ]
        },
        {
          "Name": "T2",
          "Materials": [
            "rank3-3",
            "rank3-2",
            "rank2-3",
            "rank2-2"
          ]
        },
        {
          "Name": "T3",
          "Materials": [
            "rank4-6",
            "rank4-2",
            "rank3-3",
            "rank3-2"
          ]
        }
      ]
    },
    {
      "ClassTitle": "수수께끼의 여행자 : 생명+80, 기술+5",
      "Tiers": [
        {
          "Name": "T1",
          "Materials": [
            "rank2-1",
            "rank1-1",
            "rank1-2"
          ]
        },
        {
          "Name": "T2",
          "Materials": [
            "rank3-1",
            "rank3-6",
            "rank2-1",
            "rank2-2"
          ]
        },
        {
          "Name": "T3",
          "Materials": [
            "rank4-1",
            "rank4-2",
            "rank3-1",
            "rank3-6"
          ]
        }
      ]
    },
    {
      "ClassTitle": "방랑 검객 : 생명+90 공격+25, 방어+10",
      "Tiers": [
        {
          "Name": "T1",
          "Materials": [
            "rank5-1",
            "rank5-6",
            "rank4-1",
            "rank4-2"
          ]
        },
        {
          "Name": "T2",
          "Materials": [
            "rank6-1",
            "rank6-2",
            "rank5-1",
            "rank5-6"
          ]
        },
        {
          "Name": "T3",
          "Materials": [
            "rank7-2",
            "rank7-1",
            "rank6-1",
            "rank6-2"
          ]
        },
        {
          "Name": "T4",
          "Materials": [
            "rank8-2",
            "rank8-1",
            "rank7-1",
            "rank7-7"
          ]
        }
      ]
    },
    {
      "ClassTitle": "페가수스 장군 : 방어+5, 마방+8",
      "Tiers": [
        {
          "Name": "T1",
          "Materials": [
            "rank2-6",
            "rank1-3",
            "rank1-4"
          ]
        },
        {
          "Name": "T2",
          "Materials": [
            "rank3-5",
            "rank3-2",
            "rank2-3",
            "rank2-2"
          ]
        },
        {
          "Name": "T3",
          "Materials": [
            "rank4-6",
            "rank4-2",
            "rank3-5",
            "rank3-2"
          ]
        }
      ]
    },
    {
      "ClassTitle": "페가수스 별의 기사 : 생명+90, 공격+25, 방어+10",
      "Tiers": [
        {
          "Name": "T1",
          "Materials": [
            "rank5-3",
            "rank5-2",
            "rank4-6",
            "rank4-4"
          ]
        },
        {
          "Name": "T2",
          "Materials": [
            "rank6-5",
            "rank6-3",
            "rank5-3",
            "rank5-2"
          ]
        },
        {
          "Name": "T3",
          "Materials": [
            "rank7-4",
            "rank7-7",
            "rank6-5",
            "rank6-3"
          ]
        },
        {
          "Name": "T4",
          "Materials": [
            "rank8-3",
            "rank8-2",
            "rank7-4",
            "rank7-3"
          ]
        }
      ]
    }
  ],
  "SPOpenConditions": null,
  "Skills": [
    {
      "Name": "분뢰",
      "Cost": "COST 2",
      "Entries": [
        {
          "Label": "유형",
          "Value": "물리 피해"
        },
        {
          "Label": "쿨타임",
          "Value": "2턴"
        },
        {
          "Label": "사거리",
          "Value": "1칸"
        },
        {
          "Label": "범위",
          "Value": "단일"
        }
      ],
      "IconImageUrl": "../img/skills/Skill_FlyCrash2.webp",
      "Desc": "[물리 피해]단일 적 부대를 공격하여 <font color =\"red\">1.5</font>배의 전투 피해를 준다. <br/><b>전투 전</b> [<font color =\"blue\">바람 타기</font>]획득 :<br/><b>생명이 <font color =\"red\">50</font>% 이상인 경우 전투 진입 시</b> 받는 피해량 <font color =\"red\">-15</font>%, <font color =\"red\">2</font>턴 지속.",
      "SubSkills": null,
      "Animation": null
    },
    {
      "Name": "질풍",
      "Cost": "COST 1",
      "Entries": [
        {
          "Label": "유형",
          "Value": "패시브"
        }
      ],
      "IconImageUrl": "../img/skills/16.webp",
      "Desc": "[패시브] <b>행동 종료 시 <font color =\"red\">20</font>% 확률</b>로 <font color =\"blue\">재행동</font> 가능.",
      "SubSkills": null,
      "Animation": null
    },
    {
      "Name": "전술 지휘",
      "Cost": "COST 2",
      "Entries": [
        {
          "Label": "유형",
          "Value": "지휘,패시브"
        }
      ],
      "IconImageUrl": "../img/skills/Skill_DexCommand.webp",
      "Desc": "[패시브]<b>주위 <font color =\"red\">3</font>칸 내 아군 부대가 전투 시</b> 기술 <font color =\"red\">+10</font>%.",
      "SubSkills": null,
      "Animation": null
    },
    {
      "Name": "전술 지도",
      "Cost": "COST 1",
      "Entries": [
        {
          "Label": "유형",
          "Value": "지원"
        },
        {
          "Label": "쿨타임",
          "Value": "4턴"
        },
        {
          "Label": "사거리",
          "Value": "3칸"
        },
        {
          "Label": "범위",
          "Value": "단일"
        }
      ],
      "IconImageUrl": "../img/skills/Skill_Towa1.webp",
      "Desc": "<b>자신에게 걸린 <font color =\"blue\">해제 가능한 버프</font> <font color =\"red\">3</font>개를 목표 아군에게 복사</b><br/>지속 <font color =\"red\">2</font>행동<br/>사용 후 <font color =\"red\">2</font>칸 재행동 <br/><b>(해당 재행동은 버프 지속 시간을 소모하지 않음)</b>",
      "SubSkills": null,
      "Animation": null
    },
    {
      "Name": "자유의 검",
      "Cost": "COST 2",
      "Entries": [
        {
          "Label": "유형",
          "Value": "물리 피해"
        },
        {
          "Label": "쿨타임",
          "Value": "2턴"
        },
        {
          "Label": "사거리",
          "Value": "1칸"
        },
        {
          "Label": "범위",
          "Value": "단일"
        }
      ],
      "IconImageUrl": "../img/skills/Skill_Parn2.webp",
      "Desc": "[물리 피해] 단일 적군을 공격하여 <font color =\"red\">1.4</font>배의 피해.<br/><b>전투 진입전 대상에 '모든 <font color =\"blue\">패시브 스킬</font>의 효과를 무효화'</b> 효과 부여,<br/> 지속 <font color =\"red\">1</font>행동.",
      "SubSkills": null,
      "Animation": null
    },
    {
      "Name": "검기",
      "Cost": "COST 1",
      "Entries": [
        {
          "Label": "유형",
          "Value": "패시브"
        }
      ],
      "IconImageUrl": "../img/skills/Passive_DoubleAttack.webp",
      "Desc": "[패시브]<b>공격하여 전투 후 주위 1칸 범위 내 모든 적군에게</b> <font color =\"red\">0.1</font>배 <font color =\"blue\">범위 피해</font>를 입힌다.",
      "SubSkills": null,
      "Animation": null
    },
    {
      "Name": "헤비 어택",
      "Cost": "COST 2",
      "Entries": [
        {
          "Label": "유형",
          "Value": "물리 피해"
        },
        {
          "Label": "쿨타임",
          "Value": "3턴"
        },
        {
          "Label": "사거리",
          "Value": "1칸"
        },
        {
          "Label": "범위",
          "Value": "단일"
        }
      ],
      "IconImageUrl": "../img/skills/Skill_Thump.webp",
      "Desc": "[물리 피해]단일 적 부대를 공격해 <font color =\"red\">1.2</font>배의 공격 피해를 준다. <br/><b>자신이 방어 지형에 있을 시 전투 전</b> 대상에게 <font color =\"blue\">기절</font> 시전<br/><font color =\"red\">1</font>턴 지속.",
      "SubSkills": null,
      "Animation": null
    },
    {
      "Name": "공격 대지원",
      "Cost": "COST 2",
      "Entries": [
        {
          "Label": "유형",
          "Value": "패시브"
        }
      ],
      "IconImageUrl": "../img/skills/Skill_Again.webp",
      "Desc": "[패시브]<b>행동 종료 시 주위 <font color=\"red\">2</font>칸 내 <font color=\"red\">2</font>개 다른 아군 부대가</b> 주는 피해량 <font color=\"red\">+15%</font> 및 <br/>면역: '<font color =\"blue\">공격, 지력 감소</font>', '<font color =\"blue\">침묵</font>', <font color=\"red\">1</font>턴 지속.",
      "SubSkills": null,
      "Animation": null
    },
    {
      "Name": "우세 공격",
      "Cost": "COST 2",
      "Entries": [
        {
          "Label": "유형",
          "Value": "물리 피해"
        },
        {
          "Label": "쿨타임",
          "Value": "4턴"
        },
        {
          "Label": "사거리",
          "Value": "1칸"
        },
        {
          "Label": "범위",
          "Value": "단일"
        }
      ],
      "IconImageUrl": "../img/skills/Skill_Towa2.webp",
      "Desc": "[물리 피해]단일 개체 <font color =\"red\">1.5</font>배, 자신에게 걸린 버프 <font color =\"red\">1</font>개당 피해 <font color =\"red\">+2</font>% (최대 <font color =\"red\">10</font>%) <br/><b>전투 진입 전 자신에게 <font color =\"blue\">초절 강화 특수 효과</font>가 있을 경우 선제공격</b><br/><b>앞 조건을 만족한 상태에서 적에게 <font color =\"blue\">초절 강화 특수 효과</font>가 <font color =\"red\">없을 경우</font> <font color =\"blue\">호위 무시</font></b>",
      "SubSkills": null,
      "Animation": null
    },
    {
      "Name": "고대의 진법",
      "Cost": "COST 3",
      "Entries": [
        {
          "Label": "유형",
          "Value": "선택"
        },
        {
          "Label": "쿨타임",
          "Value": "5턴"
        }
      ],
      "IconImageUrl": "../img/Super/토와.webp",
      "Desc": "[패시브] '<font color =\"blue\">강화 불가</font>' 면역, <font color =\"blue\">각성 액티브 효과</font> 보유 시 <b>병사가 받는 물리 피해</b> <font color =\"red\">-35</font>%<br/> [액티브] 이하 효과 중 택일,<b>사용 후 <font color =\"red\">3</font>칸 <font color =\"blue\">재행동</font> 가능</b>",
      "SubSkills": [
        {
          "Name": "<b><font color=\"#0070c0\">진법 - 침공</font></b>",
          "Entries": null,
          "IconImageUrl": "../img/Super/토와_1.webp",
          "Desc": "[지원]'<font color =\"blue\">진법 - 침공</font>' 효과 획득 :<br/>[지휘]<b>주위 <font color =\"red\">3</font>칸 범위 내 아군의 공격 지력 <font color =\"red\">+15</font>%,<br/><b>선공으로 전투 진입 전 고정 피해 (공격력 <font color =\"red\">0.5</font>배 수치)</b><br/><b><font color =\"blue\">액티브 스킬</font>로 적 격파 후 해당 스킬 쿨타임 <font color =\"red\">-1</font></b>,<br/>지속 <font color =\"red\">3</font>행동, <b>해제 불가</b>"
        },
        {
          "Name": "<b><font color=\"#0070c0\">진법 - 절벽</font></b>",
          "Entries": null,
          "IconImageUrl": "../img/Super/토와_2.webp",
          "Desc": "[지원]'<font color =\"blue\">진법 - 절벽</font>'효과 획득 :<br/>[지휘]<b>주위 <font color =\"red\">3</font>칸 범위 내 아군의 방어 <font color =\"red\">+15</font>%</b>,<br/>'<font color =\"blue\">고정 피해</font>' '<font color =\"blue\">위치 이동</font>' 면역, 지속 <font color =\"red\">3</font>행동, <b>해제 불가</b>"
        }
      ],
      "Animation": "../img/Super/토와_Ani.webp"
    }
  ],
  "Propose": true,
  "Bio": [
    {
      "Entry": "영웅소개",
      "Desc": "◆키/몸무게：168cm / 55kg <br/>◆출연：랑그릿사 리인카네이션  <br/>◆신체사이즈 : 85/65/88<br/>◆소속：???"
    },
    {
      "Entry": "영웅담1",
      "Desc": "침묵의 검사, 수수께끼의 여행자, 불패의 방랑 군사... <br/>토와를 가리키는 칭호 그대로, 그녀는 대륙 곳곳을 돌아다녔다. <br/>근엄하고 말이 적은 토와의 모습은 타인의 눈에는 굉장히 신비롭게 비쳐졌는데, <br/>어느 누구도 그녀가 어디에서 왔고 어디로 가는지 알 수 없었다. <br/>그녀는 마치 스쳐지나가는 서늘한 바람처럼 잠시만 눈을 돌려도 새로운 여행을 시작해 사라지곤 했다. <br/>그리고 그런 그녀의 곁에는 언제나 기계 동료 시타가 함께했다.<br/><font color=\"#DC143C\"><size=33><i>「뿌리없는 평초처럼, 그림자 없는 달처럼...」 </i>  </size=33></font>"
    },
    {
      "Entry": "영웅담2",
      "Desc": "토와의 이야기는 지도에도 없는 어느 고독한 섬에서 시작되었다. <br/>섬의 해변에서 어린 토와는 이전의 기억을 모두 잃어버린 채 깨어났는데, 바로 그 곳에서 그녀는 일생의 동료인 시타를 만났다. <br/>그 고독한 섬은 상고시대 문명이 남긴 지식을 간직한 성지였으며, 기계 생명체인 시타 역시 상고시대 문명의 유산이었다. <br/>세상과 동떨어진 섬에서 과거를 잃은 사람과 미래를 잃은 문명의 고아가 서로 만난 것이다.<br/><font color=\"#DC143C\"><size=33><i>「토와 그 아이는 내가 세상에 남겨진 의미라 할 수 있지,  <br/>그러니 넘볼 생각 하지 말라고챠하하!」 </i>  </size=33></font>"
    },
    {
      "Entry": "영웅담3",
      "Desc": "살아남기 위해 토와는 시타의 지도를 받으며 상고시대 문명이 남긴 지식과 전투 기술을 익혀나갔고, <br/>그렇게 시간은 흘러 모든 지식을 깨우친 토와는 어느덧 어른이 되었다. <br/>'살아가는 의미와 자신의 길을 찾아라'라는, 그녀를 마지막 지식으로 인도할 정보를 얻게 된 토와는 <br/>무엇을 추구해야할 지 알게 되었음에 기뻐하며 섬을 떠나기로 결정했다. <br/>그리고 시타 역시 혼잣말을 중얼거리며 합류하면서 그들의 여행이 시작되었다."
    },
    {
      "Entry": "영웅담4",
      "Desc": "여행을 시작한 토와는 섬에서 익힌 지식 말고는 아무것도 아는 것이 없다시피했다. <br/>때문에 그녀는 어지러운 세상을 떠돌며 열세에 몰린 군대를 돕고, <br/>그 과정속에서 자신을 단련시켜 나갔는데, 오랜 시간이 흐르며 이런 과정은 대륙에서 전설로 전해지게 된다. <br/>그러던 와중 우연히 들린 마을에서 자경단의 싸움을 돕게 된 토와는 당시 견습 영주였던 아레스를 만나게 되었다. <br/>그리고 이 의외의 인연 속에서 토와는 아레스를 자신의 제자로 삼기로 결정, 자경단의 군사 역할을 맡게 된다."
    },
    {
      "Entry": "영웅담5",
      "Desc": "아레스 일행과의 여행이 끝난 후 토와는 다시 자신의 길을 찾기 위한 방랑을 시작한다. <br/>그리고 얼마 지나지 않아 엘사리아로 향하는 문이 열리고 새로운 길이 펼쳐지게 되었으니, <br/>수수께끼의 여행자에 대한 전설은 이번에도 대륙에 널리 퍼질 것인가?<br/><font color=\"#DC143C\"><size=33><i>「파도가 아무리 높아도 배 밑에 있고, 산이 아무리 높아도 사람의 발 밑에 있다. <br/>과거 내가 걸어온 길이 자신의 길을 결정한다! 」 </i>  </size=33></font>"
    }
  ],
  "Performance": [
    {
      "Entry": "「소환 - 등장대사」",
      "Desc": "[토와] : 참신한 방법이구나 <br/>[시타]무슨 잠꼬대 하고있냐, 인사나 제대로 하라고!"
    },
    {
      "Entry": "「전투 - 전직1」",
      "Desc": "[시타] : 좋아 젊은이, 이건 올바른 길을 걷고있다는 증거다!"
    },
    {
      "Entry": "「전투 - 전직2」",
      "Desc": "[토와] : 『강인한 의지와 불굴의 정신은 <br/>전사에게 있어서 가장 친한 전우다.』"
    },
    {
      "Entry": "「전투 - 스킬시전1」",
      "Desc": "[토와] : 문답무용!"
    },
    {
      "Entry": "「전투 - 스킬시전2」",
      "Desc": "[토와] : 악을 벤다!"
    },
    {
      "Entry": "「전투 - 선택1」",
      "Desc": "[토와] : 『병은 신속하게』"
    },
    {
      "Entry": "「전투 - 선택2」",
      "Desc": "[토와] : 무엇을?"
    },
    {
      "Entry": "「전투 - 전투불능」",
      "Desc": "[토와] : 실패는... 성공의 어머니다..."
    },
    {
      "Entry": "「대화 - 친밀도1」",
      "Desc": "[토와] : 무슨 용건이라도? <br/>[사와] : 용건이 없으면 저리가라는 것이다, 챠하하"
    },
    {
      "Entry": "「대화 - 친밀도2」",
      "Desc": "[토와] : 『인생에 꿈이 있는것이 아니라, 꿈이 인생을을 만든다.』라...<br/> 훌륭한 마음가짐이다. "
    },
    {
      "Entry": "「대화 - 친밀도3」",
      "Desc": "[토와] : 『파도가 아무리 높아도 배 밑에 있고, 산이 아무리 높아도 사람의 발 밑에 있다.』 <br/>[시타] : 챠하하, 너도 멀리까지 와버렸구만."
    },
    {
      "Entry": "「대화 - 친밀도4」",
      "Desc": "[토와] : 『콩깍지가 씌였다』라... 무슨 뜻일까? <br/> [시타] : 어이쿠, 넌 아직 꼬맹이니까, 이건 어른이 되고나서 하는거라고!"
    },
    {
      "Entry": "「대화 - 친밀도5」",
      "Desc": "[토와] : 오랜만이구나, 변함이 없어보이네 <br/>[시타] : 어디를 말하는 겐가? 듣지 못했다만 <br/>[토와] : 조용히 해! 지휘관에게 인사하고 있으니까."
    },
    {
      "Entry": "「대화 - 일상1」",
      "Desc": "[토와] : 나는 외딴 섬에서 지식을 익혔고, 시타를 만났어.<br/>옛날 일은 전혀 기억이 나지않아"
    },
    {
      "Entry": "「대화 - 기쁨」",
      "Desc": "[토와] : 후훗...<br/>[시타] : 어이, 토와가 웃었어! 뭔짓 한 거냐 가르쳐줘어!!"
    },
    {
      "Entry": "「대화 - 분노」",
      "Desc": "[토와] :『칼부림과 작전은 별개』 <br/>[시타] : 챠하하, 잽싸게 꽁무니를 내빼라고 지휘관!"
    },
    {
      "Entry": "「대화 - 슬픔」",
      "Desc": "[토와] : 언젠가는 찾아낼수 있을까 『자신의 길』... <br/>[시타] : 풀죽어 있지말라고."
    },
    {
      "Entry": "「대화 - 수줍음」",
      "Desc": "[토와] : 읏... <br/>[시타] : 이자식, 무슨 짓 한거냐? 평범하게 안 끝날거라고! 챠하하 "
    },
    {
      "Entry": "「상호작용 - 대사1」",
      "Desc": "[토와] : 이 세상에서도 기억을 잃은 사람이 있다고 들었는데 <br/>소개해주지 않을래?"
    },
    {
      "Entry": "「상호작용 - 대사2」",
      "Desc": "[시타] : 『뿌리없는 평초처럼, 그림자 없는 달처럼...』<br/>[토와] : ......나를 놀리는 거지?<br/>[시타]:챠하하! 검을 거두라고!"
    },
    {
      "Entry": "「상호작용 - 대사3」",
      "Desc": "[토와] : 나는 강해.<br/>[시타] : 그래, 강하다고!<br/>[토와] : 내게 기대도 좋아.<br/>[시타] : 안돼! 내가 용납 못해! 챠하하!"
    },
    {
      "Entry": "「상호작용 - 호감」",
      "Desc": "[토와] : 기억을 잃은 후 『자신의 길』을 찾는것이 내 삶의 의미가 되었어.<br/>내가 방랑하면서 겪은 이야기 들어주지 않겠어?"
    },
    {
      "Entry": "「상호작용 - 반감1」",
      "Desc": "[토와] : 『무례함은 무지하기에 생긴다.』 <br/>[시타] : 내 설명은 필요 없겠구만. 빨리 사과하라고 챠하하"
    },
    {
      "Entry": "「상호작용 - 특수1」",
      "Desc": "[토와] : 많은 것을 받았어, 고마워"
    }
  ]
}];