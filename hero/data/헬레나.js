var data = [
  {
    "Skins": [
      {
        "ImageUrl": "헬레나/1.webp",
        "Desc": "기본"
      },
      {
        "ImageUrl": "헬레나/2.webp",
        "Desc": "빛의 메아리"
      },
      {
        "ImageUrl": "헬레나/3.webp",
        "Desc": "형귀뽑기"
      },
      {
        "ImageUrl": "헬레나/4.webp",
        "Desc": "한정판매"
      },
      {
        "ImageUrl": "헬레나/5.webp",
        "Desc": "한정판매"
      },
      {
        "ImageUrl": "헬레나/6.webp",
        "Desc": "한정판매"
      }
    ],
    "Name": "헬레나",
    "ChName": "海伦娜",
    "EngName": "Helena",
    "JapName": "ヘレナ",
    "CV": "이노우에 마리나",
    "Rank": [
      0
    ],
    "SP": false,
    "Groups": [
      "빛군",
      "제국"
    ],
    "HP": 40,
    "SPHP": 0,
    "Attk": 35,
    "SPAttk": 0,
    "Def": 15,
    "SPDef": 0,
    "MagicDef": 10,
    "SPMagicDef": 0,
    "HowToGet": "영웅 소환",
    "Fetters": [
      {
        "Condition": "헬레나를 출전시켜 아레나 5승"
      },
      {
        "Condition": "헬레나를 로열나이트전직"
      },
      {
        "Condition": "브렌다의 도움하에 운명의문4"
      },
      {
        "Condition": "비라쥬의 도움하에 운명의문5"
      }
    ],
    "HeartFetter": "생명+600, 방어+30, 마방+40",
    "Talent": [
      {
        "TalentName": "황금 수정의 기사",
        "Icon": "Gift_Helena.webp",
        "Abilities": [
          {
            "Desc": "1성"
          },
          {
            "Desc": "2성"
          },
          {
            "Desc": "이동전 1칸당 피해증가 <font color=\"red\">1%</font>(최대 <font color=\"red\">5%</font>) <br/>주위 <font color=\"red\">2</font>칸내 아군이 없을경우 방어상승 <font color=\"red\">10%</font>.<br/>행동종료시 자신이 통과한 모든지형에 <font color=\"red\">1</font>행동동안 [크리스탈]효과 부여:<br/>\"플레이어 적군이 이 지형에서 행동종료시 <br/><font color=\"red\">50%</font>확률로 &#39;랜덤한 장비스킬 무효화(최대생명치제외)&#39; <font color=\"red\">2</font>행동 지속.<br/>&#39;NPC일 경우 랜덤한속성 <font color=\"red\">-25%</font>&#39; <font color=\"red\">1</font>행동 지속.\"<br/>이동시 적을 무시한 이동.<br/> [크리스탈]효과가 있는 지형을 통과시 이동력소모하지 않음."
          },
          {
            "Desc": "이동전 1칸당 피해증가 <font color=\"red\">2%</font>(최대 <font color=\"red\">10%</font>) <br/>주위 <font color=\"red\">2</font>칸내 아군이 없을경우 방어상승 <font color=\"red\">15%</font>.<br/>행동종료시 자신이 통과한 모든지형에 <font color=\"red\">1</font>행동동안 [크리스탈]효과 부여:<br/>\"플레이어 적군이 이 지형에서 행동종료시 <br/><font color=\"red\">60%</font>확률로 &#39;랜덤한 장비스킬 무효화(최대생명치제외)&#39; <font color=\"red\">2</font>행동 지속.<br/>&#39;NPC일 경우 랜덤한속성 <font color=\"red\">-25%</font>&#39; <font color=\"red\">1</font>행동 지속.\"<br/>이동시 적을 무시한 이동.<br/> [크리스탈]효과가 있는 지형을 통과시 이동력소모하지 않음."
          },
          {
            "Desc": "이동전 1칸당 피해증가 <font color=\"red\">3%</font>(최대 <font color=\"red\">15%</font>) <br/>주위 <font color=\"red\">2</font>칸내 아군이 없을경우 방어상승 <font color=\"red\">20%</font>.<br/>행동종료시 자신이 통과한 모든지형에 <font color=\"red\">1</font>행동동안 [크리스탈]효과 부여:<br/>\"플레이어 적군이 이 지형에서 행동종료시 <br/><font color=\"red\">80%</font>확률로 &#39;랜덤한 장비스킬 무효화(최대생명치제외)&#39; <font color=\"red\">2</font>행동 지속.<br/>&#39;NPC일 경우 랜덤한속성 <font color=\"red\">-25%</font>&#39; <font color=\"red\">1</font>행동 지속.\"<br/>이동시 적을 무시한 이동.<br/> [크리스탈]효과가 있는 지형을 통과시 이동력소모하지 않음."
          },
          {
            "Desc": "이동전 1칸당 피해증가 <font color=\"red\">4%</font>(최대 <font color=\"red\">20%</font>) <br/>주위 <font color=\"red\">2</font>칸내 아군이 없을경우 방어상승 <font color=\"red\">25%</font>.<br/>행동종료시 자신이 통과한 모든지형에 <font color=\"red\">1</font>행동동안 [크리스탈]효과 부여:<br/>\"플레이어 적군이 이 지형에서 행동종료시 <br/><font color=\"red\">100%</font>확률로 &#39;랜덤한 장비스킬 무효화(최대생명치제외)&#39; <font color=\"red\">2</font>행동 지속.<br/>&#39;NPC일 경우 랜덤한속성 <font color=\"red\">-25%</font>&#39; <font color=\"red\">1</font>행동 지속.\"<br/>이동시 적을 무시한 이동.<br/> [크리스탈]효과가 있는 지형을 통과시 이동력소모하지 않음."
          }
        ]
      }
    ],
    "SPTalent": null,
    "Second_Awaken": {
      "Name": "빛나는 수정",
      "Stats": "공격, 방어 <font color=\"red\">+5%</font><br/><font color=\"blue\">스킬</font> 사용으로 피해를 입힐 시, 자신이 아군이 생성한 <font color=\"purple\">수정</font> 위에 있을 경우, <font color=\"red\">50%</font> 확률로 스킬 쿨타임 <font color=\"red\">-3</font>"
    },
    "Equip": {
      "EqpName": "빛나는 달의 수정갑옷(갑옷)",
      "Stats": "생명 437,방어 65",
      "Icon": "헬레나.webp",
      "Ability": "생명 (<font color=\"red\">+1, +2, +3, +4, +5</font>)%,방어 (<font color=\"red\">+1, +2, +3, +4, +5</font>)%<br/><b>행동 종료 시 <font color=\"red\">1</font>행동간 주위 <font color=\"red\">1</font>바퀴 범위에 <font color=\"blue\">빛나는 수정</font> 특수 효과 추가,<br/>자신이 <font color=\"blue\">빛나는 수정</font> 지형에 있을 경우 받는 피해 <font color=\"red\">-15%</font></b> 및<br/><b>피격 후 자신 생명 <font color=\"red\">30%</font> </b> 수치  <b><font color=\"blue\">회복</font></b>"
    },
    "Soldiers": [
      "마스터 디노",
      "홀리 페가수스",
      "근위기병",
      "그레나디어",
      "천사",
      "황가 기병",
      "하이랜더",
      "왕 도마뱀 기수",
      "무지개 페가수스"
    ],
    "Jobs": [
      {
        "Name": "스트라이크 마스터",
        "Icon": "기병",
        "SD": [
          {
            "ImageUrl": "헬레나/1/1.webp"
          },
          {
            "ImageUrl": "헬레나/2/1.webp"
          },
          {
            "ImageUrl": "헬레나/3/1.webp"
          },
          {
            "ImageUrl": "헬레나/4/1.webp"
          },
          {
            "ImageUrl": "헬레나/5/1.webp"
          },
          {
            "ImageUrl": "헬레나/6/1.webp"
          }
        ],
        "AttkRange": 1,
        "Move": 5,
        "MoveType": "기마",
        "HP": 4184,
        "HPRate": 70,
        "Attk": 609,
        "AttkRate": 94,
        "Int": 233,
        "IntRate": 39,
        "Def": 275,
        "DefRate": 61,
        "MagicDef": 254,
        "MagicDefRate": 56,
        "Skill": 113,
        "SkillRate": 32,
        "HeartFetterBouns1": "부대 생명이 70%이하일때 전투진입시 받는피해 -10%",
        "HeartFetterBouns2": "공격받아 전투진입시 입히는피해 +10%"
      },
      {
        "Name": "로열 나이트",
        "Icon": "기병",
        "SD": [
          {
            "ImageUrl": "헬레나/1/2.webp"
          },
          {
            "ImageUrl": "헬레나/2/1.webp"
          },
          {
            "ImageUrl": "헬레나/3/1.webp"
          },
          {
            "ImageUrl": "헬레나/4/1.webp"
          },
          {
            "ImageUrl": "헬레나/5/1.webp"
          },
          {
            "ImageUrl": "헬레나/6/1.webp"
          }
        ],
        "AttkRange": 1,
        "Move": 5,
        "MoveType": "기마",
        "HP": 4511,
        "HPRate": 75,
        "Attk": 578,
        "AttkRate": 89,
        "Int": 233,
        "IntRate": 39,
        "Def": 287,
        "DefRate": 64,
        "MagicDef": 283,
        "MagicDefRate": 63,
        "Skill": 113,
        "SkillRate": 32,
        "HeartFetterBouns1": "받는 범위피해 -10%",
        "HeartFetterBouns2": "생명비율이 자신보다 높은 상대에게 공격하여 전투진입시 입히는피해 +10%"
      }
    ],
    "Materials": {
      "HeartFetter": [
        "h1",
        "h2",
        "h4",
        "h3",
        "h1-1",
        "h2-1",
        "h4-1",
        "h3-1",
        "x"
      ],
      "Fetter": [
        "2",
        "2-1",
        "2-2",
        "2-3",
        "2-4"
      ],
      "Awaken": [
        "ranka2",
        "ranka6",
        "ranka2-1",
        "ranka6-1"
      ]
    },
    "ClassUp": [
      {
        "ClassTitle": "나이트 : 생명+36, 공격+9",
        "Tiers": [
          {
            "Name": "T1",
            "Materials": [
              "rank1-3"
            ]
          },
          {
            "Name": "T2",
            "Materials": [
              "rank1-3"
            ]
          }
        ]
      },
      {
        "ClassTitle": "그랜드나이트 : 공격+12, 생명+48",
        "Tiers": [
          {
            "Name": "T1",
            "Materials": [
              "rank2-3",
              "rank1-3",
              "rank1-1"
            ]
          },
          {
            "Name": "T2",
            "Materials": [
              "rank3-3",
              "rank3-2",
              "rank2-3",
              "rank2-1"
            ]
          },
          {
            "Name": "T3",
            "Materials": [
              "rank4-6",
              "rank4-1",
              "rank3-3",
              "rank3-2"
            ]
          }
        ]
      },
      {
        "ClassTitle": "스트라이크마스터 : 공격+25, 생명+90, 마방+10",
        "Tiers": [
          {
            "Name": "T1",
            "Materials": [
              "rank5-3",
              "rank5-1",
              "rank4-6",
              "rank4-1"
            ]
          },
          {
            "Name": "T2",
            "Materials": [
              "rank6-6",
              "rank6-1",
              "rank5-3",
              "rank5-1"
            ]
          },
          {
            "Name": "T3",
            "Materials": [
              "rank7-7",
              "rank7-1",
              "rank6-6",
              "rank6-1"
            ]
          },
          {
            "Name": "T4",
            "Materials": [
              "rank8-7",
              "rank8-5",
              "rank7-7",
              "rank7-1"
            ]
          }
        ]
      },
      {
        "ClassTitle": "하이랜더 : 생명+48, 공격+12",
        "Tiers": [
          {
            "Name": "T1",
            "Materials": [
              "rank2-3",
              "rank1-3",
              "rank1-1"
            ]
          },
          {
            "Name": "T2",
            "Materials": [
              "rank3-3",
              "rank3-2",
              "rank2-3",
              "rank2-1"
            ]
          },
          {
            "Name": "T3",
            "Materials": [
              "rank4-6",
              "rank4-1",
              "rank3-3",
              "rank3-2"
            ]
          }
        ]
      },
      {
        "ClassTitle": "로열나이트 : 생명+150, 공격+15, 방어+10",
        "Tiers": [
          {
            "Name": "T1",
            "Materials": [
              "rank5-3",
              "rank5-1",
              "rank4-6",
              "rank4-1"
            ]
          },
          {
            "Name": "T2",
            "Materials": [
              "rank6-6",
              "rank6-2",
              "rank5-3",
              "rank5-1"
            ]
          },
          {
            "Name": "T3",
            "Materials": [
              "rank7-7",
              "rank7-1",
              "rank6-6",
              "rank6-2"
            ]
          },
          {
            "Name": "T4",
            "Materials": [
              "rank8-7",
              "rank8-3",
              "rank7-7",
              "rank7-1"
            ]
          }
        ]
      }
    ],
    "SPOpenConditions": null,
    "Skills": [
      {
        "Name": "질풍 돌격",
        "Cost": "COST 2",
        "Entries": [
          {
            "Label": "유형",
            "Value": "물리피해"
          },
          {
            "Label": "쿨타임",
            "Value": "2턴"
          },
          {
            "Label": "사거리",
            "Value": "1칸"
          },
          {
            "Label": "범위",
            "Value": "단일"
          }
        ],
        "IconImageUrl": "../img/skills/Skill_KnightCrash2.webp",
        "Desc": "[물리 피해]단일 적 부대를 공격하여 1.5배의 전투 피해를 준다.<br/>전투 전 부대의 방어, 마방 +30%.",
        "SubSkills": null,
        "Animation": null
      },
      {
        "Name": "크리스탈 스톰",
        "Cost": "COST 1",
        "Entries": [
          {
            "Label": "유형",
            "Value": "패시브"
          }
        ],
        "IconImageUrl": "../img/skills/Skill_Helena1.webp",
        "Desc": "[패시브]행동종료시, 주위 1바퀴 범위내 모든 적군에게 0.1배의 범위피해를 주고,<br/>25%확률로 적군에게 '임의의 장비스킬 무효화(최대생명치제외)', 2턴 지속.<br/>NPC의 경우 임의의 속성 -25%, 1턴 지속.",
        "SubSkills": null,
        "Animation": null
      },
      {
        "Name": "격돌",
        "Cost": "COST 1",
        "Entries": [
          {
            "Label": "유형",
            "Value": "물리피해"
          },
          {
            "Label": "쿨타임",
            "Value": "2턴"
          },
          {
            "Label": "사거리",
            "Value": "1칸"
          },
          {
            "Label": "범위",
            "Value": "단일"
          }
        ],
        "IconImageUrl": "../img/skills/Passive_Punch.webp",
        "Desc": "[물리 피해]단일 적 부대를 공격해 1.2배의 피해를 준다.<br/>대상을 2칸 밀쳐내며 이동력 -2, '호위 불가'부여, 1턴 지속.",
        "SubSkills": null,
        "Animation": null
      },
      {
        "Name": "폭풍",
        "Cost": "COST 1",
        "Entries": [
          {
            "Label": "유형",
            "Value": "패시브"
          }
        ],
        "IconImageUrl": "../img/skills/Passive_KnightWave.webp",
        "Desc": "[패시브]공격하여 전투 진입 시 부대 생명이 80% 이상이라면<br/>전투 중 받는 피해량-30%.",
        "SubSkills": null,
        "Animation": null
      },
      {
        "Name": "수정 광휘 돌격",
        "Cost": "COST 2",
        "Entries": [
          {
            "Label": "유형",
            "Value": "물리피해"
          },
          {
            "Label": "쿨타임",
            "Value": "5턴"
          },
          {
            "Label": "사거리",
            "Value": "1칸"
          },
          {
            "Label": "범위",
            "Value": "단일"
          }
        ],
        "IconImageUrl": "../img/skills/Skill_Helena2.webp",
        "Desc": "[물리 피해]단일 적 부대를 공격해 1.6배의 피해를 준다.<br/>전투후 적군의 '장신구 스킬 무효화', 2턴 지속. 제거불가.<br/>NPC의 경우 '받는피해상승 +30%', 2턴 지속. 제거불가.",
        "SubSkills": null,
        "Animation": null
      },
      {
        "Name": "진형파괴",
        "Cost": "COST 2",
        "Entries": [
          {
            "Label": "유형",
            "Value": "물리피해"
          },
          {
            "Label": "쿨타임",
            "Value": "3턴"
          },
          {
            "Label": "사거리",
            "Value": "1칸"
          },
          {
            "Label": "범위",
            "Value": "단일"
          }
        ],
        "IconImageUrl": "../img/skills/Skill_KnightCrash3.webp",
        "Desc": "[물리 피해]단일 적 부대를 공격해 1배의 공격 피해를 준다.<br/>대상을 2칸 밀쳐내며 다음 턴 행동 불가 부여.",
        "SubSkills": null,
        "Animation": null
      },
      {
        "Name": "무기 파괴",
        "Cost": "COST 1",
        "Entries": [
          {
            "Label": "유형",
            "Value": "패시브"
          }
        ],
        "IconImageUrl": "../img/skills/Passive_BreakAtk.webp",
        "Desc": "[패시브]전투 진입전 <font color=\"red\">50%</font> 확률로 적의 공격, 지력  <font color=\"red\">-20%</font>,  <font color=\"red\">1</font>행동지속.",
        "SubSkills": null,
        "Animation": null
      },
      {
        "Name": "기사도",
        "Cost": "COST 2",
        "Entries": [
          {
            "Label": "유형",
            "Value": "지원"
          },
          {
            "Label": "쿨타임",
            "Value": "4턴"
          },
          {
            "Label": "사거리",
            "Value": "자신"
          },
          {
            "Label": "범위",
            "Value": "단일"
          }
        ],
        "IconImageUrl": "../img/skills/Skill_KnightSoul.webp",
        "Desc": "[지원]액티브 스킬, 자신 부대의 공격+30%, 동시에 [면역],[폭풍],[증원]효과 부여, 2턴 지속.<br/>사용 후 추가로 3칸 이동할 수 있고 공격이 가능하다.",
        "SubSkills": null,
        "Animation": null
      },
      {
        "Name": "피어나는 황금 수정",
        "Cost": "COST 3",
        "Entries": [
          {
            "Label": "유형",
            "Value": "물리피해"
          },
          {
            "Label": "쿨타임",
            "Value": "5턴"
          },
          {
            "Label": "사거리",
            "Value": "1칸"
          },
          {
            "Label": "범위",
            "Value": "단일"
          }
        ],
        "IconImageUrl": "../img/Super/헬레나.webp",
        "Desc": "[물리 피해]단일 적 부대를 공격해 1.6배의 피해를 준다.<br/>전투후 적군의 '무기 기술 무효화', 2턴 지속. 제거불가.<br/>NPC의 경우 '입히는피해 -30%', 2턴 지속. 제거불가.<br/>전투후 주위 2칸내 모든 적군에게 0.2배의 범위피해를 입힌다.<br/>동시에 명중한 지형에 1회합 동안 [크리스탈]효과 부여<br/>(해당 효과는 고유기술의 크리스탈 효과와 동일)",
        "SubSkills": null,
        "Animation": "../img/Super/헬레나_Ani.webp"
      }
    ],
    "Propose": true,
    "Bio": [
      {
        "Entry": "영웅소개",
        "Desc": "◆키/몸무게: 175cm / 53kg<br/>◆신체 사이즈: 90/55/87<br/>◆소속：레겐부르그 제국"
      },
      {
        "Entry": "영웅담1",
        "Desc": "레겐부르그 제국의 사천왕 중 하나로, 황금 수정 기사단 단장. <br/>전란 속에서도 행실이 바르며 도덕을 중시하는 최고의 기사. <br/>레겐부르그 국경에 있는 수정 사막을 지키는 임무를 맡았다. <br/>카콘시스 왕국에 간 적이 없어 그 명성만 알려져 있으며, 진짜 그녀가 어떤 사람인지는 베일에 싸여 있었다.<br/><font color=\"#DC143C\"><size=33><i>「무슨 일이든 정도가 있는 법이다. 전투가 살육으로 바뀌어서는 안 된다!」 </i>  </size=33></font>"
      },
      {
        "Entry": "영웅담3",
        "Desc": "그러나 황금 수정 기사단 단장이라는 화려한 신분 이면에는 이종족인 '크림조랜더'로서 사람들의 적대심과 박해 속에서 고통스럽게 자란 과거가 있다. <br/>'종언전쟁'이 끝나고 지상에 남은 크림조랜더는 기아와 병마에 시달리던 지상인들의 고통의 배출구가 되어 박해를 받았다.<br/>오랜 세월 동안 전란과 분쟁으로 엉망이 되었지만, 크림조랜더는 싸우기를 원하지 않았다. <br/>하지만 그 상황이 폭도나 선동자들을 더욱더 포악하게 바꾸고 말았다. <br/>상황이 바뀐 것은 헬레나가 사람들을 지켜야 한다는 막중한 책무를 맡게 된 때였다. 그<br/>그녀의 지시로 크림조랜더는 점차 다시 일어섰으며, 제국의 횡포로부터 권리를 되찾기 위해 저항하기 시작했다.<br/><font color=\"#DC143C\"><size=33><i>「잔혹한 폭도들...... 우린 물러서지 않겠다! 우리 검으로 일족의 행복을 지키겠다!」 </i>  </size=33></font>"
      },
      {
        "Entry": "영웅담5",
        "Desc": "그리고 지금, 운석이 떨어지고 다시 전쟁의 불꽃이 예레스 대륙에 뒤덮으려 한다. <br/>새로 탄생한 성검이 안개를 걷어내고, 과거에 한 약속이 깨지려 하는 이 순간, <br/>이상적인 '정의'를 위해 싸워 온 헬레나는 새로운 선택에 직면하게 된다...<br/><font color=\"#DC143C\"><size=33><i>「이 나라는 이미 붕괴되고 있다. 자신의 이익을 위해 음모를 꾸미는 자가 없다고 말할 수 있나? <br/>누군가가 사람들을 지켜야 한다...... 적어도 나는 그 길을 선택하겠다!」 </i>  </size=33></font>"
      }
    ],
    "Performance": [
      {
        "Entry": "「소환 - 등장대사」",
        "Desc": "황금 수정 기사단 단장 헬레나, 부름을 받고 왔습니다! 지휘관님, 지시를 내려주십시오!"
      },
      {
        "Entry": "「전투 - 전직1」",
        "Desc": "자비는 나약함이 아니다! 소중한 것을 지키려면 강해져야 해!"
      },
      {
        "Entry": "「전투 - 전직2」",
        "Desc": "좋아, 이걸로 또 새로운 전술을 사용할 수 있겠어!"
      },
      {
        "Entry": "「전투 - 스킬시전1」",
        "Desc": "황금 수정 기사단, 돌격!"
      },
      {
        "Entry": "「전투 - 스킬시전2」",
        "Desc": "사명은 내가 완수한다!"
      },
      {
        "Entry": "「전투 - 선택1」",
        "Desc": "정의를 위해!"
      },
      {
        "Entry": "「전투 - 선택2」",
        "Desc": "어떤 전술을 생각 중이지?"
      },
      {
        "Entry": "「전투 - 전투불능」",
        "Desc": "미안하다, 난 이제...... 모두를 지킬 수 없어......"
      },
      {
        "Entry": "「대화 - 친밀도1」",
        "Desc": "빈센트 말인가...... 참으로 별난 녀석이다. <br/>동료이긴 하지만, 전쟁에 대한 야심과 잔혹한 수단만큼은 인정할 수 없지."
      },
      {
        "Entry": "「대화 - 친밀도2」",
        "Desc": "카콘시스...... 분명 아름답고 풍족한 나라겠지...... <br/>침략자로서가 아닌 일개 여행자로서 그곳을 가보고 싶군."
      },
      {
        "Entry": "「대화 - 친밀도3」",
        "Desc": "지금도 여왕 폐하의 관용에 감사하고 있다. <br/>나는 폐하의 자비로 전쟁 기계가 되지 않았으니까. <br/>그리고 우리 일족도 박해에서 벗어났으니."
      },
      {
        "Entry": "「대화 - 친밀도4」",
        "Desc": "기사의 사명은 약자를 지키는 것이다. <br/>아이들이 전쟁으로 고통받는 건 절대로 용납 못 해! <br/>설령 적이라 해도, 온 힘을 다해 구해야 해......"
      },
      {
        "Entry": "「대화 - 친밀도5」",
        "Desc": "지상에 남겨진 우리 달의 일족은 수없이 많은 괴로움을 맛보았다...... <br/>모두를 지키기 위해, 진정한 평화를 위해...... 어떤 고난이라도 뛰어넘을 것이다!"
      },
      {
        "Entry": "「대화 - 일상1」",
        "Desc": "전쟁에는 정의의 싸움과 불의의 싸움이 있다. <br/>멀리서 지켜보겠습니다.이 난세에서는 선택할 수가 없지만...... <br/>기사로서 양보할 수 없는 부분은 지키고 싶다."
      },
      {
        "Entry": "「대화 - 기쁨」",
        "Desc": "훌륭한 전투였다. 순수한 '승리'의 기쁨을 느꼈었던 풋내기 기사 시절이 떠오르는군."
      },
      {
        "Entry": "「대화 - 분노」",
        "Desc": "잔혹한 폭도들...... 우린 물러서지 않겠다! 우리 검으로 일족의 행복을 지키겠다!"
      },
      {
        "Entry": "「대화 - 슬픔」",
        "Desc": "거듭되는 전쟁, 살육...... 평화로운 날이 과연 올까?"
      },
      {
        "Entry": "「대화 - 수줍음」",
        "Desc": "난 군인이라 외모나 몸매를 신경 써본 적은 없는데...... 당신이 보기엔 내가 예쁜건가?"
      },
      {
        "Entry": "「상호작용 - 대사1」",
        "Desc": "음모와 전쟁이 난무하는 이 시대에<br/>당신같이 다정하고 정의로운 지휘관이 있어 참으로 위로가 되는군."
      },
      {
        "Entry": "「상호작용 - 대사2」",
        "Desc": "이상적인 '정의'를 실현하려면 꾸준한 실천과 노력이 필요하다.<br/>같이 있어줘서 고맙군. 당신과 함께 싸우면서 희망의 빛을 찾아냈겠어!"
      },
      {
        "Entry": "「상호작용 - 대사3」",
        "Desc": "나는 전란 속에서 일족을 지키기 위해 과거를 계속 숨겨왔다......<br/>이 비밀은 다른 사람에게 말하지 말아 주십시오!"
      },
      {
        "Entry": "「상호작용 - 호감」",
        "Desc": "지켜야 할 상대가 또 한 명 늘었군...... <br/>날 인정해주셔서 대단히 기쁘군, 지휘관."
      },
      {
        "Entry": "「상호작용 - 반감1」",
        "Desc": "무슨 일이든 정도가 있다. 그만해주시죠?"
      },
      {
        "Entry": "「상호작용 - 특수1」",
        "Desc": "당신과 같이 있으면 모든 것을 잊을 수 있다...... 또 오면 좋겠군..."
      }
    ],
    "Performance2": null,
    "Misc": ""
  }
]