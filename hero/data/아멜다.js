var data = [
    {        
        "Skins": [
            {
                "ImageUrl": "아멜다/1.png",
                "Desc": "기본"
            },
            {
                "ImageUrl": "아멜다/2.png",
                "Desc": "스킨상점"
            },
            {
                "ImageUrl": "아멜다/3.png",
                "Desc": "한정판매"
            },
            {
                "ImageUrl": "아멜다/4.png",
                "Desc": "이벤트"
            },
            {
                "ImageUrl": "아멜다/5.png",
                "Desc": "한정판매"
            },           
            {
                "ImageUrl": "아멜다/6.png",
                "Desc": "한정판매"
            }
        ],       
        "Name": "아멜다",
        "CV": "우치다 마아야",
        "Rank": [ // 0 - SSR, 1 - SR, 2 - R, 3 - N
            3,
            0
        ],
        "SP": false,
        "Groups": [
            0, // 주인공
            1, // 빛
            10 // 공주
        ],
        "HP": 20,
        "Attk": 10,
        "Def": 30,
        "MagicDef": 35,
        "HowToGet": "기본지급",
        "Fetters": [ // 유대 순서를 반드시 지킬 것
            {
                "Condition": "아멜다 레벨 30 달성"
            },
            {
                "Condition": "아멜다를 출전시켜 시공의 균열 3-1[정예] 클리어"
            },
            {
                "Condition": "크리스를 도와 운명의 문3 클리어"
            },
            {
                "Condition": "쉐리를 도와 운명의 문7 클리어"
            }
        ],
        "HeartFetter": "생명+500, 방어+20, 마방+60",
        "Talent": [
            {
                "Name": "딴지 걸기",
                "Icon": "1.png",
                "Abilities": [ // 성급 순서를 반드시 지킬 것
                    {
                        "Desc": "행동 종료 시 주위 <font color=\"red\">3</font>칸 내 <font color=\"red\">1</font>개 적군 부대에게 무작위 디버프 <font color=\"red\">1</font>개 부여.<br>동시에 생명이 제일 낮은 다른 아군 부대의 생명을 아멜다 지력의 <font color=\"red\">1</font>배만큼 회복."
                    },
                    {
                        "Desc": "행동 종료 시 주위 <font color=\"red\">3</font>칸 내 <font color=\"red\">1</font>개 적군 부대에게 무작위 디버프 <font color=\"red\">1</font>개 부여.<br>동시에 생명이 제일 낮은 다른 아군 부대의 생명을 아멜다 지력의 <font color=\"red\">1.3</font>배만큼 회복."
                    },
                    {
                        "Desc": "행동 종료 시 주위 <font color=\"red\">3</font>칸 내 <font color=\"red\">2</font>개 적군 부대에게 무작위 디버프 <font color=\"red\">1</font>개 부여.<br>동시에 생명이 제일 낮은 다른 아군 부대의 생명을 아멜다 지력의 <font color=\"red\">1.6</font>배만큼 회복."
                    },
                    {
                        "Desc": "행동 종료 시 주위 <font color=\"red\">4</font>칸 내 <font color=\"red\">2</font>개 적군 부대에게 무작위 디버프 <font color=\"red\">1</font>개 부여.<br>동시에 생명이 제일 낮은 다른 아군 부대의 생명을 아멜다 지력의 <font color=\"red\">1.9</font>배만큼 회복."
                    },
                    {
                        "Desc": "행동 종료 시 주위 <font color=\"red\">4</font>칸 내 <font color=\"red\">3</font>개 적군 부대에게 무작위 디버프 <font color=\"red\">1</font>개 부여.<br>동시에 생명이 제일 낮은 다른 아군 부대의 생명을 아멜다 지력의 <font color=\"red\">2.2</font>배만큼 회복."
                    },
                    {
                        "Desc": "행동 종료 시 주위 <font color=\"red\">4</font>칸 내 <font color=\"red\">4</font>개 적군 부대에게 무작위 디버프 <font color=\"red\">1</font>개 부여.<br>동시에 생명이 제일 낮은 다른 아군 부대의 생명을 아멜다 지력의 <font color=\"red\">2.5</font>배만큼 회복."
                    }
                ]
            }
        ],
        "SPTalent": null,
        "Equip": {
            "Name": "■ ■ ■ ■(무기)",
            "Stats": "생명 583,지력 85",
            "Icon": "아멜다.png",
            "Ability": "공격하여 피해를 입힌 후 적군에게 '이동력 -1, 호위불가' 디버프 부여, 1턴 지속"
        },
        "Soldiers": [
            0,
            1,
            7,
            15,
            17,
            44,
            58
        ],
        "Jobs": [
            {
                "Name": "세인트",
                "Icon": "Icon_Occupation_Monk.png",
                "SD": [
                    {
                        "ImageUrl": "아멜다/1/1.png"
                    },
                    {
                        "ImageUrl": "아멜다/2/1.png"
                    },
                    {
                        "ImageUrl": "아멜다/3/1.png"
                    },
                    {
                        "ImageUrl": "아멜다/4/1.png"
                    },
                    {
                        "ImageUrl": "아멜다/5/1.png"
                    },
                    {
                        "ImageUrl": "아멜다/6/1.png"
                    }
                ],
                "AttkRange": 2,
                "Move": 3,
                "MoveType": "Walk", // "Walk, Ride, Fly, Water, FieldArmy"
                "HP": 3378,
                "HPRate": 56,
                "Attk": 311,
                "AttkRate": 47,
                "Int": 491,
                "IntRate": 81,
                "Def": 250,
                "DefRate": 55,
                "MagicDef": 399,
                "MagicDefRate": 88,
                "Skill": 81,
                "SkillRate": 23,
                "HeartFetterBouns1": "체력이 80%이상일 시 치명타를 받을확률 -10%",
                "HeartFetterBouns2": "범위피해 +10%"
            },
            {
                "Name": "오라클",
                "Icon": "Icon_Occupation_Monk.png",
                "SD": [
                    {
                        "ImageUrl": "아멜다/1/2.png"
                    },
                    {
                        "ImageUrl": "아멜다/2/1.png"
                    },
                    {
                        "ImageUrl": "아멜다/3/1.png"
                    },
                    {
                        "ImageUrl": "아멜다/4/1.png"
                    },
                    {
                        "ImageUrl": "아멜다/5/1.png"
                    },
                    {
                        "ImageUrl": "아멜다/6/1.png"
                    }
                ],
                "AttkRange": 2,
                "Move": 3,
                "MoveType": "Walk", // "Walk, Ride, Fly, Water, FieldArmy"
                "HP": 3227,
                "HPRate": 53,
                "Attk": 311,
                "AttkRate": 47,
                "Int": 491,
                "IntRate": 81,
                "Def": 230,
                "DefRate": 51,
                "MagicDef": 408,
                "MagicDefRate": 90,
                "Skill": 81,
                "SkillRate": 23,
                "HeartFetterBouns1": "체력이 100%시 받는 피해 -10%",
                "HeartFetterBouns2": "공격하여 전투시 입히는피해+10%"
            }
        ],
        "Materials": {
            "HeartFetter": [
                "h3",
                "h4",
                "h2",
                "h1",
                "h3-1",
                "h4-1",
                "h2-1",
                "h1-1",
                "x",
            ],
            "Fetter": [
                "3",
                "3-1",
                "3-2",
                "3-3",
                "3-4"
            ],
            "Awaken": [
                "ranka1",
                "ranka3",
                "ranka1-1",
                "ranka3-1"
            ]
        },
        "ClassUp": [ // 2개씩 한묶음으로 표시됨.
            {
                "ClassTitle": "견습 시스터 : 지력+3, 마방+1",
                "Tiers": [
                    {
                        "Name": "T1",
                        "Materials": [
                            "rank1-4"
                        ]
                    }
                ]
            },               
            {
                "ClassTitle": "시스터 : 마방+6, 지력+5",
                "Tiers": [
                    {
                        "Name": "T1",
                        "Materials": [
                            "rank1-4"
                        ]
                    },
                    {
                        "Name": "T2",
                        "Materials": [
                            "rank1-4"
                        ]
                    }
                ]
            },
            {
                "ClassTitle": "마터 : 마방+8, 지력+7",
                "Tiers": [
                    {
                        "Name": "T1",
                        "Materials": [
                            "rank2-5",
                            "rank1-5",
                            "rank1-4"
                        ]
                    },
                    {
                        "Name": "T2",
                        "Materials": [
                            "rank3-4",
                            "rank3-5",
                            "rank2-5",
                            "rank2-4"
                        ]
                    },
                    {
                        "Name": "T3",
                        "Materials": [
                            "rank4-5",
                            "rank4-4",
                            "rank3-4",
                            "rank3-5"
                        ]
                    }
                ]
            },
            {
                "ClassTitle": "세인트 : 지력+25, 마방+10, 생명+90",
                "Tiers": [
                    {
                        "Name": "T1",
                        "Materials": [
                            "rank5-4",
                            "rank5-5",
                            "rank4-5",
                            "rank4-4"
                        ]
                    },
                    {
                        "Name": "T2",
                        "Materials": [
                            "rank6-8",
                            "rank6-7",
                            "rank5-4",
                            "rank5-5"
                        ]
                    },
                    {
                        "Name": "T3",
                        "Materials": [
                            "rank7-5",
                            "rank7-6",
                            "rank6-8",
                            "rank6-7"
                        ]
                    },
                    {
                        "Name": "T4",
                        "Materials": [
                            "rank8-6",
                            "rank8-4",
                            "rank7-5",
                            "rank7-6"
                        ]
                    }
                ]
            },
            {
                "ClassTitle": "주교 : 마방+8, 생명+48",
                "Tiers": [
                    {
                        "Name": "T1",
                        "Materials": [
                            "rank2-5",
                            "rank1-5",
                            "rank1-4"
                        ]
                    },
                    {
                        "Name": "T2",
                        "Materials": [
                            "rank3-4",
                            "rank3-5",
                            "rank2-5",
                            "rank2-4"
                        ]
                    },
                    {
                        "Name": "T3",
                        "Materials": [
                            "rank4-5",
                            "rank4-4",
                            "rank3-4",
                            "rank3-5"
                        ]
                    }
                ]
            },
            {
                "ClassTitle": "오라클 : 마방+16, 생명+90, 지력+15",
                "Tiers": [
                    {
                        "Name": "T1",
                        "Materials": [
                            "rank5-4",
                            "rank5-5",
                            "rank4-5",
                            "rank4-4"
                        ]
                    },
                    {
                        "Name": "T2",
                        "Materials": [
                            "rank6-8",
                            "rank6-7",
                            "rank5-4",
                            "rank5-5"
                        ]
                    },
                    {
                        "Name": "T3",
                        "Materials": [
                            "rank7-5",
                            "rank7-6",
                            "rank6-8",
                            "rank6-7"
                        ]
                    },
                    {
                        "Name": "T4",
                        "Materials": [
                            "rank8-6",
                            "rank8-4",
                            "rank7-5",
                            "rank7-6"
                        ]
                    }
                ]
            },
            {
                "ClassTitle": "프리스트 : 마방+8, 생명+48",
                "Tiers": [
                    {
                        "Name": "T1",
                        "Materials": [
                            "rank2-5",
                            "rank1-5",
                            "rank1-4"
                        ]
                    },
                    {
                        "Name": "T2",
                        "Materials": [
                            "rank3-4",
                            "rank3-5",
                            "rank2-5",
                            "rank2-4"
                        ]
                    },
                    {
                        "Name": "T3",
                        "Materials": [
                            "rank4-5",
                            "rank4-4",
                            "rank3-4",
                            "rank3-5"
                        ]
                    }
                ]
            }            
        ],
        "SPOpenConditions": null,
        "Skills": [
            {
                "Name": "힐",
                "IsPassive": false,
                "Cost": 1,
                "Type": 5, // 0 - 해당사항 없음, 1 - 물리피해, 2 - 마법피해, 3 - 초절강화, 4 - 지원, 5 - 치료
                "CoolTime": 0, // 0 - 해당사항 없음
                "Distance": 2, // 0 - 자신, 1 ~ - 사거리
                "Range": 1, // 1 - 단일, 2~98 - 범위, 99 - 맵전체
                "IconImageUrl": "../img/skills/17.png",
                "Desc": "[치료] 액티브 스킬. 단일 부대의 생명을 회복시킨다.</br>회복량은 사용자 지력의 5배. 그리고 디버프를 1개 제거한다.",
                "Animation": null
            },
            {
                "Name": "프로텍션",
                "IsPassive": false,
                "Cost": 1,
                "Type": 4, // 0 - 해당사항 없음, 1 - 물리피해, 2 - 마법피해, 3 - 초절강화, 4 - 지원, 5 - 치료
                "CoolTime": 1, // 0 - 해당사항 없음
                "Distance": 3, // 0 - 자신 / 해당사항 없음, 1 ~ - 사거리
                "Range": 1, // 0 - 해당사항 없음, 1 - 단일, 2~97 - 범위, 98 - 직선, 99 - 맵전체
                "IconImageUrl": "../img/skills/18.png",
                "Desc": "[지원]액티브 스킬, 1개 부대의 방어 +20% 및</br>면역: '방어, 마방 감소', '치료 방해', 2턴 지속.",
                "Animation": null
            },
            {
                "Name": "성언",
                "IsPassive": false,
                "Cost": 2,
                "Type": 2, // 0 - 해당사항 없음, 1 - 물리피해, 2 - 마법피해, 3 - 초절강화, 4 - 지원, 5 - 치료
                "CoolTime": 1, // 0 - 해당사항 없음
                "Distance": 2, // 0 - 자신 / 해당사항 없음, 1 ~ - 사거리
                "Range": 1, // 0 - 해당사항 없음, 1 - 단일, 2~97 - 범위, 98 - 직선, 99 - 맵전체
                "IconImageUrl": "../img/skills/19.png",
                "Desc": "[마법 피해] 단일 적 부대를 공격해 1.5배의 피해를 준다.</br>'마물'에게 더 큰 피해를 준다. 전투 후 생명이 가장 적은 아군의 생명을 회복시킨다.</br>회복량은 사용자 지력의 4배.",
                "Animation": null
            },
            {
                "Name": "토네이도",
                "IsPassive": false,
                "Cost": 2,
                "Type": 2, // 0 - 해당사항 없음, 1 - 물리피해, 2 - 마법피해, 3 - 초절강화, 4 - 지원, 5 - 치료
                "CoolTime": 3, // 0 - 해당사항 없음
                "Distance": 3, // 0 - 자신 / 해당사항 없음, 1 ~ - 사거리
                "Range": 3, // 0 - 해당사항 없음, 1 - 단일, 2~97 - 범위, 98 - 직선, 99 - 맵전체
                "IconImageUrl": "../img/skills/20.png",
                "Desc": "[마법 피해] 범위 내 다수의 적군을 공격 하여 0.3배 범위 피해를 입힌다.</br>'비병'과 전투 시 더 큰 피해를 준다.</br>적군의 공격, 지력 -20%, 2턴 지속.",
                "Animation": null
            },
            {
                "Name": "메타 발언",
                "IsPassive": false,
                "Cost": 2,
                "Type": 2, // 0 - 해당사항 없음, 1 - 물리피해, 2 - 마법피해, 3 - 초절강화, 4 - 지원, 5 - 치료
                "CoolTime": 5, // 0 - 해당사항 없음
                "Distance": 3, // 0 - 자신 / 해당사항 없음, 1 ~ - 사거리
                "Range": 3, // 0 - 해당사항 없음, 1 - 단일, 2~97 - 범위, 98 - 직선, 99 - 맵전체
                "IconImageUrl": "../img/skills/21.png",
                "Desc": "[마법 피해] 0.3배의 광역 피해를 주고</br>범위 내 적군의 버프를 1개 제거하며 무작위 디버프를 1개 부여한다.",
                "Animation": null
            },
            {
                "Name": "명상",
                "IsPassive": true,
                "Cost": 1,
                "Type": 0, // 0 - 해당사항 없음, 1 - 물리피해, 2 - 마법피해, 3 - 초절강화, 4 - 지원, 5 - 치료
                "CoolTime": 0, // 0 - 해당사항 없음
                "Distance": 0, // 0 - 자신 / 해당사항 없음, 1 ~ - 사거리
                "Range": 0, // 0 - 해당사항 없음, 1 - 단일, 2~97 - 범위, 98 - 직선, 99 - 맵전체
                "IconImageUrl": "../img/skills/22.png",
                "Desc": "[패시브] 부대 생명이 90%이상일 시 지력, 마방 +10%.",
                "Animation": null
            },
            {
                "Name": "매스 힐",
                "IsPassive": false,
                "Cost": 2,
                "Type": 5, // 0 - 해당사항 없음, 1 - 물리피해, 2 - 마법피해, 3 - 초절강화, 4 - 지원, 5 - 치료
                "CoolTime": 1, // 0 - 해당사항 없음
                "Distance": 3, // 0 - 자신 / 해당사항 없음, 1 ~ - 사거리
                "Range": 3, // 0 - 해당사항 없음, 1 - 단일, 2~97 - 범위, 98 - 직선, 99 - 맵전체
                "IconImageUrl": "../img/skills/23.png",
                "Desc": "[치료] 액티브 스킬. 범위 내 아군들의 생명을 회복시킨다.</br>회복량은 사용자 지력의 3배. 그리고 디버프를 1개 제거한다.",
                "Animation": null
            },
            {
                "Name": "성룡의 숨결",
                "IsPassive": false,
                "Cost": 2,
                "Type": 2, // 0 - 해당사항 없음, 1 - 물리피해, 2 - 마법피해, 3 - 초절강화, 4 - 지원, 5 - 치료
                "CoolTime": 5, // 0 - 해당사항 없음
                "Distance": 5, // 0 - 자신 / 해당사항 없음, 1 ~ - 사거리
                "Range": 98, // 0 - 해당사항 없음, 1 - 단일, 2~97 - 범위, 98 - 직선, 99 - 맵전체
                "IconImageUrl": "../img/skills/24.png",
                "Desc": "[마법 피해] 3개의 라인에 있는 모든 적군을 공격하여 0.36배 범위 피해를 입힌다.</br>'마물'과 전투 시 더 큰 피해를 입힌다. 버프를 1개 제거한다.",
                "Animation": null
            },
            {
                "Name": "레지스트",
                "IsPassive": false,
                "Cost": 1,
                "Type": 4, // 0 - 해당사항 없음, 1 - 물리피해, 2 - 마법피해, 3 - 초절강화, 4 - 지원, 5 - 치료
                "CoolTime": 1, // 0 - 해당사항 없음
                "Distance": 3, // 0 - 자신 / 해당사항 없음, 1 ~ - 사거리
                "Range": 1, // 0 - 해당사항 없음, 1 - 단일, 2~97 - 범위, 98 - 직선, 99 - 맵전체
                "IconImageUrl": "../img/skills/25.png",
                "Desc": "[지원]액티브 스킬, 1개 부대의 마방 +30% 및 </br>면역: '기절','강화 방해','이동력 감소', 2턴지속.",
                "Animation": null
            },
            {
                "Name": "조롱 대회",
                "IsPassive": false,
                "Cost": 3,
                "Type": 4, // 0 - 해당사항 없음, 1 - 물리피해, 2 - 마법피해, 3 - 초절강화, 4 - 지원, 5 - 치료
                "CoolTime": 5, // 0 - 해당사항 없음
                "Distance": 0, // 0 - 자신 / 해당사항 없음, 1 ~ - 사거리
                "Range": 1, // 0 - 해당사항 없음, 1 - 단일, 2~97 - 범위, 98 - 직선, 99 - 맵전체
                "IconImageUrl": "/img/Super/Skill_Super36.png",
                "Desc": "[지원] 액티브 스킬. 범위 내 아군들의 생명을 회복시킨다.</br>회복량은 사용자 지력의 3배. 동시에 [조롱] 획득.<br>[조롱]: \"행동 종료시, 주위 4칸 이내 2기 적군에게 1개의 디버프 부여\", 2턴 지속.",
                "Animation": "/img/Super/아멜다.gif"
            }
        ],
        "Bio": [
            {
                "Entry": "영웅소개",
                "Desc": "◆키/몸무게：163cm / 46kg<br>◆신체 사이즈：82/46/84<br>◆소속：새내기 용사"
            },
            {
                "Entry": "영웅담1",
                "Desc": "칼자스 출신의 소녀로 아직 남아있는 빛의 후예이다. 그 혈통 탓에 하이보레안 제국의 위협을 받아 어린 시절부터 같이 보낸 매튜, 그레니어와 함께 도망 길에 나선다. 그리고 여신의 힘을 받아 빛의 후예로서 성검을 다시 만드는 사명을 짊어지게 된다. 시공의 균열을 뛰어넘는 힘을 갖고 있어 대륙에 퍼진 성검의 기억을 통해 옛 시대의 영웅을 소환할 수 있다.<br><font color=#DC143C><size=33><i>「내 몸속에는 대단한 힘이 있어. 나를 잘 지켜주라고!」</i> </size> </font>"
            },
            {
                "Entry": "영웅담2",
                "Desc": "이상할 정도로 활발하며 사람을 어지럽게 할 만큼 조롱하는 능력이 뛰어나다. 몸 안에 성검 랑그릿사의 조각이 있기에 여신 루시리스를 강림시킬 수 있다. 그리고 여신이 강림하면 마치 마녀처럼 몸이 떠오르게 된다."
            },
            {
                "Entry": "영웅담3",
                "Desc": "모험을 시작했을 당시에는 드디어 무미건조한 시골 생활에서 벗어나 낯설고 다채로운 바깥 세계로 떠날 수 있어서 기쁘기까지 했다. 하지만 현실의 잔혹함을 마주한 그녀의 생각은 빠르게 바뀌었다. 어둠의 위협이 점차 다가오고 주저할 때마다 무고한 사람이 희생되었다. 이제 아멜다는 평온한 생활로 다시 돌아갈 수 없음을 깨닫게 되었다.<br><font color=#DC143C><size=33><i>「칼자스의 숲에서 보냈던 아무 걱정 없던 소년 시절로는 다시 돌아갈 수 없겠지...」</i> </size> </font>"
            },
            {
                "Entry": "영웅담4",
                "Desc": "다른 시공에서 온 영웅들이 아멜다의 주위로 모여들고 이런 분위기는 그녀에게 「자신 역시 전설 속 영웅」이 된 느낌을 들게 하였다. 늦은 밤, 아멜다는 조용히 혼자서 「시간의 시계」를 열어보곤 한다. 시간의 시계를 열면 과거 성검의 기억을 볼 수 있음을 알기에 어쩌면 자신의 혈통에 관한 것도 알 수 있지 않을까 하고 기대하고 있는 것이다. 생각해보니 그레니어의 아버지는 아멜다에게 그녀의 부모가 누군지 말해준 적이 없었다. 그저 두 남자아이에게 그녀를 보호해주라 부탁했을 뿐이다. 다음에 기회가 된다면 꼭 아저씨께 확실히 물어봐야겠야겠다고 생각했다."
            },
            {
                "Entry": "영웅담5",
                "Desc": "매번 시공의 균열에 들어갈 때마다 성검의 힘 일부분이 아멜다의 몸속에 모여들었다. 그리고 아멜다 역시 빛의 영혼이 가진 힘을 느낄 수 있었다. 그것은 랑그릿사가 그녀에게 건네는 작은 속삭임이었다. 성검이 다시 만들어지는 순간, 자신의 운명이 어떻게 될지는 아멜다 자신도 알 수 없었다. 하지만 그녀는 랑그릿사가 고대 지크하르트 왕의 영혼으로 만들어졌다는 것을 알고 있었다. 만약 자신의 영혼을 바쳐야 한다면 그녀 역시 망설이지 않을 것이다. 그저 이 사실을 어떻게 동료들에게 털어놓아야 할지 모를 뿐...<br><font color=#DC143C><size=33><i>「대단한 사명을 짊어졌다지만... 나 역시 그저 여자아이일 뿐인걸...」</i> </size> </font></i> </size> </font>"
            }
        ],
        "Performance": [
            {
                "Entry": "「전투 - 전직1」",
                "Desc": "휴우... 이걸로 된 건가? 어때? 조금은 믿음직스러워졌을까?"
            },
            {
                "Entry": "「전투 - 전직2」",
                "Desc": "엣?! 이걸로 된 거야? 뭔가 대단한 걸~ 기뻐!"
            },
            {
                "Entry": "「전투 - 스킬시전1」",
                "Desc": "간다-앗!"
            },
            {
                "Entry": "「전투 - 스킬시전2」",
                "Desc": "잘 부탁드려요~"
            },
            {
                "Entry": "「전투 - 선택1」",
                "Desc": "여기~ 여기!"
            },
            {
                "Entry": "「전투 - 선택2」",
                "Desc": "뭔가 도와드릴 일이 있나요?"
            },
            {
                "Entry": "「전투 - 전투불능」",
                "Desc": "우웃... 뭔가 싫은 느낌이..."
            },
            {
                "Entry": "「대화 - 친밀도1」",
                "Desc": "방 청소는 잘 해놔야지! 정리 정돈도 잘 해놓고!"
            },
            {
                "Entry": "「대화 - 친밀도2」",
                "Desc": "진짜... 항상 무리만하고 상처는 괜찮아?"
            },
            {
                "Entry": "「대화 - 친밀도3」",
                "Desc": "에헤헷... 기분 좋은 아침이네, 같이 산책할까?"
            },
            {
                "Entry": "「대화 - 친밀도4」",
                "Desc": "이제부터 우린... 어떻게 될까? 엣?! 아니, 아... 아무것도 아니야, 비밀이야! 비밀!"
            },
            {
                "Entry": "「대화 - 친밀도5」",
                "Desc": "무리하는 거 아니야. 처음에는 무서웠지만 계속 나를 지켜줬으니까 이제는 안 무서워. 헤헷, 방금 말은 좀 바보같았나?"
            },
            {
                "Entry": "「대화 - 일상1」",
                "Desc": "제 몸에 말도 안되는 뭔가가 있는거 같아요. 언제까지나 저를 지켜주세요."
            },
            {
                "Entry": "「대화 - 기쁨」",
                "Desc": "성검의 기억이 우리를 이끌어 줄꺼야!"
            },
            {
                "Entry": "「대화 - 분노」",
                "Desc": "알겠어? 진짜로 싫은 것에는 어떤 가치도 느낄 수 없다구."
            },
            {
                "Entry": "「대화 - 슬픔」",
                "Desc": "칼자스 숲에 있던 그 시절에는... 다시 돌아갈 수 없어..."
            },
            {
                "Entry": "「대화 - 수줍음」",
                "Desc": "엄청난 사명을 짊어지고 있다고 해도 저는 평범한 여자에요..."
            },
            {
                "Entry": "「상호작용 - 대사1」",
                "Desc": "저, 저라고 해도 부끄러워져요..."
            },
            {
                "Entry": "「상호작용 - 대사2」",
                "Desc": "앗... 우우..."
            },
            {
                "Entry": "「상호작용 - 대사3」",
                "Desc": "안 돼! 들킬 수도 있잖아!"
            },
            {
                "Entry": "「상호작용 - 대사4」",
                "Desc": "으웃... 앗..."
            },
            {
                "Entry": "「상호작용 - 대사5」",
                "Desc": "당신만 좋다면 계속 해주세요..."
            },
            {
                "Entry": "「상호작용 - 호감1」",
                "Desc": "좀... 좀 더... 부드럽게..."
            },
            {
                "Entry": "「상호작용 - 호감2」",
                "Desc": "좀 더 가까이 와주세요... 가슴이 두근거리는 걸 알고 계신가요..."
            },
            {
                "Entry": "「상호작용 - 반감1」",
                "Desc": "엣!? 여... 여긴 안 돼!"
            },
            {
                "Entry": "「상호작용 - 반감2」",
                "Desc": "참! 하지 마!"
            },
            {
                "Entry": "「상호작용 - 반감3」",
                "Desc": "여긴 개인적인 장소가 아니에요... 알고 계신가요?"
            },
            {
                "Entry": "「상호작용 - 특수1」",
                "Desc": "자~ 또 데이트 해요! 약속하는 거에요!"
            },
            {
                "Entry": "「상호작용 - 특수2」",
                "Desc": "그럼, 또·만·나·요!"
            }
        ]
    }
];