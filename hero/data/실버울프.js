var data = [
  {
    "Skins": [
      {
        "ImageUrl": "실버울프/1.webp",
        "Desc": "기본"
      },
      {
        "ImageUrl": "실버울프/2.webp",
        "Desc": "스킨상점"
      },
      {
        "ImageUrl": "실버울프/3.webp",
        "Desc": "서밋아레나 S8 골드스킨"
      }
    ],
    "Name": "실버울프",
    "CV": "시무라 토모유키",
    "ChName": "银狼",
    "EngName": "SilverWolf",
    "JapName": "シルバーウルフ",
    "Rank": [
      1
    ],
    "SP": false,
    "Groups": [
      "기원",
      "유성"
    ],
    "HP": 40,
    "SPHP": 0,
    "Attk": 35,
    "SPAttk": 0,
    "Def": 10,
    "SPDef": 0,
    "MagicDef": 10,
    "SPMagicDef": 0,
    "HowToGet": "영웅 소환",
    "Fetters": [
      {
        "Condition": "실버울프를 출전시켜 시공의 균열 3-6 클리어"
      },
      {
        "Condition": "실버울프를 출전시켜 시공의 균열 3-1[정예] 클리어"
      },
      {
        "Condition": "프레아의 도움 받아 '비밀 상단!?' 클리어"
      },
      {
        "Condition": "디하르트의 도움 받아 시공 아레나 클리어"
      }
    ],
    "HeartFetter": "생명+650, 방어+40, 마방+40",
    "Talent": [
      {
        "TalentName": "늑대의 본성",
        "Icon": "23.webp",
        "Abilities": [
          {
            "Desc": "1성"
          },
          {
            "Desc": "보유한 버프 1개마다 공격 방어 <font color=\"red\">+3%</font> 최대 <font color=\"red\">+9%</font>.<br/> 치명타 발생 시 전투 후 상대의 버프 <font color=\"red\">1개</font> 탈취."
          },
          {
            "Desc": "보유한 버프 1개마다 공격 방어 <font color=\"red\">+4%</font> 최대 <font color=\"red\">+12%</font>.<br/> 치명타 발생 시 전투 후 상대의 버프 <font color=\"red\">1개</font> 탈취."
          },
          {
            "Desc": "보유한 버프 1개마다 공격 방어 <font color=\"red\">+5%</font> 최대 <font color=\"red\">+15%</font>.<br/> 치명타 발생 시 전투 후 상대의 버프 <font color=\"red\">2개</font> 탈취."
          },
          {
            "Desc": "보유한 버프 1개마다 공격 방어 <font color=\"red\">+6%</font> 최대 <font color=\"red\">+18%</font>.<br/> 치명타 발생 시 전투 후 상대의 버프 <font color=\"red\">2개</font> 탈취."
          },
          {
            "Desc": "보유한 버프 1개마다 공격 방어 <font color=\"red\">+7%</font> 최대 <font color=\"red\">+21%</font>.<br/> 치명타 발생 시 전투 후 상대의 버프 <font color=\"red\">2개</font> 탈취."
          }
        ]
      }
    ],
    "SPTalent": null,
    "Second_Awaken": {
      "Name": "질풍파공",
      "Stats": "공격 <font color=\"red\">+5%</font><br/>선공으로 전투 진입 후 <b>적에게 <font color=\"blue\">버프</font>가 없으면</b><br/>실버울프 공격 <font color=\"red\">1.5</font>배 수치 <font color=\"blue\">고정 피해</font>"
    },
    "Equip": {
      "EqpName": "의적의 가면(투구)",
      "Stats": "생명 437,마방 59",
      "Icon": "실버울프.webp",
      "Ability": "마방 +5%<br/>공격하여 전투진입시, 상대보다 강화버프가 2배 많을 경우 상대보다 먼저 공격"
    },
    "Soldiers": [
      "사무라이",
      "미스트 댄서",
      "밴디트",
      "투석기",
      "다크엘프 저격수",
      "하이엘프",
      "데몬헌터",
      "얼굴 없는 자",
      "엘리시움 암살자"
    ],
    "Jobs": [
      {
        "Name": "쉐도우",
        "Icon": "암살자",
        "SD": [
          {
            "ImageUrl": "실버울프/1/1.webp"
          },
          {
            "ImageUrl": "실버울프/2/1.webp"
          },
          {
            "ImageUrl": "실버울프/3/1.webp"
          }
        ],
        "AttkRange": 2,
        "Move": 3,
        "MoveType": "보행",
        "HP": 3597,
        "HPRate": 59,
        "Attk": 516,
        "AttkRate": 79,
        "Int": 233,
        "IntRate": 38,
        "Def": 252,
        "DefRate": 56,
        "MagicDef": 258,
        "MagicDefRate": 57,
        "Skill": 306,
        "SkillRate": 87,
        "HeartFetterBouns1": "공격하여 전투 진입시 받는피해 -10%",
        "HeartFetterBouns2": "공격받아 전투 진입시 치명타피해 +10%"
      },
      {
        "Name": "레인저",
        "Icon": "궁병",
        "SD": [
          {
            "ImageUrl": "실버울프/1/2.webp"
          },
          {
            "ImageUrl": "실버울프/2/1.webp"
          },
          {
            "ImageUrl": "실버울프/3/1.webp"
          }
        ],
        "AttkRange": 2,
        "Move": 3,
        "MoveType": "보행",
        "HP": 3199,
        "HPRate": 53,
        "Attk": 516,
        "AttkRate": 79,
        "Int": 233,
        "IntRate": 38,
        "Def": 234,
        "DefRate": 52,
        "MagicDef": 316,
        "MagicDefRate": 70,
        "Skill": 239,
        "SkillRate": 68,
        "HeartFetterBouns1": "공격받아 전투 진입시 받는 물리피해 -10%",
        "HeartFetterBouns2": "부대 생명이 80%이상일때 치명타피해 +10%"
      }
    ],
    "Materials": {
      "HeartFetter": [
        "h3",
        "h4",
        "h2",
        "h1",
        "h3-1",
        "h4-1",
        "h2-1",
        "h1-1",
        "x"
      ],
      "Fetter": [
        "6",
        "6-1",
        "6-2",
        "6-3",
        "6-4"
      ],
      "Awaken": [
        "ranka3",
        "ranka6",
        "ranka3-1",
        "ranka6-1"
      ]
    },
    "ClassUp": [
      {
        "ClassTitle": "밴디트 : 공격+5, 기술+6",
        "Tiers": [
          {
            "Name": "T1",
            "Materials": [
              "rank1-2"
            ]
          },
          {
            "Name": "T2",
            "Materials": [
              "rank1-2"
            ]
          }
        ]
      },
      {
        "ClassTitle": "소드맨 : 생명+80, 기술+5",
        "Tiers": [
          {
            "Name": "T1",
            "Materials": [
              "rank2-1",
              "rank1-1",
              "rank1-2"
            ]
          },
          {
            "Name": "T2",
            "Materials": [
              "rank3-1",
              "rank3-6",
              "rank2-1",
              "rank2-2"
            ]
          },
          {
            "Name": "T3",
            "Materials": [
              "rank4-1",
              "rank4-2",
              "rank3-1",
              "rank3-6"
            ]
          }
        ]
      },
      {
        "ClassTitle": "닌자 : 기술+8, 공격+7",
        "Tiers": [
          {
            "Name": "T1",
            "Materials": [
              "rank2-6",
              "rank1-2",
              "rank1-1"
            ]
          },
          {
            "Name": "T2",
            "Materials": [
              "rank3-2",
              "rank3-1",
              "rank2-3",
              "rank2-6"
            ]
          },
          {
            "Name": "T3",
            "Materials": [
              "rank4-3",
              "rank4-1",
              "rank3-2",
              "rank3-1"
            ]
          }
        ]
      },
      {
        "ClassTitle": "쉐도우 : 공격+25, 생명+90, 기술+10",
        "Tiers": [
          {
            "Name": "T1",
            "Materials": [
              "rank5-2",
              "rank5-1",
              "rank4-3",
              "rank4-1"
            ]
          },
          {
            "Name": "T2",
            "Materials": [
              "rank6-4",
              "rank6-3",
              "rank5-2",
              "rank5-1"
            ]
          },
          {
            "Name": "T3",
            "Materials": [
              "rank7-3",
              "rank7-6",
              "rank6-4",
              "rank6-3"
            ]
          },
          {
            "Name": "T4",
            "Materials": [
              "rank8-8",
              "rank8-5",
              "rank7-3",
              "rank7-6"
            ]
          }
        ]
      },
      {
        "ClassTitle": "스나이퍼 : 공격+12, 생명+48",
        "Tiers": [
          {
            "Name": "T1",
            "Materials": [
              "rank2-6",
              "rank1-2",
              "rank1-5"
            ]
          },
          {
            "Name": "T2",
            "Materials": [
              "rank3-2",
              "rank3-1",
              "rank2-2",
              "rank2-6"
            ]
          },
          {
            "Name": "T3",
            "Materials": [
              "rank4-3",
              "rank4-1",
              "rank3-2",
              "rank3-1"
            ]
          }
        ]
      },
      {
        "ClassTitle": "레인저 : 공격+25, 생명+90, 기술+10",
        "Tiers": [
          {
            "Name": "T1",
            "Materials": [
              "rank5-2",
              "rank5-1",
              "rank4-3",
              "rank4-1"
            ]
          },
          {
            "Name": "T2",
            "Materials": [
              "rank6-3",
              "rank6-1",
              "rank5-2",
              "rank5-1"
            ]
          },
          {
            "Name": "T3",
            "Materials": [
              "rank7-3",
              "rank7-2",
              "rank6-3",
              "rank6-1"
            ]
          },
          {
            "Name": "T4",
            "Materials": [
              "rank8-3",
              "rank8-5",
              "rank7-4",
              "rank7-3"
            ]
          }
        ]
      }
    ],
    "SPOpenConditions": null,
    "Skills": [
      {
        "Name": "냉혈",
        "Cost": "COST 2",
        "Entries": [
          {
            "Label": "유형",
            "Value": "지원"
          },
          {
            "Label": "쿨타임",
            "Value": "2턴"
          },
          {
            "Label": "사거리",
            "Value": "자신"
          },
          {
            "Label": "범위",
            "Value": "단일"
          }
        ],
        "IconImageUrl": "../img/skills/Skill_Coldblood.webp",
        "Desc": "[지원]사용 후 자신 부대 치명타율, 치명타 피해량 <font color=\"red\">+15%</font>, <font color=\"red\">2턴</font> 지속.<br/>사용 후 추가로 <font color=\"red\">2칸</font> 이동할 수 있고 다시 공격할 수 있다.",
        "SubSkills": null,
        "Animation": null
      },
      {
        "Name": "기습",
        "Cost": "COST 1",
        "Entries": [
          {
            "Label": "유형",
            "Value": "패시브"
          }
        ],
        "IconImageUrl": "../img/skills/Passive_Ass1.webp",
        "Desc": "[패시브]전투 중 치명타가 발생한 경우<br/>전투 후 적에게 [고정 피해]를 준다.(피해량은 영웅 공격의 2배)",
        "SubSkills": null,
        "Animation": null
      },
      {
        "Name": "암살",
        "Cost": "COST 2",
        "Entries": [
          {
            "Label": "유형",
            "Value": "물리 피해"
          },
          {
            "Label": "쿨타임",
            "Value": "2턴"
          },
          {
            "Label": "사거리",
            "Value": "2칸"
          },
          {
            "Label": "범위",
            "Value": "단일"
          }
        ],
        "IconImageUrl": "../img/skills/Skill_BladeCrash2.webp",
        "Desc": "[물리 피해]호위를 무시하고 단일 적 부대를 공격해 1.2배의 피해를 주고<br/>치명타율 +20%.",
        "SubSkills": null,
        "Animation": null
      },
      {
        "Name": "무기 파괴",
        "Cost": "COST 1",
        "Entries": [
          {
            "Label": "유형",
            "Value": "패시브"
          }
        ],
        "IconImageUrl": "../img/skills/Passive_BreakAtk.webp",
        "Desc": "[패시브]전투 진입전 <font color=\"red\">50%</font> 확률로 적의 공격, 지력  <font color=\"red\">-20%</font>,  <font color=\"red\">1</font>행동지속.",
        "SubSkills": null,
        "Animation": null
      },
      {
        "Name": "무자비",
        "Cost": "COST 2",
        "Entries": [
          {
            "Label": "유형",
            "Value": "패시브"
          }
        ],
        "IconImageUrl": "../img/skills/Passive_Ass3.webp",
        "Desc": "[패시브]치명타 피해량 +20%. 전투 중 치명타가 발생한 경우<br/>전투 종료 후 받는 모든 피해량 -30%, 1턴 지속.",
        "SubSkills": null,
        "Animation": null
      },
      {
        "Name": "트릭스터",
        "Cost": "COST 1",
        "Entries": [
          {
            "Label": "유형",
            "Value": "패시브"
          }
        ],
        "IconImageUrl": "../img/skills/Skill_TrickStep.webp",
        "Desc": "[패시브]치명타 확률+10%.<br/>전투 중 적 부대를 격퇴할 시 3칸 재이동 가능.",
        "SubSkills": null,
        "Animation": null
      },
      {
        "Name": "다리 저격",
        "Cost": "COST 2",
        "Entries": [
          {
            "Label": "유형",
            "Value": "물리 피해"
          },
          {
            "Label": "쿨타임",
            "Value": "2턴"
          },
          {
            "Label": "사거리",
            "Value": "2칸"
          },
          {
            "Label": "범위",
            "Value": "단일"
          }
        ],
        "IconImageUrl": "../img/skills/Skill_ArrowCrash2.webp",
        "Desc": "[물리 피해]단일 적을 공격 하여 1.4배 피해를 입힌다.<br/>'비병'을 상대할 시 더 큰 피해를 준다.<br/>전투 후 적의 이동력-1, 1턴 지속.(제거 불가)",
        "SubSkills": null,
        "Animation": null
      },
      {
        "Name": "구급",
        "Cost": "COST 1",
        "Entries": [
          {
            "Label": "유형",
            "Value": "치료"
          },
          {
            "Label": "쿨타임",
            "Value": "3턴"
          },
          {
            "Label": "사거리",
            "Value": "자신"
          },
          {
            "Label": "범위",
            "Value": "단일"
          }
        ],
        "IconImageUrl": "../img/skills/Skill_PHeal2.webp",
        "Desc": "[치료]액티브 스킬, 자신 부대 생명을 100% 회복한다.",
        "SubSkills": null,
        "Animation": null
      },
      {
        "Name": "간파",
        "Cost": "COST 1",
        "Entries": [
          {
            "Label": "유형",
            "Value": "패시브"
          }
        ],
        "IconImageUrl": "../img/skills/Passive_Cancel.webp",
        "Desc": "[패시브]치명타율 +10%. 피해를 줄 때 50%의 확률로 적 부대의 패시브 스킬을 무효화한다.<br/>2턴 지속, 치명타 발생 시 반드시 발동.",
        "SubSkills": null,
        "Animation": null
      },
      {
        "Name": "무영풍뢰파",
        "Cost": "COST 3",
        "Entries": [
          {
            "Label": "유형",
            "Value": "물리 피해"
          },
          {
            "Label": "쿨타임",
            "Value": "3턴"
          },
          {
            "Label": "사거리",
            "Value": "2칸"
          },
          {
            "Label": "범위",
            "Value": "단일"
          }
        ],
        "IconImageUrl": "../img/Super/실버울프.webp",
        "Desc": "[물리 피해] <b>단일 적 부대를 공격할때 호위를 무시하고</b> <font color=\"red\">1.4</font>배의 피해를 준다.<br/><b>전투 진입 전 자신에게</b> 이동력 <font color=\"red\">+1</font>, 기술 <font color=\"red\">+30%</font>, 받는 피해 <font color=\"red\">-20%</font> 효과를 부여한다.<br/>지속 <font color=\"red\">2</font>행동<br/><b>치명타 발동시</b> 해당 스킬 쿨타임<font color=\"red\">-1</font>",
        "SubSkills": null,
        "Animation": "../img/Super/실버울프_Ani.webp"
      }
    ],
    "Propose": false,
    "Bio": [
      {
        "Entry": "영웅소개",
        "Desc": "◆키/몸무게：178cm / 69kg<br/>◆출연：랑그릿사III<br/>◆소속：정의로운 동료"
      },
      {
        "Entry": "영웅담1",
        "Desc": "'의적'을 자처하는 도적 두목. 자신의 신분을 감추기 위해 언제나 얼굴을 가리고 자신을 실버울프라 부르며 본명을 숨기고 있다. 적절한 시기에 나타나 디하르트 일행에게 도움을 준 믿음직하고 강력한 아군.<br/><font color=\"#DC143C\"><size=33><i>「내 뒤에 서지 마라... 나도 모르게 죽일 수 있으니...」</i>  </size=33></font>"
      },
      {
        "Entry": "영웅담2",
        "Desc": "과묵하나 의리를 중시하고 동료를 위해서라면 자신의 안전을 신경 쓰지 않는다. 지하세계에 많은 추종자가 있기에 강력한 정보망을 구축할 수 있었다."
      },
      {
        "Entry": "영웅담3",
        "Desc": "라카스 독립군이 바랄 왕국의 대군에게 포위당해 위험에 처하자, 의적 부대를 이끌고 나타나 디하르트 일행을 구해주었다. 오랜 전란 속에서 의적의 강력한 정보 능력을 십분 발휘하여 여러 차례에 걸쳐 승리의 기회를 제공해 주었다."
      },
      {
        "Entry": "영웅담4",
        "Desc": "전쟁이 끝난 후 실버울프는 새로운 세계에 더는 「의적」이 필요 없다는 것을 깨달았다.그의 정보 능력을 높이 산 엘스리드 왕가에게 정보원으로 고용되고 이후 정보 수집 방면에서뿐만 아니라 외교 부문에서도 활약하게 된다."
      },
      {
        "Entry": "영웅담5",
        "Desc": "현대에 영령으로 소환되었지만 과묵한 건 여전하다. 때때로 갑자기 사라진 후 대단한 정보를 가지고 돌아오기도 한다. <br/><font color=\"#DC143C\"><size=33><i>「긴말할 필요 없다. 가자.」</i>  </size=33></font>"
      }
    ],
    "Performance": [
      {
        "Entry": "「소환 - 등장대사」",
        "Desc": "나는 천하의 대의적 실버울프님이다! 소환에 응해 찾아왔다."
      },
      {
        "Entry": "「전투 - 전직1」",
        "Desc": "앗..."
      },
      {
        "Entry": "「전투 - 전직2」",
        "Desc": "힘이 모이는군!"
      },
      {
        "Entry": "「전투 - 스킬시전1」",
        "Desc": "냉정하게!"
      },
      {
        "Entry": "「전투 - 스킬시전2」",
        "Desc": "발도!"
      },
      {
        "Entry": "「전투 - 선택1」",
        "Desc": "뭐..."
      },
      {
        "Entry": "「전투 - 선택2」",
        "Desc": "알았다!"
      },
      {
        "Entry": "「전투 - 전투불능」",
        "Desc": "미안...."
      },
      {
        "Entry": "「대화 - 친밀도1」",
        "Desc": "뭐..."
      },
      {
        "Entry": "「대화 - 친밀도2」",
        "Desc": "맡겨다오!"
      },
      {
        "Entry": "「대화 - 친밀도3」",
        "Desc": "내 뒤에 서지 말아주겠나? ...나도 모르게 살기를 내뿜게 되니...미안하군."
      },
      {
        "Entry": "「대화 - 친밀도4」",
        "Desc": "별로 말이 없다고? 음...그건... 그런 설정이라서...아니, 아무것도 아니다..."
      },
      {
        "Entry": "「대화 - 친밀도5」",
        "Desc": "도구에게도 도구의 행복이 있지. 나는 사람으로선 결점이 많지만 너같은 사람에게 다뤄지는건 꽤나 기쁜일이지."
      },
      {
        "Entry": "「대화 - 일상1」",
        "Desc": "이름은 그저 기호일 뿐이다... 하지만 나는 네 살의를 구상화 할 수 있지."
      },
      {
        "Entry": "「대화 - 일상2」",
        "Desc": "「실버울프」가 바로 나다."
      },
      {
        "Entry": "「대화 - 기쁨」",
        "Desc": "하하... 아무것도 아니다. 신경쓰지 마라."
      },
      {
        "Entry": "「대화 - 분노」",
        "Desc": "흥! 악은 반드시 쓰러뜨린다!"
      },
      {
        "Entry": "「상호작용 - 대사1」",
        "Desc": "나쁘지 않군!"
      },
      {
        "Entry": "「상호작용 - 대사2」",
        "Desc": "후후."
      },
      {
        "Entry": "「상호작용 - 대사3」",
        "Desc": "괜찮군..."
      },
      {
        "Entry": "「상호작용 - 대사4」",
        "Desc": "아하."
      },
      {
        "Entry": "「상호작용 - 대사5」",
        "Desc": "좋은 느낌인데!"
      },
      {
        "Entry": "「상호작용 - 반감1」",
        "Desc": "안된다!"
      },
      {
        "Entry": "「상호작용 - 반감2」",
        "Desc": "흥!"
      },
      {
        "Entry": "「상호작용 - 반감3」",
        "Desc": "여기까지다!"
      },
      {
        "Entry": "「상호작용 - 특수1」",
        "Desc": "정말 좋군!"
      },
      {
        "Entry": "「상호작용 - 특수2」",
        "Desc": "그럼 다음에!"
      }
    ],
    "Performance2": null,
    "Misc": ""
  }
]