var data = [
  {
    "Skins": [
      {
        "ImageUrl": "제리올/1.webp",
        "Desc": "기본"
      },
      {
        "ImageUrl": "제리올/2.webp",
        "Desc": "한정판매"
      }
    ],
    "Name": "제리올레이라",
    "CV": "나가츠카 타쿠마",
    "ChName": "杰利奥鲁&蕾拉",
    "EngName": "Gerold & Layla",
    "JapName": "ジェリオール&レイラ",
    "Rank": [
      0
    ],
    "SP": false,
    "Groups": [
      "기원",
      "전략"
    ],
    "HP": 15,
    "SPHP": 0,
    "Attk": 35,
    "SPAttk": 0,
    "Def": 35,
    "SPDef": 0,
    "MagicDef": 15,
    "SPMagicDef": 0,
    "HowToGet": "영웅 소환,시공의 레스토랑 이벤트",
    "Fetters": [
      {
        "Condition": "제리올&레이라를 출전시켜 아레나에서 5회 승리"
      },
      {
        "Condition": "제리올&레이라를 출전시켜 시공의 균열 3-1[정예] 클리어"
      },
      {
        "Condition": "티아리스의 도움 받아 강철과 원한 클리어"
      },
      {
        "Condition": "디하르트의 도움 받아 고통의 배신 클리어"
      }
    ],
    "HeartFetter": "생명+550, 방어+40, 마방+60",
    "Talent": [
      {
        "TalentName": "월하의 서약",
        "Icon": "55.webp",
        "Abilities": [
          {
            "Desc": "1성"
          },
          {
            "Desc": "2성"
          },
          {
            "Desc": "공격 시 근접 공격 피해감면 영향을 받지 않는다.<br/>원거리 공격 후 아군에게 제리올&레이라 <br/>공격+지력의 <font color=\"red\">0.5</font>배 생명을 회복 <font color=\"red\">1</font>개 디버프 제거.<br/> 근거리 공격 후 자신 부대의 생명을 입힌 피해 <font color=\"red\">10%</font>만큼 회복<br/> 자신에게 &#39;면역&#39; 시전 <font color=\"red\">1</font>행동 지속."
          },
          {
            "Desc": "공격 시 근접 공격 피해감면 영향을 받지 않는다.<br/>원거리 공격 후 아군에게 제리올&레이라 <br/>공격+지력의 <font color=\"red\">0.8</font>배 생명을 회복 <font color=\"red\">1</font>개 디버프 제거.<br/> 근거리 공격 후 자신 부대의 생명을 입힌 피해 <font color=\"red\">15%</font>만큼 회복<br/> 자신에게 &#39;면역&#39; 시전 <font color=\"red\">1</font>행동 지속."
          },
          {
            "Desc": "공격 시 근접 공격 피해감면 영향을 받지 않는다.<br/>원거리 공격 후 아군에게 제리올&레이라 <br/>공격+지력의 <font color=\"red\">1.1</font>배 생명을 회복 <font color=\"red\">1</font>개 디버프 제거.<br/> 근거리 공격 후 자신 부대의 생명을 입힌 피해 <font color=\"red\">20%</font>만큼 회복<br/> 자신에게 &#39;면역&#39; 시전 <font color=\"red\">1</font>행동 지속."
          },
          {
            "Desc": "공격 시 근접 공격 피해감면 영향을 받지 않는다.<br/>원거리 공격 후 아군에게 제리올&레이라 <br/>공격+지력의 <font color=\"red\">1.5</font>배 생명을 회복 <font color=\"red\">1</font>개 디버프 제거.<br/> 근거리 공격 후 자신 부대의 생명을 입힌 피해 <font color=\"red\">30%</font>만큼 회복<br/> 자신에게 &#39;면역&#39; 시전 <font color=\"red\">1</font>행동 지속."
          }
        ]
      }
    ],
    "SPTalent": null,
    "Second_Awaken": {
      "Name": "영원한 맹세",
      "Stats": "<font color=\"blue\">액티브 스킬</font> 피해량 <font color=\"red\">+10%</font><br/><font color=\"blue\">레이라의 축복</font>(<font color=\"blue\">제리올의 호위</font>) 지속시간 <font color=\"red\">+1</font>행동 "
    },
    "Equip": {
      "EqpName": "같은 마음의 수호(망치)",
      "Stats": "공격 107,지력 107",
      "Icon": "제리올.webp",
      "Ability": "공격, 지력 <font color=\"red\">+10%</font><br/>영웅 통상 공격 사거리 <font color=\"red\">+1</font>,<br/><b>선공으로 전투 진입 시</b>, <b>전투 후 생명이 가장 낮은 아군 <font color=\"blue\">회복</font></b>, <br/><b>회복량은 술자의 공격+지력 <font color=\"red\">1</font>배</b> 수치<br/><b>재행동 발동 시, 자신에게 걸린 버프의 지속 시간이 소모되지 않음</b>"
    },
    "Soldiers": [
      "세인트 나이트",
      "다크엘프 저격수",
      "마녀",
      "마법사",
      "뱀파이어 배트",
      "마스터 디노",
      "듀라한",
      "스파이더 엘프",
      "머신 나이트",
      "어둠의 연금술사",
      "요정예언자",
      "안개의 유령"
    ],
    "Jobs": [
      {
        "Name": "다크 나이트",
        "Icon": "기병",
        "SD": [
          {
            "ImageUrl": "제리올/1/1.webp"
          },
          {
            "ImageUrl": "제리올/2/1.webp"
          }
        ],
        "AttkRange": 1,
        "Move": 5,
        "MoveType": "기마",
        "HP": 3925,
        "HPRate": 65,
        "Attk": 531,
        "AttkRate": 82,
        "Int": 417,
        "IntRate": 70,
        "Def": 302,
        "DefRate": 67,
        "MagicDef": 280,
        "MagicDefRate": 62,
        "Skill": 118,
        "SkillRate": 34,
        "HeartFetterBouns1": "'강화버프'가 5개이상일때 전투중 받는피해 -10%",
        "HeartFetterBouns2": "근접공격으로 공격하여 전투진입시 입히는피해 +10%"
      },
      {
        "Name": "로열 엔보이",
        "Icon": "승려",
        "SD": [
          {
            "ImageUrl": "제리올/1/2.webp"
          },
          {
            "ImageUrl": "제리올/2/1.webp"
          }
        ],
        "AttkRange": 2,
        "Move": 3,
        "MoveType": "보행",
        "HP": 3455,
        "HPRate": 58,
        "Attk": 501,
        "AttkRate": 77,
        "Int": 480,
        "IntRate": 80,
        "Def": 270,
        "DefRate": 60,
        "MagicDef": 338,
        "MagicDefRate": 75,
        "Skill": 91,
        "SkillRate": 26,
        "HeartFetterBouns1": "'약화버프'를 보유중일때 전투중 받는피해 -10%",
        "HeartFetterBouns2": "원거리공격으로 공격하여 전투진입시 입히는피해 +10%"
      }
    ],
    "Materials": {
      "HeartFetter": [
        "h4",
        "h1",
        "h3",
        "h2",
        "h4-1",
        "h1-1",
        "h3-1",
        "h2-1",
        "x"
      ],
      "Fetter": [
        "2",
        "2-1",
        "2-2",
        "2-3",
        "2-4"
      ],
      "Awaken": [
        "ranka2",
        "ranka6",
        "ranka2-1",
        "ranka6-1"
      ]
    },
    "ClassUp": [
      {
        "ClassTitle": "나이트 : 생명+36, 공격+9",
        "Tiers": [
          {
            "Name": "T1",
            "Materials": [
              "rank1-3"
            ]
          },
          {
            "Name": "T2",
            "Materials": [
              "rank1-3"
            ]
          }
        ]
      },
      {
        "ClassTitle": "하이랜더 : 생명+48, 공격+12",
        "Tiers": [
          {
            "Name": "T1",
            "Materials": [
              "rank2-3",
              "rank1-3",
              "rank1-1"
            ]
          },
          {
            "Name": "T2",
            "Materials": [
              "rank3-3",
              "rank3-2",
              "rank2-3",
              "rank2-1"
            ]
          },
          {
            "Name": "T3",
            "Materials": [
              "rank4-6",
              "rank4-1",
              "rank3-3",
              "rank3-2"
            ]
          }
        ]
      },
      {
        "ClassTitle": "다크나이트 : 공격+25, 방어+10, 지력+15",
        "Tiers": [
          {
            "Name": "T1",
            "Materials": [
              "rank5-3",
              "rank5-5",
              "rank4-6",
              "rank4-4"
            ]
          },
          {
            "Name": "T2",
            "Materials": [
              "rank6-4",
              "rank6-1",
              "rank5-3",
              "rank5-5"
            ]
          },
          {
            "Name": "T3",
            "Materials": [
              "rank7-7",
              "rank7-6",
              "rank6-4",
              "rank6-1"
            ]
          },
          {
            "Name": "T4",
            "Materials": [
              "rank8-8",
              "rank8-3",
              "rank7-7",
              "rank7-6"
            ]
          }
        ]
      },
      {
        "ClassTitle": "헤럴드 : 생명+80, 공격+7, 지력+7",
        "Tiers": [
          {
            "Name": "T1",
            "Materials": [
              "rank2-5",
              "rank1-5",
              "rank1-4"
            ]
          },
          {
            "Name": "T2",
            "Materials": [
              "rank3-4",
              "rank3-3",
              "rank2-5",
              "rank2-4"
            ]
          },
          {
            "Name": "T3",
            "Materials": [
              "rank4-5",
              "rank4-4",
              "rank3-4",
              "rank3-3"
            ]
          }
        ]
      },
      {
        "ClassTitle": "로열엔보이 : 생명+90, 공격+15, 지력+25",
        "Tiers": [
          {
            "Name": "T1",
            "Materials": [
              "rank5-4",
              "rank5-5",
              "rank4-5",
              "rank4-4"
            ]
          },
          {
            "Name": "T2",
            "Materials": [
              "rank6-8",
              "rank6-7",
              "rank5-4",
              "rank5-5"
            ]
          },
          {
            "Name": "T3",
            "Materials": [
              "rank7-5",
              "rank7-6",
              "rank6-8",
              "rank6-7"
            ]
          },
          {
            "Name": "T4",
            "Materials": [
              "rank8-6",
              "rank8-4",
              "rank7-5",
              "rank7-6"
            ]
          }
        ]
      }
    ],
    "SPOpenConditions": null,
    "Skills": [
      {
        "Name": "질풍 돌격",
        "Cost": "COST 2",
        "Entries": [
          {
            "Label": "유형",
            "Value": "물리피해"
          },
          {
            "Label": "쿨타임",
            "Value": "2턴"
          },
          {
            "Label": "사거리",
            "Value": "1칸"
          },
          {
            "Label": "범위",
            "Value": "단일"
          }
        ],
        "IconImageUrl": "../img/skills/Skill_KnightCrash2.webp",
        "Desc": "[물리 피해]단일 적 부대를 공격하여 1.5배의 전투 피해를 준다.<br/>전투 전 부대의 방어, 마방 +30%.",
        "SubSkills": null,
        "Animation": null
      },
      {
        "Name": "재이동",
        "Cost": "COST 1",
        "Entries": [
          {
            "Label": "유형",
            "Value": "패시브"
          }
        ],
        "IconImageUrl": "../img/skills/13.webp",
        "Desc": "[패시브]공격하여 전투 진입 시 주는 피해량 +10%.<br/>남은 이동력이 있을 경우 공격 후 재이동 가능.",
        "SubSkills": null,
        "Animation": null
      },
      {
        "Name": "마무술",
        "Cost": "COST 1",
        "Entries": [
          {
            "Label": "유형",
            "Value": "지원"
          },
          {
            "Label": "쿨타임",
            "Value": "4턴"
          },
          {
            "Label": "사거리",
            "Value": "자신"
          },
          {
            "Label": "범위",
            "Value": "단일"
          }
        ],
        "IconImageUrl": "../img/skills/Skill_MagicAttack.webp",
        "Desc": "[지원]'마무'시전: 영웅이 입히는 모든 마법 피해를 물리 피해로 취급.<br/>공격의 10%를 지력에 추가, 4턴 지속.",
        "SubSkills": null,
        "Animation": null
      },
      {
        "Name": "맹렬한 돌격",
        "Cost": "COST 2",
        "Entries": [
          {
            "Label": "유형",
            "Value": "물리피해"
          },
          {
            "Label": "쿨타임",
            "Value": "2턴"
          },
          {
            "Label": "사거리",
            "Value": "1칸"
          },
          {
            "Label": "범위",
            "Value": "단일"
          }
        ],
        "IconImageUrl": "../img/skills/39.webp",
        "Desc": "[물리 피해]단일 적 부대를 공격해 1.4배의 공격 피해를 준다.<br/>대상을 2칸 밀쳐내며 방어 -20%, '호위 불가' 부여, 2턴지속.",
        "SubSkills": null,
        "Animation": null
      },
      {
        "Name": "공격 지휘",
        "Cost": "COST 2",
        "Entries": [
          {
            "Label": "유형",
            "Value": "패시브"
          }
        ],
        "IconImageUrl": "../img/skills/2.webp",
        "Desc": "주위 <font color=\"red\">2</font>칸 내 모든 아군 공격, 지력 <font color=\"red\">+10%</font>.",
        "SubSkills": null,
        "Animation": null
      },
      {
        "Name": "증폭술",
        "Cost": "COST 1",
        "Entries": [
          {
            "Label": "유형",
            "Value": "지원"
          },
          {
            "Label": "쿨타임",
            "Value": "4턴"
          },
          {
            "Label": "사거리",
            "Value": "자신"
          },
          {
            "Label": "범위",
            "Value": "단일"
          }
        ],
        "IconImageUrl": "../img/skills/Skill_PhysicalAttack.webp",
        "Desc": "[지원]'증폭'시전: 영웅이 입히는 모든 물리 피해를 마법 피해로 취급.<br/>지력의 10%를 공격에 추가, 4턴 지속.",
        "SubSkills": null,
        "Animation": null
      },
      {
        "Name": "성언",
        "Cost": "COST 2",
        "Entries": [
          {
            "Label": "유형",
            "Value": "마법피해"
          },
          {
            "Label": "쿨타임",
            "Value": "1턴"
          },
          {
            "Label": "사거리",
            "Value": "2칸"
          },
          {
            "Label": "범위",
            "Value": "단일"
          }
        ],
        "IconImageUrl": "../img/skills/19.webp",
        "Desc": "단일 적에게 <font color=\"red\">1.5</font>배의 피해 <br/>'마물'에게 더 큰 피해를 준다<br/>전투 후 생명이 백분비가 가장 적은 아군의 <font color=\"green\">생명 회복</font><br/><font color=\"green\">회복량</font>은 사용자 지력의 <font color=\"red\">4</font>배.",
        "SubSkills": null,
        "Animation": null
      },
      {
        "Name": "명상",
        "Cost": "COST 1",
        "Entries": [
          {
            "Label": "유형",
            "Value": "패시브"
          }
        ],
        "IconImageUrl": "../img/skills/22.webp",
        "Desc": "[패시브] 부대 생명이 90%이상일 시 지력, 마방 +10%.",
        "SubSkills": null,
        "Animation": null
      },
      {
        "Name": "레이라의 축복",
        "Cost": "COST 3",
        "Entries": [
          {
            "Label": "유형",
            "Value": "변신"
          },
          {
            "Label": "쿨타임",
            "Value": "4턴"
          },
          {
            "Label": "사거리",
            "Value": "자신"
          },
          {
            "Label": "범위",
            "Value": "단일"
          }
        ],
        "IconImageUrl": "../img/Super/제리올.webp",
        "Desc": "[레이라의 축복]<br/>레이라가 자신의 힘을 제리올이랑 합체하여 다크 나이트로 변신하며 '고급 증폭술'획득<br/>영웅이 입히는 모든 피해를 물리 피해로 간주하고 지력의 50% 수치를 공격에 추가. 지속 2회합<br/>사용후 3칸 이동가능하며 다시 공격 할 수 있다.<br/>지속 2회합이 끝나면 [제리올의 호위]로 변경<br/>해당 스킬은 면역 및 해제 불가",
        "SubSkills": null,
        "Animation": "../img/Super/제리올_Ani.webp"
      }
    ],
    "Propose": false,
    "Bio": [
      {
        "Entry": "영웅소개",
        "Desc": "◆키/몸무게：——<br/>◆출연：랑그릿사III<br/>◆소속：라카스 왕국"
      },
      {
        "Entry": "영웅담1",
        "Desc": "서로 믿고 의지하는 제리올과 레이라는 라카스 왕국에서도 유명한 한 쌍이었다.고상한 성품의 제리올은 지용을 겸비한 라카스의 제일 기사이자 디하르트의 검술 스승인 동시에 동경의 대상이었다. 레이라는 제리올과 자신의 고향을 사랑하는 헌신적인 여성이었다.그들은 행복한 기사와 여인의 본보기였다. 그래, 본래 그랬어야 했다..."
      },
      {
        "Entry": "영웅담2",
        "Desc": "제국이 라카스 왕국의 부유성을 습격한 첫 번째 전투에서 제리올은 제국 원수 알테뮬러와 일대일 대결을 하던 도중 불행히도 중상을 입고 말았다. 제국의 추격을 피해 도망가던 와중에 짐이 되고 싶지 않았던 제리올은 디하르트와 레이라에게 자신을 두고 가라 했고 그 이후 행방이 묘연해졌다.<br/><font color=\"#DC143C\"><size=33><i>「레이라... 이게 내 마지막 부탁이야. 네가 걱정되지 않게 디하르트와 같이 도망가!」</i>  </size=33></font>"
      },
      {
        "Entry": "영웅담3",
        "Desc": "격렬한 전투 끝에 디하르트는 바랄 왕국의 다크 나이트가 쓴 가면을 벗겨낸다. 그리고 가면 아래에는... 오래전 실종된 제리올이 있었다. 제리올은 마충사 라그에게 조종당해 어둠의 세력의 도구가 되어있었다.그때 레이라는 절망적인 심정으로 실종된 제리올을 찾아 헤매고 있었다. 그리고 그런 그녀를 기다리고 있던 것은..."
      },
      {
        "Entry": "영웅담4",
        "Desc": "자아를 되찾은 제리올은 얼마 살지 못한다는 걸 깨닫고, 마지막으로 사랑하는 사람을 찾아갔다... 흑기사의 가면을 쓴 채, 레이라를 공격하는 모든 적을 물리친 그는 결국 연인을 등지고 홀로 광야 속으로 사라졌다...<br/><font color=\"#DC143C\"><size=33><i>「이건 제리올의 유품이에요. 그는 마지막으로 '레이라, 행복해야 해...'라는 말을 남기고 떠났어요.」</i>  </size=33></font>"
      },
      {
        "Entry": "영웅담5",
        "Desc": "어쩌면 이 한 쌍의 연인은 동시에 두 명이 소환된 유일한 케이스가 아닐지도 모른다. 하지만... 이들이 가장 행복한 한 쌍이라는 것은 확실하다. 역사의 수레바퀴에 의해 헤어진 제리올과 레이라는 이 시대에 이르러 다시 만날 수 있었다.<br/><font color=\"#DC143C\"><size=33><i>「저희 둘을 다시 만날 수 있게 해준 빛의 여신께 감사합니다. 본래 하나인 두 생명, 이제 다시 떨어지지 않길 바랍니다.」</i>  </size=33></font>"
      }
    ],
    "Performance": [
      {
        "Entry": "「소환 - 등장대사」",
        "Desc": "( 제리올 : ) 원래 하나의 생명이었던 둘! ( 레이라 : ) 두 번 다시 헤어지지 않길!"
      },
      {
        "Entry": "「전투 - 전직1」",
        "Desc": "( 제리올 : ) 레이라, 이 힘이 느껴져?"
      },
      {
        "Entry": "「전투 - 전직2」",
        "Desc": "( 레이라 : ) 제리올, 당신이 자랑스러워요."
      },
      {
        "Entry": "「전투 - 스킬시전1」",
        "Desc": "( 제리올 : ) 함께 가자!"
      },
      {
        "Entry": "「전투 - 스킬시전2」",
        "Desc": "( 레이라 : ) 나도 지지 않겠어!"
      },
      {
        "Entry": "「전투 - 선택1」",
        "Desc": "( 제리올 : ) 듣고 있어."
      },
      {
        "Entry": "「전투 - 선택2」",
        "Desc": "( 레이라 : ) 준비는 끝났어."
      },
      {
        "Entry": "「전투 - 전투불능」",
        "Desc": "( 레이라 : ) 제리올... (제리올 : ) 레이라..."
      },
      {
        "Entry": "「대화 - 친밀도1」",
        "Desc": "( 제리올 : ) 나는 고통스런 과거를 짊어지고 있다. 그래서 지금 다시 만난 이 순간의 소중함을 잘 알고 있어."
      },
      {
        "Entry": "「대화 - 친밀도2」",
        "Desc": "( 레이라 : ) 우린 떨어질 수 없어. 성검도 우리의 영혼을 같이 묶어 버렸으니까."
      },
      {
        "Entry": "「대화 - 친밀도3」",
        "Desc": "( 제리올 : ) 내 인생은 어둠의 사자에 의해 엉망으로 망가져버렸지. 그러니 널 위해 그 녀석을 쓰러뜨리겠어!"
      },
      {
        "Entry": "「대화 - 친밀도4」",
        "Desc": "( 레이라 : ) 우리를 다시 만나게 해준 이 은혜, 뭐라 감사해야 할지..."
      },
      {
        "Entry": "「대화 - 친밀도5」",
        "Desc": "( 제리올 : ) 레이라, 내 사랑... ( 레이라 : ) 제리올, 우리 조금 자제하는게 좋지 않을까요?"
      },
      {
        "Entry": "「대화 - 일상1」",
        "Desc": "( 제리올 : ) 언제라도 싸울 수 있어. ( 레이라 : ) 무리는 하지마!"
      },
      {
        "Entry": "「대화 - 기쁨」",
        "Desc": "( 레이라 : ) 우리들 너무 눈에 띄지 않아?"
      },
      {
        "Entry": "「대화 - 분노」",
        "Desc": "( 제리올 : ) 어둠에 떨어진 고통은 지금도 똑똑히 기억하고 있어..."
      },
      {
        "Entry": "「대화 - 슬픔」",
        "Desc": "( 레이라 : ) 사랑하는 사람을 잃어본 적 있어? 나는 그 마음을 알고 있어..."
      },
      {
        "Entry": "「상호작용 - 대사1」",
        "Desc": "( 제리올 : ) 이건 나에게 주는 포상인가?"
      },
      {
        "Entry": "「상호작용 - 대사2」",
        "Desc": "( 레이라 : ) 두명이니까 조금 까다로운가?"
      },
      {
        "Entry": "「상호작용 - 대사3」",
        "Desc": "( 제리올 : ) 우리 둘 다 네게 고마워하고 있어."
      },
      {
        "Entry": "「상호작용 - 대사4」",
        "Desc": "( 레이라 : ) 헤헤..."
      },
      {
        "Entry": "「상호작용 - 대사5」",
        "Desc": "( 레이라 : ) 다음번엔 우리 부부가 신경써 줄게."
      },
      {
        "Entry": "「상호작용 - 반감1」",
        "Desc": "( 제리올 : ) 그 손 치워."
      },
      {
        "Entry": "「상호작용 - 반감2」",
        "Desc": "( 레이라 : ) ...이러지마. 지나치잖아?"
      },
      {
        "Entry": "「상호작용 - 반감3」",
        "Desc": "( 제리올 : ) 무슨 뜻이지?"
      },
      {
        "Entry": "「상호작용 - 특수1」",
        "Desc": "( 제리올 : ) 오늘은 이걸로! (레이라 : ) 다음에 봐!"
      },
      {
        "Entry": "「상호작용 - 특수2」",
        "Desc": "( 레이라 : ) 그러면 다음에 만나!"
      }
    ],
    "Performance2": null,
    "Misc": ""
  }
]