var data = [
  {
    "Skins": [
      {
        "ImageUrl": "티아리스/1.webp",
        "Desc": "기본"
      },
      {
        "ImageUrl": "티아리스/2.webp",
        "Desc": "한정판매"
      },
      {
        "ImageUrl": "티아리스/3.webp",
        "Desc": "형귀스킨"
      },
      {
        "ImageUrl": "티아리스/4.webp",
        "Desc": "한정판매"
      },
      {
        "ImageUrl": "티아리스/5.webp",
        "Desc": "한정판매"
      },
      {
        "ImageUrl": "티아리스/6.webp",
        "Desc": "한정판매"
      },
      {
        "ImageUrl": "티아리스/7.webp",
        "Desc": "한정판매"
      }
    ],
    "Name": "티아리스",
    "CV": "노토 마미코",
    "ChName": "蒂亚莉丝",
    "EngName": "Tiaris",
    "JapName": "ティアリス",
    "Rank": [
      0
    ],
    "SP": false,
    "Groups": [
      "주인공",
      "기원",
      "공주"
    ],
    "HP": 10,
    "SPHP": 0,
    "Attk": 10,
    "SPAttk": 0,
    "Def": 40,
    "SPDef": 0,
    "MagicDef": 40,
    "SPMagicDef": 0,
    "HowToGet": "영웅 소환",
    "Fetters": [
      {
        "Condition": "티아리스를 출전시켜 아레나에서 5회 승리"
      },
      {
        "Condition": "티아리스를 홀리 나이트전직"
      },
      {
        "Condition": "제리올&레이라의 도움 받아 '성숙한 기품' 클리어"
      },
      {
        "Condition": "디하르트의 도움 받아 '성장하기 싫어' 클리어"
      }
    ],
    "HeartFetter": "생명+450, 방어+20, 마방+70",
    "Talent": [
      {
        "TalentName": "치유의 빛",
        "Icon": "52.webp",
        "Abilities": [
          {
            "Desc": "1성"
          },
          {
            "Desc": "2성"
          },
          {
            "Desc": "행동 종료 시 주위 <font color=\"red\">2</font>칸 내 아군에게 [치유] 부여 :<br/> 전투 후 <font color=\"red\">50%</font> 확률로 티아리스 지력의 <font color=\"red\">3</font>배만큼 생명 회복."
          },
          {
            "Desc": "행동 종료 시 주위 <font color=\"red\">2</font>칸 내 아군에게 [치유] 부여 :<br/> 전투 후 <font color=\"red\">60%</font> 확률로 티아리스 지력의 <font color=\"red\">3</font>배만큼 생명 회복."
          },
          {
            "Desc": "행동 종료 시 주위 <font color=\"red\">2</font>칸 내 아군에게 [치유] 부여 :<br/> 전투 후 <font color=\"red\">80%</font> 확률로 티아리스 지력의 <font color=\"red\">3</font>배만큼 생명 회복."
          },
          {
            "Desc": "행동 종료 시 주위 <font color=\"red\">2</font>칸 내 아군에게 [치유] 부여 :<br/> 전투 후 <font color=\"red\">100%</font> 확률로 티아리스 지력의 <font color=\"red\">3</font>배만큼 생명 회복."
          }
        ]
      }
    ],
    "SPTalent": null,
    "Second_Awaken": null,
    "Equip": {
      "EqpName": "소녀의 희망(투구)",
      "Stats": "생명 364,마방 65",
      "Icon": "티아리스.webp",
      "Ability": "생명 <font color=\"red\">+10%</font><br/>단일 아군에게 단일 대상 스킬 사용시, <font color=\"blue\">진격의 가호</font> 효과 부여.<br/>지속 <font color=\"red\">1</font>행동."
    },
    "Soldiers": [
      "신관",
      "세인트 나이트",
      "투석기",
      "볼트 레인저",
      "데몬헌터",
      "마녀",
      "템플러",
      "공성 발리스타",
      "꽃과 바람의 성가대"
    ],
    "Jobs": [
      {
        "Name": "홀리 나이트",
        "Icon": "승려",
        "SD": [
          {
            "ImageUrl": "티아리스/1/1.webp"
          },
          {
            "ImageUrl": "티아리스/2/1.webp"
          },
          {
            "ImageUrl": "티아리스/3/1.webp"
          },
          {
            "ImageUrl": "티아리스/4/1.webp"
          },
          {
            "ImageUrl": "티아리스/5/1.webp"
          },
          {
            "ImageUrl": "티아리스/6/1.webp"
          },
          {
            "ImageUrl": "티아리스/7/1.webp"
          }
        ],
        "AttkRange": 2,
        "Move": 5,
        "MoveType": "기마",
        "HP": 3327,
        "HPRate": 55,
        "Attk": 335,
        "AttkRate": 52,
        "Int": 443,
        "IntRate": 74,
        "Def": 272,
        "DefRate": 60,
        "MagicDef": 407,
        "MagicDefRate": 90,
        "Skill": 96,
        "SkillRate": 27,
        "HeartFetterBouns1": "생명이 100%일때 받는피해 -10%",
        "HeartFetterBouns2": "공격하여 전투진입시 입히는피해 +10%"
      },
      {
        "Name": "유니콘 마스터",
        "Icon": "마법사",
        "SD": [
          {
            "ImageUrl": "티아리스/1/2.webp"
          },
          {
            "ImageUrl": "티아리스/2/1.webp"
          },
          {
            "ImageUrl": "티아리스/3/1.webp"
          },
          {
            "ImageUrl": "티아리스/4/1.webp"
          },
          {
            "ImageUrl": "티아리스/5/1.webp"
          },
          {
            "ImageUrl": "티아리스/6/1.webp"
          },
          {
            "ImageUrl": "티아리스/7/1.webp"
          }
        ],
        "AttkRange": 2,
        "Move": 5,
        "MoveType": "기마",
        "HP": 3619,
        "HPRate": 60,
        "Attk": 308,
        "AttkRate": 47,
        "Int": 481,
        "IntRate": 80,
        "Def": 304,
        "DefRate": 68,
        "MagicDef": 348,
        "MagicDefRate": 77,
        "Skill": 125,
        "SkillRate": 36,
        "HeartFetterBouns1": "원거리 공격을 받아 전투진입시 받는 물리피해 -10%",
        "HeartFetterBouns2": "마법공격을 받아 전투진입시 입히는피해 +10%"
      }
    ],
    "Materials": {
      "HeartFetter": [
        "h1",
        "h2",
        "h4",
        "h3",
        "h1-1",
        "h2-1",
        "h4-1",
        "h3-1",
        "x"
      ],
      "Fetter": [
        "3",
        "3-1",
        "3-2",
        "3-3",
        "3-4"
      ],
      "Awaken": [
        "ranka1",
        "ranka3",
        "ranka1-1",
        "ranka3-1"
      ]
    },
    "ClassUp": [
      {
        "ClassTitle": "시스터 : 마방+6, 지력+5",
        "Tiers": [
          {
            "Name": "T1",
            "Materials": [
              "rank1-4"
            ]
          },
          {
            "Name": "T2",
            "Materials": [
              "rank1-4"
            ]
          }
        ]
      },
      {
        "ClassTitle": "주교 : 마방+8, 생명+48",
        "Tiers": [
          {
            "Name": "T1",
            "Materials": [
              "rank2-5",
              "rank1-5",
              "rank1-4"
            ]
          },
          {
            "Name": "T2",
            "Materials": [
              "rank3-4",
              "rank3-5",
              "rank2-5",
              "rank2-4"
            ]
          },
          {
            "Name": "T3",
            "Materials": [
              "rank4-5",
              "rank4-4",
              "rank3-4",
              "rank3-5"
            ]
          }
        ]
      },
      {
        "ClassTitle": "세인트나이트 : 생명+48, 방어+8",
        "Tiers": [
          {
            "Name": "T1",
            "Materials": [
              "rank2-6",
              "rank1-3",
              "rank1-4"
            ]
          },
          {
            "Name": "T2",
            "Materials": [
              "rank3-4",
              "rank3-3",
              "rank2-5",
              "rank2-3"
            ]
          },
          {
            "Name": "T3",
            "Materials": [
              "rank4-6",
              "rank4-5",
              "rank3-4",
              "rank3-3"
            ]
          }
        ]
      },
      {
        "ClassTitle": "홀리나이트 : 생명+90, 방어+16, 마방+10",
        "Tiers": [
          {
            "Name": "T1",
            "Materials": [
              "rank5-3",
              "rank5-4",
              "rank4-5",
              "rank4-6"
            ]
          },
          {
            "Name": "T2",
            "Materials": [
              "rank6-3",
              "rank6-8",
              "rank5-3",
              "rank5-4"
            ]
          },
          {
            "Name": "T3",
            "Materials": [
              "rank7-5",
              "rank7-7",
              "rank6-3",
              "rank6-8"
            ]
          },
          {
            "Name": "T4",
            "Materials": [
              "rank8-6",
              "rank8-3",
              "rank7-5",
              "rank7-7"
            ]
          }
        ]
      },
      {
        "ClassTitle": "유니콘나이트 : 마방+5, 지력+12",
        "Tiers": [
          {
            "Name": "T1",
            "Materials": [
              "rank2-6",
              "rank1-3",
              "rank1-4"
            ]
          },
          {
            "Name": "T2",
            "Materials": [
              "rank3-3",
              "rank3-5",
              "rank2-3",
              "rank2-4"
            ]
          },
          {
            "Name": "T3",
            "Materials": [
              "rank4-6",
              "rank4-1",
              "rank3-3",
              "rank3-5"
            ]
          }
        ]
      },
      {
        "ClassTitle": "유니콘마스터 : 방어+10, 마방+10, 지력+25",
        "Tiers": [
          {
            "Name": "T1",
            "Materials": [
              "rank5-3",
              "rank5-5",
              "rank4-6",
              "rank4-4"
            ]
          },
          {
            "Name": "T2",
            "Materials": [
              "rank6-4",
              "rank6-1",
              "rank5-3",
              "rank5-5"
            ]
          },
          {
            "Name": "T3",
            "Materials": [
              "rank7-1",
              "rank7-2",
              "rank6-4",
              "rank6-1"
            ]
          },
          {
            "Name": "T4",
            "Materials": [
              "rank8-7",
              "rank8-4",
              "rank7-1",
              "rank7-2"
            ]
          }
        ]
      }
    ],
    "SPOpenConditions": null,
    "Skills": [
      {
        "Name": "힐",
        "Cost": "COST 1",
        "Entries": [
          {
            "Label": "유형",
            "Value": "치료"
          },
          {
            "Label": "쿨타임",
            "Value": "0턴"
          },
          {
            "Label": "사거리",
            "Value": "2칸"
          },
          {
            "Label": "범위",
            "Value": "단일"
          }
        ],
        "IconImageUrl": "../img/skills/17.webp",
        "Desc": "[치료] 액티브 스킬. 단일 부대의 생명을 회복시킨다.<br/>회복량은 사용자 지력의 5배. 그리고 디버프를 1개 제거한다.",
        "SubSkills": null,
        "Animation": null
      },
      {
        "Name": "프로텍션",
        "Cost": "COST 1",
        "Entries": [
          {
            "Label": "유형",
            "Value": "지원"
          },
          {
            "Label": "쿨타임",
            "Value": "1턴"
          },
          {
            "Label": "사거리",
            "Value": "3칸"
          },
          {
            "Label": "범위",
            "Value": "단일"
          }
        ],
        "IconImageUrl": "../img/skills/18.webp",
        "Desc": "[지원]액티브 스킬, 1개 부대의 방어 +20% 및 <br/>면역: '방어, 마방 감소', '치료 방해', 2턴 지속.",
        "SubSkills": null,
        "Animation": null
      },
      {
        "Name": "매스 힐",
        "Cost": "COST 2",
        "Entries": [
          {
            "Label": "유형",
            "Value": "치료"
          },
          {
            "Label": "쿨타임",
            "Value": "1턴"
          },
          {
            "Label": "사거리",
            "Value": "3칸"
          },
          {
            "Label": "범위",
            "Value": "3칸"
          }
        ],
        "IconImageUrl": "../img/skills/23.webp",
        "Desc": "범위 내 아군들의 <font color=\"green\">생명 회복</font>시킨다.<br/>회복량은 사용자 지력의 <font color=\"red\">3</font>배. 그리고 <font color=\"blue\">디버프</font>를 <font color=\"red\">1</font>개 제거한다.",
        "SubSkills": null,
        "Animation": null
      },
      {
        "Name": "진격의 가호",
        "Cost": "COST 2",
        "Entries": [
          {
            "Label": "유형",
            "Value": "지원"
          },
          {
            "Label": "쿨타임",
            "Value": "2턴"
          },
          {
            "Label": "사거리",
            "Value": "3칸"
          },
          {
            "Label": "범위",
            "Value": "단일"
          }
        ],
        "IconImageUrl": "../img/skills/Skill_ValBuff.webp",
        "Desc": "[지원]액티브 스킬, 단일 부대의 공격시 주는 피해량 +30% 및<br/>받는 피해량 -50%, 2턴 지속.",
        "SubSkills": null,
        "Animation": null
      },
      {
        "Name": "기적",
        "Cost": "COST 2",
        "Entries": [
          {
            "Label": "유형",
            "Value": "지원"
          },
          {
            "Label": "쿨타임",
            "Value": "4턴"
          },
          {
            "Label": "사거리",
            "Value": "3칸"
          },
          {
            "Label": "범위",
            "Value": "4칸"
          }
        ],
        "IconImageUrl": "../img/skills/Skill_PalCrash.webp",
        "Desc": "범위 내 아군들에게 시전자 지력 <font color=\"red\">3</font>배의 생명 <font color=\"green\">회복</font> 및<br/><font color=\"blue\">주는 피해량</font> <font color=\"red\">+15%</font>, 받는 피해량 <font color=\"red\">-15%</font> 부여<br/>지속 <font color=\"red\">2</font>행동",
        "SubSkills": null,
        "Animation": null
      },
      {
        "Name": "광역 마방",
        "Cost": "COST 2",
        "Entries": [
          {
            "Label": "유형",
            "Value": "지원"
          },
          {
            "Label": "쿨타임",
            "Value": "2턴"
          },
          {
            "Label": "사거리",
            "Value": "3칸"
          },
          {
            "Label": "범위",
            "Value": "3칸"
          }
        ],
        "IconImageUrl": "../img/skills/Skill_MassMDefKup.webp",
        "Desc": "범위 내 아군들의 마방<font color=\"Red\">+30%</font>,<br/>'<font color=\"blue\">기절, 강화 방해, 이동력 감소</font> 면역 부여<br/>지속 <font color=\"red\">2</font>행동",
        "SubSkills": null,
        "Animation": null
      },
      {
        "Name": "파이어볼",
        "Cost": "COST 1",
        "Entries": [
          {
            "Label": "유형",
            "Value": "마법피해"
          },
          {
            "Label": "쿨타임",
            "Value": "1턴"
          },
          {
            "Label": "사거리",
            "Value": "2칸"
          },
          {
            "Label": "범위",
            "Value": "단일"
          }
        ],
        "IconImageUrl": "../img/skills/Skill_Fireball.webp",
        "Desc": "단일 개체에게 <font color=\"red\">1.5</font>배의 피해.<br/>'보병'에게 더큰 피해를 준다.",
        "SubSkills": null,
        "Animation": null
      },
      {
        "Name": "홀리 미사일",
        "Cost": "COST 2",
        "Entries": [
          {
            "Label": "유형",
            "Value": "마법피해"
          },
          {
            "Label": "쿨타임",
            "Value": "3턴"
          },
          {
            "Label": "사거리",
            "Value": "6칸"
          },
          {
            "Label": "범위",
            "Value": "직선"
          }
        ],
        "IconImageUrl": "../img/skills/Skill_SacredRay.webp",
        "Desc": "[마법 피해]1개의 라인에 있는 모든 적군을 공격하여 0.33배 범위 피해를 입힌다.<br/>'마물'과 전투 시 더큰 피해를 입힌다. 1개 버프 제거.",
        "SubSkills": null,
        "Animation": null
      },
      {
        "Name": "공격 지원",
        "Cost": "COST 1",
        "Entries": [
          {
            "Label": "유형",
            "Value": "패시브"
          }
        ],
        "IconImageUrl": "../img/skills/Passive_AtkBuf5.webp",
        "Desc": "[패시브]행동 종료 시 근접한 <font color=\"red\">1개</font> 아군 부대의 공격, 지력 <font color=\"red\">+15%</font> 및<br/>면역: '공격, 지력 감소', '침묵', <font color=\"red\">1턴</font> 지속.",
        "SubSkills": null,
        "Animation": null
      },
      {
        "Name": "진격의 송가",
        "Cost": "COST 3",
        "Entries": [
          {
            "Label": "유형",
            "Value": "지원"
          },
          {
            "Label": "쿨타임",
            "Value": "9턴"
          },
          {
            "Label": "사거리",
            "Value": "자신"
          },
          {
            "Label": "범위",
            "Value": "3칸"
          }
        ],
        "IconImageUrl": "../img/Super/티아리스.webp",
        "Desc": "범위 내 모든 아군 이동력 <font color=\"red\">+1</font>, 공격,지력<font color=\"red\">+15%</font> <br/>선공으로 전투 진입 시 피해량 <font color=\"red\">+30%</font>, 받는 피해량 <font color=\"red\">-50%</font>, <font color=\"green\">받는 반사 피해량</font> <font color=\"red\">-30%</font><br/><font color=\"blue\">공격, 지력 감소 면역</font> 부여 <br/>지속 <font color=\"red\">2</font>행동",
        "SubSkills": null,
        "Animation": "../img/Super/티아리스_Ani.webp"
      }
    ],
    "Propose": true,
    "Bio": [
      {
        "Entry": "영웅소개",
        "Desc": "◆키/몸무게：145cm / 32kg<br/>◆신체 사이즈：65/52/68<br/>◆출연：랑그릿사III<br/>◆소속：라카스 왕국"
      },
      {
        "Entry": "영웅담1",
        "Desc": "라카스 왕국 부유성 후작의 딸로 마법에 재능이 있는 활발한 소녀. 전쟁 초기에 아버지가 죽는 걸 목격한 일은 그녀의 가슴 속에 지워지지 않는 그늘로 남게 되었다.디하르트와 같이 도망친 후 나라를 되찾기 위한 길에 나섰으며 마침내 전설 속에 그 자취를 남긴다.<br/><font color=\"#DC143C\"><size=33><i>「난 아직 11살이야! 너무 많은 걸 요구하지 마!」</i>  </size=33></font>"
      },
      {
        "Entry": "영웅담2",
        "Desc": "천진난만하고 활기찬 소녀로 자신만의 백마 탄 왕자님을 꿈꾸고 있다. 겉으로는 단순히 명랑하고 장난기가 많은 모습이나 내면에 큰 슬픔을 감추고 있는 용감하고 굳센 여자아이.어릴 때부터 풍족하게 생활한 덕분에 아직 귀족의 습관이 남아 있다. 깨끗한 것을 좋아하고 더럽고 냄새나는 것을 싫어하며 이건 전투 중에도 마찬가지다."
      },
      {
        "Entry": "영웅담3",
        "Desc": "디하르트와 함께하며 라카스 독립군의 중요 인물로 거듭난다. 원래는 단순하고 세상 경험 부족한 아가씨였지만 차츰 믿을 수 있는 강력한 조력자가 되어간다.<br/><font color=\"#DC143C\"><size=33><i>「그렇다곤 하지만 그 근육질 녀석들은 참을 수 없는걸!」</i>  </size=33></font>"
      },
      {
        "Entry": "영웅담4",
        "Desc": "전쟁이 끝난 후 티아리스는 아버지가 남겨놓은 라카스 옛 지방의 영지와 재산을 물려받는다. 새로운 후작이 된 티아리스는 엘스리드 왕국과 루인 국왕의 중요한 지지자가 되어 대륙의 평화와 왕국의 재건에 커다란 공헌을 한다."
      },
      {
        "Entry": "영웅담5",
        "Desc": "11살의 나이로 현세에 소환된 것에 불만을 갖고 있다. 이 세계에서 좀 더 성숙한 어른이 되고 싶어... <br/><font color=\"#DC143C\"><size=33><i>「나, 도움이 되고 있어? 미안해, 나도 즈파파파~ 라던가 비리비리~같은 마법을 쓸 수 있으면 좋을 텐데!」</i>  </size=33></font>"
      }
    ],
    "Performance": [
      {
        "Entry": "「소환 - 등장대사」",
        "Desc": "티아리스야! 잘 부탁해. 티아라고 불러도 좋아."
      },
      {
        "Entry": "「전투 - 전직1」",
        "Desc": "기분 좋다.... 이런식으로 변하는 것도 흔치 않은 경험이네."
      },
      {
        "Entry": "「전투 - 전직2」",
        "Desc": "음~ 전혀 커지지 않았네.... 아, 키 말하는거야!"
      },
      {
        "Entry": "「전투 - 스킬시전1」",
        "Desc": "아직 끝나지 않았어!"
      },
      {
        "Entry": "「전투 - 스킬시전2」",
        "Desc": "으음, 효과가 있다!"
      },
      {
        "Entry": "「전투 - 선택1」",
        "Desc": "어? 벌써 내 차례야?"
      },
      {
        "Entry": "「전투 - 선택2」",
        "Desc": "그...그래."
      },
      {
        "Entry": "「전투 - 전투불능」",
        "Desc": "싫어...무서워...."
      },
      {
        "Entry": "「대화 - 친밀도1」",
        "Desc": "아...안녕! 나, 폐를 끼친건 아니겠지?"
      },
      {
        "Entry": "「대화 - 친밀도2」",
        "Desc": "내가 좀 더 강했다면...아버지도...."
      },
      {
        "Entry": "「대화 - 친밀도3」",
        "Desc": "나, 도움이 되고 있어? 미안해, 나도 즈파파파~ 라던가 비리비리~같은 마법을 쓸 수 있으면 좋을텐데."
      },
      {
        "Entry": "「대화 - 친밀도4」",
        "Desc": "요즘 나도 쓸 수 있는 마법을 연구하고 있어! 만약 성공한다면, 역사적인 순간을 기념해서 마법에 나 자신의 이름을 붙일거야!"
      },
      {
        "Entry": "「대화 - 친밀도5」",
        "Desc": "응! 좋아해~! 내 머리를 쓰다듬어 주니까. 다...다른 곳? 아직 안된다니까!"
      },
      {
        "Entry": "「대화 - 일상1」",
        "Desc": "태평한 것도 생활 방식 중 하나야. 적합한 형태가 있다면 사람들이 따르는 귀족이 될 수 있어!"
      },
      {
        "Entry": "「대화 - 기쁨」",
        "Desc": "즐거운 일도 슬픈 일도 매일 반복되고 있어....그러니 웃을 수 있을때는 웃을 수 있을만큼 웃는거야!"
      },
      {
        "Entry": "「대화 - 분노」",
        "Desc": "사람의 고민은 제각각이야! 내가 키 작은 것만 고민한다고 생각하지 마!"
      },
      {
        "Entry": "「대화 - 슬픔」",
        "Desc": "부유성... 우리집 이었는데....하지만 그 집은, 이제 존재하지 않아...."
      },
      {
        "Entry": "「대화 - 수줍음」",
        "Desc": "매일 백마 탄 왕자님의 꿈을 꾸고 있어. 응. 이게 귀족병이라는 거겠지!"
      },
      {
        "Entry": "「상호작용 - 대사1」",
        "Desc": "아! 장난친거지!"
      },
      {
        "Entry": "「상호작용 - 대사2」",
        "Desc": "후후후...."
      },
      {
        "Entry": "「상호작용 - 대사3」",
        "Desc": "이대로라면... 조금 지나치다 생각해... 하지만 다른 사람 따윈 신경쓰지 않아! 헤헤!"
      },
      {
        "Entry": "「상호작용 - 대사4」",
        "Desc": "정말~ 부끄러워!"
      },
      {
        "Entry": "「상호작용 - 대사5」",
        "Desc": "당신을 보면 부끄러운 기분이 들어. 이게 사랑인걸까...."
      },
      {
        "Entry": "「상호작용 - 호감1」",
        "Desc": "저기.... 당신이 내가 기대했던 백마 탄 왕자님이지? 분명 그렇겠지...."
      },
      {
        "Entry": "「상호작용 - 호감2」",
        "Desc": "나 혼자 즐거워 했던건, 괴로운 기억에서 도망치고 있었을 뿐이야.... 겨우, 나도 받아들일 수 있게 되었네..."
      },
      {
        "Entry": "「상호작용 - 반감1」",
        "Desc": "...만지지마, 변태 지휘관!"
      },
      {
        "Entry": "「상호작용 - 반감2」",
        "Desc": "그만둬!"
      },
      {
        "Entry": "「상호작용 - 반감3」",
        "Desc": "욕심이 지나치잖아!"
      },
      {
        "Entry": "「상호작용 - 특수1」",
        "Desc": "당신과 함께 있으면 정말 안심할 수 있어... 이런 기회가 더 많았으면 좋겠다!"
      },
      {
        "Entry": "「상호작용 - 특수2」",
        "Desc": "오늘 또 만날 수 있어? 바이바이!"
      }
    ],
    "Performance2": null,
    "Misc": ""
  }
]