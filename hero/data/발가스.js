var data = [
{
  "Skins": [
    {
      "ImageUrl": "발가스/1.webp",
      "Desc": "기본"
    },
    {
      "ImageUrl": "발가스/2.webp",
      "Desc": "이벤트"
    },
    {
      "ImageUrl": "발가스/3.webp",
      "Desc": "이벤트"
    }
  ],
  "Name": "발가스",
  "CV": "모가미 츠구오",
  "ChName": "巴尔加斯",
  "EngName": "Vargas",
  "JapName": "バルガス",
  "Rank": [
    1
  ],
  "SP": false,
  "Groups": [
    2,
    6
  ],
  "HP": 40,
  "SPHP": 0,
  "Attk": 10,
  "SPAttk": 0,
  "Def": 35,
  "SPDef": 0,
  "MagicDef": 10,
  "SPMagicDef": 0,
  "HowToGet": "영웅 소환",
  "Fetters": [
    {
      "Condition": "발가스를 출전시켜 시공의 균열 3-6 클리어"
    },
    {
      "Condition": "발가스를 로열 가디언으로 전직"
    },
    {
      "Condition": "이멜다의 도움 하에 '찾는 물건' 클리어"
    },
    {
      "Condition": "베른하르트의 도움 하에 '부드러운 사나이 클리어'"
    }
  ],
  "HeartFetter": "생명+1150, 방어+50, 마방+30",
  "Talent": [
    {
      "Name": "용장의 신력",
      "Icon": "18.webp",
      "Abilities": [
        {
          "Desc": "1성"
        },
        {
          "Desc": "부대 체력이 높을수록 받는 물리 피해량이 최대 <font color=\"red\">15%</font> 감소.<br/> 사망 시 부활 및 생명을 <font color=\"red\">15%</font> 회복.<br/> 이 효과는 전투마다 <font color=\"red\">1회</font> 발동할 수 있다."
        },
        {
          "Desc": "부대 체력이 높을수록 받는 물리 피해량이 최대 <font color=\"red\">18%</font> 감소.<br/> 사망 시 부활 및 생명을 <font color=\"red\">18%</font> 회복.<br/> 이 효과는 전투마다 <font color=\"red\">1회</font> 발동할 수 있다."
        },
        {
          "Desc": "부대 체력이 높을수록 받는 물리 피해량이 최대 <font color=\"red\">21%</font> 감소.<br/> 사망 시 부활 및 생명을 <font color=\"red\">21%</font> 회복.<br/> 이 효과는 전투마다 <font color=\"red\">1회</font> 발동할 수 있다."
        },
        {
          "Desc": "부대 체력이 높을수록 받는 물리 피해량이 최대 <font color=\"red\">25%</font> 감소.<br/> 사망 시 부활 및 생명을 <font color=\"red\">25%</font> 회복.<br/> 이 효과는 전투마다 <font color=\"red\">1회</font> 발동할 수 있다."
        },
        {
          "Desc": "부대 체력이 높을수록 받는 물리 피해량이 최대 <font color=\"red\">30%</font> 감소.<br/> 사망 시 부활 및 생명을 <font color=\"red\">30%</font> 회복.<br/> 이 효과는 전투마다 <font color=\"red\">1회</font> 발동할 수 있다."
        }
      ]
    }
  ],
  "SPTalent": null,
  "Second_Awaken": {
    "Name" : "강인한 신력",
    "Stats": "방어 <font color=\"red\">+5</font>%<br/>자신의 <font color=\"blue\">불굴</font> 효과 발동 후 '<font color=\"blue\">주는 피해량</font> <font color=\"red\">+15</font>%' 효과 획득<br/>지속 <font color=\"red\">1</font>턴<br/><b>해제 불가</b>"
  },
  "Equip": {
    "Name": "염룡의 빛(갑옷)",
    "Stats": "생명 437,방어 65",
    "Icon": "발가스.webp",
    "Ability": "방어 +10%<br/><font color=\"blue\">전력 찌르기</font>효과를 보유시<br/>원거리 <font color=\"red\">2</font>칸 공격에 대해 <font color=\"blue\">반격</font>."
  },
  "Soldiers": [
    20,
    22,
    26,
    32,
    35,
    36,
    55,
    74,
    83,
    94
  ],
  "Jobs": [
    {
      "Name": "로열 가디언",
      "Icon": "Icon_Occupation_Marines.webp",
      "SD": [
        {
          "ImageUrl": "발가스/1/1.webp"
        },
        {
          "ImageUrl": "발가스/2/1.webp"
        },
        {
          "ImageUrl": "발가스/3/1.webp"
        }
      ],
      "AttkRange": 1,
      "Move": 3,
      "MoveType": "Walk",
      "HP": 5310,
      "HPRate": 88,
      "Attk": 424,
      "AttkRate": 65,
      "Int": 210,
      "IntRate": 35,
      "Def": 412,
      "DefRate": 91,
      "MagicDef": 179,
      "MagicDefRate": 39,
      "Skill": 77,
      "SkillRate": 22,
      "HeartFetterBouns1": "디버프가 있는 대상에게 공격받아 전투 진입시 받는피해 -10%",
      "HeartFetterBouns2": "근접 공격받아 전투 진입시 입히는피해 +10%"
    },
    {
      "Name": "용사",
      "Icon": "Icon_Occupation_Infantryman.webp",
      "SD": [
        {
          "ImageUrl": "발가스/1/2.webp"
        },
        {
          "ImageUrl": "발가스/2/2.webp"
        },
        {
          "ImageUrl": "발가스/3/2.webp"
        }
      ],
      "AttkRange": 1,
      "Move": 3,
      "MoveType": "Walk",
      "HP": 4996,
      "HPRate": 83,
      "Attk": 450,
      "AttkRate": 69,
      "Int": 231,
      "IntRate": 38,
      "Def": 402,
      "DefRate": 89,
      "MagicDef": 247,
      "MagicDefRate": 54,
      "Skill": 90,
      "SkillRate": 25,
      "HeartFetterBouns1": "공격하여 전투 진입시 받는피해 -10%",
      "HeartFetterBouns2": "공격하여 전투 진입시 입히는피해 +10%"
    }
  ],
  "Materials": {
    "HeartFetter": [
      "h1",
      "h2",
      "h4",
      "h3",
      "h1-1",
      "h2-1",
      "h4-1",
      "h3-1",
      "x"
    ],
    "Fetter": [
      "5",
      "5-1",
      "5-2",
      "5-3",
      "5-4"
    ],
    "Awaken": [
      "ranka2",
      "ranka5",
      "ranka2-1",
      "ranka5-1"
    ]
  },
  "ClassUp": [
    {
      "ClassTitle": "서전트 : 공격+5, 방어+6",
      "Tiers": [
        {
          "Name": "T1",
          "Materials": [
            "rank1-2"
          ]
        },
        {
          "Name": "T2",
          "Materials": [
            "rank1-2"
          ]
        }
      ]
    },
    {
      "ClassTitle": "제네럴 : 생명+80, 공격+7",
      "Tiers": [
        {
          "Name": "T1",
          "Materials": [
            "rank2-2",
            "rank1-2",
            "rank1-1"
          ]
        },
        {
          "Name": "T2",
          "Materials": [
            "rank3-6",
            "rank3-1",
            "rank2-2",
            "rank2-1"
          ]
        },
        {
          "Name": "T3",
          "Materials": [
            "rank4-2",
            "rank4-6",
            "rank3-6",
            "rank3-1"
          ]
        }
      ]
    },
    {
      "ClassTitle": "가디언 : 생명+48, 방어+8",
      "Tiers": [
        {
          "Name": "T1",
          "Materials": [
            "rank2-2",
            "rank1-2",
            "rank1-1"
          ]
        },
        {
          "Name": "T2",
          "Materials": [
            "rank3-6",
            "rank3-1",
            "rank2-2",
            "rank2-6"
          ]
        },
        {
          "Name": "T3",
          "Materials": [
            "rank4-2",
            "rank4-6",
            "rank3-6",
            "rank3-1"
          ]
        }
      ]
    },
    {
      "ClassTitle": "로열가디언 : 공격+15, 방어+16, 마방+10",
      "Tiers": [
        {
          "Name": "T1",
          "Materials": [
            "rank5-6",
            "rank5-3",
            "rank4-2",
            "rank4-6"
          ]
        },
        {
          "Name": "T2",
          "Materials": [
            "rank6-2",
            "rank6-1",
            "rank5-6",
            "rank5-3"
          ]
        },
        {
          "Name": "T3",
          "Materials": [
            "rank7-1",
            "rank7-7",
            "rank6-2",
            "rank6-1"
          ]
        },
        {
          "Name": "T4",
          "Materials": [
            "rank8-2",
            "rank8-1",
            "rank7-1",
            "rank7-7"
          ]
        }
      ]
    },
    {
      "ClassTitle": "글래디에이터 : 생명+48, 방어+8",
      "Tiers": [
        {
          "Name": "T1",
          "Materials": [
            "rank2-1",
            "rank1-1",
            "rank1-2"
          ]
        },
        {
          "Name": "T2",
          "Materials": [
            "rank3-1",
            "rank3-6",
            "rank2-1",
            "rank2-6"
          ]
        },
        {
          "Name": "T3",
          "Materials": [
            "rank4-1",
            "rank4-2",
            "rank3-1",
            "rank3-6"
          ]
        }
      ]
    },
    {
      "ClassTitle": "용사 : 생명+90, 공격+15, 방어+16",
      "Tiers": [
        {
          "Name": "T1",
          "Materials": [
            "rank5-1",
            "rank5-6",
            "rank4-1",
            "rank4-2"
          ]
        },
        {
          "Name": "T2",
          "Materials": [
            "rank6-1",
            "rank6-2",
            "rank5-1",
            "rank5-6"
          ]
        },
        {
          "Name": "T3",
          "Materials": [
            "rank7-2",
            "rank7-1",
            "rank6-1",
            "rank6-2"
          ]
        },
        {
          "Name": "T4",
          "Materials": [
            "rank8-1",
            "rank8-7",
            "rank7-2",
            "rank7-4"
          ]
        }
      ]
    }
  ],
  "SPOpenConditions": null,
  "Skills": [
    {
      "Name": "전력 찌르기",
      "Cost": "COST 2",
      "Entries": [
        {
          "Label": "유형",
          "Value": "물리 피해"
        },
        {
          "Label": "쿨타임",
          "Value": "2턴"
        },
        {
          "Label": "사거리",
          "Value": "1칸"
        },
        {
          "Label": "범위",
          "Value": "단일"
        }
      ],
      "IconImageUrl": "../img/skills/Skill_SpearCrash1.webp",
      "Desc": "[물리 피해]단일 적 부대를 공격해 <font color=\"#DC143C\">1.5배</font>의 전투 피해를 준다.<br/>전투 전 [전력 찌르기] 효과 획득: 방어의 <font color=\"#DC143C\">1.6배</font>로 공격을 대체, <font color=\"#DC143C\">2턴</font> 지속.",
      "SubSkills": null,
      "Animation": null
    },
    {
      "Name": "수호",
      "Cost": "COST 1",
      "Entries": [
        {
          "Label": "유형",
          "Value": "패시브"
        }
      ],
      "IconImageUrl": "../img/skills/26.webp",
      "Desc": "[패시브]받는 물리 피해량 -5%,<br/>근접한 아군 부대가 물리 공격을 받을 시 대신하여 전투에 진입한다.",
      "SubSkills": null,
      "Animation": null
    },
    {
      "Name": "공격 지원",
      "Cost": "COST 1",
      "Entries": [
        {
          "Label": "유형",
          "Value": "패시브"
        }
      ],
      "IconImageUrl": "../img/skills/Passive_AtkBuf5.webp",
      "Desc": "[패시브]행동 종료 시 근접한 <font color=\"red\">1개</font> 아군 부대의 공격, 지력 <font color=\"red\">+15%</font> 및<br/>면역: '공격, 지력 감소', '침묵', <font color=\"red\">1턴</font> 지속.",
      "SubSkills": null,
      "Animation": null
    },
    {
      "Name": "철벽수비",
      "Cost": "COST 2",
      "Entries": [
        {
          "Label": "유형",
          "Value": "액티브"
        },
        {
          "Label": "쿨타임",
          "Value": "2턴"
        },
        {
          "Label": "사거리",
          "Value": "자신"
        },
        {
          "Label": "범위",
          "Value": "단일"
        }
      ],
      "IconImageUrl": "../img/skills/Skill_IronBody.webp",
      "Desc": "[패시브]근접한 아군 부대를 대신해 물리 공격을 받는다.<br/>[액티브]호위 범위가 2칸으로 늘어나며 치명타 피격확률 -20%,<br/>[전력 찌르기]효과 획득: 방어의 1.6배로 공격을 대체, 2턴 지속.",
      "SubSkills": null,
      "Animation": null
    },
    {
      "Name": "방패벽",
      "Cost": "COST 2",
      "Entries": [
        {
          "Label": "유형",
          "Value": "패시브"
        }
      ],
      "IconImageUrl": "../img/skills/Passive_Parry.webp",
      "Desc": "[패시브]근접 공격을 받아 전투 진입 시 25%확률로 발동, 받는 물리 피해량 <font color=\"red\">-50%</font>.",
      "SubSkills": null,
      "Animation": null
    },
    {
      "Name": "배수의 진",
      "Cost": "COST 1",
      "Entries": [
        {
          "Label": "유형",
          "Value": "패시브"
        }
      ],
      "IconImageUrl": "../img/skills/Passive_DefBuf3.webp",
      "Desc": "[패시브]부대 생명이 50% 이하로 전투 진입 시 방어, 마방 +10%.",
      "SubSkills": null,
      "Animation": null
    },
    {
      "Name": "공격 위협",
      "Cost": "COST 2",
      "Entries": [
        {
          "Label": "유형",
          "Value": "패시브"
        }
      ],
      "IconImageUrl": "../img/skills/Passive_AuraAtkDown.webp",
      "Desc": "[패시브]행동 종료 시 주위 2칸 내 모든 적 부대 공격, 지력 -15%, 1턴 지속.",
      "SubSkills": null,
      "Animation": null
    },
    {
      "Name": "전투 함성",
      "Cost": "COST 1",
      "Entries": [
        {
          "Label": "유형",
          "Value": "패시브"
        }
      ],
      "IconImageUrl": "../img/skills/Passive_Snarl.webp",
      "Desc": "[패시브]공격하여 전투 진입 전 적 부대의 버프 1개 제거 및<br/>공격, 방어 -10%, 1턴 지속.",
      "SubSkills": null,
      "Animation": null
    },
    {
      "Name": "관통",
      "Cost": "COST 2",
      "Entries": [
        {
          "Label": "유형",
          "Value": "물리 피해"
        },
        {
          "Label": "쿨타임",
          "Value": "2턴"
        },
        {
          "Label": "사거리",
          "Value": "1칸"
        },
        {
          "Label": "범위",
          "Value": "단일"
        }
      ],
      "IconImageUrl": "../img/skills/43.webp",
      "Desc": "[물리 피해] 단일 적 부대를 공격해 1.5배의 전투 피해를 준다.<br/>전투 후, 적 방어 -30%, 2턴 지속.",
      "SubSkills": null,
      "Animation": null
    },
    {
      "Name": "염룡의 혼",
      "Cost": "COST 3",
      "Entries": [
        {
          "Label": "유형",
          "Value": "액티브"
        },
        {
          "Label": "쿨타임",
          "Value": "3턴"
        },
        {
          "Label": "사거리",
          "Value": "자신"
        },
        {
          "Label": "범위",
          "Value": "단일"
        }
      ],
      "IconImageUrl": "../img/Super/발가스.webp",
      "Desc": "[패시브]근접한 아군을 대신해 <b>물리 피해</b>를 받는다.<br/> 고유 스킬 <font color=\"blue\">불굴</font> 효과 발동 후 [<font color=\"blue\">굳건함</font>] 효과 획득 : <br/>[<font color=\"blue\">굳건함</font>]: <b>전장에 아군이 있을 경우</b> 생명 <font color=\"red\">30</font>% 수치로 <font color=\"blue\">부활</font><br/><b>단, 부대 최대 생명 <font color=\"red\">-35</font>% 감소</b>(패널티)<br/>(<b>해제 불가, 면역 불가, 굳건함의 <font color=\"blue\">불굴</font> 효과는 전장에서 최대 <font color=\"red\">2</font>회 발동</b>)<br/>[액티브]호위 범위가 <font color=\"red\">3</font>칸으로 증가, 받는 치명타 확률 <font color=\"red\">-30</font>%, <br/>'<font color=\"blue\">치료 불가 면역</font>' 효과 부여, '<font color=\"blue\">전력 찌르기</font>' 효과 획득, <br/>지속 <font color=\"red\">3</font>행동, <b>상술된 모든 효과는 제거 불가</b>",
      "SubSkills": null,
      "Animation": "../img/Super/발가스_Ani.webp"
    }
  ],
  "Propose": false,
  "Bio": [
    {
      "Entry": "영웅소개",
      "Desc": "◆키/몸무게：199cm / 92kg<br/>◆출연：랑그릿사II<br/>◆소속：레이갈드 제국"
    },
    {
      "Entry": "영웅담1",
      "Desc": "베른하르트 황제를 따르는 사천왕 중 한명. 염룡병단의 지휘관이자 '맹장'의 칭호를 받은 인물. 머리까지 근육인 거친 남자로 보이지만 사실은 오랜 실전 경험으로 다져진 교활한 작전 능력도 겸비하고 있다.언제나 병사들과 동고동락하기에 병사들의 깊은 존경을 받고 있다.<br/><font color=\"#DC143C\"><size=33><i>「대머리 장군이라 한 게 누구냐, 어서 나오지 못해!」</i>  </size=33></font>"
    },
    {
      "Entry": "영웅담2",
      "Desc": "군인 집안에서 태어나 전선에서 성장한 역전의 장군. 하지만 아버지가 전장에서 도망친 죄인이란 사실이 평생 그의 가슴에 한으로 맺혀 있었다.부하를 지켜주는 것을 자신의 책임으로 여기며 부하를 버리고 도망치는 행위를 지휘관의 가장 큰 수치라고 생각한다."
    },
    {
      "Entry": "영웅담3",
      "Desc": "이론에 얽매이지 않고 다년간의 실전경험을 바탕으로 염룡병단을 지휘하며 뛰어난 전술을 펼친다. 양공 작전으로 부하를 보내 적의 주력 부대를 견제하고 자신은 텅 빈 칼자스 성을 포위하는 등, 한때 빛의 군단에게 굉장한 골칫거리였던 인물."
    },
    {
      "Entry": "영웅담4",
      "Desc": "원래는 부인 엘리자의 곁에서 아이가 태어나는 것을 지켜봐야 했던 발가스. 하지만 그는 부하들과 함께 싸우기 위해 전선으로 돌아왔다. 그리고 그것이 그의 생애 마지막 싸움이었다.<br/><font color=\"#DC143C\"><size=33><i>「에리자에게 유언을 전해주었으면 한다. 마지막까지... 나는 부하들과 함께... 싸웠다고...」</i>  </size=33></font>"
    },
    {
      "Entry": "영웅담5",
      "Desc": "「젊은이들이 전선에 나가 죽는 것을 보고만 있을 수 없다」는 생각으로 전장에 나가 모두와 함께 싸웠으며 신뢰받는 동료가 됐다. 그래도 때때로 볼 수 없었던 아이를 생각하며 깊은 밤, 혼자 슬퍼하곤 한다."
    }
  ],
  "Performance": [
    {
      "Entry": "「소환 - 등장대사」",
      "Desc": "오, 불러줘서 고맙다! 염룡병단 단장 발가스다. 선봉은 내게 맡겨라!"
    },
    {
      "Entry": "「전투 - 전직1」",
      "Desc": "오! 좋다 좋아! 귀중한 자원에 감사하지!"
    },
    {
      "Entry": "「전투 - 전직2」",
      "Desc": "카하하! 나쁘지 않군! 좋은 느낌이다!"
    },
    {
      "Entry": "「전투 - 스킬시전1」",
      "Desc": "순진하긴!"
    },
    {
      "Entry": "「전투 - 스킬시전2」",
      "Desc": "그 정도냐!"
    },
    {
      "Entry": "「전투 - 선택1」",
      "Desc": "하하!"
    },
    {
      "Entry": "「전투 - 선택2」",
      "Desc": "맡겨줘!"
    },
    {
      "Entry": "「전투 - 전투불능」",
      "Desc": "뒤는 맡기겠다..."
    },
    {
      "Entry": "「대화 - 친밀도1」",
      "Desc": "하하하, 기운이 넘쳐 보이는군!"
    },
    {
      "Entry": "「대화 - 친밀도2」",
      "Desc": "오! 좋다! 그런 기세다! 이러면 병사들도 분명 사기가 오를거다!"
    },
    {
      "Entry": "「대화 - 친밀도3」",
      "Desc": "아쉽냐고? ...당연히 아쉽지, 마지막까지 내 아이를 볼 수 없었으니... 하지만 후회는 하지 않는다. 어쨌든 나는 군인이니까!"
    },
    {
      "Entry": "「대화 - 친밀도4」",
      "Desc": "이건...역사책인가? 보자...오, 어미처럼 이쁘고 상냥하게 컸다고? 정말...정말 잘됐구나!"
    },
    {
      "Entry": "「대화 - 친밀도5」",
      "Desc": "내 눈은 틀리지 않았다! 너는 정말 대단한 지휘관이다. 군인인 내가 보증하지!앞으로도 잘 부탁하네!"
    },
    {
      "Entry": "「대화 - 일상1」",
      "Desc": "단장으로서 나는 앞장서서 돌격해야하지! 전장에서 전사하는 것은 군인의 가장 큰 영광이다!"
    },
    {
      "Entry": "「대화 - 일상2」",
      "Desc": "전장에 만약이란 없다. 그런게 있다면 멀리서라도 태어난 아이를 보고 싶군..."
    },
    {
      "Entry": "「대화 - 기쁨」",
      "Desc": "오오오! 바로 그 기세다!"
    },
    {
      "Entry": "「대화 - 분노」",
      "Desc": "내 사람들을 건들 셈이냐? 우선 내 시체를 넘어가야 할거다!"
    },
    {
      "Entry": "「상호작용 - 대사1」",
      "Desc": "음, 배려해줘서 고맙군!"
    },
    {
      "Entry": "「상호작용 - 대사2」",
      "Desc": "으음...."
    },
    {
      "Entry": "「상호작용 - 대사3」",
      "Desc": "정말 대단한 마사지 실력이군!"
    },
    {
      "Entry": "「상호작용 - 대사4」",
      "Desc": "으하하! 하하!"
    },
    {
      "Entry": "「상호작용 - 대사5」",
      "Desc": "기분은 좋지만, 슬슬 쉬는게 어때."
    },
    {
      "Entry": "「상호작용 - 반감1」",
      "Desc": "으음! 여기엔 엣 상처가 있다고! 아야야..."
    },
    {
      "Entry": "「상호작용 - 반감2」",
      "Desc": "건들지 마!"
    },
    {
      "Entry": "「상호작용 - 반감3」",
      "Desc": "조금 불편할 뿐이다! 이 정도로 티를 내진 않아!"
    },
    {
      "Entry": "「상호작용 - 특수1」",
      "Desc": "신경 써줘서 고맙군! 나도 최선을 다하지!"
    },
    {
      "Entry": "「상호작용 - 특수2」",
      "Desc": "고생했다! 일찍 쉬도록!"
    }
  ],
  "Performance2": null,  
  "Misc": ""
}];