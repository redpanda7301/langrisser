var data = [
{
  "Skins": [
    {
      "ImageUrl": "마리엘/1.webp",
      "Desc": "기본"
    },
    {
      "ImageUrl": "마리엘/2.webp",
      "Desc": "빛의 메아리"
    },
    {
      "ImageUrl": "마리엘/3.webp",
      "Desc": "형귀뽑기"
    },
    {
      "ImageUrl": "마리엘/4.webp",
      "Desc": "서밋아레나 S11 패자스킨"
    }
  ],
  "Name": "마리엘",
  "ChName": "玛丽埃尔",
  "EngName": "Mariel",
  "JapName": "マリエル",
  "CV": "야마모토 아야노",
  "Rank": [
    0
  ],
  "SP": false,
  "Groups": [
    1,
    11
  ],
  "HP": 25,
  "SPHP": 0,
  "Attk": 40,
  "SPAttk": 0,
  "Def": 15,
  "SPDef": 0,
  "MagicDef": 20,
  "SPMagicDef": 0,
  "HowToGet": "영웅 소환",
  "Fetters": [
    {
      "Condition": "마리엘을 출전시켜 아레나 5승"
    },
    {
      "Condition": "클래스 신성 신관 전직"
    },
    {
      "Condition": "탄생의 빛의 도움을 받아 운명의문 4 클리어"
    },
    {
      "Condition": "로자리아의 도움을 받아 운명의문 5 클리어"
    }
  ],
  "HeartFetter": "생명+800, 방어+50, 마방+50",
  "Talent": [
    {
      "Name": "정의의 관",
      "Icon": "Gift_Mariel.webp",
      "Abilities": [
        {
          "Desc": "1성"
        },
        {
          "Desc": "2성"
        },
        {
          "Desc": "<b>생명 <font color=\"red\">50</font>% 이상일 경우 선공으로 전투 진입시 받는 피해 <font color=\"red\">-10%</font> 공격<font color=\"red\">+10</font>%</b><br/><b>스테이지 시작 시 [<font color=\"purple\">계율</font>] 효과 획득</b><br/>[<font color=\"purple\">계율</font>] : 치료량 <font color=\"red\">50</font>% 받는 모든 피해 <font color=\"red\">-25</font>% 주는 모든피해 <font color=\"red\">-50%</font><br/><b>선공 후</b> [<font color=\"purple\">계율</font>] 효과는 사라지며 재행동 가능"
        },
        {
          "Desc": "<b>생명 <font color=\"red\">50</font>% 이상일 경우 선공으로 전투 진입시 받는 피해 <font color=\"red\">-15%</font> 공격<font color=\"red\">+13</font>%</b><br/><b>스테이지 시작 시 [<font color=\"purple\">계율</font>] 효과 획득</b><br/>[<font color=\"purple\">계율</font>] : 치료량 <font color=\"red\">50</font>% 받는 모든 피해 <font color=\"red\">-25</font>% 주는 모든피해 <font color=\"red\">-50%</font><br/><b>선공 후</b> [<font color=\"purple\">계율</font>] 효과는 사라지며 재행동 가능"
        },
        {
          "Desc": "<b>생명 <font color=\"red\">50</font>% 이상일 경우 선공으로 전투 진입시 받는 피해 <font color=\"red\">-20%</font> 공격<font color=\"red\">+16</font>%</b><br/><b>스테이지 시작 시 [<font color=\"purple\">계율</font>] 효과 획득</b><br/>[<font color=\"purple\">계율</font>] : 치료량 <font color=\"red\">50</font>% 받는 모든 피해 <font color=\"red\">-25</font>% 주는 모든피해 <font color=\"red\">-50%</font><br/><b>선공 후</b> [<font color=\"purple\">계율</font>] 효과는 사라지며 재행동 가능"
        },
        {
          "Desc": "<b>생명 <font color=\"red\">50</font>% 이상일 경우 선공으로 전투 진입시 받는 피해 <font color=\"red\">-30%</font> 공격<font color=\"red\">+20</font>%</b><br/><b>스테이지 시작 시 [<font color=\"purple\">계율</font>] 효과 획득</b><br/>[<font color=\"purple\">계율</font>] : 치료량 <font color=\"red\">50</font>% 받는 모든 피해 <font color=\"red\">-25</font>% 주는 모든피해 <font color=\"red\">-50%</font><br/><b>선공 후</b> [<font color=\"purple\">계율</font>] 효과는 사라지며 재행동 가능"
        }
      ]
    }
  ],
  "SPTalent": null,
  "Second_Awaken": {
    "Name" : "전투의 바람",
    "Stats":"공격 <font color=\"red\">+5</font>%<br/><font color=\"blue\">성스러운 분노의 일격</font>으로 적을 격파후, 행동 종료시<br/><font color=\"blue\">성스러운 분노</font> 획득<br/>반대일 경우 적 <font color=\"blue\">기절</font><br/>지속 <font color=\"red\">1</font>행동"
  },
  "Equip": {
    "Name": "성스러운 프레일(무기)",
    "Stats": "생명 583,공격 85",
    "Icon": "마리엘.webp",
    "Ability": "공격 <font color=\"red\">+10</font>%<br/><font color=\"purple\">계율</font>보유 시 스킬 사용 후 스킬 쿨타임 <font color=\"red\">-1</font><br/><font color=\"purple\">계율</font> 미보유 시 영웅이 주는 피해량 <font color=\"red\">+10</font>%<br/>쿨타임 <font color=\"red\">2</font>행동"
  },
  "Soldiers": [
    31,
    20,
    24,
    4,
    3,
    29,
    67,
    91
  ],
  "Jobs": [
    {
      "Name": "이단 심판자",
      "Icon": "Icon_Occupation_Monk.webp",
      "SD": [
        {
          "ImageUrl": "마리엘/1/1.webp"
        },
        {
          "ImageUrl": "마리엘/2/1.webp"
        },
        {
          "ImageUrl": "마리엘/3/1.webp"
        },
        {
          "ImageUrl": "마리엘/4/1.webp"
        }
        
      ],
      "AttkRange": 1,
      "Move": 3,
      "MoveType": "Walk",
      "HP": 4083,
      "HPRate": 73,
      "Attk": 576,
      "AttkRate": 83,
      "Int": 290,
      "IntRate": 45,
      "Def": 325,
      "DefRate": 72,
      "MagicDef": 339,
      "MagicDefRate": 73,
      "Skill": 138,
      "SkillRate": 37,
      "HeartFetterBouns1": "<b>공격하여 전투 진입시</b>, <br/>부대 받는 피해감소 <font color=\"red\">-10</font>%",
      "HeartFetterBouns2": "<b>부대 체력이 <font color=\"red\">100</font>%일 경우</b><br/>주는 피해량 <font color=\"red\">+10</font>%"
    },
    {
      "Name": "신성 신관",
      "Icon": "Icon_Occupation_Monk.webp",
      "SD": [
        {
          "ImageUrl": "마리엘/1/2.webp"
        },
        {
          "ImageUrl": "마리엘/2/1.webp"
        },
        {
          "ImageUrl": "마리엘/3/1.webp"
        },
        {
          "ImageUrl": "마리엘/4/1.webp"
        }
      ],
      "AttkRange": 1,
      "Move": 3,
      "MoveType": "Walk",
      "HP": 4404,
      "HPRate": 76,
      "Attk": 547,
      "AttkRate": 78,
      "Int": 290,
      "IntRate": 47,
      "Def": 351,
      "DefRate": 82,
      "MagicDef": 339,
      "MagicDefRate": 80,
      "Skill": 138,
      "SkillRate": 37,
      "HeartFetterBouns1": "<b>자신보다 생명 백분비 높은 적에게 공격 받을시</b>,<br/>받는 피해량 <font color=\"red\">-10</font>%",
      "HeartFetterBouns2": "치유량 <font color=\"red\">+10</font>%"
    }
  ],
  "Materials": {
    "HeartFetter": [
      "h4",
      "h1",
      "h3",
      "h2",
      "h4-1",
      "h1-1",
      "h3-1",
      "h2-1",
      "x"
    ],
    "Fetter": [
      "3",
      "3-1",
      "3-2",
      "3-3",
      "3-4"
    ],
    "Awaken": [
      "ranka1",
      "ranka3",
      "ranka1-1",
      "ranka3-1"
    ]
  },
  "ClassUp": [
    {
      "ClassTitle": "성직자 : 생명력+36, 공격+9",
      "Tiers": [
        {
          "Name": "T1",
          "Materials": [
            "rank1-4"
          ]
        },
        {
          "Name": "T2",
          "Materials": [
            "rank1-4"
          ]
        }
      ]
    },
    {
      "ClassTitle": "마터 : 마방+8, 지력+7",
      "Tiers": [
        {
          "Name": "T1",
          "Materials": [
            "rank2-5",
            "rank1-5",
            "rank1-4"
          ]
        },
        {
          "Name": "T2",
          "Materials": [
            "rank3-4",
            "rank3-5",
            "rank2-5",
            "rank2-4"
          ]
        },
        {
          "Name": "T3",
          "Materials": [
            "rank4-5",
            "rank4-4",
            "rank3-4",
            "rank3-5"
          ]
        }
      ]
    },
    {
      "ClassTitle": "신앙인 : 공격+12, 방어+5",
      "Tiers": [
        {
          "Name": "T1",
          "Materials": [
            "rank2-5",
            "rank1-5",
            "rank1-4"
          ]
        },
        {
          "Name": "T2",
          "Materials": [
            "rank3-4",
            "rank3-5",
            "rank2-5",
            "rank2-4"
          ]
        },
        {
          "Name": "T3",
          "Materials": [
            "rank4-5",
            "rank4-4",
            "rank3-4",
            "rank3-5"
          ]
        }
      ]
    },
    {
      "ClassTitle": "인퀴지터 : 공격+25, 방어+10, 마방+10",
      "Tiers": [
        {
          "Name": "T1",
          "Materials": [
            "rank5-4",
            "rank5-5",
            "rank4-4",
            "rank4-5"
          ]
        },
        {
          "Name": "T2",
          "Materials": [
            "rank6-7",
            "rank6-8",
            "rank5-4",
            "rank5-5"
          ]
        },
        {
          "Name": "T3",
          "Materials": [
            "rank7-5",
            "rank7-6",
            "rank6-7",
            "rank6-8"
          ]
        },
        {
          "Name": "T4",
          "Materials": [
            "rank8-4",
            "rank8-6",
            "rank7-5",
            "rank7-6"
          ]
        }
      ]
    },
    {
      "ClassTitle": "홀리 메이지 : 생명+80, 공격+7",
      "Tiers": [
        {
          "Name": "T1",
          "Materials": [
            "rank2-5",
            "rank1-5",
            "rank1-4"
          ]
        },
        {
          "Name": "T2",
          "Materials": [
            "rank3-4",
            "rank3-5",
            "rank2-5",
            "rank2-4"
          ]
        },
        {
          "Name": "T3",
          "Materials": [
            "rank4-5",
            "rank4-4",
            "rank3-4",
            "rank3-5"
          ]
        }
      ]
    },
    {
      "ClassTitle": "홀리 프리스트 : 공격+25, 방어+10, 마방+10",
      "Tiers": [
        {
          "Name": "T1",
          "Materials": [
            "rank5-4",
            "rank5-5",
            "rank4-4",
            "rank4-5"
          ]
        },
        {
          "Name": "T2",
          "Materials": [
            "rank6-7",
            "rank6-8",
            "rank5-4",
            "rank5-5"
          ]
        },
        {
          "Name": "T3",
          "Materials": [
            "rank7-5",
            "rank7-6",
            "rank6-7",
            "rank6-8"
          ]
        },
        {
          "Name": "T4",
          "Materials": [
            "rank8-4",
            "rank8-6",
            "rank7-5",
            "rank7-6"
          ]
        }
      ]
    }
  ],
  "SPOpenConditions": null,
  "Skills": [
    {
      "Name": "헤비 어택",
      "Cost": "COST 2",
      "Entries": [
        {
          "Label": "유형",
          "Value": "물리 피해"
        },
        {
          "Label": "쿨타임",
          "Value": "3턴"
        },
        {
          "Label": "사거리",
          "Value": "1칸"
        },
        {
          "Label": "범위",
          "Value": "단일"
        }
      ],
      "IconImageUrl": "../img/skills/Skill_Thump.webp",
      "Desc": "[물리 피해]단일 적 부대를 공격해 <font color =\"red\">1.2</font>배의 공격 피해를 준다. <br/><b>자신이 방어 지형에 있을 시 전투 전</b> 대상에게 <font color =\"blue\">기절</font> 시전<br/><font color =\"red\">1</font>행동 지속.",
      "SubSkills": null,
      "Animation": null
    },
    {
      "Name": "진격",
      "Cost": "COST 1",
      "Entries": [
        {
          "Label": "유형",
          "Value": "패시브"
        }
      ],
      "IconImageUrl": "../img/skills/Passive_AtkBuf2.webp",
      "Desc": "[패시브]<b>전투 진입 시 자신의 부대 공격력</b> <font color=\"red\">+7</font>%.",
      "SubSkills": null,
      "Animation": null
    },
    {
      "Name": "공격 지휘",
      "Cost": "COST 2",
      "Entries": [
        {
          "Label": "유형",
          "Value": "지휘,패시브"
        }
      ],
      "IconImageUrl": "../img/skills/2.webp",
      "Desc": "[지휘]주위 <font color=\"red\">2</font>칸 내 모든 아군 공격, 지력 <font color=\"red\">+10</font>%",
      "SubSkills": null,
      "Animation": null
    },
    {
      "Name": "광역 강화",
      "Cost": "COST 2",
      "Entries": [
        {
          "Label": "유형",
          "Value": "지원"
        },
        {
          "Label": "쿨타임",
          "Value": "2턴"
        },
        {
          "Label": "사거리",
          "Value": "3칸"
        },
        {
          "Label": "범위",
          "Value": "3칸"
        }
      ],
      "IconImageUrl": "../img/skills/Skill_MassATKup.webp",
      "Desc": "[지원]액티브 스킬, 범위 내 아군들의 공격, 지력 <font color=\"red\">+20</font>%.<br/>그리고 '공격, 지력 감소','침묵' 면역 부여, <font color=\"red\">2</font>행동 지속.",
      "SubSkills": null,
      "Animation": null
    },
    {
      "Name": "공격 대지원",
      "Cost": "COST 2",
      "Entries": [
        {
          "Label": "유형",
          "Value": "패시브"
        }
      ],
      "IconImageUrl": "../img/skills/Skill_Again.webp",
      "Desc": "[패시브]행동 종료 시 주위 <font color=\"red\">2칸</font> 내 <font color=\"red\">2개</font> 다른 아군 부대가 주는 피해량 <font color=\"red\">+15%</font> 및 <br/>면역: '공격, 지력 감소', '침묵', <font color=\"red\">1턴</font> 지속.",
      "SubSkills": null,
      "Animation": null
    },
    {
      "Name": "헌신",
      "Cost": "COST 1",
      "Entries": [
        {
          "Label": "유형",
          "Value": "패시브"
        }
      ],
      "IconImageUrl": "../img/skills/Skill_Dedication.webp",
      "Desc": "[패시브] 전투 후 <font color=\"red\">40%</font>의 확률로 다른 아군 부대의 생명을 회복시킨다.<br/>회복량은 부대 최대 생명의 <font color=\"red\">15%</font>.",
      "SubSkills": null,
      "Animation": null
    },
    {
      "Name": "구원",
      "Cost": "COST 1",
      "Entries": [
        {
          "Label": "유형",
          "Value": "치료"
        },
        {
          "Label": "쿨타임",
          "Value": "-"
        },
        {
          "Label": "사거리",
          "Value": "1칸"
        },
        {
          "Label": "범위",
          "Value": "단일"
        }
      ],
      "IconImageUrl": "../img/skills/Mariel/1.webp",
      "Desc": "[치료] 액티브 스킬. 단일 개체 생명 회복, <br/>회복량은 술자 공격력 <font color=\"red\">3.5배</font> 수치, 디버프 <font color=\"red\">2</font>개 제거.",
      "SubSkills": null,
      "Animation": null
    },
    {
      "Name": "기원",
      "Cost": "COST 1",
      "Entries": [
        {
          "Label": "유형",
          "Value": "지원"
        },
        {
          "Label": "쿨타임",
          "Value": "3턴"
        },
        {
          "Label": "사거리",
          "Value": "자신"
        },
        {
          "Label": "범위",
          "Value": "단일"
        }
      ],
      "IconImageUrl": "../img/skills/Mariel/2.webp",
      "Desc": "[지원]액티브 스킬, 자신에게 걸린 디버프 <font color=\"red\">2</font>개 제거 및 <br/>부대 생명 <font color=\"red\">30</font>% 회복, 사용 후 재행동 <font color=\"red\">2</font>칸.",
      "SubSkills": null,
      "Animation": null
    },
    {
      "Name": "성왕의 비호",
      "Cost": "COST 2",
      "Entries": [
        {
          "Label": "유형",
          "Value": "지원"
        },
        {
          "Label": "쿨타임",
          "Value": "3턴"
        },
        {
          "Label": "사거리",
          "Value": "2칸"
        },
        {
          "Label": "범위",
          "Value": "3칸"
        }
      ],
      "IconImageUrl": "../img/skills/Mariel/3.webp",
      "Desc": "[지원]액티브. <b>범위 내 아군</b> 생명 회복, 회복량은 술자 공격력의 <font color=\"red\">2.5</font>배 수치, <br/><b>디버프 <font color=\"red\">1</font>개 제거 및 <font color=\"red\">2</font>개의 무작위 <font color=\"blue\">면역</font> 효과 부여</b>지속 <font color=\"red\">2</font>행동.<br/><b>자신에게 걸린 [<font color=\"purple\">계율</font>] 효과가 사라진 후, 해당 스킬은 [<font color=\"blue\">성왕의 맹렬한 공격</font>]으로 변경</b>",
      "SubSkills": [
      {
          "Name": "성왕의 맹렬한 공격",
          "Cost": null,
          "Entries": [
            {
              "Label": "유형",
              "Value": "물리 피해"
            },
            {
              "Label": "쿨타임",
              "Value": "3턴"
            },
            {
              "Label": "사거리",
              "Value": "1칸"
            },
            {
              "Label": "범위",
              "Value": "단일"
            }
          ],
          "IconImageUrl": "../img/skills/Mariel/4.webp",
          "Desc": "[물리 피해] 단일 적 부대를 공격해 <font color=\"red\">1.4</font>배 피해, <br/>적을 위치 이동 시킴 : 목표를 <font color=\"red\">3</font>칸 밀침, <br/>전투 후 피해량 <font color=\"red\">50</font>% 수치로 생명 회복, 재이동 <font color=\"red\">2</font>칸",
          "SubSkills": null,
          "Animation": null
        }
      ],
      "Animation": null
    },
    {
      "Name": "성스러운 분노의 일격",
      "Cost": "COST 3",
      "Entries": [
        {
          "Label": "유형",
          "Value": "지원"
        },
        {
          "Label": "쿨타임",
          "Value": "7턴"
        },
        {
          "Label": "사거리",
          "Value": "1칸"
        },
        {
          "Label": "범위",
          "Value": "단일"
        }
      ],
      "IconImageUrl": "../img/Super/마리엘.webp",
      "Desc": "[패시브]자신이 [<font color=\"blue\">계율</font>] 효과를 보유 하지 않을 경우, 이동력 <font color=\"red\">+3</font>. <br/>자신이 [<font color=\"blue\">계율</font>] 효과를 보유하고 있을 경우 <br/><b>행동 종료 시 생명력이 가장 낮은 아군 <font color=\"red\">2</font>명 <font color=\"blue\">생명 회복</font></b>, <br/>회복량은 마리엘 공격력의 <font color=\"red\">1.5</font>배 수치<br/>[물리피해] 단일 개체 <font color=\"red\">1.8</font>배 피해, <b>전투 전</b> 적의 <font color=\"blue\">버프</font> <font color=\"red\">3</font>개 제거 및 <br/>적을 <font color=\"blue\">위치 이동</font> 시킴<br/> 적을 <font color=\"red\">3</font>칸 밀침. <br/><b>호위 받는 적을 공격 시 피해 <font color=\"red\">+15</font>% 추가 증가 </b>, <br/>자신은 [<font color=\"blue\">성스러운 분노</font>] 효과 획득 :<br/>[<font color=\"blue\">성스러운 분노</font>]:주는 피해량 <font color=\"red\">+20</font>%, <font color=\"blue\">호위 무시</font> 공격,<br/><b><font color=\"blue\">스킬</font>로 피해를 줬을경우 </b><font color=\"blue\">스킬</font> 쿨타임 <font color=\"red\">-3</font><br/>지속 <font color=\"red\">1</font>행동 <br/><b><font color=\"blue\">성스러운 분노</font> 효과는 면역 및 제거 불가</b>",
      "SubSkills": null,
      "Animation": "../img/Super/마리엘_Ani.webp"
    }
  ],
  "Propose": true,
  "Bio": [
    {
      "Entry": "영웅소개",
      "Desc": "◆키/몸무게: 175cm / 52kg<br/>◆출연: 랑그릿사 리인카네이션 -전생-<br/>◆신체 사이즈: 96/60/94<br/>◆소속: 빛의 군단"
    },
    {
      "Entry": "영웅담1",
      "Desc": "명문 살라스 가문의 장녀. 루시리스 교에서 가장 성실한 사제이며, 빛의 군단을 지키는 것을 자신의 책무로 삼고 있다. <br/>선조가 창립한 '살라스 도서실'도 운영하고 있다. 탄생의 빛에게는 빼놓을 수 없는 소중한 파트너이며, 항상 온화한 분위기를 풍기고 있으나, <br/>때때로 폭주하는 일면도 있다. 그럴 때마다 손에 든 메이스는 적을 위협하기에 충분한 위력을 발휘한다.<br/><font color=\"#DC143C\"><size=33><i>「저는 살라스 가문의 장녀이며, 교회 도서실을 담당하는 사제이기도 하며, <br/>베스트셀러 작가이기도 하지만, 빛의 여신을 믿는 신자이기도 합니다. <br/>하지만 애석하게도 제가 수녀라는 사실을 아는 사람은 거의 없네요.」</i></size=33></font>"
    },
    {
      "Entry": "영웅담2",
      "Desc": "귀족의 몸으로 태어나 국가의 쇠퇴에 따라 가족을 잃었지만, 생활에 부족함이 있었던 적은 없었다. <br/>가문의 명예는 잃었지만 마리엘은 선조의 책무를 잊지 않고 있다. 지금도 조용히 빛의 군단을 뒷바라지하며, <br/>그 현명함으로 탄생의 빛을 보좌할 뿐 아니라, 루시리스 교 또한 착실히 지키고 있다. <br/>빛의 군단에 경제적인 면에서도 조력하고 있다.<br/><font color=\"#DC143C\"><size=33><i>「어떤 세상에서도 제시카 님은 제가 가장 마음을 터놓을 수 있는 친구입니다. <br/>사령관 님이라고 해도 방해는 하지 말아줬으면 해요.」</i></size=33></font>"
    },
    {
      "Entry": "영웅담3",
      "Desc": "중대한 책임을 짊어지고 있으면서도, 마리엘은 자신의 꿈을 포기하지 않고 <br/>여가 시간에 익명으로 연애소설을 집필하고 있다. 본래 반듯한 생활을 보내고 있던 마리엘에게 있어서 소설 집필은 <br/>처음으로 자신의 다른 일면을 보여줄 수 있는 기회이기도 했다. 의외로 많은 애독자를 얻었고, <br/>이는 그녀가 소설을 계속 집필할 수 있는 원동력이 되었다."
    },
    {
      "Entry": "영웅담4",
      "Desc": "그러면서 마리엘은 점차 자신의 감정을 표현하는 법을 배웠다. 그녀는 이야기를 쓰는 것을 좋아해, <br/>항상 메모 노트를 가지고 다니며 아이디어가 떠오를 때마다 적어두고 있다. <br/>언젠가 사람들이 모두 놀랄 만한, 온 세상에 이름을 떨칠 작품을 집필하는 것을 목표로 삼고, 노력을 아끼지 않고 있다.<br/><font color=\"#DC143C\"><size=33><i>「책이란, 사람의 마음속 깊은 곳에 숨겨둔 미묘한 감정을 증폭시키는 것입니다. <br/>행복한 사람은 더 행복을 느끼고, 괴로운 사람은 더욱 괴로워지고 마는 것이지요.」</i></size=33></font>"
    },
    {
      "Entry": "영웅담5",
      "Desc": "새로운 대륙으로 온 마리엘은 아직도 계속해서 과거의 꿈을 좇고 있다. <br/>이 대륙에 찾아온 이유는 그녀를 옭아맸던 모든 속박에서 벗어나기 위함이기도 했다. <br/>예전부터 더욱 자유분방한 성격이 된 그녀는, 엘사리아에서 도서관을 경영하면서 자유롭게 창작 활동을 펼치고 있다.<br/><font color=\"#DC143C\"><size=33><i>「전 엘사리아 대륙이 너무 좋아요. 언젠가 영웅 이야기도 써보고 싶네요. <br/>주제는...... 뭐가 좋을까요?」</i> </size=33></font>"
    }
  ],
  "Performance": [
    {
      "Entry": "「소환 - 등장대사」",
      "Desc": "처음 뵙겠습니다, 저는 마리엘입니다. 당신의 존함을 여쭈어봐도 괜찮을지요?"
    },
    {
      "Entry": "「전투 - 전직1」",
      "Desc": "이 안경이 어쩌면 저의 비밀병기일지도 모른다고 생각해보신 적, 없으신가요?"
    },
    {
      "Entry": "「전투 - 전직2」",
      "Desc": "더 해주세요....... 점점 좋아지기 시작했어요. 사령관 님."
    },
    {
      "Entry": "「전투 - 스킬시전1」",
      "Desc": "이길지도 모르겠네요."
    },
    {
      "Entry": "「전투 - 스킬시전2」",
      "Desc": "승리를 위해, 여신님의 이름으로."
    },
    {
      "Entry": "「전투 - 선택1」",
      "Desc": "선택받아 버린 건가요?"
    },
    {
      "Entry": "「전투 - 선택2」",
      "Desc": "잠깐 좀 도와주시겠어요? 사령관 님."
    },
    {
      "Entry": "「전투 - 전투불능」",
      "Desc": "어머....... '벌'이란 건, 겨우 이 정도인가요?"
    },
    {
      "Entry": "「대화 - 친밀도1」",
      "Desc": "어떤 세상에서도 제시카 님은 제가 가장 마음을 터놓을 수 있는 친구입니다. <br/>사령관 님이라고 해도 방해는 하지 말아줬으면 해요."
    },
    {
      "Entry": "「대화 - 친밀도2」",
      "Desc": "전 엘사리아 대륙이 너무 좋아요. 언젠가 영웅 이야기도 써보고 싶네요. <br/>주제는...... 뭐가 좋을까요?"
    },
    {
      "Entry": "「대화 - 친밀도3」",
      "Desc": "제가 왜 무기로 '메이스'를 고른 건지, 드디어 물어봐주셨네요. <br/>그 이유는 그저 제가 평범한 빛의 수녀이기 때문이랍니다."
    },
    {
      "Entry": "「대화 - 친밀도4」",
      "Desc": "책 또한 제게 있어 소중한 친구이긴 해요. <br/>그래도 밤새 사령관 님께서 제 귀에 속삭여 주시는 것을...... 더 원하는 저도 있네요."
    },
    {
      "Entry": "「대화 - 친밀도5」",
      "Desc": "저는 살라스 가문의 장녀이며, 교회 도서실을 담당하는 사제이기도 하며, <br/>베스트셀러 작가이기도 하고, 빛의 여신을 믿는 신자이기도 하지만, <br/>제가 수녀라는 사실을 아는 사람은 거의 없네요."
    },
    {
      "Entry": "「대화 - 일상1」",
      "Desc": "책이란, 사람의 마음속 깊은 곳에 숨겨둔 미묘한 감정을 증폭시키는 것입니다. <br/>행복한 사람은 더 행복을 느끼고, 괴로운 사람은 더욱 괴로워지고 마는 것이지요."
    },
    {
      "Entry": "「대화 - 기쁨」",
      "Desc": "익명으로 소설을 쓰는 것은 제게 있어서 그렇게 대담한 도전은 아닙니다. <br/>사실은 저, 이렇게 사람이 혼란스러워하는 모습을 보는 걸 좋아한답니다."
    },
    {
      "Entry": "「대화 - 분노」",
      "Desc": "제 앞에서 '독서 좋아하는 여자는 지루해'같은 말은 하지 않는 게 신상에 이로울걸요? <br/>수녀를 화나게 하면 무섭다는 걸 모르시는 모양이네요."
    },
    {
      "Entry": "「대화 - 슬픔」",
      "Desc": "제시카 님의 기분을 상하게 하고 싶지는 않지만, 제가 좋아하는 건 루시리스 교의 가르침을 거스르는 것들 뿐이에요....... <br/>이 둘을 병행하려면 어떻게 해야 할까요?"
    },
    {
      "Entry": "「대화 - 수줍음」",
      "Desc": "우후훗, 사령관 님과 도서실에서 단 둘이 있다니....... <br/>두근거리네요. 뭔가 설레는 일이 일어날 것만 같아요......."
    },
    {
      "Entry": "「상호작용 - 대사1」",
      "Desc": "저의 이 마음, 예상하지 못하셨나 봐요? <br/>괜찮아요, 긴장하지 마세요. 사령관 '님'......."
    },
    {
      "Entry": "「상호작용 - 대사2」",
      "Desc": "후아암....... 어머 사령관 님, 단정치 못한 모습 보여드려 죄송합니다. <br/>하지만, 창작에는 약간 이렇게 반쯤 꿈에 취해있는 상태가 딱 좋답니다."
    },
    {
      "Entry": "「상호작용 - 대사3」",
      "Desc": "청소 말고 싫어하는 건 뭐냐구요? 글쎄요....... 운동도 질색이에요. <br/>하지만 사령관 님께서 권유해주신다면, 해봐도 나쁘진 않을 것 같네요."
    },
    {
      "Entry": "「상호작용 - 호감1」",
      "Desc": "으음~ ......오늘은 사령관 님께 어떤 책을 추천해드리면 좋을까? <br/>그런 거 말고, 안 보이는 데서 단 둘이 몰래 즐길 수 있는 것 같은 건...... 어떠세요?"
    },
    {
      "Entry": "「상호작용 - 반감1」",
      "Desc": "실례지만, 점심 휴식 시간이라서요."
    },
    {
      "Entry": "「상호작용 - 특수1」",
      "Desc": "다음엔 좀 더 재밌는 걸 하지 않을래요?"
    }
  ],
  "Performance2": null,  
  "Misc": ""
}];