var data = [
  {
    "Skins": [
      {
        "ImageUrl": "레인폴스/1.webp",
        "Desc": "기본"
      },
      {
        "ImageUrl": "레인폴스/2.webp",
        "Desc": "빛의 메아리"
      },
      {
        "ImageUrl": "레인폴스/3.webp",
        "Desc": "형귀 스킨"
      }
    ],
    "Name": "레인폴스",
    "ChName": "雷因法鲁斯",
    "EngName": "Rainforce",
    "JapName": "レインフォルス",
    "CV": "하야미 쇼",
    "Rank": [
      0
    ],
    "SP": false,
    "Groups": [
      "전략",
      "전설",
      "인외"
    ],
    "HP": 20,
    "SPHP": 0,
    "Attk": 20,
    "SPAttk": 0,
    "Def": 35,
    "SPDef": 0,
    "MagicDef": 25,
    "SPMagicDef": 0,
    "HowToGet": "영웅 소환",
    "Fetters": [
      {
        "Condition": "시공 3-6 일반"
      },
      {
        "Condition": "크림조니아왕자 전직"
      },
      {
        "Condition": "기자로프와 함께 운명의문4"
      },
      {
        "Condition": "시그마와 함께 운명의문5"
      }
    ],
    "HeartFetter": "생명+650, 방어+50, 마방+60",
    "Talent": [
      {
        "TalentName": "크림조니아의 힘",
        "Icon": "80.webp",
        "Abilities": [
          {
            "Desc": "1성"
          },
          {
            "Desc": "2성"
          },
          {
            "Desc": "매 적에게 피해를 입힌 후 [크림조니아의 힘] 효과 획득<br/>[크림조니아의 힘] : 생명 이외 모든 능력치 <font color=\"red\">+1%</font> 중첩가능. 최대 <font color=\"red\">7</font>회 중첩<br/>행동 종료시 [크림조니아의 힘] 중첩이 <font color=\"red\">7</font>개일 경우 재행동 <font color=\"red\">1</font>회 가능.<br/>재행동 종료시 가장 가까운 아군의 곁으로 이동하며 <br/>[크림조니아의 힘] 누적 효과 제거"
          },
          {
            "Desc": "매 적에게 피해를 입힌 후 [크림조니아의 힘] 효과 획득<br/>[크림조니아의 힘] : 생명 이외 모든 능력치 <font color=\"red\">+2%</font> 중첩가능. 최대 <font color=\"red\">7</font>회 중첩<br/>행동 종료시 [크림조니아의 힘] 중첩이 <font color=\"red\">7</font>개일 경우 재행동 <font color=\"red\">1</font>회 가능.<br/>재행동 종료시 가장 가까운 아군의 곁으로 이동하며 <br/>[크림조니아의 힘] 누적 효과 제거"
          },
          {
            "Desc": "매 적에게 피해를 입힌 후 [크림조니아의 힘] 효과 획득<br/>[크림조니아의 힘] : 생명 이외 모든 능력치 <font color=\"red\">+3%</font> 중첩가능. 최대 <font color=\"red\">7</font>회 중첩<br/>행동 종료시 [크림조니아의 힘] 중첩이 <font color=\"red\">7</font>개일 경우 재행동 <font color=\"red\">1</font>회 가능.<br/>재행동 종료시 가장 가까운 아군의 곁으로 이동하며 <br/>[크림조니아의 힘] 누적 효과 제거"
          },
          {
            "Desc": "매 적에게 피해를 입힌 후 [크림조니아의 힘] 효과 획득<br/>[크림조니아의 힘] : 생명 이외 모든 능력치 <font color=\"red\">+4%</font> 중첩가능. 최대 <font color=\"red\">7</font>회 중첩<br/>행동 종료시 [크림조니아의 힘] 중첩이 <font color=\"red\">7</font>개일 경우 재행동 <font color=\"red\">1</font>회 가능.<br/>재행동 종료시 가장 가까운 아군의 곁으로 이동하며 <br/>[크림조니아의 힘] 누적 효과 제거"
          }
        ]
      }
    ],
    "SPTalent": null,
    "Second_Awaken": {
      "Name": "잔월의 유지",
      "Stats": "공격 <font color=\"red\">+5%</font><br/>고유 스킬 <font color=\"purple\">크림조의 힘</font> 효과 제거 시 <font color=\"red\">2</font>개를 보류 시킴"
    },
    "Equip": {
      "EqpName": "크림존의 심장(장신구)",
      "Stats": "공격 75,기술 37",
      "Icon": "레인폴스.webp",
      "Ability": "공격 <font color=\"red\">+5%</font>, 이동력 <font color=\"red\">+1</font><br/>고유 스킬 <font color=\"green\">크림조니아의 힘</font> 중첩이, <br/><font color=\"red\">4</font> 이상일 경우<font color=\"blue\">이동시 적 통과 가능</font>, <br/><font color=\"red\">7</font> 이상일 경우 피해를 <b>입힌 후</b> 적에게 <font color=\"blue\">강력한 디버프</font><font color=\"red\">1</font>개 부여,"
    },
    "Soldiers": [
      "퇴마사",
      "사무라이",
      "밴디트",
      "화염 저격수",
      "하이엘프",
      "광전사",
      "중장 보병",
      "개조인간",
      "사막용병"
    ],
    "Jobs": [
      {
        "Name": "크림조니아 왕자",
        "Icon": "암살자",
        "SD": [
          {
            "ImageUrl": "레인폴스/1/1.webp"
          },
          {
            "ImageUrl": "레인폴스/2/1.webp"
          },
          {
            "ImageUrl": "레인폴스/3/1.webp"
          }
        ],
        "AttkRange": 2,
        "Move": 3,
        "MoveType": "보행",
        "HP": 4056,
        "HPRate": 68,
        "Attk": 530,
        "AttkRate": 82,
        "Int": 294,
        "IntRate": 49,
        "Def": 293,
        "DefRate": 65,
        "MagicDef": 273,
        "MagicDefRate": 61,
        "Skill": 316,
        "SkillRate": 90,
        "HeartFetterBouns1": "생명이 80%이상일때 치명타공격을 받을 확률 -10%",
        "HeartFetterBouns2": "생명이 80%이상일때 치명타확률 +10%"
      },
      {
        "Name": "검성",
        "Icon": "보병",
        "SD": [
          {
            "ImageUrl": "레인폴스/1/2.webp"
          },
          {
            "ImageUrl": "레인폴스/2/2.webp"
          },
          {
            "ImageUrl": "레인폴스/3/1.webp"
          }
        ],
        "AttkRange": 1,
        "Move": 3,
        "MoveType": "보행",
        "HP": 4370,
        "HPRate": 73,
        "Attk": 588,
        "AttkRate": 90,
        "Int": 294,
        "IntRate": 49,
        "Def": 305,
        "DefRate": 68,
        "MagicDef": 293,
        "MagicDefRate": 65,
        "Skill": 273,
        "SkillRate": 78,
        "HeartFetterBouns1": "원거리 공격을 받아 전투진입시 받는 마법피해 -10%",
        "HeartFetterBouns2": "생명이 80%이상일때 치명타피해 +10%"
      }
    ],
    "Materials": {
      "HeartFetter": [
        "h2",
        "h3",
        "h1",
        "h4",
        "h2-1",
        "h3-1",
        "h1-1",
        "h4-1",
        "x"
      ],
      "Fetter": [
        "4",
        "4-1",
        "4-2",
        "4-3",
        "4-4"
      ],
      "Awaken": [
        "ranka4",
        "ranka5",
        "ranka4-1",
        "ranka5-1"
      ]
    },
    "ClassUp": [
      {
        "ClassTitle": "크림조니아낭인 : 공격+5, 기술+6",
        "Tiers": [
          {
            "Name": "T1",
            "Materials": [
              "rank1-2"
            ]
          },
          {
            "Name": "T2",
            "Materials": [
              "rank1-2"
            ]
          }
        ]
      },
      {
        "ClassTitle": "스나이퍼 : 공격+12, 생명+48",
        "Tiers": [
          {
            "Name": "T1",
            "Materials": [
              "rank2-6",
              "rank1-2",
              "rank1-5"
            ]
          },
          {
            "Name": "T2",
            "Materials": [
              "rank3-2",
              "rank3-1",
              "rank2-2",
              "rank2-6"
            ]
          },
          {
            "Name": "T3",
            "Materials": [
              "rank4-3",
              "rank4-1",
              "rank3-2",
              "rank3-1"
            ]
          }
        ]
      },
      {
        "ClassTitle": "크림조니아귀족 : 공격+7, 기술+8",
        "Tiers": [
          {
            "Name": "T1",
            "Materials": [
              "rank2-6",
              "rank1-2",
              "rank1-1"
            ]
          },
          {
            "Name": "T2",
            "Materials": [
              "rank3-2",
              "rank3-1",
              "rank2-3",
              "rank2-6"
            ]
          },
          {
            "Name": "T3",
            "Materials": [
              "rank4-3",
              "rank4-1",
              "rank3-2",
              "rank3-1"
            ]
          }
        ]
      },
      {
        "ClassTitle": "크림조니아왕자 : 생명+90, 공격+15, 기술+16",
        "Tiers": [
          {
            "Name": "T1",
            "Materials": [
              "rank5-2",
              "rank5-1",
              "rank4-3",
              "rank4-1"
            ]
          },
          {
            "Name": "T2",
            "Materials": [
              "rank6-4",
              "rank6-3",
              "rank5-2",
              "rank5-1"
            ]
          },
          {
            "Name": "T3",
            "Materials": [
              "rank7-3",
              "rank7-6",
              "rank6-4",
              "rank6-3"
            ]
          },
          {
            "Name": "T4",
            "Materials": [
              "rank8-8",
              "rank8-5",
              "rank7-3",
              "rank7-6"
            ]
          }
        ]
      },
      {
        "ClassTitle": "소드맨 : 생명+80, 기술+5",
        "Tiers": [
          {
            "Name": "T1",
            "Materials": [
              "rank2-1",
              "rank1-1",
              "rank1-2"
            ]
          },
          {
            "Name": "T2",
            "Materials": [
              "rank3-1",
              "rank3-6",
              "rank2-1",
              "rank2-2"
            ]
          },
          {
            "Name": "T3",
            "Materials": [
              "rank4-1",
              "rank4-2",
              "rank3-1",
              "rank3-6"
            ]
          }
        ]
      },
      {
        "ClassTitle": "검성 : 생명+90, 공격+25, 방어+10",
        "Tiers": [
          {
            "Name": "T1",
            "Materials": [
              "rank5-1",
              "rank5-2",
              "rank4-1",
              "rank4-3"
            ]
          },
          {
            "Name": "T2",
            "Materials": [
              "rank6-1",
              "rank6-3",
              "rank5-1",
              "rank5-2"
            ]
          },
          {
            "Name": "T3",
            "Materials": [
              "rank7-2",
              "rank7-4",
              "rank6-1",
              "rank6-3"
            ]
          },
          {
            "Name": "T4",
            "Materials": [
              "rank8-1",
              "rank8-5",
              "rank7-2",
              "rank7-4"
            ]
          }
        ]
      }
    ],
    "SPOpenConditions": null,
    "Skills": [
      {
        "Name": "음속검",
        "Cost": "COST 1",
        "Entries": [
          {
            "Label": "유형",
            "Value": "물리피해"
          },
          {
            "Label": "쿨타임",
            "Value": "1턴"
          },
          {
            "Label": "사거리",
            "Value": "6칸"
          },
          {
            "Label": "범위",
            "Value": "직선"
          }
        ],
        "IconImageUrl": "../img/skills/Skill_Lainforce1.webp",
        "Desc": "[물리 피해] 라인에 있는 모든 적을 관통하며 0.18배 범위 피해를 입힌다.<br/>치명타 발생시 스킬 쿨타임 -1턴.",
        "SubSkills": null,
        "Animation": null
      },
      {
        "Name": "트릭스터",
        "Cost": "COST 1",
        "Entries": [
          {
            "Label": "유형",
            "Value": "패시브"
          }
        ],
        "IconImageUrl": "../img/skills/Skill_TrickStep.webp",
        "Desc": "[패시브]치명타 확률+10%.<br/>전투 중 적 부대를 격퇴할 시 3칸 재이동 가능.",
        "SubSkills": null,
        "Animation": null
      },
      {
        "Name": "바람 칼날",
        "Cost": "COST 2",
        "Entries": [
          {
            "Label": "유형",
            "Value": "물리피해"
          },
          {
            "Label": "쿨타임",
            "Value": "2턴"
          },
          {
            "Label": "사거리",
            "Value": "2칸"
          },
          {
            "Label": "범위",
            "Value": "단일"
          }
        ],
        "IconImageUrl": "../img/skills/0.webp",
        "Desc": "[물리 피해]단일 적 부대를 공격해 1.5배의 피해를 준다.<br/>근접전 병사를 이끌고 있다면 병사도 함께 공격한다.",
        "SubSkills": null,
        "Animation": null
      },
      {
        "Name": "치명",
        "Cost": "COST 1",
        "Entries": [
          {
            "Label": "유형",
            "Value": "패시브"
          }
        ],
        "IconImageUrl": "../img/skills/Passive_CriBuf.webp",
        "Desc": "[패시브]공격하여 전투 진입 시 공격 <font color=\"red\">+12%</font>.",
        "SubSkills": null,
        "Animation": null
      },
      {
        "Name": "월식",
        "Cost": "COST 2",
        "Entries": [
          {
            "Label": "유형",
            "Value": "물리피해"
          },
          {
            "Label": "쿨타임",
            "Value": "5턴"
          },
          {
            "Label": "사거리",
            "Value": "4칸"
          },
          {
            "Label": "범위",
            "Value": "2칸"
          }
        ],
        "IconImageUrl": "../img/skills/Skill_Lainforce3.webp",
        "Desc": "[물리 피해] 스킬 사용 지점으로 순간이동 하며 <br/>주위 2칸 범위의 모든 적에게 0.3배의 피해 및 강화버프를 1개 강탈한다.<br/>치명타 발생시 강력한 디버프 1개 부여.",
        "SubSkills": null,
        "Animation": null
      },
      {
        "Name": "무자비",
        "Cost": "COST 2",
        "Entries": [
          {
            "Label": "유형",
            "Value": "패시브"
          }
        ],
        "IconImageUrl": "../img/skills/Passive_Ass3.webp",
        "Desc": "[패시브]치명타 피해량 +20%. 전투 중 치명타가 발생한 경우<br/>전투 종료 후 받는 모든 피해량 -30%, 1턴 지속.",
        "SubSkills": null,
        "Animation": null
      },
      {
        "Name": "전술 지휘",
        "Cost": "COST 2",
        "Entries": [
          {
            "Label": "유형",
            "Value": "패시브"
          }
        ],
        "IconImageUrl": "../img/skills/Skill_DexCommand.webp",
        "Desc": "[패시브] 주위 3칸 내 아군 부대가 전투 시 기술 +20%.",
        "SubSkills": null,
        "Animation": null
      },
      {
        "Name": "별 가르기",
        "Cost": "COST 2",
        "Entries": [
          {
            "Label": "유형",
            "Value": "물리피해"
          },
          {
            "Label": "쿨타임",
            "Value": "3턴"
          },
          {
            "Label": "사거리",
            "Value": "7칸"
          },
          {
            "Label": "범위",
            "Value": "직선"
          }
        ],
        "IconImageUrl": "../img/skills/Skill_Lainforce2.webp",
        "Desc": "[물리 피해] 라인에 있는 모든 적을 관통하며 0.33배 범위 피해를 입힌다.<br/>공격 중 스킬의 가장 먼 사거리로 이동하며<br/>2명 이상의 적에게 적중 시 적중당한 적의 위치를 랜덤하게 변경한다.",
        "SubSkills": null,
        "Animation": null
      },
      {
        "Name": "군단 정비",
        "Cost": "COST 1",
        "Entries": [
          {
            "Label": "유형",
            "Value": "패시브"
          }
        ],
        "IconImageUrl": "../img/skills/Passive_BothBuf1.webp",
        "Desc": "[패시브]부대 생명이 90% 이상일 시 공격,방어 +10%.",
        "SubSkills": null,
        "Animation": null
      },
      {
        "Name": "태양",
        "Cost": "COST 3",
        "Entries": [
          {
            "Label": "유형",
            "Value": "물리피해"
          },
          {
            "Label": "쿨타임",
            "Value": "5턴"
          },
          {
            "Label": "사거리",
            "Value": "4칸"
          },
          {
            "Label": "범위",
            "Value": "직선"
          }
        ],
        "IconImageUrl": "../img/Super/레인폴스.webp",
        "Desc": "[물리 피해] 3개의 라인에 있는 모든 적군을 공격하여 0.2배 범위 피해를 입히고,<br/>[연소] 부여: 행동종료시 레인폴스의 공격 1배의 [고정피해],중첩가능, 2턴 지속.<br/>공격시 기술사거리의 가장 먼 위치로 이동한다.<br/>2명 이상 적중시 [전송]발동: 적중한 적군의 위치를 서로바꾸며 [연소]추가부여.<br/>3명 이상 적중시 쿨타임 초기화 및 [연소]추가부여",
        "SubSkills": null,
        "Animation": "../img/Super/레인폴스_Ani.webp"
      }
    ],
    "Propose": false,
    "Bio": [
      {
        "Entry": "영웅소개",
        "Desc": "◆키/몸무게: 195cm /60kg<br/>◆출연: 랑그릿사V<br/>◆소속: 크림조니아"
      },
      {
        "Entry": "영웅담1",
        "Desc": "레인폴스는 '붉은 달 크림조'의 통치 계급의 말예이자, 왕족의 정통한 왕위계승자이다. 크림조에서는 이미 고갈된 마나를 손에 넣기 위해, 그는 지상으로 내려왔다. 목적을 달성하기 위해서라면 수단을 가리지 않는 자이기는 하나, 선악으로 구분지을 수 없는 자이기도 하다.<font color=\"#a52a2aff\"><size=33><i>'그래, 우리 크림조니아는 달에서 온 종족이다. 놀랐나?'</i> </size=33></font>"
      },
      {
        "Entry": "영웅담2",
        "Desc": "한때 자유분방하게 살아왔던 크림조의 왕자는, 왕족으로서 짊어져야 할 책임에서 벗어나기를 원했다. 하지만, 왕족에서 내린 지시로 그는 파사의 검(사실은 인조 병기 페이리아의 기동 장치)의 개발에 착수한다.훗날 크림조랜더라고 불리는 노동 계급이 통치 계급에 반란을 일으켰을 때, 붉은 달은 심각한 피해를 입고 지상을 통치할 힘을 잃게 된다. 진·파사의 검도 카오스의 손에 오염되어 마검 알하자드로 모습을 바꾸게 된다. 결국 레인폴스는 그 싸움의 유일한 생존자가 되어 크림조로 돌아갔다."
      },
      {
        "Entry": "영웅담3",
        "Desc": "반란으로 크나큰 피해를 입은 크림조는 정체 상태에 빠졌으며, 살아남은 대부분의 사람들도 동면 상태에 들어갔다. 그 사이 그들의 생존이 걸린 '마나'도 곧 바닥을 드러냈다.레인폴스는 자신의 별을 구하기 위해 200년에 한 번 오는 기회를 이용하여 지상으로 돌아와, '세계수'를 심어 마나를 모으는 계획을 추진한다. 한때 반란군에 가담했던 브렌다와 사랑에 빠진 것은 그조차 예상하지 못한 일이었다. 하지만 이 관계는 참모 제로에 의해 파국을 맞게 되고, 레인폴스는 종족의 영광을 되찾기 위해 알하자드를 찾는 여정을 떠난다."
      },
      {
        "Entry": "영웅담4",
        "Desc": "그는 연방 내 3개의 세력을 선동하며 마검의 정보를 수집했다. 동시에 세계수의 힘을 이용하기 위해, 세계수와 대화할 수 있는 힘을 지닌 람다를 납치한다.하지만 그의 계획은 알하자드를 뺏어 '푸른 달 페이리아'를 기동시킨 마장군 가이엘의 손에 무너졌으며, 마지막 결전에서는 시그마 일행에게 패배하게 된다. 일련의 사건을 거치며 각오를 다진 레인폴스는 지구의 통치를 포기하고, 시그마와 함께 푸른 달을 파괴한다. 모든 것이 끝나자, 레인폴스는 알하자드를 들고 크림조로 돌아간다."
      },
      {
        "Entry": "영웅담5",
        "Desc": "최근 들어 레인폴스에 관한 소문이 대륙에 퍼지고 있다. 그가 혼돈의 신의 지배를 받는다는 소문도 있는가 하면, 크림조를 이끌고 다시 지구를 통치하려 한다는 소문도 있다. 모든 것은 베일에 싸여있다...<font color=\"#a52a2aff\"><size=33><i>'다음 초승달이 뜰 때 다시 만나자.'</i> </size=33></font>"
      }
    ],
    "Performance": [
      {
        "Entry": "「소환 - 등장대사」",
        "Desc": "설마 이 별의 흙을 다시 밟게 될 줄은... 나는 레인폴스, 크림조니아의 태자다."
      },
      {
        "Entry": "「전투 - 전직1」",
        "Desc": "이 별을 초월한 힘, 똑똑히 보도록 해라."
      },
      {
        "Entry": "「전투 - 전직2」",
        "Desc": "마지막 크림조니아로서 이 정도는 당연하다."
      },
      {
        "Entry": "「전투 - 스킬시전1」",
        "Desc": "전부 쓰러트린다."
      },
      {
        "Entry": "「전투 - 스킬시전2」",
        "Desc": "크림조니아를 위하여!"
      },
      {
        "Entry": "「전투 - 선택1」",
        "Desc": "때가 됐군."
      },
      {
        "Entry": "「전투 - 선택2」",
        "Desc": "시작하지."
      },
      {
        "Entry": "「전투 - 전투불능」",
        "Desc": "여기서 멈출 수는..."
      },
      {
        "Entry": "「대화 - 친밀도1」",
        "Desc": "붉은 달 '크림조', 그것이 접근한다는 것은... 이것은 피할 수 없는 운명의 윤회라는 건가?"
      },
      {
        "Entry": "「대화 - 친밀도2」",
        "Desc": "'크림조'... 나는 어떤 대가를 치르더라도 구해내야 한다! 그것이 나의 사명이다!"
      },
      {
        "Entry": "「대화 - 친밀도3」",
        "Desc": "브렌다, 그녀는 어디서 뭘하고 있을지... 레클리오 마을에서 보내던 나날로는 이제 돌아갈 수 없겠지."
      },
      {
        "Entry": "「대화 - 친밀도4」",
        "Desc": "푸른 달 '페이리아'는 인공적으로 만들어진 천체다. 모든 것을 끝낼 종언의 달이지."
      },
      {
        "Entry": "「대화 - 친밀도5」",
        "Desc": "'크림조'를 구하는 사명을 완수하더라도, 난 다시 통치자로서 군림할 수는 없다... 레클리오 마을에서 그 사실을 배웠지."
      },
      {
        "Entry": "「대화 - 일상1」",
        "Desc": "그래, 우리 크림조니아는 달에서 온 종족이다. 놀랐나?"
      },
      {
        "Entry": "「대화 - 기쁨」",
        "Desc": "지구는 푸르고 아름답다... 두 번 다시 붉은 달의 야망으로 지구를 뒤덮어서는 안 된다."
      },
      {
        "Entry": "「대화 - 분노」",
        "Desc": "크림조니아의 왕을 모욕하지 마라!"
      },
      {
        "Entry": "「대화 - 슬픔」",
        "Desc": "과거는 흘려보내자. 그런 끔찍한 전쟁은 더는 떠올리고 싶지 않다...."
      },
      {
        "Entry": "「상호작용 - 대사1」",
        "Desc": "내게도 동료가 있었다. 제로, 아이젤... 하지만, 분수에 맞지 않는 야망 탓에 전부 잃어버리고 말았다."
      },
      {
        "Entry": "「상호작용 - 대사2」",
        "Desc": "랑그릿사, 그리고 알하자드의 힘... 그 정체는 푸른 달 '페이리아'를 조종하기 위한 것... 보통 사람은 다룰 수 없는 힘이다."
      },
      {
        "Entry": "「상호작용 - 대사3」",
        "Desc": "아이러니하군... 지상인이 지키며 믿어왔던 랑그릿사의 힘은, 더 강대한 힘을 부리기 위한 도구에 불과했었지... 후후, 너라면 이해하겠지."
      },
      {
        "Entry": "「상호작용 - 호감1」",
        "Desc": "함께 해준 것에 감사를 표한다. 그 슬픈 과거를 조금은 잊을 수 있게 됐다...."
      },
      {
        "Entry": "「상호작용 - 반감1」",
        "Desc": "그만둬라."
      },
      {
        "Entry": "「상호작용 - 특수1」",
        "Desc": "다음 초승달이 뜰 때 다시 만나자."
      },
      {
        "Entry": "「상호작용 - 특수2」",
        "Desc": "안녕이다."
      }
    ],
    "Performance2": null,
    "Misc": ""
  }
]