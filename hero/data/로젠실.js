var data = [
  {
    "Skins": [
      {
        "ImageUrl": "로젠실/1.webp",
        "Desc": "기본"
      },
      {
        "ImageUrl": "로젠실/2.webp",
        "Desc": "빛의 메아리"
      },
      {
        "ImageUrl": "로젠실/3.webp",
        "Desc": "서밋아레나 S8 패자스킨"
      },
      {
        "ImageUrl": "로젠실/4.webp",
        "Desc": "형귀뽑기"
      }
    ],
    "Name": "로젠실",
    "ChName": "罗泽希尔",
    "EngName": "Rozenciel",
    "JapName": "ローゼンシル",
    "CV": "타나카 리에",
    "Rank": [
      0
    ],
    "SP": false,
    "Groups": [
      "제국",
      "공주",
      "인외"
    ],
    "HP": 20,
    "SPHP": 0,
    "Attk": 10,
    "SPAttk": 0,
    "Def": 35,
    "SPDef": 0,
    "MagicDef": 35,
    "SPMagicDef": 0,
    "HowToGet": "영웅 소환",
    "Fetters": [
      {
        "Condition": "레벨 30 달성"
      },
      {
        "Condition": "시공 정예 3-1 로젠실 출전하여 클리어"
      },
      {
        "Condition": "란포드의 도움을 받아 운명의문 4"
      },
      {
        "Condition": "클로테르의 도움을 받아 운명의문 5"
      }
    ],
    "HeartFetter": "생명+400, 방어+30, 마방+30",
    "Talent": [
      {
        "TalentName": "수정여왕",
        "Icon": "Gift_Rozenciel.webp",
        "Abilities": [
          {
            "Desc": "1성"
          },
          {
            "Desc": "2성"
          },
          {
            "Desc": "받는 범위 피해 <font color=\"red\">-10%</font><br/><b>아군에게 스킬 사용 시 <font color=\"red\">디버프</font> <font color=\"red\">1</font>개 제거 및 <font color=\"red\">1</font>개의 <font color=\"blue\">수정 장벽</font> 효과 부여:</b> <br/><font color=\"red\">디버프</font> 1개 면역 누적 가능 최대 <font color=\"red\">5</font>중첩 <b>해제 불가</b>. <br/><b>주위 <font color=\"red\">1</font> 칸 안에 있는 모든 아군이 행동 종료 시</b> <br/> 로젠실 지력 <font color=\"red\">1</font> 배 수치 생명 회복 및 <font color=\"blue\">수정 장벽</font> 1개 획득"
          },
          {
            "Desc": "받는 범위 피해 <font color=\"red\">-15%</font><br/><b>아군에게 스킬 사용 시 <font color=\"red\">디버프</font> <font color=\"red\">1</font>개 제거 및 <font color=\"red\">2</font>개의 <font color=\"blue\">수정 장벽</font> 효과 부여:</b> <br/><font color=\"red\">디버프</font> 1개 면역 누적 가능 최대 <font color=\"red\">5</font>중첩 <b>해제 불가</b>. <br/><b>주위 <font color=\"red\">2</font> 칸 안에 있는 모든 아군이 행동 종료 시</b> <br/> 로젠실 지력 <font color=\"red\">1.5</font> 배 수치 생명 회복 및 <font color=\"blue\">수정 장벽</font> 1개 획득"
          },
          {
            "Desc": "받는 범위 피해 <font color=\"red\">-20%</font><br/><b>아군에게 스킬 사용 시 <font color=\"red\">디버프</font> <font color=\"red\">1</font>개 제거 및 <font color=\"red\">2</font>개의 <font color=\"blue\">수정 장벽</font> 효과 부여:</b> <br/><font color=\"red\">디버프</font> 1개 면역 누적 가능 최대 <font color=\"red\">5</font>중첩 <b>해제 불가</b>. <br/><b>주위 <font color=\"red\">2</font> 칸 안에 있는 모든 아군이 행동 종료 시</b> <br/> 로젠실 지력 <font color=\"red\">2</font> 배 수치 생명 회복 및 <font color=\"blue\">수정 장벽</font> 1개 획득"
          },
          {
            "Desc": "받는 범위 피해 <font color=\"red\">-25%</font><br/><b>아군에게 스킬 사용 시 <font color=\"red\">디버프</font> <font color=\"red\">1</font>개 제거 및 <font color=\"red\">3</font>개의 <font color=\"blue\">수정 장벽</font> 효과 부여:</b> <br/><font color=\"red\">디버프</font> 1개 면역 누적 가능 최대 <font color=\"red\">5</font>중첩 <b>해제 불가</b>. <br/><b>주위 <font color=\"red\">3</font> 칸 안에 있는 모든 아군이 행동 종료 시</b> <br/> 로젠실 지력 <font color=\"red\">2.5</font> 배 수치 생명 회복 및 <font color=\"blue\">수정 장벽</font> 1개 획득"
          }
        ]
      }
    ],
    "SPTalent": null,
    "Second_Awaken": null,
    "Equip": {
      "EqpName": "순결한 수정의 면사(투구)",
      "Stats": "생명 364,마방 65",
      "Icon": "로젠실.webp",
      "Ability": "생명 <font color=\"red\">+10%</font><br/>자신이 보유한 <font color=\"purple\">수정 장벽</font> <font color=\"red\">1</font>스택 당 <font color=\"green\">치료 효과</font> <font color=\"red\">+3%</font><br/><b>영웅 사망 시</b> 주위 <font color=\"red\">2</font>칸 범위 안 다른 아군 <font color=\"green\">생명 회복</font><br/>회복량은 로젠실 지력 <font color=\"red\">2</font>배 수치"
    },
    "Soldiers": [
      "마녀",
      "하이엘프",
      "신관",
      "홀리 페가수스",
      "무녀",
      "왕녀 친위대",
      "천공 사수",
      "영종의 투승"
    ],
    "Jobs": [
      {
        "Name": "수정여황",
        "Icon": "마법사",
        "SD": [
          {
            "ImageUrl": "로젠실/1/1.webp"
          },
          {
            "ImageUrl": "로젠실/2/1.webp"
          },
          {
            "ImageUrl": "로젠실/3/1.webp"
          },
          {
            "ImageUrl": "로젠실/4/1.webp"
          }
        ],
        "AttkRange": 2,
        "Move": 3,
        "MoveType": "비행",
        "HP": 3666,
        "HPRate": 81,
        "Attk": 295,
        "AttkRate": 41,
        "Int": 447,
        "IntRate": 79,
        "Def": 278,
        "DefRate": 61,
        "MagicDef": 317,
        "MagicDefRate": 66,
        "Skill": 99,
        "SkillRate": 36,
        "HeartFetterBouns1": "받는 범위피해량 <font color=\"red\">-10%</font>",
        "HeartFetterBouns2": "부대 생명력이 <font color=\"red\">100%</font> 일시<br/> 주는 피해량 <font color=\"red\">+10%</font>"
      },
      {
        "Name": "빛나는 성녀",
        "Icon": "승려",
        "SD": [
          {
            "ImageUrl": "로젠실/1/2.webp"
          },
          {
            "ImageUrl": "로젠실/2/1.webp"
          },
          {
            "ImageUrl": "로젠실/3/1.webp"
          },
          {
            "ImageUrl": "로젠실/4/1.webp"
          }
        ],
        "AttkRange": 2,
        "Move": 3,
        "MoveType": "비행",
        "HP": 3497,
        "HPRate": 81,
        "Attk": 321,
        "AttkRate": 41,
        "Int": 412,
        "IntRate": 75,
        "Def": 278,
        "DefRate": 58,
        "MagicDef": 348,
        "MagicDefRate": 68,
        "Skill": 85,
        "SkillRate": 36,
        "HeartFetterBouns1": "<b>부대 생명력이 <font color=\"red\">80%</font> 이상시</b><br/>받는 치명타 확률<font color=\"red\">-10%</font>",
        "HeartFetterBouns2": "<b>약화효과를 보유한 적군 부대와 전투시</b>주는 피해량 <font color=\"red\">+10%</font>"
      }
    ],
    "Materials": {
      "HeartFetter": [
        "h2",
        "h3",
        "h1",
        "h4",
        "h2-1",
        "h3-1",
        "h1-1",
        "h4-1",
        "x"
      ],
      "Fetter": [
        "4",
        "4-1",
        "4-2",
        "4-3",
        "4-4"
      ],
      "Awaken": [
        "ranka4",
        "ranka5",
        "ranka4-1",
        "ranka5-1"
      ]
    },
    "ClassUp": [
      {
        "ClassTitle": "비법술사 : 마방+4, 지력+9",
        "Tiers": [
          {
            "Name": "T1",
            "Materials": [
              "rank1-5"
            ]
          },
          {
            "Name": "T2",
            "Materials": [
              "rank1-5"
            ]
          }
        ]
      },
      {
        "ClassTitle": "전도사 : 생명+48, 마방+8",
        "Tiers": [
          {
            "Name": "T1",
            "Materials": [
              "rank2-5",
              "rank1-4",
              "rank1-5"
            ]
          },
          {
            "Name": "T2",
            "Materials": [
              "rank3-4",
              "rank3-5",
              "rank2-4",
              "rank2-5"
            ]
          },
          {
            "Name": "T3",
            "Materials": [
              "rank4-4",
              "rank4-5",
              "rank3-4",
              "rank3-5"
            ]
          }
        ]
      },
      {
        "ClassTitle": "빛나는 성녀 : 생명+90, 방어+10, 지력+25",
        "Tiers": [
          {
            "Name": "T1",
            "Materials": [
              "rank5-4",
              "rank5-5",
              "rank4-4",
              "rank4-5"
            ]
          },
          {
            "Name": "T2",
            "Materials": [
              "rank6-8",
              "rank6-7",
              "rank5-4",
              "rank5-5"
            ]
          },
          {
            "Name": "T3",
            "Materials": [
              "rank7-5",
              "rank7-6",
              "rank6-8",
              "rank6-7"
            ]
          },
          {
            "Name": "T4",
            "Materials": [
              "rank8-4",
              "rank8-6",
              "rank7-5",
              "rank7-6"
            ]
          }
        ]
      },
      {
        "ClassTitle": "수정여왕 : 생명+80, 방어+5",
        "Tiers": [
          {
            "Name": "T1",
            "Materials": [
              "rank2-4",
              "rank1-4",
              "rank1-5"
            ]
          },
          {
            "Name": "T2",
            "Materials": [
              "rank3-5",
              "rank3-6",
              "rank2-5",
              "rank2-4"
            ]
          },
          {
            "Name": "T3",
            "Materials": [
              "rank4-4",
              "rank4-5",
              "rank3-5",
              "rank3-6"
            ]
          }
        ]
      },
      {
        "ClassTitle": "수정여황 : 생명+150, 방어+10, 마방+10",
        "Tiers": [
          {
            "Name": "T1",
            "Materials": [
              "rank5-4",
              "rank5-5",
              "rank4-4",
              "rank4-5"
            ]
          },
          {
            "Name": "T2",
            "Materials": [
              "rank6-5",
              "rank6-7",
              "rank5-4",
              "rank5-5"
            ]
          },
          {
            "Name": "T3",
            "Materials": [
              "rank7-5",
              "rank7-6",
              "rank6-5",
              "rank6-7"
            ]
          },
          {
            "Name": "T4",
            "Materials": [
              "rank8-4",
              "rank8-8",
              "rank7-6",
              "rank7-4"
            ]
          }
        ]
      }
    ],
    "SPOpenConditions": null,
    "Skills": [
      {
        "Name": "빙결",
        "Cost": "COST 1",
        "Entries": [
          {
            "Label": "유형",
            "Value": "마법 피해"
          },
          {
            "Label": "쿨타임",
            "Value": "1턴"
          },
          {
            "Label": "사거리",
            "Value": "2칸"
          },
          {
            "Label": "범위",
            "Value": "단일"
          }
        ],
        "IconImageUrl": "../img/skills/Skill_IceBolt.webp",
        "Desc": "[마법 피해]단일 적 부대를 공격해 1.5배의 피해를 준다<br/>'창병'에게 더 큰 피해를 준다.",
        "SubSkills": null,
        "Animation": null
      },
      {
        "Name": "방어막 파괴",
        "Cost": "COST 1",
        "Entries": [
          {
            "Label": "유형",
            "Value": "패시브"
          }
        ],
        "IconImageUrl": "../img/skills/Passive_BreakMDef.webp",
        "Desc": "[패시브]전투 진입전 50% 확률로<br/>적 마방 -25%, 지속 1행동",
        "SubSkills": null,
        "Animation": null
      },
      {
        "Name": "방어 대지원",
        "Cost": "COST 2",
        "Entries": [
          {
            "Label": "유형",
            "Value": "패시브"
          }
        ],
        "IconImageUrl": "../img/skills/Passive_MDefUp.webp",
        "Desc": "[패시브]행동 종료 시 주위 2칸 내 2개 다른 아군 부대의 '받는 피해' -15% 및<br/>면역: '방어,마방 감소','치료 방해', 1턴 지속.",
        "SubSkills": null,
        "Animation": null
      },
      {
        "Name": "찬란한 제국의 수정",
        "Cost": "COST 2",
        "Entries": [
          {
            "Label": "유형",
            "Value": "초절강화"
          },
          {
            "Label": "쿨타임",
            "Value": "3턴"
          },
          {
            "Label": "사거리",
            "Value": "자신"
          },
          {
            "Label": "범위",
            "Value": "맵전체"
          }
        ],
        "IconImageUrl": "../img/skills/Rozenciel/1.webp",
        "Desc": "[초절강화] 액티브 스킬, 범위 내 모든 '제국의 빛' 아군의 <br/>공격, 방어 능력치가 크게 상승하고<br/><b>선공으로 전투 진입시 주는 피해량 <font color=\"red\">+12</font></b>% 및 <br/><b>피격으로 전투 진입전 적 버프 <font color=\"red\">1</font>개 제거</b>, <font color=\"red\">4</font>행동 지속.<br/>",
        "SubSkills": null,
        "Animation": null
      },
      {
        "Name": "빙룡의 숨결",
        "Cost": "COST 2",
        "Entries": [
          {
            "Label": "유형",
            "Value": "마법 피해"
          },
          {
            "Label": "쿨타임",
            "Value": "5턴"
          },
          {
            "Label": "사거리",
            "Value": "5칸"
          },
          {
            "Label": "범위",
            "Value": "직선"
          }
        ],
        "IconImageUrl": "../img/skills/Skill_FrostRay2.webp",
        "Desc": "<font color=\"red\">3</font>개의 직선 상의 모든 적군에게 <font color=\"red\">0.36</font>배 범위 피해를 입히며 <font color=\"blue\">이동력 <font color\"red\">-1</font></font> 부여,<br/> 지속 <font color=\"red\">2</font><br/>'창병'에게 더 큰 피해를 입힌다.",
        "SubSkills": null,
        "Animation": null
      },
      {
        "Name": "수정 치유술",
        "Cost": "COST 1",
        "Entries": [
          {
            "Label": "유형",
            "Value": "치료"
          },
          {
            "Label": "쿨타임",
            "Value": "-"
          },
          {
            "Label": "사거리",
            "Value": "2칸"
          },
          {
            "Label": "범위",
            "Value": "단일"
          }
        ],
        "IconImageUrl": "../img/skills/Rozenciel/2.webp",
        "Desc": "[액티브] <b>단일 개체</b> 지력 <font color=\"red\">3</font>배 수치 치료 및 '<font color=\"blue\">수정 치유</font>' 효과 부여 :<br/>피격 후 로젠실 지력 <font color=\"red\">1.5</font>배 수치 생명 회복, 지속 <font color=\"red\">2</font>행동",
        "SubSkills": null,
        "Animation": null
      },
      {
        "Name": "집단 수정 치유",
        "Cost": "COST 2",
        "Entries": [
          {
            "Label": "유형",
            "Value": "치료"
          },
          {
            "Label": "쿨타임",
            "Value": "2턴"
          },
          {
            "Label": "사거리",
            "Value": "3칸"
          },
          {
            "Label": "범위",
            "Value": "3칸"
          }
        ],
        "IconImageUrl": "../img/skills/Rozenciel/3.webp",
        "Desc": "[액티브]<b>범위 내 아군에게</b> 지력 <font color=\"red\">3</font>배 수치 치료 및 '<font color=\"blue\">수정 치유</font>' 효과 부여 : <br/><b>피격 후</b> 로젠실 지력 <font color=\"red\">1.5</font>배 수치 생명 회복, 지속 <font color=\"red\">2</font>행동",
        "SubSkills": null,
        "Animation": null
      },
      {
        "Name": "기적",
        "Cost": "COST 2",
        "Entries": [
          {
            "Label": "유형",
            "Value": "지원"
          },
          {
            "Label": "쿨타임",
            "Value": "4턴"
          },
          {
            "Label": "사거리",
            "Value": "3칸"
          },
          {
            "Label": "범위",
            "Value": "4칸"
          }
        ],
        "IconImageUrl": "../img/skills/Skill_PalCrash.webp",
        "Desc": "범위 내 아군들에게 시전자 지력 <font color=\"red\">3</font>배의 생명 <font color=\"green\">회복</font> 및<br/><font color=\"blue\">주는 피해량</font> <font color=\"red\">+15%</font>, 받는 피해량 <font color=\"red\">-15%</font> 부여<br/>지속 <font color=\"red\">2</font>행동",
        "SubSkills": null,
        "Animation": null
      },
      {
        "Name": "수정 결계",
        "Cost": "COST 3",
        "Entries": [
          {
            "Label": "유형",
            "Value": "변신"
          },
          {
            "Label": "쿨타임",
            "Value": "5턴"
          },
          {
            "Label": "사거리",
            "Value": "자신"
          },
          {
            "Label": "범위",
            "Value": "4칸"
          }
        ],
        "IconImageUrl": "../img/Super/로젠실.webp",
        "Desc": "[변신]<b>대규모 범위 내 다수의 부대 생명을 로젠실 지력 <font color=\"red\">3</font>배 수치로 회복</b> 및 <br/>'<font color=\"blue\">수정 치유</font>' 효과 획득:<br/><b>피격으로 피해를 입은 후</b> 로젠실 지력 <font color=\"red\">1.5</font>배 수치로 생명 회복, 지속 <font color=\"red\">2</font>행동<br/>사용 후 '<font color=\"blue\">수정 결인</font>' 상태 진입 :<br/><b><font color=\"red\">공격 및 스킬 사용 불가 및 스스로 이동 및 위치 변경 불가</font>(디메리트)</b>, <br/>치유량 <font color=\"red\">+20%</font>, 받는 피해량 <font color=\"red\">-20%</font>, <b><font color=\"purple\">고유 스킬</font> 지휘 효과 범위</b> <font color=\"red\">+1</font>, <font color=\"red\">1</font>회 부활, <br/><b>행동 종료 시 자신의 <font color=\"purple\">고유 스킬</font> 범위 안에 있는 다른 아군을</b><br/>로젠실 지력 <font color=\"red\">2</font>배 수치만큼 회복 및 '<font color=\"blue\">수정 장벽</font>' <font color=\"red\">2</font>개 효과 부여, 지속 <font color=\"red\">2</font>행동<br/>'<font color=\"blue\">수정 결인</font>'의 효과는 <b>해제 불가 및 면역 불가</b>, <br/><b>해당 스킬 지속 시간 동안 <font color=\"blue\">불굴</font> 효과</b>는 최대 <font color=\"red\">1</font>회 발동)",
        "SubSkills": null,
        "Animation": "../img/Super/로젠실_Ani.webp"
      }
    ],
    "Propose": true,
    "Bio": [
      {
        "Entry": "영웅소개",
        "Desc": "◆키/몸무게: 168cm / ?<br/>◆신체 사이즈: 81/51/81<br/>◆소속: 레겐부르그 제국"
      },
      {
        "Entry": "영웅담1",
        "Desc": "과거 레겐부르그 연방의 공주였으며, 지금은 레겐부르그 제국의 고귀한 여왕. <br/>그녀는 변혁만이 희망을 가져올 수 있다고 굳게 믿으며, 오로지 자신의 신념에 따라 분골쇄신하며 아버지와 동생이 남긴 나라를 지키고 있다. <br/>수정처럼 여리고 약해져, 지금이라도 무너질 듯이 쇠퇴한 레겐부르그를......<br/><font color=\"#DC143C\"><size=33><i>「일어나십시오, 나를 맞이한 소환자여. 레겐부르그의 여왕 로젠실이, 그대의 고통을 덜어드리겠습니다!」</i></size=33></font>"
      },
      {
        "Entry": "영웅담2",
        "Desc": "참혹했던 종언전쟁이 겨우 끝을 맞이한 순간, 많은 이들은 기나긴 어둠을 빠져나와 겨우 햇살을 맞이하는 해방감으로 가득했다. <br/>허나 이는 로젠실에겐 괴롭고 잔혹한 시련의 시작일 뿐이었다. 긴 세월에 걸친 전쟁은 레겐부르그를 급격히 쇠퇴시켰다. <br/>검은 안개 탓에 대지는 메마르고, 기아가 일어나 많은 백성의 마음을 좀먹었다. <br/>거기에 쐐기를 박듯 폭발적으로 창궐한 수정병이 쇠락한 나라를 유린했으며, <br/>내부의 속지에서까지 터진 반란으로 수많은 폭도가 왕성에 몰려들었다.<br/>그럼에도, 제국의 유일한 왕위후계자인 로젠실에겐 슬퍼할 시간조차 주어지지 않았다. 그녀는 자신의 감정을 억누른 채, <br/>그 가녀린 몸에 레겐부르그의 부흥과 백성의 치료라는 막대한 사명을 짊어져야만 했다.<br/><font color=\"#DC143C\"><size=33><i>「제 곁을 떠난 이들을 원망한 적은 없습니다. <br/>제가 할 수 있는 건, 마땅한 책임을 짊어진 채 백성을 이끌어 가는 것뿐입니다.」</i></size=33></font>"
      },
      {
        "Entry": "영웅담3",
        "Desc": "로젠실은 각지에 퍼진 수정의 조사를 지휘하는 와중, 자신도 생명을 앗아가는 '수정병'에 감염되었음을 깨닫는다. <br/>동시에 그녀는 자신의 몸이 조금씩 '수정화' 되어가고 있음을 알아차린다. <br/>하지만 다른 감염자들처럼 목숨을 잃는 대신, 오히려 영원에 가까운 긴 수명을 얻게 된다. <br/>그리고 그녀가 감염자들의 몸을 잠깐 만지기만 해도, 수정병의 증세는 거짓말처럼 사라졌다. <br/>하지만 이때의 로젠실은 꿈에도 알지 못했다. 그녀의 몸은 이미 기자로프의 계략에 의해 <br/>'생명력'의 그릇으로 바뀌어 있었으며, 사라진 줄만 알았던 수정은 그저 그녀의 몸속으로 이동했을 뿐이라는 사실을."
      },
      {
        "Entry": "영웅담4",
        "Desc": "사람들은 '수정병'의 발원지가 종언전쟁 때 추락한 '붉은 달'의 전함이라는 사실을 깨닫는다. <br/>이 때문에 레겐부르그의 백성과 크림조랜더들 사이에 깊은 골이 패이며 격렬한 내전을 불러일으켰다. <br/>로젠실은 무력을 동원하여 내전을 진압할 수밖에 없었다. <br/>그녀는 통치력을 높여 두 번 다시 같은 비극이 일어나지 않도록 국가 체제를 연방에서 제국으로 바꿨다. <br/>그 후, 그녀는 불로불사의 힘으로 레겐부르그를 150년동안 통치해왔다. 이 결단은 레겐부르그의 비참한 운명의 흐름을 잠시 늦추기는 했지만, <br/>기근은 피할 수 없었다. 결국 그녀는 애머시스트의 부추김에 넘어가 카콘시스를 침략하게 되며, <br/>이는 훗날 대륙전쟁의 불씨가 된다.<br/><font color=\"#DC143C\"><size=33><i>「이젠 싸우는 수밖에 레겐부르그는 살아남을 길이 없습니다. 이 또한 전부 제가 무능한 탓...... <br/>란포드 원수님, 당신의 기대에 부응해드리지 못했습니다.」</i> </size=33></font>"
      },
      {
        "Entry": "영웅담5",
        "Desc": "로젠실은 직접 병사를 이끌고 카콘시스로 진군했다. 어쩌면 그녀는 이미 이런 결과를 예상했는지도 모른다. <br/>하지만 설령 그녀를 기다리는 운명이 긴 시간에 걸쳐 계획된 파멸의 길이라고 해도, <br/>150년 전의 자신과 마찬가지로 그저 고독히 앞으로 나아가는 수밖에 없었다......<br/><font color=\"#DC143C\"><size=33><i>「제 마음은 이미 수정처럼 단단히 얼어붙었습니다. 이제 와서 천 번의 슬픔, 만 번의 상처를 겪는다 한들, <br/>균열이 조금 늘어날 뿐. 고통을 느끼지는 않습니다.」</i> </size=33></font>"
      }
    ],
    "Performance": [
      {
        "Entry": "「소환 - 등장대사」",
        "Desc": "일어나십시오, 나를 맞이한 소환자여. 레겐부르그의 여왕 로젠실이, 그대의 고통을 덜어드리겠습니다!"
      },
      {
        "Entry": "「전투 - 전직1」",
        "Desc": "체제건 능력이건, 변혁하는 자만이 미래를 가져올 수 있습니다."
      },
      {
        "Entry": "「전투 - 전직2」",
        "Desc": "이제 더욱 많은 백성을 구할 수 있겠죠......"
      },
      {
        "Entry": "「전투 - 스킬시전1」",
        "Desc": "레겐부르그를 위하여!"
      },
      {
        "Entry": "「전투 - 스킬시전2」",
        "Desc": "수정이여, 제국의 적을 쓰러트리세요!"
      },
      {
        "Entry": "「전투 - 선택1」",
        "Desc": "무슨 일이죠?"
      },
      {
        "Entry": "「전투 - 선택2」",
        "Desc": "제가 바로, 레겐부르그입니다!"
      },
      {
        "Entry": "「전투 - 전투불능」",
        "Desc": "결국...... 산산이 부서질 운명인가요?"
      },
      {
        "Entry": "「대화 - 친밀도1」",
        "Desc": "수정은 저주일 뿐입니다. 사람들에게 고통과 욕망을 가져다주고, <br/>제게는 영원의 생명이라는 고문을 가져다주었죠."
      },
      {
        "Entry": "「대화 - 친밀도2」",
        "Desc": "클로테르...... 그의 성장은 기쁘지만, 전쟁에 끌어들인 건 후회하고 있습니다. <br/>언젠가 그가 행복해지길 바라 마지않습니다."
      },
      {
        "Entry": "「대화 - 친밀도3」",
        "Desc": "기자로프...... 그는 제가 어렸을 적 품었던 꿈을, <br/>그 야심과 음모로 전부 파괴했습니다. 그의 그림자에서 벗어나는 날이 올까요?"
      },
      {
        "Entry": "「대화 - 친밀도4」",
        "Desc": "이제 레겐부르그가 살아남을 길은 싸우는 것 뿐입니다. 이 또한 전부 제가 무능한 탓...... <br/>란포드 원수님, 당신의 기대에 부응해드리지 못했습니다."
      },
      {
        "Entry": "「대화 - 친밀도5」",
        "Desc": "제 곁을 떠난 이들을 원망한 적은 없습니다. 제가 할 수 있는 건, <br/>마땅한 책임을 짊어진 채 백성을 이끌어 가는 것뿐입니다."
      },
      {
        "Entry": "「대화 - 일상1」",
        "Desc": "가끔씩, 알단이 제 곁을 떠난 날을 떠올립니다. <br/>하지만 그것도 한 세기도 넘게 지난 일이군요......"
      },
      {
        "Entry": "「대화 - 기쁨」",
        "Desc": "안개가 걷혔군요. 하늘도 레겐부르그에 미소짓는 걸까요."
      },
      {
        "Entry": "「대화 - 분노」",
        "Desc": "크루거...... 그가 지옥에서 돌아온 목적이 레겐부르그의 멸망이라면, <br/>저는 악귀가 되어서라도 막아내겠습니다!"
      },
      {
        "Entry": "「대화 - 슬픔」",
        "Desc": "제 마음은 이미 수정처럼 단단히 얼어붙었습니다. 이제 와서 천 번의 슬픔, 만 번의 상처를 겪는다 한들, <br/>균열이 조금 늘어날 뿐. 고통을 느끼지는 않습니다."
      },
      {
        "Entry": "「대화 - 수줍음」",
        "Desc": "네? 시공의 균열에서 소녀 시절의 저와 만나셨다구요? 아아, 어쩌죠. 뭔가 부끄럽습니다......"
      },
      {
        "Entry": "「상호작용 - 대사1」",
        "Desc": "멜파니 공주와는 만난 적이 있나요? 총명하고 세심한, 하지만 불쌍한 아이입니다. <br/>그 아이를 볼 때마다 떠올리고 말아요. 옛날, 전쟁의 소용돌이 속에서 발버둥 치던 저 자신을......"
      },
      {
        "Entry": "「상호작용 - 대사2」",
        "Desc": "제가 두려우신가요? 네, 무리도 아니죠. 저는 이제 광물과 얼음 결정으로 만들어진 괴물이나 다름없죠. <br/>저를 보고도 도망치지 않은 것만으로도 감사해야할 지도 모르겠군요."
      },
      {
        "Entry": "「상호작용 - 대사3」",
        "Desc": "어째서일까요? 가끔 먼 옛날 일을 떠올린답니다...... 그 시절 연방의 대왕이셨던 아바마마께선, <br/>제가 신부가 되는 것을 보고 싶다고 하셨죠......"
      },
      {
        "Entry": "「상호작용 - 호감1」",
        "Desc": "옛날이야기만 하는 할머니 같은 저와 어울려주셔서 고마워요. <br/>괜찮으시다면, 함께 정원이라도 산책할까요?"
      },
      {
        "Entry": "「상호작용 - 반감1」",
        "Desc": "무례는 삼가해주셨으면 하군요!"
      },
      {
        "Entry": "「상호작용 - 특수1」",
        "Desc": "함께 해주셔서 즐거웠습니다. 저는 슬슬 가보겠습니다."
      }
    ],
    "Performance2": null,
    "Misc": ""
  }
]