var data = [
{
  "Skins": [
    {
      "ImageUrl": "세계수의현자/1.webp",
      "Desc": "기본"
    },
    {
      "ImageUrl": "세계수의현자/2.webp",
      "Desc": "빛의 메아리"
    },
    {
      "ImageUrl": "세계수의현자/3.webp",
      "Desc": "한정판매"
    }
  ],
  "Name": "세계수의현자",
  "ChName": "树之贤者",
  "EngName": "Sage of the Trees",
  "JapName": "世界樹の賢者",
  "CV": "오키아유 료타로",
  "Rank": [
    0
  ],
  "SP": false,
  "Groups": [
    7,
    10
  ],
  "HP": 20,
  "SPHP": 0,
  "Attk": 15,
  "SPAttk": 0,
  "Def": 35,
  "SPDef": 0,
  "MagicDef": 30,
  "SPMagicDef": 0,
  "HowToGet": "영웅 소환",
  "Fetters": [
    {
      "Condition": "세계수의 현자를 출전시켜 시공 3-6 클리어"
    },
    {
      "Condition": "세계수의 현자를 출전시켜 시공 3-1 정예 클리어"
    },
    {
      "Condition": "미지의기사의 도움하에 운명의문4"
    },
    {
      "Condition": "리스틸의 도움하에 운명의문5"
    }
  ],
  "HeartFetter": "생명+400, 방어+40, 마방+40",
  "Talent": [
    {
      "TalentName": "영원한 수호",
      "Icon": "Gift_MOTT.webp",
      "Abilities": [
        {
          "Desc": "1성"
        },
        {
          "Desc": "2성"
        },
        {
          "Desc": "자신 주위 <font color=\"red\">1</font>칸내 아군 있을 시 지력 <font color=\"red\">+10%</font><br/>근접 공격 피해감면 영향을 받지않는다.<br/>사망시 자신을 중심으로한 <font color=\"red\">3</font>칸 범위내 아군의 모든 약화효과를 제거한다.<br/>동시에 명중한 모든 지형에 '<font color=\"purple\">눈부신 빛</font>'부여 <font color=\"red\">4</font>행동 지속.<br/>:\"적군의 이동 방식이 [비행][기마] [야전]의 경우 이동 시 이동력 소모+<font color=\"red\">1</font><br/>아군의 이동방식이 [보행][수병]인 경우 이 지형에서 전투후 <font color=\"red\">2</font>칸 재이동 가능."
        },
        {
          "Desc": "자신 주위 <font color=\"red\">2</font>칸내 아군 있을 시 지력 <font color=\"red\">+13%</font><br/>근접 공격 피해감면 영향을 받지않는다.<br/>사망시 자신을 중심으로한 <font color=\"red\">4</font>칸 범위내 아군의 모든 약화효과를 제거한다.<br/>동시에 명중한 모든 지형에 '<font color=\"purple\">눈부신 빛</font>'부여 <font color=\"red\">4</font>행동 지속.<br/>:\"적군의 이동 방식이 [비행][기마] [야전]의 경우 이동 시 이동력 소모+<font color=\"red\">1</font><br/>아군의 이동방식이 [보행][수병]인 경우 이 지형에서 전투후 <font color=\"red\">2</font>칸 재이동 가능."
        },
        {
          "Desc": "자신 주위 <font color=\"red\">2</font>칸내 아군 있을 시 지력 <font color=\"red\">+16%</font><br/>근접 공격 피해감면 영향을 받지않는다.<br/>사망시 자신을 중심으로한 <font color=\"red\">4</font>칸 범위내 아군의 모든 약화효과를 제거한다.<br/>동시에 명중한 모든 지형에 '<font color=\"purple\">눈부신 빛</font>'부여 <font color=\"red\">4</font>행동 지속.<br/>:\"적군의 이동 방식이 [비행][기마] [야전]의 경우 이동 시 이동력 소모+<font color=\"red\">1</font><br/>아군의 이동방식이 [보행][수병]인 경우 이 지형에서 전투후 <font color=\"red\">2</font>칸 재이동 가능."
        },
        {
          "Desc": "자신 주위 <font color=\"red\">3</font>칸내 아군 있을 시 지력 <font color=\"red\">+20%</font><br/>근접 공격 피해감면 영향을 받지않는다.<br/>사망시 자신을 중심으로한 <font color=\"red\">5</font>칸 범위내 아군의 모든 약화효과를 제거한다.<br/>동시에 명중한 모든 지형에 '<font color=\"purple\">눈부신 빛</font>'부여 <font color=\"red\">4</font>행동 지속.<br/>:\"적군의 이동 방식이 [비행][기마] [야전]의 경우 이동 시 이동력 소모+<font color=\"red\">1</font><br/>아군의 이동방식이 [보행][수병]인 경우 이 지형에서 전투후 <font color=\"red\">2</font>칸 재이동 가능."
        }
      ]
    }
  ],
  "SPTalent": null,
  "Second_Awaken": {
    "Name" : "세상의 위엄",
    "Stats": "지력 <font color=\"red\">+10%</font><br/>일반 공격이 <font color=\"red\">1.5</font>배 피해를 입히며, 선공 후 아군의 <font color=\"purple\">눈부신 빛</font> 위에 있는 아군 부대 생명 <font color=\"green\">회복</font><br/>지력 <font color=\"red\">2</font>배 수치"
  },
  "Equip": {
    "EqpName": "과거의 빛(투구)",
    "Stats": "생명 364,마방 65",
    "Icon": "세계수의현자.webp",
    "Ability": "생명 <font color=\"red\">+10%</font><br/><b>부대의 일반 공격 사거리 <font color=\"red\">+1</font></b><br/><b>선공으로 전투 진입 후 목표 및 주위 <font color=\"red\">1</font>칸에</b><br/><font color=\"purple\">눈부신 빛</font>효과 부여<br/>지속<font color=\"red\">1</font>행동"
  },
  "Soldiers": [
    44,
    20,
    17,
    4,
    3,
    45,
    19,
    76,
    95
  ],
  "Jobs": [
    {
      "Name": "대현자",
      "Icon": "Icon_Occupation_Monk.webp",
      "SD": [
        {
          "ImageUrl": "세계수의현자/1/1.webp"
        },
        {
          "ImageUrl": "세계수의현자/2/1.webp"
        },
        {
          "ImageUrl": "세계수의현자/3/1.webp"
        }
      ],
      "AttkRange": 2,
      "Move": 3,
      "MoveType": "Walk",
      "HP": 3279,
      "HPRate": 55,
      "Attk": 307,
      "AttkRate": 47,
      "Int": 544,
      "IntRate": 91,
      "Def": 250,
      "DefRate": 56,
      "MagicDef": 348,
      "MagicDefRate": 77,
      "Skill": 92,
      "SkillRate": 26,
      "HeartFetterBouns1": "공격을 받아 전투진입시 받는 물리피해 -10%",
      "HeartFetterBouns2": "생명이 80%이상일때 전투진입시 입히는피해 +10%"
    },
    {
      "Name": "위저드",
      "Icon": "Icon_Occupation_Magician.webp",
      "SD": [
        {
          "ImageUrl": "세계수의현자/1/2.webp"
        },
        {
          "ImageUrl": "세계수의현자/2/1.webp"
        },
        {
          "ImageUrl": "세계수의현자/3/1.webp"
        }
      ],
      "AttkRange": 2,
      "Move": 3,
      "MoveType": "Walk",
      "HP": 3279,
      "HPRate": 55,
      "Attk": 256,
      "AttkRate": 39,
      "Int": 544,
      "IntRate": 91,
      "Def": 250,
      "DefRate": 56,
      "MagicDef": 370,
      "MagicDefRate": 82,
      "Skill": 92,
      "SkillRate": 26,
      "HeartFetterBouns1": "'약화효과'를 보유중인 부대와 공격진입시 받는피해 -10%",
      "HeartFetterBouns2": "생명비율이 낮은적을 공격하여 전투진입시 입히는피해 +10%"
    }
  ],
  "Materials": {
    "HeartFetter": [
      "h4",
      "h1",
      "h3",
      "h2",
      "h4-1",
      "h1-1",
      "h3-1",
      "h2-1",
      "x"
    ],
    "Fetter": [
      "4",
      "4-1",
      "4-2",
      "4-3",
      "4-4"
    ],
    "Awaken": [
      "ranka4",
      "ranka5",
      "ranka4-1",
      "ranka5-1"
    ]
  },
  "ClassUp": [
    {
      "ClassTitle": "메이지 : 마방+4, 지력+9",
      "Tiers": [
        {
          "Name": "T1",
          "Materials": [
            "rank1-5"
          ]
        },
        {
          "Name": "T2",
          "Materials": [
            "rank1-5"
          ]
        }
      ]
    },
    {
      "ClassTitle": "아크메이지 : 생명+48, 지력+12",
      "Tiers": [
        {
          "Name": "T1",
          "Materials": [
            "rank2-4",
            "rank1-5",
            "rank1-4"
          ]
        },
        {
          "Name": "T2",
          "Materials": [
            "rank3-5",
            "rank3-4",
            "rank2-4",
            "rank2-5"
          ]
        },
        {
          "Name": "T3",
          "Materials": [
            "rank4-4",
            "rank4-5",
            "rank3-5",
            "rank3-4"
          ]
        }
      ]
    },
    {
      "ClassTitle": "위저드 : 생명+90, 마방+10, 지력+25",
      "Tiers": [
        {
          "Name": "T1",
          "Materials": [
            "rank5-5",
            "rank5-4",
            "rank4-4",
            "rank4-5"
          ]
        },
        {
          "Name": "T2",
          "Materials": [
            "rank6-7",
            "rank6-4",
            "rank5-5",
            "rank5-4"
          ]
        },
        {
          "Name": "T3",
          "Materials": [
            "rank7-6",
            "rank7-5",
            "rank6-7",
            "rank6-4"
          ]
        },
        {
          "Name": "T4",
          "Materials": [
            "rank8-4",
            "rank8-9",
            "rank7-3",
            "rank7-4"
          ]
        }
      ]
    },
    {
      "ClassTitle": "마터 : 마방+8, 지력+7",
      "Tiers": [
        {
          "Name": "T1",
          "Materials": [
            "rank2-5",
            "rank1-5",
            "rank1-4"
          ]
        },
        {
          "Name": "T2",
          "Materials": [
            "rank3-4",
            "rank3-5",
            "rank2-5",
            "rank2-4"
          ]
        },
        {
          "Name": "T3",
          "Materials": [
            "rank4-5",
            "rank4-4",
            "rank3-4",
            "rank3-5"
          ]
        }
      ]
    },
    {
      "ClassTitle": "대현자 : 지력+25, 마방+10, 생명+90",
      "Tiers": [
        {
          "Name": "T1",
          "Materials": [
            "rank5-4",
            "rank5-5",
            "rank4-5",
            "rank4-4"
          ]
        },
        {
          "Name": "T2",
          "Materials": [
            "rank6-8",
            "rank6-7",
            "rank5-4",
            "rank5-5"
          ]
        },
        {
          "Name": "T3",
          "Materials": [
            "rank7-5",
            "rank7-6",
            "rank6-8",
            "rank6-7"
          ]
        },
        {
          "Name": "T4",
          "Materials": [
            "rank8-6",
            "rank8-4",
            "rank7-5",
            "rank7-6"
          ]
        }
      ]
    }
  ],
  "SPOpenConditions": null,
  "Skills": [
    {
      "Name": "썬더",
      "Cost": "COST 1",
      "Entries": [
        {
          "Label": "유형",
          "Value": "마법 피해"
        },
        {
          "Label": "쿨타임",
          "Value": "1턴"
        },
        {
          "Label": "사거리",
          "Value": "2칸"
        },
        {
          "Label": "범위",
          "Value": "단일"
        }
      ],
      "IconImageUrl": "../img/skills/Skill_ThunderBolt.webp",
      "Desc": "[마법 피해]단일 적 부대를 공격해 1.5배의 피해를 준다.<br/>'기병'에게 더큰 피해를 준다.",
      "SubSkills": null,
      "Animation": null
    },
    {
      "Name": "방어 지원",
      "Cost": "COST 1",
      "Entries": [
        {
          "Label": "유형",
          "Value": "패시브"
        }
      ],
      "IconImageUrl": "../img/skills/Passive_DefBuf2.webp",
      "Desc": "[패시브]행동 종료 시 근접한 1개 아군 부대의 방어 +15% 및<br/>면역: '방어, 마방 감소','치료 방해', 1턴 지속.",
      "SubSkills": null,
      "Animation": null
    },
    {
      "Name": "공정한 빛",
      "Cost": "COST 2",
      "Entries": [
        {
          "Label": "유형",
          "Value": "마법 피해"
        },
        {
          "Label": "쿨타임",
          "Value": "2턴"
        },
        {
          "Label": "사거리",
          "Value": "3칸"
        },
        {
          "Label": "범위",
          "Value": "직선"
        }
      ],
      "IconImageUrl": "../img/skills/Skill_MOTT3.webp",
      "Desc": "[마법 피해]범위 내 적들을 공격해 0.2배의 광역 피해를 주며<br/>명중한 지형에 '<font color=\"purple\">눈부신 빛</font>'부여, 2턴 지속.<br/>:\"적군의 이동 방식이 [비행], [기마], [야전]의 경우 이동시 소모되는 이동력 소모+1<br/>아군의 이동방식이 [보행], [수병]인 경우 이 해당 지형에서 전투후 2칸 재이동 가능.",
      "SubSkills": null,
      "Animation": null
    },
    {
      "Name": "초월 구현화",
      "Cost": "COST 2",
      "Entries": [
        {
          "Label": "유형",
          "Value": "초절강화"
        },
        {
          "Label": "쿨타임",
          "Value": "3턴"
        },
        {
          "Label": "사거리",
          "Value": "자신"
        },
        {
          "Label": "범위",
          "Value": "맵전체"
        }
      ],
      "IconImageUrl": "../img/skills/Skill_MOTT2.webp",
      "Desc": "[초절강화] 액티브 스킬, 범위 내 모든 '초월 영역' 아군의 <br/>공격, 방어 능력치가 크게 상승하고 원거리 피해 +12%,<br/>피해를 입히지 않고 행동종료시 약화효과 1개 제거, 4턴 지속.<br/>(이효과는 다른 초절강화 스킬과 중첩불가)",
      "SubSkills": null,
      "Animation": null
    },
    {
      "Name": "뇌룡의 숨결",
      "Cost": "COST 2",
      "Entries": [
        {
          "Label": "유형",
          "Value": "마법 피해"
        },
        {
          "Label": "쿨타임",
          "Value": "5턴"
        },
        {
          "Label": "사거리",
          "Value": "5칸"
        },
        {
          "Label": "범위",
          "Value": "직선"
        }
      ],
      "IconImageUrl": "../img/skills/Skill_ThundeRay2.webp",
      "Desc": "[마법 피해] 3개의 라인에 있는 모든 적군을 공격하여 0.36배 범위 피해를 입힌다.<br/>'기병'과 전투 시 더 큰 피해를 입힌다. 적군의 방어 -20%, 2턴 지속.",
      "SubSkills": null,
      "Animation": null
    },
    {
      "Name": "턴언데드",
      "Cost": "COST 2",
      "Entries": [
        {
          "Label": "유형",
          "Value": "마법 피해"
        },
        {
          "Label": "쿨타임",
          "Value": "2턴"
        },
        {
          "Label": "사거리",
          "Value": "2칸"
        },
        {
          "Label": "범위",
          "Value": "단일"
        }
      ],
      "IconImageUrl": "../img/skills/Skill_ValkCrash2.webp",
      "Desc": "[마법 피해]단일 적 부대를 공격해 1.5배의 피해를 준다.<br/>'마물'에게 더큰 피해를 준다. 전투 후 대상의 버프를 2개 제거한다.",
      "SubSkills": null,
      "Animation": null
    },
    {
      "Name": "불굴",
      "Cost": "COST 1",
      "Entries": [
        {
          "Label": "유형",
          "Value": "패시브"
        }
      ],
      "IconImageUrl": "../img/skills/Skill_NerverDie.webp",
      "Desc": "[패시브]사망 시 부활 및 생명 20%회복. 최대 1회 가능.",
      "SubSkills": null,
      "Animation": null
    },
    {
      "Name": "역전의 기억",
      "Cost": "COST 2",
      "Entries": [
        {
          "Label": "유형",
          "Value": "패시브"
        }
      ],
      "IconImageUrl": "../img/skills/Skill_MOTT1.webp",
      "Desc": "[패시브] 주위 2칸 내 영웅의 직업이 [보병],[창병],[기병]의<br/>아군이 공격하여 전투진입시 입히는 피해 +12%, 받는피해 -12%.",
      "SubSkills": null,
      "Animation": null
    },
    {
      "Name": "고요한 숨결",
      "Cost": "COST 3",
      "Entries": [
        {
          "Label": "유형",
          "Value": "마법 피해"
        },
        {
          "Label": "쿨타임",
          "Value": "5턴"
        },
        {
          "Label": "사거리",
          "Value": "5칸"
        },
        {
          "Label": "범위",
          "Value": "직선"
        }
      ],
      "IconImageUrl": "../img/Super/세계수의현자.webp",
      "Desc": "[지휘]주위 2칸내 모든 아군이 받는 치명타 확률 - 30%,<br/>받는 원거리피해 -15%, 받는 치료효과 +15%.<br/>[마법피해]3개의 라인에 있는 모든 적군을 공격하여 0.36배 범위 피해를 입히고,<br/>2칸 후퇴 시킨다. 동시에 명중한 모든 지형에 '<font color=\"purple\">눈부신 빛</font>' 2턴 부여<br/>:\"적 부대의 이동 방식이 [비행], [기마], [야전]의 경우 이동시 소모되는 이동력 소모+1<br/>아군의 이동방식이 [보행], [수병]인 경우 이 지형에서 전투후 2칸 재이동 가능.",
      "SubSkills": null,
      "Animation": "../img/Super/세계수의현자_Ani.webp"
    }
  ],
  "Propose": false,
  "Bio": [
    {
      "Entry": "영웅소개",
      "Desc": "◆키/몸무게: 185cm / 70kg<br/>◆소속：정의로운 동료"
    },
    {
      "Entry": "영웅담1",
      "Desc": "수수께끼의 현자. 그가 언제 어디서 왔는지는 알 수 없으며, <br/>그의 과거와 내력에 관한 것은 모두 베일에 싸여 있다. <br/>이 세계에서 유일하게 그를 이해하는 건, 그가 수천 년에 걸쳐 소중하게 지켜온 북방의 세계수뿐이다. <br/>그의 진짜 모습을 본 사람은 거의 없다.<br/><font color=\"#DC143C\"><size=33><i>「나는 모든 것을 버린 이름 없는 사람이다...... 내 유일한 소원은 세계수를 지키는 것이다.」 </i>  </size=33></font>"
    },
	{
      "Entry": "영웅담2",
      "Desc": "연방 시절, 그는 세계수 곁에서 예레스 대륙의 무수한 흥망성쇠를 지켜봐 왔다.<br/>그러나 그는 간섭하는 것을 완강히 거부하며 세계수 곁에서 세계의 변화를 냉담하게 방관할 뿐이었다.<br/><font color=\"#DC143C\"><size=33><i>「나는 여기서 세계수를 지킨다. 이 대륙의 마지막 희망을 지키는 것이다. 그게 내가 할 수 있는 유일한 일이다. 세상을 떠난 동료들을 위해, 다시는 돌아오지 않을 그 시간을 위해......」</i>  </size=33></font>"
    },
    {
      "Entry": "영웅담3",
      "Desc": "미지의 기사의 인도로 매튜 일행은 세계수의 현자를 찾아와 도움을 요청한다.<br/>세계수의 현자 또한 세계수의 예언에 따라 성검 군단과 함께 떠나기로 한다. <br/>그는 매튜 일행을 도우면서 마지막 세계수를 지키겠다고 결심했다."
    },
    {
      "Entry": "영웅담4",
      "Desc": "미지의 기사가 매튜를 소개하면서 세계수의 현자의 비밀도 밝혀졌다.<br/>그는 과거에 예레스 대륙의 영웅이었던 란디우스였다.<br/>크림조인 그는 수명이 길어 오랜 세월 속에서 과거의 전우들이나 연인과 이별을 경험했다.<br/>그 경험은 무엇보다 유대를 중시하는 그에게 견딜 수 없는 고통을 안겨주었다.<br/>그 결과, 왕국의 수호자라는 신분을 버리고 지금은 리스틸의 화신이 된 북단의 세계수를 지키게 된 것이다.<br/><font color=\"#DC143C\"><size=33><i>「아니, 란디우스라는 이름은 버렸어...... 난 모든 것을 버렸어......」</i></size=33></font>"
    },
    {
      "Entry": "영웅담5",
      "Desc": "성검 군단과 합류한 세계수의 현자는 영웅의 의지를 조금씩 되찾았고, <br/>그가 남긴 희망의 씨앗과 함께 다시 대륙을 지키겠다고 결심했다.<br/><font color=\"#DC143C\"><size=33><i>「난 지키겠어... 리스틸, 그리고 네가 지키고자 했던 모든 걸...... 내 목숨이 다할 때까지.」 </i>  </size=33></font>"
    }
  ],
  "Performance": [
    {
      "Entry": "「소환 - 등장대사」",
      "Desc": "나는 모든 것을 버린 이름 없는 사람이다...... 내 유일한 소원은 세계수를 지키는 것이다."
    },
    {
      "Entry": "「전투 - 전직1」",
      "Desc": "강인함의 끝은...... 과연 무엇인가?"
    },
    {
      "Entry": "「전투 - 전직2」",
      "Desc": "이 힘은...... 과거에 내가 지닌 힘과 다른 것 같군."
    },
    {
      "Entry": "「전투 - 스킬시전1」",
      "Desc": "포기해라!"
    },
    {
      "Entry": "「전투 - 스킬시전2」",
      "Desc": "이 앞으로는 갈 수 없다!"
    },
    {
      "Entry": "「전투 - 선택1」",
      "Desc": "내일을 위해!"
    },
    {
      "Entry": "「전투 - 선택2」",
      "Desc": "준비는 되어있다."
    },
    {
      "Entry": "「전투 - 전투불능」",
      "Desc": "설마 나도......"
    },
    {
      "Entry": "「대화 - 친밀도1」",
      "Desc": "아니, 란디우스라는 이름은 버렸어...... 난 모든 것을 버렸어......"
    },
    {
      "Entry": "「대화 - 친밀도2」",
      "Desc": "크림조랜더에게 주어진 기나긴 수명......<br/>과연 은혜일까, 아니면 저주일까?"
    },
    {
      "Entry": "「대화 - 친밀도3」",
      "Desc": "에밀리, 미안하군. 내가 버린 것을 짊어지게 해서...... 못된 짓을 해버렸군."
    },
    {
      "Entry": "「대화 - 친밀도4」",
      "Desc": "마족은 인간에게 그저 장애물이자 위험한 존재라고 생각하나? <br/>옛날엔 나도 그렇게 생각했었을지도 모르지......"
    },
    {
      "Entry": "「대화 - 친밀도5」",
      "Desc": "난 지키겠어...... <br/>리스틸, 그리고 네가 지키고자 했던 모든 걸...... 내 목숨이 다할 때까지."
    },
    {
      "Entry": "「대화 - 일상1」",
      "Desc": "새벽이 가면 황혼이 오고, 어두운 밤이 가면 새벽이 온다...... <br/>인간 세상에서 영원히 바뀌지 않는 건 그것뿐이다."
    },
    {
      "Entry": "「대화 - 기쁨」",
      "Desc": "매튜...... 넌 수많은 인연은 물론 미래로 나아가는 의지와 용기를 지니고 있어. 마음이 놓이는군."
    },
    {
      "Entry": "「대화 - 분노」",
      "Desc": "영원히 죽지 않기 위해 태어났다면, 왜 살아가야 하지? <br/>하지만 죽음을 위한 삶은 그저 터무니없는 촌극이 아닌가."
    },
    {
      "Entry": "「대화 - 슬픔」",
      "Desc": "무한한 생명이라 해도 언젠가 끝이 찾아오지. <br/>그때 난 어떤 얼굴로 동료들을 만나야 할까......"
    },
    {
      "Entry": "「상호작용 - 대사1」",
      "Desc": "자기 행동에 가치가 있고,<br/> 죽음을 뛰어넘을 수 있다고 믿으려면 엄청난 힘과 긍지, 그리고 사랑이 필요하지."
    },
    {
      "Entry": "「상호작용 - 대사2」",
      "Desc": "우리의 행동이 후세에 어떻게 평가받을지......<br/>우리의 신념과 바람을 인정해줄까?"
    },
    {
      "Entry": "「상호작용 - 대사3」",
      "Desc": "난 여기서 세계수를 지키겠다. 이 대륙의 마지막 희망을 지키는 거다.<br/>그건 내가 할 수 있는 유일한 일이지. 세상을 떠난 동료들을 위해,<br/>다시는 돌아오지 않을 그 시간을 위해......"
    },
    {
      "Entry": "「상호작용 - 호감1」",
      "Desc": "이 끝없는 기억 속에서 자네를 만났다는 걸 가슴 깊이 새겨두지."
    },
    {
      "Entry": "「상호작용 - 반감1」",
      "Desc": "이건 실수한 게 아닌지......"
    },
    {
      "Entry": "「상호작용 - 특수1」",
      "Desc": "또 만나자, 꼭......"
    }
  ],
  "Performance2": null,  
  "Misc": ""
}];