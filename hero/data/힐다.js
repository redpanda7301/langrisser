var data = [
{
  "Skins": [
    {
      "ImageUrl": "힐다/1.webp",
      "Desc": "기본"
    },
    {
      "ImageUrl": "힐다/2.webp",
      "Desc": "빛의 메아리"
    },
    {
      "ImageUrl": "힐다/3.webp",
      "Desc": "서밋아레나 시즌7 패자 스킨"
    }
  ],
  "Name": "힐다",
  "CV": "아라나미 카즈사",
  "Rank": [
    0
  ],
  "SP": false,
  "Groups": [
    2,
    6,
    11
  ],
  "HP": 20,
  "SPHP": 0,
  "Attk": 30,
  "SPAttk": 0,
  "Def": 40,
  "SPDef": 0,
  "MagicDef": 10,
  "SPMagicDef": 0,
  "HowToGet": "영웅 소환",
  "Fetters": [
    {
      "Condition": "레벨 30 달성"
    },
    {
      "Condition": "로열나이트 전직"
    },
    {
      "Condition": "베르너의 도움을 받아 운명의문 4 클리어"
    },
    {
      "Condition": "플로렌티아의 도움을 받아 운명의문 5 클리어"
    }
  ],
  "HeartFetter": "생명+1000, 방어+50, 마방+30",
  "Talent": [
    {
      "Name": "핏빛 복수",
      "Icon": "Gift_Hilder.webp",
      "Abilities": [
        {
          "Desc": "1성"
        },
        {
          "Desc": "2성"
        },
        {
          "Desc": "사망 시 생명 <font color=\"red\">20</font>% 수치로 부활 (최대 <font color=\"red\">1</font>회)<br/><font color=\"red\">2</font>개 이상의 버프 보유 시 받는 모든 피해 <font color=\"red\">-10%</font><br/>공격 스탯의 <font color=\"red\">5</font>%  수치만큼 방마방 상승 <br/> 주위 <font color=\"red\">2</font>칸 내 아군 <font color=\"red\">1</font>명이 피격으로 전투 진입 시<br/>공격자는 해당 공격에서 <b><font color=\"blue\">치명타 발동 불가</font></b> <br/>쿨타임<font color=\"red\">3</font>턴"
        },
        {
          "Desc": "사망 시 생명 <font color=\"red\">30</font>% 수치로 부활 (최대 <font color=\"red\">1</font>회)<br/><font color=\"red\">2</font>개 이상의 버프 보유 시 받는 모든 피해 <font color=\"red\">-15%</font><br/>공격 스탯의 <font color=\"red\">8</font>%  수치만큼 방마방 상승 <br/> 주위 <font color=\"red\">3</font>칸 내 아군 <font color=\"red\">1</font>명이 피격으로 전투 진입 시<br/>공격자는 해당 공격에서 <b><font color=\"blue\">치명타 발동 불가</font></b> <br/>쿨타임<font color=\"red\">2</font>턴"
        },
        {
          "Desc": "사망 시 생명 <font color=\"red\">40</font>% 수치로 부활 (최대 <font color=\"red\">1</font>회)<br/><font color=\"red\">2</font>개 이상의 버프 보유 시 받는 모든 피해 <font color=\"red\">-20%</font><br/>공격 스탯의 <font color=\"red\">11</font>% 수치만큼 방마방 상승 <br/> 주위 <font color=\"red\">3</font>칸 내 아군 <font color=\"red\">1</font>명이 피격으로 전투 진입 시<br/>공격자는 해당 공격에서 <b><font color=\"blue\">치명타 발동 불가</font></b> <br/>쿨타임<font color=\"red\">2</font>턴"
        },
        {
          "Desc": "사망 시 생명 <font color=\"red\">50</font>% 수치로 부활 (최대 <font color=\"red\">1</font>회)<br/><font color=\"red\">2</font>개 이상의 버프 보유 시 받는 모든 피해 <font color=\"red\">-25%</font><br/>공격 스탯의 <font color=\"red\">15</font>% 수치만큼 방마방 상승 <br/> 주위 <font color=\"red\">3</font>칸 내 아군 <font color=\"red\">1</font>명이 피격으로 전투 진입 시<br/>공격자는 해당 공격에서 <b><font color=\"blue\">치명타 발동 불가</font></b> <br/>쿨타임<font color=\"red\">1</font>턴"
        }
      ]
    }
  ],
  "SPTalent": null,
  "Equip": null,
  "Soldiers": [
    25,
    22,
    21,
    74,
    52,
    58,
    61
  ],
  "Jobs": [
    {
      "Name": "마샬",
      "Icon": "Icon_Occupation_Marines.webp",
      "SD": [
        {
          "ImageUrl": "힐다/1/1.webp"
        },
        {
          "ImageUrl": "힐다/2/1.webp"
        },
        {
          "ImageUrl": "힐다/3/1.webp"
        }
      ],
      "AttkRange": 1,
      "Move": 3,
      "MoveType": "Walk",
      "HP": 5372,
      "HPRate": 91,
      "Attk": 548,
      "AttkRate": 87,
      "Int": 247,
      "IntRate": 40,
      "Def": 357,
      "DefRate": 82,
      "MagicDef": 207,
      "MagicDefRate": 28,
      "Skill": 106,
      "SkillRate": 37,
      "HeartFetterBouns1": "공격하여 전투진입 시<br/> 부대 받는 피해감소 <font color=\"red\">-10</font>%",
      "HeartFetterBouns2": "공격받아 전투진입 시<br?> 입히는 피해  <font color=\"red\">+10</font>%"
    },
    {
      "Name": "로열나이트",
      "Icon": "Icon_Occupation_Cavalry.webp",
      "SD": [
        {
          "ImageUrl": "힐다/1/2.webp"
        },
        {
          "ImageUrl": "힐다/2/2.webp"
        },
        {
          "ImageUrl": "힐다/3/2.webp"
        }
      ],
      "AttkRange": 1,
      "Move": 5,
      "MoveType": "Ride",
      "HP": 5057,
      "HPRate": 89,
      "Attk": 577,
      "AttkRate": 87,
      "Int": 247,
      "IntRate": 41,
      "Def": 311,
      "DefRate": 70,
      "MagicDef": 260,
      "MagicDefRate": 60,
      "Skill": 118,
      "SkillRate": 33,
      "HeartFetterBouns1": "부대 체력이 <font color=\"red\">80</font>% 이상일 시,<br/> 받는 치명타확률 <font color=\"red\">-10</font>% 감소",
      "HeartFetterBouns2": "공격하여 전투진입 시,<br/> 주는 피해량 <font color=\"red\">+10</font>% 증가"
    }
  ],
  "Materials": {
    "HeartFetter": [
      "h4",
      "h1",
      "h3",
      "h2",
      "h4-1",
      "h1-1",
      "h3-1",
      "h2-1",
      "x"
    ],
    "Fetter": [
      "2",
      "2-1",
      "2-2",
      "2-3",
      "2-4"
    ],
    "Awaken": [
      "ranka2",
      "ranka6",
      "ranka2-1",
      "ranka6-1"
    ]
  },
  "ClassUp": [
    {
      "ClassTitle": "기병 : 생명력+36, 공격+9",
      "Tiers": [
        {
          "Name": "T1",
          "Materials": [
            "rank1-3"
          ]
        },
        {
          "Name": "T2",
          "Materials": [
            "rank1-3"
          ]
        }
      ]
    },
    {
      "ClassTitle": "소드우먼 : 생명+80, 기술+5",
      "Tiers": [
        {
          "Name": "T1",
          "Materials": [
            "rank2-1",
            "rank1-1",
            "rank1-2"
          ]
        },
        {
          "Name": "T2",
          "Materials": [
            "rank3-1",
            "rank3-6",
            "rank2-1",
            "rank2-2"
          ]
        },
        {
          "Name": "T3",
          "Materials": [
            "rank4-1",
            "rank4-2",
            "rank3-1",
            "rank3-6"
          ]
        }
      ]
    },
    {
      "ClassTitle": "제네럴 : 생명+80, 공격+7",
      "Tiers": [
        {
          "Name": "T1",
          "Materials": [
            "rank2-2",
            "rank1-2",
            "rank1-1"
          ]
        },
        {
          "Name": "T2",
          "Materials": [
            "rank3-6",
            "rank3-1",
            "rank2-2",
            "rank2-1"
          ]
        },
        {
          "Name": "T3",
          "Materials": [
            "rank4-2",
            "rank4-6",
            "rank3-6",
            "rank3-1"
          ]
        }
      ]
    },
    {
      "ClassTitle": "마샬 : 생명+150, 공격+15, 방어+10",
      "Tiers": [
        {
          "Name": "T1",
          "Materials": [
            "rank5-6",
            "rank5-3",
            "rank4-2",
            "rank4-6"
          ]
        },
        {
          "Name": "T2",
          "Materials": [
            "rank6-2",
            "rank6-1",
            "rank5-6",
            "rank5-3"
          ]
        },
        {
          "Name": "T3",
          "Materials": [
            "rank7-1",
            "rank7-7",
            "rank6-2",
            "rank6-1"
          ]
        },
        {
          "Name": "T4",
          "Materials": [
            "rank8-2",
            "rank8-1",
            "rank7-1",
            "rank7-7"
          ]
        }
      ]
    },
    {
      "ClassTitle": "하이랜더 : 생명+48, 공격+12",
      "Tiers": [
        {
          "Name": "T1",
          "Materials": [
            "rank2-3",
            "rank1-3",
            "rank1-1"
          ]
        },
        {
          "Name": "T2",
          "Materials": [
            "rank3-3",
            "rank3-2",
            "rank2-3",
            "rank2-1"
          ]
        },
        {
          "Name": "T3",
          "Materials": [
            "rank4-6",
            "rank4-1",
            "rank3-3",
            "rank3-2"
          ]
        }
      ]
    },
    {
      "ClassTitle": "로열나이트 : 생명+150, 공격+15, 방어+10",
      "Tiers": [
        {
          "Name": "T1",
          "Materials": [
            "rank5-3",
            "rank5-1",
            "rank4-6",
            "rank4-1"
          ]
        },
        {
          "Name": "T2",
          "Materials": [
            "rank6-6",
            "rank6-2",
            "rank5-3",
            "rank5-1"
          ]
        },
        {
          "Name": "T3",
          "Materials": [
            "rank7-7",
            "rank7-1",
            "rank6-6",
            "rank6-2"
          ]
        },
        {
          "Name": "T4",
          "Materials": [
            "rank8-7",
            "rank8-3",
            "rank7-7",
            "rank7-1"
          ]
        }
      ]
    }
  ],
  "SPOpenConditions": null,
  "Skills": [
    {
      "Name": "진형파괴",
      "Cost": "COST 2",
      "Entries": [
        {
          "Label": "유형",
          "Value": "물리 피해"
        },
        {
          "Label": "쿨타임",
          "Value": "3턴"
        },
        {
          "Label": "사거리",
          "Value": "1칸"
        },
        {
          "Label": "범위",
          "Value": "단일"
        }
      ],
      "IconImageUrl": "https://storage.googleapis.com/langmosheet-data/img/skills/Skill_KnightCrash3.webp",
      "Desc": "[물리 피해]단일 적 부대를 공격해 <font color=\"red\">1</font>배의 공격 피해를 준다.<br/>대상을 <font color=\"red\">2</font>칸 밀쳐내며 다음 턴 행동 불가 부여.",
      "SubSkills": null,
      "Animation": null
    },
    {
      "Name": "격돌",
      "Cost": "COST 1",
      "Entries": [
        {
          "Label": "유형",
          "Value": "물리 피해"
        },
        {
          "Label": "쿨타임",
          "Value": "2턴"
        },
        {
          "Label": "사거리",
          "Value": "1칸"
        },
        {
          "Label": "범위",
          "Value": "단일"
        }
      ],
      "IconImageUrl": "https://storage.googleapis.com/langmosheet-data/img/skills/Passive_Punch.webp",
      "Desc": "[물리 피해]단일 적 부대를 공격해 1.2배의 피해를 준다.<br/>대상을 <font color=\"red\">2</font>칸 밀쳐내며 이동력 <font color=\"red\">-2</font>, '호위 불가'부여, <font color=\"red\">1</font>턴 지속.",
      "SubSkills": null,
      "Animation": null
    },
    {
      "Name": "부러진 창",
      "Cost": "COST 1",
      "Entries": [
        {
          "Label": "유형",
          "Value": "패시브"
        }
      ],
      "IconImageUrl": "https://storage.googleapis.com/langmosheet-data/img/skills/Hilder/1.webp",
      "Desc": "[패시브]근접 공격으로 전투 진입 시 공격 <font color=\"red\">+15</font>%,<br/>전투 전 <b><font color=\"blue\">적의 무기 스킬 무효화</font>(최대 생명 수치 제외)</b>,<br/>목표가 NPC 일 경우 해당 효과는 '공격 지력 <font color=\"red\">-25</font>%'로 대체, <br/>지속 <font color=\"red\">1</font>행동",
      "SubSkills": null,
      "Animation": null
    },
    {
      "Name": "전생의 결의",
      "Cost": "COST 2",
      "Entries": [
        {
          "Label": "유형",
          "Value": "초절강화"
        },
        {
          "Label": "쿨타임",
          "Value": "3턴"
        },
        {
          "Label": "사거리",
          "Value": "자신"
        },
        {
          "Label": "범위",
          "Value": "맵전체"
        }
      ],
      "IconImageUrl": "https://storage.googleapis.com/langmosheet-data/img/skills/SuperBuff_Reincar2.webp",
      "Desc": "[초절강화] 액티브 스킬, 범위 내 모든 '<b>전생</b>' 아군의 <br/>공격, 방어 능력치가 크게 상승하고<br/>단일 개체 스킬 피해 <font color=\"red\">+12</font>% 및 반격 피해 <font color=\"red\">+12</font>%, 지속 <font color=\"red\">4</font>행동<br/>(이효과는 다른 초절강화 스킬과 중첩불가)",
      "SubSkills": null,
      "Animation": null
    },
    {
      "Name": "방어 진형",
      "Cost": "COST 2",
      "Entries": [
        {
          "Label": "유형",
          "Value": "물리 피해"
        },
        {
          "Label": "쿨타임",
          "Value": "2턴"
        },
        {
          "Label": "사거리",
          "Value": "5칸"
        },
        {
          "Label": "범위",
          "Value": "직선"
        }
      ],
      "IconImageUrl": "https://storage.googleapis.com/langmosheet-data/img/skills/Hilder/2.webp",
      "Desc": "[패시브] 인접한 아군이 받는 모든 공격 호위<br/>[액티브]직선 상 적에게 <font color =\"red\">0.1</font>배 물리 피해 및 자신의 호위 범위 <font color=\"red\">2</font>칸으로 증가   <br/>피격 치명타 확률 <font color=\"red\">-20</font>%, 지속 <font color=\"red\">2</font>행동<br/>적이 혼합 부대일 경우 해당 고정 피해는 면역 불가.<br/>스킬 명중한 지형에 '<b><font color=\"blue\">함몰</font></b>' 효과 부여 : <br/>해당 지형으로 이동한 적군은 '<b><font color=\"blue\">재이동</font> 및 <font color=\"blue\">재행동</font></b> 불가'",
      "SubSkills": null,
      "Animation": null
    },
    {
      "Name": "창진",
      "Cost": "COST 2",
      "Entries": [
        {
          "Label": "유형",
          "Value": "액티브"
        },
        {
          "Label": "쿨타임",
          "Value": "2턴"
        },
        {
          "Label": "사거리",
          "Value": "자신"
        },
        {
          "Label": "범위",
          "Value": "단일"
        }
      ],
      "IconImageUrl": "https://storage.googleapis.com/langmosheet-data/img/skills/Skill_BitterlyHit.webp",
      "Desc": "[패시브]근접한 아군 부대를 대신해 물리 공격을 받는다.<br/>[액티브]호위 범위가 <font color=\"#DC143C\">2칸</font>으로 늘어나며 방어 <font color=\"#DC143C\">+20%</font>.<br/>근접 전투 종료 후 [미늘]의 고정 피해가 반드시 발동한다. <font color=\"#DC143C\">2턴</font> 지속.",
      "SubSkills": null,
      "Animation": null
    },
    {
      "Name": "방패벽",
      "Cost": "COST 2",
      "Entries": [
        {
          "Label": "유형",
          "Value": "패시브"
        }
      ],
      "IconImageUrl": "https://storage.googleapis.com/langmosheet-data/img/skills/Passive_Parry.webp",
      "Desc": "[패시브]근접 공격을 받아 전투 진입 시 <font color=\"red\">25%</font>확률로 발동, 받는 물리 피해량 <font color=\"red\">-50%</font>.",
      "SubSkills": null,
      "Animation": null
    },
    {
      "Name": "부러진 화살",
      "Cost": "COST 1",
      "Entries": [
        {
          "Label": "유형",
          "Value": "패시브"
        }
      ],
      "IconImageUrl": "https://storage.googleapis.com/langmosheet-data/img/skills/Hilder/3.webp",
      "Desc": "[패시브]<b>원거리 <font color =\"blue\">물리 공격</font>받아 전투 진입 시</b> 공격 <font color=\"red\">+15</font>%, 받는 피해량 <font color=\"red\">-10</font>%<br/>최대 <font color=\"red\">3</font>칸 <b><font color=\"blue\">원거리 물리 공격 반격</font></b>",
      "SubSkills": null,
      "Animation": null
    },
    {
      "Name": "핏빛 호령",
      "Cost": "COST 3",
      "Entries": [
        {
          "Label": "유형",
          "Value": "변환"
        },
        {
          "Label": "쿨타임",
          "Value": "-"
        },
        {
          "Label": "사거리",
          "Value": "자신"
        },
        {
          "Label": "범위",
          "Value": "단일"
        }
      ],
      "IconImageUrl": "../img/Super/힐다.webp",
      "Desc": "[패시브] 인접한 아군이 받는 모든 공격 호위, '<b><font color=\"blue\">이동력 저하</font></b>' 면역. <br/>[지휘] 주위 <font color=\"red\">2</font>칸 범위 내 모든 아군은 위치 이동 면역, 피격 고정 피해 <font color=\"red\">-35</font>%,<br/>[액티브] 이하 스킬 중 <font color=\"red\">1</font>개 선택<br/>액티브 스킬 사용 시 쿨타임 <font color=\"red\">3</font>턴으로 변경",
      "SubSkills": [
        {
          "Name": "<b><font color=\"#0070c0\">호령-창진</font></b>",
          "Entries": null,
          "IconImageUrl": null,
          "Desc": "[액티브] 호위 범위 <font color=\"red\">2</font>칸으로 확장. 지휘 범위 <font color=\"red\">3</font>칸으로 확장<br/>영웅 병종을 <img src=\"../img/troop/Icon_Occupation_Marines.webp\" width=\"20px\"><font color=\"green\">창병</font>으로 변경 <br/>피격으로 전투 진입 전 적에게 이동한 거리 <font color=\"red\">1</font>칸 당 받는 피해 <font color=\"red\">+10</font>% 부여 <br/>(최대 <font color=\"red\">50</font>%), 지속 <font color=\"red\">3</font>행동"
        },
        {
          "Name": "<b><font color=\"#0070c0\">호령-기마돌격</font></b>",
          "Entries": null,
          "IconImageUrl": null,
          "Desc": "[액티브] 호위 범위 <font color=\"red\">2</font>칸으로 확장. 지휘 범위 <font color=\"red\">3</font>칸으로 확장<br/>영웅 병종을 <img src=\"../img/troop/Icon_Occupation_Cavalry.webp\" width=\"20px\"><font color=\"blue\">기병</font>으로 변경 <br/>보병과 전투 시 피해 <font color=\"red\">+20</font>%, 지속 <font color=\"red\">3</font>행동"
        }
      ],
      "Animation": "../img/Super/힐다_Ani.webp"
    }
  ],
  "Propose": true,
  "Bio": [
    {
      "Entry": "영웅소개",
      "Desc": "◆키/몸무게: 173cm / 53kg<br/>◆출연: 랑그릿사 리인카네이션 -전생-<br/>◆신체 사이즈: 92/63/88<br/>◆소속: 그레스덴 제국"
    },
    {
      "Entry": "영웅담1",
      "Desc": "그레스덴 제국의 여자 원수. 명문세가인 유스티스 가문의 장녀로 <br/>제국 재상 플로렌티아의 친언니이기도 하다. <br/>미소지은 채 무섭고 잔인한 말을 내뱉으며, 죽음에 대해서도 일반 사람을 훨씬 뛰어넘는 <br/>무자비한 여신처럼 죽음에 대해서도 다른 사람보다 훨씬 냉소적인 태도를 취한다.<br/>이 매력적이고 강한 여자 원수에게는 수많은 무서운 소문이 뒤따르고 있으며, <br/>적의 눈에 비친 힐다는 그야말로 광기어린 악몽 그 자체라 할 수 있다.<br/><font color=\"#DC143C\"><size=33><i>「후후후, 다 큰 어른이 전장에서 죽는 자들을 동정한다고? 흐음... 꽤나 귀엽게 구네.」</i></size=33></font>"
    },
    {
      "Entry": "영웅담2",
      "Desc": "과거 힐다는 기품있고 강인한 귀족 아가씨였으나 양친이 <br/>적군에게 살해당하는 참상을 직접 목격하면서 무력한 자신을 원망하게 되었다. <br/>그리고 힐다는 인자하고 부드러운 감정을 철저히 묻어버리고 제국 사관 학교에 들어가게 된다.<br/>그 후 힐다는 절대적인 승리와 철저한 복수만을 추구하며 살게 되었다.<br/><font color=\"#DC143C\"><size=33><i>「이 멈춘 회중시계는... 순진한 힐다가 부모와 함께 죽은 순간을 기념하는 거라고 할 수 있지.」</i></size=33></font>"
    },
    {
      "Entry": "영웅담3",
      "Desc": "제국 사관 학교에서 놀랄만한 실전 능력과 전술적인 재능을 보여준 힐다는 <br/>잔혹한 전쟁을 수없이 겪으며 자신을 피와 영광으로 단련했다.<br/>전신에 피칠갑을 한 채 승리의 언덕에서 발 아래의 적을 굽어볼 때의 힐다는 <br/>마치 얼굴에 잔인한 미소를 짓는 가면을 쓴 듯 했다.<br/>'핏빛 복수의 여신'이라는 칭호는 힐다가 내거는 승리의 깃발과 함께 전 대륙에 전해지게 된다.<br/><font color=\"#DC143C\"><size=33><i>「무기에 새겨진 승리의 인장은 내가 겪어온 전장을 보여주는 <br/>영광스러운 훈장이자 훗날의 내 묘비명이라고 할 수 있지.」</i></size=33></font>"
    },
    {
      "Entry": "영웅담4",
      "Desc": "힐다는 복수에 대한 강한 집착을 갖고 있다. <br/>그래서 힐다는 '엘리시온 계획'을 위해 원수와 손을 잡으려는 여동생을 쉽사리 용서할 수 없었다. <br/>힐다가 보기에 그것은 일종의 배반과 같았으므로. <br/>하지만 유스티스 가문을 짊어져야 한다는 사명과 제국 군인으로서의 영광, <br/>그리고 책임감이 힐다로 하여금 결국 여동생의 곁에 서도록 만들었고, <br/>또한 운명에 저항하는 웅대한 계획을 돕게 만들었다.<br/>여전히 힐다는 매일 밤 꿈을 꾼다. 도망칠 수도 없는, 부모가 죽는 그 악몽을..."
    },
    {
      "Entry": "영웅담5",
      "Desc": "새로운 대륙에 소환된 힐다는 얼마 지나지 않아 <br/>자신의 원수들이 오래전에 역사의 먼지로 변해버렸다는 것과, <br/>어쩔 수 없이 자신의 책임과 사명을 다시 찾아내야 한다는 것을 깨달았다.<br/>하지만 여동생과의 갈등과 오해는 어떻게 해야 풀어낼 수 있을지...<br/><font color=\"#DC143C\"><size=33><i>「플로레는 아직도 그 번거로운 작전과 순진한 생각을 고집하고 있는거야? <br/>후후, 그렇게 어리숙한 재상 나으리 함께 해야 한다니, 앞날이 걱정되지 않아?」</i></size=33></font>"
    }
  ],
  "Performance": [
    {
      "Entry": "「소환 - 등장대사」",
      "Desc": "흐음? 너 같은 녀석이 절대적인 승리를 갈망하고 있다고? 그렇다면 내 발목이나 잡지나 말라고? 후후후"
    },
    {
      "Entry": "「전투 - 전직1」",
      "Desc": "후후후, 나는 훨씬 더 거대한 승리를 갈망하고 있다고 "
    },
    {
      "Entry": "「전투 - 전직2」",
      "Desc": "이 힘은... 아하하... 아무래도 내 무기에 핏자국이 더욱 늘어날거 같구나"
    },
    {
      "Entry": "「전투 - 스킬시전1」",
      "Desc": "아하하하하!"
    },
    {
      "Entry": "「전투 - 스킬시전2」",
      "Desc": "저리 꺼져라! 나약한 것!"
    },
    {
      "Entry": "「전투 - 선택1」",
      "Desc": "내게 명령하는걸까?"
    },
    {
      "Entry": "「전투 - 선택2」",
      "Desc": "어머? 죽여버리면 되잖아. 아하하"
    },
    {
      "Entry": "「전투 - 전투불능」",
      "Desc": "나의 피도 새빨갰었구나..."
    },
    {
      "Entry": "「대화 - 친밀도1」",
      "Desc": "플로레는 아직도 그 번거로운 작전과 순진한 생각을 고집하고 있는거야? <br/>후후, 그렇게 어리숙한 재상 나으리 함께 해야 한다니, 앞날이 걱정되지 않아?"
    },
    {
      "Entry": "「대화 - 친밀도2」",
      "Desc": "후후후, 다 큰 어른이 전장에서 죽는 자들을 동정한다고? 흐음... 꽤나 귀엽게 구네."
    },
    {
      "Entry": "「대화 - 친밀도3」",
      "Desc": "약자는 전장에 나설 자격이 없다. 짓밟힐 운명에서 벗어날 수 없기 때문이지<br/>그렇게 간단한 것을 아직도 모르는 거야?"
    },
    {
      "Entry": "「대화 - 친밀도4」",
      "Desc": "내 무기의 흠집 갯수를 세는거야? 귀엽구나... <br/>그래, 이 흠집의 수는 내가 승리한 횟수야. 그리고 나의 전쟁이 남긴 명예로운 한장이지"
    },
    {
      "Entry": "「대화 - 친밀도5」",
      "Desc": "이 멈춘 회중시계는... <br/>순진한 힐다가 부모와 함께 죽은 순간을 기념하는 거라고 할 수 있지."
    },
    {
      "Entry": "「대화 - 일상1」",
      "Desc": "포로를 고문할때, 포로가 가장 소중히 여기는것부터 파괴하는거야,<br/> 그렇지... 그리고 나서 이익이 될 만한것을 제시하는 것도 괜찮아. <br/>고통스러울거 같다고? 아하하하.<br/>내 경험담이였는데 말이야. 좀더 듣고싶지 않니?"
    },
    {
      "Entry": "「대화 - 기쁨」",
      "Desc": "이상하구나, 혹시 「핏빛 복수의 여신」라는 별명에 대해 모르는거 같은데? <br/>나를 전혀 무서워하지 않는거 같은데... <br/>후후후 몰라서 그런걸까? 너는 참 재미있구나"
    },
    {
      "Entry": "「대화 - 분노」",
      "Desc": "잊을래야 잊을수가 없는 아픔이 있지. 이 가증스러운 아픔은 매일 나를 괴롭히고 있지. <br/>후후후... 내 고통을 씻어낼수 있는 건 적들이 내뿜는 피 뿐이라고!!"
    },
    {
      "Entry": "「대화 - 슬픔」",
      "Desc": "그때 내게 힘이 있었다면... 내 눈 앞에 있는 행복을 지킬수 있었을텐데... <br/>어머? 어째서 내가 이런 말을 하는걸까?"
    },
    {
      "Entry": "「대화 - 수줍음」",
      "Desc": "어머어머, 나는 네가 나를 쳐다볼때의 눈빛을 좋아한다고? <br/>마치 승자의 당당함이 느껴지는거 같아서 말이야"
    },
    {
      "Entry": "「상호작용 - 대사1」",
      "Desc": "어머나? 어째서 나한테 그런짓을 하는지는 모르겠지만 <br/>그래도 조금은 기쁘다고 느껴버렸어"
    },
    {
      "Entry": "「상호작용 - 대사2」",
      "Desc": "적에게는 돌만 던져댈수밖에 없었던 그 여린 소녀... <br/>그것은 내겐 악몽같은 거였어<br/> 그래도 지금은 적들에게는 악몽과 같은 존재니까 후후후"
    },
    {
      "Entry": "「상호작용 - 대사3」",
      "Desc": "내가 허가할때 까지는 너무 떨어지지 마라. <br/> 흐응? 후후후후 오해하지 말라고. <br/>이것은 부탁이 아니라 명령이니까"
    },
    {
      "Entry": "「상호작용 - 호감1」",
      "Desc": "어머... 움직이지 마렴, 너에게서 달콤한 승리의 냄새가 나니까 말이야"
    },
    {
      "Entry": "「상호작용 - 반감1」",
      "Desc": "아하하... 내게 고문을 당하고 싶다고?"
    },
    {
      "Entry": "「상호작용 - 특수1」",
      "Desc": "내일도 살아있는 널 만나고 싶어져.<br/>어머? 어째서 나를 그런 표정으로 보는걸까?"
    },
    {
      "Entry": "「상호작용 - 특수2」",
      "Desc": "여기까지 하도록 하죠"
    }
  ]
}];