var data = [
{
  "Skins": [
    {
      "ImageUrl": "알파/1.webp",
      "Desc": "기본"
    },
    {
      "ImageUrl": "알파/2.webp",
      "Desc": "빛의 메아리"
    },
    {
      "ImageUrl": "알파/3.webp",
      "Desc": "한정 판매"
    }
  ],
  "Name": "알파",
  "ChName": "阿尔法",
  "EngName": "Alpha",
  "JapName": "",
  "CV": "이토 시즈카",
  "Rank": [
    0
  ],
  "SP": false,
  "Groups": [
    2,
    4
  ],
  "HP": 10,
  "SPHP": 0,
  "Attk": 40,
  "SPAttk": 0,
  "Def": 25,
  "SPDef": 0,
  "MagicDef": 25,
  "SPMagicDef": 0,
  "HowToGet": "영웅 소환",
  "Fetters": [
    {
      "Condition": "알파를 출전시켜 아레나 5승 달성"
    },
    {
      "Condition": "알파를 출전시켜 시공의 균열 3-1[정예] 클리어"
    },
    {
      "Condition": "입실론의 도움 받아 운명의문4 클리어"
    },
    {
      "Condition": "초월자의 도움 받아 운명의문5 클리어"
    }
  ],
  "HeartFetter": "생명+900, 방어+60, 마방+30",
  "Talent": [
    {
      "TalentName": "불멸의 자수정",
      "Icon": "Gift_Alpha.webp",
      "Abilities": [
        {
          "Desc": "1성"
        },
        {
          "Desc": "2성"
        },
        {
          "Desc": "피해량 <font color=\"red\">+5%</font>.<br/><b>상성 관계의 부대 와 전투 시</b> 공격, 방어<font color=\"red\">+5%</font><br/><b>스테이지 진입 후 자신에게 무작위로 '<font color=\"purple\">자수정 장막</font>'이나 '<font color=\"purple\">자수정 방패</font>'중 <font color=\"red\">1</font>개 획득</b><br/><font color=\"purple\">자수정 장막</font> : 받는 마법 피해량 <font color =\"red\">-20%</font> <br/><font color=\"purple\">자수정 방패</font> : 받는 물리 피해량 <font color =\"red\">-20%</font> <br/>'<font color=\"purple\">자수정 장막</font>', '<font color=\"purple\">자수정 방패</font>'는 <b>동시 발동 불가</b><br/><b>면역 및 해제 불가</b><br/><b>턴 시작 시 상호 교체</b>"
        },
        {
          "Desc": "피해량 <font color=\"red\">+8%</font>.<br/><b>상성 관계의 부대 와 전투 시</b> 공격, 방어<font color=\"red\">+8%</font><br/><b>스테이지 진입 후 자신에게 무작위로 '<font color=\"purple\">자수정 장막</font>'이나 '<font color=\"purple\">자수정 방패</font>'중 <font color=\"red\">1</font>개 획득</b><br/><font color=\"purple\">자수정 장막</font> : 받는 마법 피해량 <font color =\"red\">-25%</font> <br/><font color=\"purple\">자수정 방패</font> : 받는 물리 피해량 <font color =\"red\">-25%</font> <br/>'<font color=\"purple\">자수정 장막</font>', '<font color=\"purple\">자수정 방패</font>'는 <b>동시 발동 불가</b><br/><b>면역 및 해제 불가</b><br/><b>턴 시작 시 상호 교체</b>"
        },
        {
          "Desc": "피해량 <font color=\"red\">+11%</font>.<br/><b>상성 관계의 부대 와 전투 시</b> 공격, 방어<font color=\"red\">+11%</font><br/><b>스테이지 진입 후 자신에게 무작위로 '<font color=\"purple\">자수정 장막</font>'이나 '<font color=\"purple\">자수정 방패</font>'중 <font color=\"red\">1</font>개 획득</b><br/><font color=\"purple\">자수정 장막</font> : 받는 마법 피해량 <font color =\"red\">-30%</font> <br/><font color=\"purple\">자수정 방패</font> : 받는 물리 피해량 <font color =\"red\">-30%</font> <br/>'<font color=\"purple\">자수정 장막</font>', '<font color=\"purple\">자수정 방패</font>'는 <b>동시 발동 불가</b><br/><b>면역 및 해제 불가</b><br/><b>턴 시작 시 상호 교체</b>"
        },
        {
          "Desc": "피해량 <font color=\"red\">+15%</font>.<br/><b>상성 관계의 부대 와 전투 시</b> 공격, 방어<font color=\"red\">+15%</font><br/><b>스테이지 진입 후 자신에게 무작위로 '<font color=\"purple\">자수정 장막</font>'이나 '<font color=\"purple\">자수정 방패</font>'중 <font color=\"red\">1</font>개 획득</b><br/><font color=\"purple\">자수정 장막</font> : 받는 마법 피해량 <font color =\"red\">-40%</font> <br/><font color=\"purple\">자수정 방패</font> : 받는 물리 피해량 <font color =\"red\">-40%</font> <br/>'<font color=\"purple\">자수정 장막</font>', '<font color=\"purple\">자수정 방패</font>'는 <b>동시 발동 불가</b><br/><b>면역 및 해제 불가</b><br/><b>턴 시작 시 상호 교체</b>"
        }
      ]
    }
  ],
  "SPTalent":null,
  "Second_Awaken":{
    "Awaken_Name" : "수정 코어 충전",
    "Stats":"공격, 방어 <font color=\"red\">+5%</font><br/><font color=\"blue\">패시브 봉인 면역</font><br/><font color=\"blue\">자수정 폭발</font> 스킬 피해량<font color=\"red\">+10%</font>"
  },
  "Equip": {
    "EqpName": "개조육체 α EX(갑옷)",
    "Stats": "생명 437,방어 65",
    "Icon": "알파.webp",
    "Ability": "생명,방어 <font color=\"red\">+5%</font> 이동력 <font color=\"red\">+1</font><br/>자신에게 '<font color=\"purple\">자수정 방패</font>'가 있을 경우 피격 물리 공격을 피해량 <font color=\"red\">60%</font> <font color=\"green\">반사</font><br/>'<font color=\"purple\">자수정 장막</font>'이 있을 경우 피격 마법 공격을 피해량 <font color=\"red\">60%</font> <font color=\"green\">반사</font><br/>둘다 없는 경우 영웅이 주는 피해량 <font color=\"red\">+15%</font>"
  },
  "Soldiers": [
    28,
    36,
    25,
    22,
    75,
    26,
    33,
    21,
    77,
    78,
    94,
    101
  ],
  "Jobs": [
    {
      "Name": "소드 마스터",
      "Icon": "Icon_Occupation_Infantryman.webp",
      "SD": [
        {
          "ImageUrl": "알파/1/1.webp"
        },
        {
          "ImageUrl": "알파/2/1.webp"
        },
        {
          "ImageUrl": "알파/3/1.webp"
        }
      ],
      "AttkRange": 1,
      "Move": 3,
      "MoveType": "Walk",
      "HP": 3933,
      "HPRate": 78,
      "Attk": 579,
      "AttkRate": 89,
      "Int": 256,
      "IntRate": 43,
      "Def": 321,
      "DefRate": 76,
      "MagicDef": 253,
      "MagicDefRate": 58,
      "Skill": 154,
      "SkillRate": 44,
      "HeartFetterBouns1": "<b>선공으로 전투 진입 시</b><br/>부대가 받는 피해량 <font color=\"red\">-10%</font>",
      "HeartFetterBouns2": "<b>근접 공격 받아 전투 진입 시</b><br/>주는 피해량 <font color=\"red\">+10%</font>",
    },
    {
      "Name": "애머시스트 마스터",
      "Icon": "Icon_Occupation_Marines.webp",
      "SD": [
        {
          "ImageUrl": "알파/1/2.webp"
        },
        {
          "ImageUrl": "알파/2/1.webp"
        },
        {
          "ImageUrl": "알파/3/1.webp"
        }
      ],
      "AttkRange": 1,
      "Move": 3,
      "MoveType": "Walk",
      "HP": 4863,
      "HPRate": 80,
      "Attk": 579,
      "AttkRate": 88,
      "Int": 233,
      "IntRate": 39,
      "Def": 357,
      "DefRate": 79,
      "MagicDef": 220,
      "MagicDefRate": 48,
      "Skill": 172,
      "SkillRate": 56,
      "HeartFetterBouns1": "<b>원거리 공격 받아 전투 진입 시</b><br/>받는 피해량 <font color=\"red\">-10%</font>",
      "HeartFetterBouns2": "<b>디버프가 걸린 부대와 전투 시</b><br/>주는 피해량 <font color=\"red\">+10%</font>"
    }
  ],
  "Materials": {
    "HeartFetter": [
      "h3",
      "h4",
      "h2",
      "h1",
      "h3-1",
      "h4-1",
      "h2-1",
      "h1-1",
      "x"
    ],
    "Fetter": [
      "3",
      "3-1",
      "3-2",
      "3-3",
      "3-4"
    ],
    "Awaken": [
      "ranka3",
      "ranka1",
      "ranka3-1",
      "ranka1-1"
    ]
  },
  "ClassUp": [
    {
      "ClassTitle": "서전트 : 공격+5, 방어+6",
      "Tiers": [
        {
          "Name": "T1",
          "Materials": [
            "rank1-2"
          ]
        },
        {
          "Name": "T2",
          "Materials": [
            "rank1-2"
          ]
        }
      ]
    },
    {
      "ClassTitle": "소드맨 : 생명+80, 기술+5",
      "Tiers": [
        {
          "Name": "T1",
          "Materials": [
            "rank2-1",
            "rank1-2",
            "rank1-1"
          ]
        },
        {
          "Name": "T2",
          "Materials": [
            "rank3-6",
            "rank3-1",
            "rank2-2",
            "rank2-1"
          ]
        },
        {
          "Name": "T3",
          "Materials": [
            "rank4-1",
            "rank4-2",
            "rank3-6",
            "rank3-1"
          ]
        }
      ]
    },
    {
      "ClassTitle": "소드 마스터 : 공격+25, 방어+10, 마방+10",
      "Tiers": [
        {
          "Name": "T1",
          "Materials": [
            "rank5-1",
            "rank5-6",
            "rank4-1",
            "rank4-2"
          ]
        },
        {
          "Name": "T2",
          "Materials": [
            "rank6-1",
            "rank6-2",
            "rank5-1",
            "rank5-6"
          ]
        },
        {
          "Name": "T3",
          "Materials": [
            "rank7-2",
            "rank7-1",
            "rank6-1",
            "rank6-2"
          ]
        },
        {
          "Name": "T4",
          "Materials": [
            "rank8-2",
            "rank8-1",
            "rank7-1",
            "rank7-7"
          ]
        }
      ]
    },
    {
        "ClassTitle": "투사 : 생명+48, 공격+12",
        "Tiers": [
          {
            "Name": "T1",
            "Materials": [
              "rank2-2",
              "rank1-1",
              "rank1-2"
            ]
          },
          {
            "Name": "T2",
            "Materials": [
              "rank3-1",
              "rank3-6",
              "rank2-2",
              "rank2-6"
            ]
          },
          {
            "Name": "T3",
            "Materials": [
              "rank4-2",
              "rank4-6",
              "rank3-1",
              "rank3-6"
            ]
          }
        ]
      },
    {
      "ClassTitle": "애머시스트 마스터 : 생명+90, 공격+25, 방어+10",
      "Tiers": [
        {
          "Name": "T1",
          "Materials": [
            "rank5-6",
            "rank5-3",
            "rank4-2",
            "rank4-6"
          ]
        },
        {
          "Name": "T2",
          "Materials": [
            "rank6-2",
            "rank6-1",
            "rank5-6",
            "rank5-3"
          ]
        },
        {
          "Name": "T3",
          "Materials": [
            "rank7-1",
            "rank7-7",
            "rank6-2",
            "rank6-1"
          ]
        },
        {
          "Name": "T4",
          "Materials": [
            "rank8-2",
            "rank8-1",
            "rank7-1",
            "rank7-7"
          ]
        }
      ]
    }
  ],
  "SPOpenConditions": null,
  "Skills": [
    {
        "Name": "관통",
        "Cost": "COST 2",
        "Entries": [
          {
            "Label": "유형",
            "Value": "물리 피해"
          },
          {
            "Label": "쿨타임",
            "Value": "2턴"
          },
          {
            "Label": "사거리",
            "Value": "1칸"
          },
          {
            "Label": "범위",
            "Value": "단일"
          }
        ],
        "IconImageUrl": "../img/skills/43.webp",
        "Desc": "[물리 피해] 단일 적 부대를 공격해 <font color=\"red\">1.5</font>배의 전투 피해를 준다.<br/>전투 후, 적 방어 <font color=\"red\">-30%</font>, <font color=\"red\">2</font>행동 지속.",
        "SubSkills": null,
        "Animation": null
    },
    {
        "Name": "증원",
        "Cost": "COST 1",
        "Entries": [
          {
            "Label": "유형",
            "Value": "패시브"
          }
        ],
        "IconImageUrl": "../img/skills/Passive_PHeal1.webp",
        "Desc": "행동 종료 시 자신 부대 생명 <font color=\"red\">20%</font> <font color=\"green\">회복</font>.",
        "SubSkills": null,
        "Animation": null
    },
    {
      "Name": "힘의 외침",
      "Cost": "COST 2",
      "Entries": [
        {
          "Label": "유형",
          "Value": "물리 피해"
        },
        {
          "Label": "쿨타임",
          "Value": "2턴"
        },
        {
          "Label": "사거리",
          "Value": "1칸"
        },
        {
          "Label": "범위",
          "Value": "단일"
        }
      ],
      "IconImageUrl": "../img/skills/36.webp",
      "Desc": "[물리 피해] 단일 적군을 공격하여 <font color=\"red\">1.3</font>배 피해를 준다.<br/>전투 전 상대 버프 <font color=\"red\">2</font>개 제거와 동시에 상대의 공격 및 지력 <font color=\"red\">-20%</font> <br/><font color=\"red\">1</font>행동 지속.",
      "SubSkills": null,
      "Animation": null
    },
    {
        "Name": "응수",
        "Cost": "COST 1",
        "Entries": [
          {
            "Label": "유형",
            "Value": "패시브"
          }
        ],
        "IconImageUrl": "../img/skills/Skill_Parry.webp",
        "Desc": "[패시브]근접한 아군 부대를 대신해 물리 공격을 받는다.<br/>공격을 받을 시 공격 <font color=\"red\">7%</font> 만큼 방어 상승",
        "SubSkills": null,
        "Animation": null
    },
    {
        "Name": "수정 칼날",
        "Cost": "COST 2",
        "Entries": [
          {
            "Label": "유형",
            "Value": "선택"
          },          
          {
            "Label": "쿨타임",
            "Value": "2턴"
          },
          {
            "Label": "사거리",
            "Value": ""
          },
          {
            "Label": "범위",
            "Value": ""
          }
        ],
        "IconImageUrl": "../img/skills/Alpha/1.webp",
        "Desc": "[액티브]이하 스킬 중 하나 선택해서 사용",
        "SubSkills": [
          {
            "Name": "<font color=\"blue\">장막</font>",
            "Entries": [        
            {
              "Label": "유형",
              "Value": "물리 피해"
            },
            {
              "Label": "쿨타임",
              "Value": ""
            },
            {
              "Label": "사거리",
              "Value": "1칸"
            },
            {
              "Label": "범위",
              "Value": "단일"
            }
          ],
            "IconImageUrl": "../img/skills/Alpha/1_1.webp",
            "Desc": "[물리 피해]단일 적군을 공격하여 <font color=\"red\">1.5</font>배 피해를 준다.<br/><b>전투 전</b> <font color=\"purple\">자수정 장막</font> 효과 획득<br/>전투 후 해당 전투에서 입힌 피해량 <font color =\"red\">30%</font> 수치 생명 회복"
          },
          {
            "Name": "<font color=\"blue\">방패</font>",
            "Entries": [        
            {
              "Label": "유형",
              "Value": "물리 피해"
            },
            {
              "Label": "쿨타임",
              "Value": ""
            },
            {
              "Label": "사거리",
              "Value": "1칸"
            },            
            {
              "Label": "범위",
              "Value": "단일"
            }],
            "IconImageUrl": "../img/skills/Alpha/1_2.webp",
            "Desc": "[물리 피해]단일 적군을 공격하여 <font color=\"red\">1.5</font>배 피해를 준다.<br/><b>전투 전</b> <font color=\"purple\">자수정 방패</font> 효과 획득<br/>전투 후 해당 전투에서 입힌 피해량 <font color =\"red\">30%</font> 수치 생명 회복"
          }
        ],
        "Animation": null
      },
      {
        "Name": "수정 호위",
        "Cost": "COST 2",
        "Entries": [
          {
            "Label": "유형",
            "Value": "액티브"
          },
          {
            "Label": "쿨타임",
            "Value": "2턴"
          },
          {
            "Label": "사거리",
            "Value": "자신"
          },
          {
            "Label": "범위",
            "Value": "단일"
          }
        ],
        "IconImageUrl": "../img/skills/Alpha/2.webp",
        "Desc": "[패시브]근접한 아군을 대신해 모든 공격을 받는다.<br/>[액티브]호위 범위가 <font color=\"red\">2</font>칸으로 증가, 공격 지력 <font color=\"red\">+20%</font> 및 '<font color=\"blue\">증원</font>'과 '<font color=\"blue\">응수</font>' 획득, <br/>지속 <font color=\"red\">2</font>행동 <br/><font color=\"blue\">증원</font> : 행동 종료 시 자신 부대 생명 <font color=\"red\">20%</font> 회복 <br/><font color=\"blue\">응수</font> : 공격을 받을 시 공격 <font color=\"red\">7%</font> 만큼 방어 상승",
        "SubSkills": null,
        "Animation": null
      },
    {
      "Name": "재생하는 수정 핵",
      "Cost": "COST 1",
      "Entries": [
        {
          "Label": "유형",
          "Value": "패시브"
        }
      ],
      "IconImageUrl": "../img/skills/Alpha/3.webp",
      "Desc": "[패시브]근접한 아군을 대신해 모든 공격을 받는다.<br/><b>공격을 받은 후 자신의 공격</b> <font color =\"red\">1.5</font>배 수치 생명 회복",
      "SubSkills": null,
      "Animation": null
    },
    {
      "Name": "정면일격",
      "Cost": "COST 2",
      "Entries": [
        {
          "Label": "유형",
          "Value": "물리 피해"
        },
        {
          "Label": "쿨타임",
          "Value": "5턴"
        },
        {
          "Label": "사거리",
          "Value": "1칸"
        },
        {
          "Label": "범위",
          "Value": "단일"
        }
      ],
      "IconImageUrl": "../img/skills/Skill_Support.webp",
      "Desc": "[물리 피해] 단일 적군을 공격하여 <font color=\"red\">1.7</font>배 피해를 준다.<br/>전투 전 공격의 <font color=\"red\">20%</font>가 방어에 추가되며, 적 부대에게 '모든 패시브 스킬 봉인'부여 <br/><font color=\"red\">2</font>행동 지속,<br/>이 효과는 제거되지 않는다.",
      "SubSkills": null,
      "Animation": null
    },
    {
        "Name": "자수정 폭발",
        "Cost": "COST 3",
        "Entries": [
          {
            "Label": "유형",
            "Value": "물리피해"
          },
          {
            "Label": "쿨타임",
            "Value": "6턴"
          },
          {
            "Label": "사거리",
            "Value": "자신"
          },
          {
            "Label": "범위",
            "Value": "4칸"
          }
        ],
        "IconImageUrl": "../img/Super/알파.webp",
        "Desc": "[패시브] 사망 시 생명 <font color=\"red\">100%</font> 수치로 <font color=\"blue\">부활</font> 및 <font color=\"blue\">모든 스킬 쿨타임 <font color=\"red\">-5</font></font> 및 <br/><b>'<font color=\"purple\">자수정 장막</font>','<font color=\"purple\">자수정 방패</font>' 영구 상실 및 재획득 불가(디메리트)</b><br/>최대 <font color=\"red\">1</font>회 발동.<br/>[액티브]<b>자신과 주위 <font color=\"red\">4</font>칸 <font color=\"red\">0.3</font>배 범위 피해</b><br/>사용 전 <b>자신에게 걸린</b> 모든 <font color =\"blue\">디버프 해제</font><br/>자신의 생명이 낮을 수록 피해량 증가 <br/>최대 <font color =\"red\">100%</font>",
        "SubSkills": null,
        "Animation": "../img/Super/알파_Ani.webp"
      }
  ],
  "Propose": false,
  "Bio": [
    {
      "Entry": "영웅소개",
      "Desc": "◆키/몸무게: 175cm / 55kg<br/>◆출연：초월자 진영"
    },
    {
      "Entry": "영웅담1",
      "Desc": "종언전쟁이 끝날 무렵, 마검 알하자드에 의해 격추된 비행선은 대륙에 수백년 동안 계속되는 수정 오염을 초래했다.<br/>기억체로 존재하던 기자로프는 새로울 초월자 계획을 실현시키기 위해, 오염된 수정을 이용하여 첫번째 클론인 알파를 만들어 낸다.<br/>냉담함, 오만함, 그리고 충성심...... 알파는 의지도, 불필요한 감정도 가지고 있지 않다. 그저 기자로프의 계획의 중요한 실행수단으로 묵묵히 전투를 계속할 뿐이다.<br/><font color=\"#DC143C\"><size=33><i>「난 주인님이 만든 도구다. 죽기 위해 살고 있지.」 </i>  </size=33></font>"
    },    
    {
      "Entry": "영웅담2",
      "Desc": "수정 코어의 계획의 중요한 실행자인 알파는 레겐부르그에 잠입하여, 로젠실 여왕이 신뢰하는 장군 중 한명인 암흑 수정 철위군단장 애머시스트가 되었다.<br/>제국의 장군으로 움직일 시절, 알파는 철위군단을 지휘하며 카콘시스 성을 불사르고, 식물의 클론 뮤와 함께 수정화된 세계수의 에너지를 모아 수정병을 퍼뜨려,<br/>여왕의 몸에 수정의 씨앗을 심는데에 성공했다.<br/><font color=\"#DC143C\"><size=33><i>「로젠실? 이 세상에 없어서는 안 될, 딱하고 어리석은 여자...... 그녀를 위대한 계획의 일부로 편입시키는 데에는 그 이상의 감정은 필요 없다.」 </i>  </size=33></font>"
    },
    {
      "Entry": "영웅담3",
      "Desc": "다른 클론과 달리, 수정의 클론 알파는 생명력을 흡수하는 능력을 지니고 있다. 그러므로 육체가 심하게 손상된 상황에서도,<br/>기자로프의 리버스 프로토콜만 있으면 수정의 힘으로 다시 부활할 수 있다. 뮤와 함께 임무를 수행하는 동안, 그녀는 주인이 자신에게 부여한,<br>\"뮤의 보호자로 그녀를 모든 위험에서 지킨다.\"는 임무를 계속 수행했다.<br/><font color=\"#DC143C\"><size=33><i>「수정이란...... 생명을 이어주는 힘을 가진 것. 그리고 한 순간에 세상을 파괴시킬 수 있는 멋진 힘을 가진 것이지.」 </i>  </size=33></font>"
    },
    {
      "Entry": "영웅담4",
      "Desc": "뮤와 함께 임무를 수행하면서, 두 사람은 서로 마음을 터놓게 되었고, 알파는 점차 이 어린 클론에게 특별한 감정을 품게 되었다.<br/>뮤가 가장 신뢰하는 파트너가 된 알파는 식물의 힘으로 마나 에너지를 모으는 뮤가 끊임없이 발아로 고통받는 모습을 보게 된다.<br/>몇 번의 부활을 거치는 동안, 그녀는 세계수의 새싹에서 나오는 신경독의 영향으로, 처음으로 기자로프에게 반항하는 의지를 가지게 된다.<br/>알파는 어떤 대가를 치르더라도, 뮤를 반드시 전투에서 물러서게하여, 더는 발아의 고통을 느끼지 않도록 하겠다고 마음 먹었다."
    },
    {
      "Entry": "영웅담5",
      "Desc": "수정 코어, 마나 에너지, 카오스의 세 가지 힘의 결집으로 인해, 초월자가 탄생하여 예레스 대륙에 끝을 고했다.<br/>마지막 전투에서 알파와 뮤는 성검 군단에게 크게 패하고 만다. 알파는 뮤를 지키기 위해 자폭의 길을 택한다. <br/>그녀는 자신의 죽음과 맞바꿔, 뮤에게 기자로프의 지배에서 벗어날 수 있는 기회를 주었다. 산산조각이 나, 모든 것을 잃은......알파는 이 전투의 생존자로 남지는 못했지만, <br/>기억체로 세계수 속에 계속 머물며, 뮤의 미래를 지켜볼 것이다.<br/><font color=\"#DC143C\"><size=33><i>「언젠가 뮤는 평범한, 다른 사람과 인연을 맺는 소녀가 되겠지. 그렇지만 난...... 그 순간을 볼 수는 없을 것 같군.」 </i>  </size=33></font>"
    }
  ],
  "Performance": [
    {
      "Entry": "「소환 - 등장대사」",
      "Desc": "인간은 생명의 말로에 직면하는 죽음을 두려워하지. <br/>그러니까 나는 여기에 왔다. <br/>그들에게 죽음은 지옥에서 내려주는 자비라는 걸 보이기 위해."
    },
    {
      "Entry": "「전투 - 전직1」",
      "Desc": "또 숭고한 목적에 한 걸음 다가간 모양이다."
    },
    {
      "Entry": "「전투 - 전직2」",
      "Desc": "능력 상승. 하지만 내 힘에 극한이 있는 게 아니야."
    },
    {
      "Entry": "「전투 - 스킬시전1」",
      "Desc": "천한 벌레 녀석! 죽어!"
    },
    {
      "Entry": "「전투 - 스킬시전2」",
      "Desc": "역류의 힘! 수정붕괴!"
    },
    {
      "Entry": "「전투 - 선택1」",
      "Desc": "새로운 지령인가."
    },
    {
      "Entry": "「전투 - 선택2」",
      "Desc": "마스터가 내린 지령은 너를 죽이는 것이다."
    },
    {
      "Entry": "「전투 - 전투불능」",
      "Desc": "전투 속행은 무의미인가... 철퇴한다."
    },
    {
      "Entry": "「대화 - 친밀도1」",
      "Desc": "수정... 생명이 끊어지지 않게 하기 위한 것...<br/>그리고... 한순간에 세계를 파괴할 수 있는 멋진 것이다."
    },
    {
      "Entry": "「대화 - 친밀도2」",
      "Desc": "뮤는 내가 유일하게 지키고 싶은 것. 그것뿐이다."
    },
    {
      "Entry": "「대화 - 친밀도3」",
      "Desc": "로젠실? 이 세상에 없어서는 안 될 슬프고도 어리석은 여자. <br/>그녀를 위대한 계획의 일환에 짜 넣는 데에 그 이상의 감정은 필요 없다."
    },
    {
      "Entry": "「대화 - 친밀도4」",
      "Desc": "울부짖고, 분노하고, 미쳐버리고, 절망하다... <br/>클로테르라는 남자가 주인을 잃어버린 아픔... 나는 이해할 수 있다."
    },
    {
      "Entry": "「대화 - 친밀도5」",
      "Desc": "마스터의 목적은 인류의 진화를 실현하는 거다.<br/>비천한 너희에게는 그 위대함이 이해되지 않겠지."
    },
    {
      "Entry": "「대화 - 일상1」",
      "Desc": "나는 마스터가 만든 도구다. 죽기 위해 살고 있지."
    },
    {
      "Entry": "「대화 - 기쁨」",
      "Desc": "계획 실행은 완벽하다만 유열이란 감정은 부질없다."
    },
    {
      "Entry": "「대화 - 분노」",
      "Desc": "아무런 가치도 없는 것?<br/>그것들은 당연히 쓰레기처럼 태워야지."
    },
    {
      "Entry": "「대화 - 슬픔」",
      "Desc": "언젠가 뮤는 평범한... 타인과의 유대를 바라는 여자애가 되겠지. <br/>하지만 나는... 그날을 볼 수는 없겠지."
    },
    {
      "Entry": "「대화 - 수줍음」",
      "Desc": "나에게는 과거의 기억이 없어. 하지만 그걸로 상관없다.<br/>쓸데없는 감정은 힘을 약하게 할 뿐이다."
    },
    {
      "Entry": "「상호작용 - 대사1」",
      "Desc": "마스터의 위대한 계획을 어지럽히는 자들은 전부 내가 섬멸하지."
    },
    {
      "Entry": "「상호작용 - 대사2」",
      "Desc": "레겐부르그의 파멸은 피할 수 없다.<br/>마스터가 신이 되는 과정을 누구도 방해할 수 없는 것과 마찬가지로 말이야."
    },
    {
      "Entry": "「상호작용 - 대사3」",
      "Desc": "죽음은 생명이 다다르는 말로.<br/>수정은 생환의 공포를 초래한다."
    },
    {
      "Entry": "「상호작용 - 호감」",
      "Desc": "행동력은 나쁘지 않군. 너도 마스터의 힘이 될 것 같다."
    },
    {
      "Entry": "「상호작용 - 반감1」",
      "Desc": "어리석은 인류! 너희의 더러운 신앙으로 나를 설복할 수 있을 거라 생각하지 마라!"
    },
    {
      "Entry": "「상호작용 - 특수1」",
      "Desc": "다음에도 우리의 계획에 얌전히 따르도록 해라."
    },
    {
      "Entry": "「상호작용 - 특수2」",
      "Desc": "끝내도록 하지. 시간 낭비였다."
    }
  ],
  "Performance2": null,  
  "Misc": ""
}];